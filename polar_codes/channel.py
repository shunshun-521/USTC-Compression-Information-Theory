...
def align_llr_to_decoder(llr):
    """
    将信道 LLR（传输顺序，对应 polar_encode 输出）重排为译码器内部顺序。
    polar_encode 含比特倒序置换，故需使用逆置换对齐因子图。
    """
    llr = np.asarray(llr, dtype=np.float64)
    brp = bit_reversal_permutation(len(llr))
    inv = np.empty(len(llr), dtype=int)
    inv[brp] = np.arange(len(llr))
    return llr[inv]
