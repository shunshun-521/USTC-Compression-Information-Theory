...
def awgn_channel(s, sigma, rng=None):
    ...
    if rng is None:
        noise = np.random.normal(0.0, sigma, size=np.shape(s))
    else:
...
def prepare_decoder_llr(llr, N):
    """
    将信道 LLR 转换为 SC/SCL/BP 译码器所需的比特倒序排列。
    编码器输出经 B_N 置换，译码器因子图按倒序索引处理。
    """
    n = int(np.log2(N))
    br = np.array([int(format(i, f"0{n}b")[::-1], 2) for i in range(N)])
