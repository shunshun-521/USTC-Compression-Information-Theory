...
def awgn_channel(s, sigma, rng=None):
    ...
    if rng is None:
        noise = np.random.normal(0, sigma, size=np.asarray(s).shape)
    else:
        noise = rng.normal(0, sigma, size=np.asarray(s).shape)
    return s + noise
...
def compute_llr(y, sigma):
    ...
    返回形状与 y 相同的 LLR 数组
    """
    return 2.0 * np.asarray(y) / (sigma ** 2)
...
def reorder_llr_for_decoder(llr, N):
    """将自然顺序信道 LLR 重排为 SC/SCL 译码器所需顺序"""
    from encoder import bit_reversal_permutation
    rev = bit_reversal_permutation(N)
    return np.asarray(llr)[rev]
