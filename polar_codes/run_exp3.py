...
def run_unit_tests():
    """单元测试：编码器、SC、BP 高信噪比校验。"""
    assert np.array_equal(polar_encode(np.array([1, 0, 1, 1])), [1, 1, 0, 1])
    ...
    for _ in range(50):
        ...
        llr = np.where(polar_encode(u_sent) == 0, 50.0, -50.0)
        u_hat, _ = bp.decode(llr)
        if not np.array_equal(u_hat[info_idx], u_sent[info_idx]):
            errors += 1
    assert errors == 0, f"BP 高信噪比校验失败: {errors} 帧错误"
    print("单元测试通过。")
...
for N in N_LIST:
    ...
    def scl_d(llr_ch):
        u, _pm = SCLDecoder(N, fb, list_size=4).decode(llr_ch)
        return u, None
