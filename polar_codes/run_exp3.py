...
def run_unit_tests():
    ...
    frozen_b = frozen_bits.astype(bool)
    ...
    for _ in range(100):
        ...
        if not np.array_equal(uh[info_idx], u_sent[info_idx]):
            errors += 1
    assert errors == 0, f'SC 无损验证失败: {errors}'
    print('单元测试通过。')
...
for N in N_LIST:
    ...
    def scl_d(llr_ch):
        u, _ = SCLDecoder(N, frozen_b, list_size=4).decode(llr_ch)
        return u, None
    ...
    bp_decoder = BPDecoder(N, frozen_b, max_iter=MAX_ITER)
