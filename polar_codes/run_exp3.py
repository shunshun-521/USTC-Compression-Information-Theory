...
N_LIST = [256] if BP_QUICK else ([256, 512] if QUICK else [256, 512])
RATE = 0.5
DESIGN_EBN0 = 2.5
MAX_ITER = 20 if BP_QUICK else 50
MAX_FRAMES = 500 if BP_QUICK else (2000 if QUICK else 100000)
MIN_ERRORS = 5 if BP_QUICK else (10 if QUICK else 100)
EB_N0_RANGE = np.arange(2.0, 4.5, 0.5) if BP_QUICK else (
    np.arange(2.0, 5.5, 0.5) if QUICK else np.arange(1.0, 5.5, 0.25)
)
...
for N in N_LIST:
    ...
    print(f"N={N}: BP 仿真 (max_iter={MAX_ITER})")
    r_bp = run_simulation(
