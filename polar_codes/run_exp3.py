...
def main():
    ...
    for N in p['n_list']:
        ...
        bp_decoder = BPDecoder(N, frozen_bits, max_iter=p['max_iter'])
        ...
        r_bp = run_simulation(
            N, K, p['eb_n0_range'], bp_d, 'bp',
            p['max_frames'], p['min_errors'],
            info_indices=info_idx,
        )
        all_results[f'BP (max_iter={p["max_iter"]})'] = r_bp
        ...
        if plt is not None:
            ...
            ax.set_title(f'BP Average Iterations (N={N}, max_iter={p["max_iter"]})')
