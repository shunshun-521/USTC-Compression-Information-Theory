...
frozen_t = np.ones(N_t, dtype=int); frozen_t[info_t] = 0
rng = np.random.default_rng(0)
for _ in range(100):
    u_t = np.zeros(N_t, dtype=int); u_t[info_t] = rng.integers(0, 2, K_t)
...
DESIGN_EBN0 = 2.5
MAX_ITER = 50
MAX_FRAMES = 300 if QUICK else int(os.environ.get('POLAR_MAX_FRAMES', 100000))
MIN_ERRORS = 15 if QUICK else int(os.environ.get('POLAR_MIN_ERRORS', 100))
