...
def run_unit_tests():
    ...
    assert np.array_equal(x, (build_generator_matrix(4) @ u) % 2)
    print("单元测试通过。")


os.makedirs("results", exist_ok=True)

N_LIST = [256] if os.environ.get("POLAR_FAST_SIM") else [256, 512]
RATE = 0.5
DESIGN_EBN0 = 2.5
MAX_ITER = 50
sim_p = fast_sim_params()
MAX_FRAMES = sim_p["max_frames"]
MIN_ERRORS = sim_p["min_errors"]
EB_N0_RANGE = np.arange(1.0, 5.5, 0.5) if os.environ.get("POLAR_FAST_SIM") else np.arange(1.0, 5.5, 0.25)
