"""
实验三：BP 译码
"""
import os
import sys
import numpy as np

sys.path.insert(0, os.path.dirname(__file__))

from construction import ga_construction
from encoder import polar_encode
from channel import bpsk_modulate, compute_llr, eb_n0_to_sigma
from decoder_sc import sc_decode
from decoder_scl import SCLDecoder
from decoder_bp import BPDecoder
from simulation import run_simulation
from utils import save_results_csv, plot_bler_curves, find_capacity_limit

try:
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt
except ImportError:
    plt = None


def run_unit_tests():
    u = np.array([1, 0, 1, 1])
    x = polar_encode(u)
    assert np.array_equal(x, [1, 1, 0, 1]), f"编码器错误: {x}"
    print("单元测试通过。")


os.makedirs('results', exist_ok=True)
run_unit_tests()

N_LIST = [256, 512]
RATE = 0.5
DESIGN_EBN0 = 2.5
MAX_ITER = 50
MAX_FRAMES = 100000
MIN_ERRORS = 100
EB_N0_RANGE = np.arange(1.0, 5.5, 0.25)

for N in N_LIST:
    K = N // 2
    info_idx, _, _ = ga_construction(N, K, DESIGN_EBN0)
    frozen_bits = np.ones(N, dtype=int)
    frozen_bits[info_idx] = 0

    all_results = {}

    print(f"\n{'=' * 60}\nN={N}, K={K}\n{'=' * 60}")

    results_sc = run_simulation(
        N, K, EB_N0_RANGE,
        lambda llr: (sc_decode(llr, frozen_bits), None),
        'sc', MAX_FRAMES, MIN_ERRORS, info_indices=info_idx,
    )
    all_results['SC'] = results_sc
    save_results_csv(results_sc, f'results/exp3_sc_N{N}_R0.5.csv')

    scl = SCLDecoder(N, frozen_bits, list_size=4)
    results_scl = run_simulation(
        N, K, EB_N0_RANGE,
        lambda llr: (scl.decode(llr)[0], None),
        'scl', MAX_FRAMES, MIN_ERRORS, info_indices=info_idx,
    )
    all_results['SCL (L=4)'] = results_scl
    save_results_csv(results_scl, f'results/exp3_scl_N{N}_R0.5.csv')

    bp_decoder = BPDecoder(N, frozen_bits, max_iter=MAX_ITER)
    results_bp = run_simulation(
        N, K, EB_N0_RANGE,
        lambda llr: bp_decoder.decode(llr),
        'bp', MAX_FRAMES, MIN_ERRORS, info_indices=info_idx,
    )
    all_results[f'BP (max_iter={MAX_ITER})'] = results_bp
    save_results_csv(results_bp, f'results/exp3_bp_N{N}_R0.5.csv')

    shannon_db = find_capacity_limit(RATE)
    plot_bler_curves(
        all_results,
        f'SC vs SCL vs BP (N={N}, R={RATE})',
        f'results/fig3_bp_N{N}_bler.png',
        shannon_limit_db=shannon_db,
    )

    if plt is not None:
        eb_n0_vals = [r['eb_n0_db'] for r in results_bp]
        avg_iters = [r['avg_iters'] for r in results_bp]
        fig, ax = plt.subplots(figsize=(7, 4))
        ax.plot(eb_n0_vals, avg_iters, 'o-', color='purple')
        ax.set_xlabel('Eb/N0 (dB)')
        ax.set_ylabel('Avg Iterations')
        ax.set_title(f'BP Average Iterations (N={N}, max_iter={MAX_ITER})')
        ax.grid(True, alpha=0.4)
        plt.tight_layout()
        plt.savefig(f'results/fig3_bp_N{N}_iters.png', dpi=150)
        plt.savefig(f'results/fig3_bp_N{N}_iters.pdf')
        plt.close()

print("\n实验三完成。")
