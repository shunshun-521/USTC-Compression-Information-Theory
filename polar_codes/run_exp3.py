...
for N in N_LIST:
    ...
    print(f"\nN={N} SC")
    r_sc = run_simulation(
        N, K, EB_N0_RANGE, sc_d, "sc", MAX_FRAMES, MIN_ERRORS, frozen_bits=frozen_bits
    )
    all_results["SC"] = r_sc
    ...
    r_scl = run_simulation(
        N, K, EB_N0_RANGE, scl_d, "scl", MAX_FRAMES, MIN_ERRORS, frozen_bits=frozen_bits
    )
    ...
    print(f"N={N} BP")
    r_bp = run_simulation(
        N, K, EB_N0_RANGE, bp_d, "bp", MAX_FRAMES, MIN_ERRORS, frozen_bits=frozen_bits
    )
...
# 合并 N=256/512 的 BP 对比图（fig3_bp_bler.png）
from utils import load_results_csv

combined = {}
for N in N_LIST:
    combined[f"BP N={N}"] = load_results_csv(f"results/exp3_bp_N{N}_R0.5.csv")
