...
DESIGN_EBN0 = 2.5
MAX_ITER = 50
MAX_FRAMES = 400 if QUICK else 100000
...
for N in N_LIST:
    ...
    r_sc = run_simulation(
        N, K, EB_N0_RANGE, sc_d, "sc", MAX_FRAMES, MIN_ERRORS, frozen_bits=frozen_bool
    )
    all_results["SC"] = r_sc
    ...
    r_scl = run_simulation(
        N, K, EB_N0_RANGE, scl_d, "scl", MAX_FRAMES, MIN_ERRORS, frozen_bits=frozen_bool
    )
    ...
    r_bp = run_simulation(
        N, K, EB_N0_RANGE, bp_d, "bp", MAX_FRAMES, MIN_ERRORS, frozen_bits=frozen_bool
    )
