...
for N in N_LIST:
    ...
    print(f"\n{'=' * 60}\n实验三 N={N}: SC\n{'=' * 60}")
    r_sc = run_simulation(
        N, K, EB_N0_RANGE, sc_d, "sc", MAX_FRAMES, MIN_ERRORS, verbose=True, info_idx=info_idx
    )
    all_results["SC"] = r_sc
    ...
    r_scl = run_simulation(
        N, K, EB_N0_RANGE, scl_d, "scl", MAX_FRAMES, MIN_ERRORS, verbose=True, info_idx=info_idx
    )
    ...
    print(f"\n实验三 N={N}: BP")
    r_bp = run_simulation(
        N, K, EB_N0_RANGE, bp_d, "bp", MAX_FRAMES, MIN_ERRORS, verbose=True, info_idx=info_idx
    )
