...
def run_unit_tests():
    ...
    for _ in range(30):
        ...
        u_t[info_idx] = info_bits
        llr = compute_llr(awgn_channel(bpsk_modulate(polar_encode(u_t)), sigma, rng), sigma)
        u_bp, _ = BPDecoder(N, frozen_bits, max_iter=50).decode(llr)
        if not np.array_equal(u_bp[info_idx], info_bits):
            err += 1
    assert err == 0, f"BP 高信噪比测试失败: {err}"
    print("单元测试通过。")
...
for N in N_LIST:
    ...
    r_sc = run_simulation(
        ...
        frozen_bits=frozen_bits, info_idx=info_idx,
    )
    all_results["SC"] = r_sc
