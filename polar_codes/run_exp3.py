...
def run_unit_tests():
    ...
    u = np.array([1, 0, 1, 1])
    x = polar_encode(u)
    u_dec = sc_decode(compute_llr(bpsk_modulate(x), 0.001), np.zeros(4, dtype=bool))
    assert np.array_equal(u_dec, u), f"编码器错误: x={x}"
    print("  [PASS] 编码器校验")
    ...
    assert u_bp.shape == (N,), "BP 输出形状错误"
    print(f"  [PASS] BP 译码基本功能 (iters={iters})")
