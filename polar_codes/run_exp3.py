...
fb_t[info_t] = False
u0 = np.zeros(N_t, dtype=int)
u0[info_t] = np.ones(K_t, dtype=int)
llr0 = compute_llr(bpsk_modulate(polar_encode(u0)), 0.01)
u_bp, _ = BPDecoder(N_t, fb_t, max_iter=50).decode(llr0)
assert np.array_equal(u_bp[info_t], u0[info_t]), "BP 无噪校验失败"
print("实验三：单元测试通过")
...
for N in N_LIST:
    ...
    def scl_d(llr_ch):
        u, _ = SCLDecoder(N, fb_bool, list_size=4).decode(llr_ch)
        return u, None
