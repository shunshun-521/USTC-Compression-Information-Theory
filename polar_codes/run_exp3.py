...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        llr = 100.0 * (1 - 2 * polar_encode(u))
        assert np.array_equal(u, sc_decode(llr, frozen)), "SC 无损译码失败"
    print("单元测试通过。")
...
def main():
    ...
    for N in N_LIST:
        ...
        scl = SCLDecoder(N, frozen_bool, list_size=4)
        ...
        bp = BPDecoder(N, frozen_bool, max_iter=MAX_ITER)
