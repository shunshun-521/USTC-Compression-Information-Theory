...
for N in N_LIST:
    ...
    print(f"\n{'=' * 60}\n实验三 N={N}: SC\n{'=' * 60}")
    r_sc = run_simulation(
        N, K, EB_N0_RANGE, sc_d, "sc", info_indices=info_idx, verbose=True
    )
    all_results["SC"] = r_sc
    ...
    r_scl = run_simulation(
        N, K, EB_N0_RANGE, scl_d, "scl", info_indices=info_idx, verbose=True
    )
    ...
    print(f"\n实验三 N={N}: BP")
    r_bp = run_simulation(
        N, K, EB_N0_RANGE, bp_d, "bp", info_indices=info_idx, verbose=True
    )
...
# 兼容目录结构中的 fig3_bp_bler.png（使用 N=256 图）
import shutil

src = "results/fig3_bp_N256_bler.png"
dst = "results/fig3_bp_bler.png"
