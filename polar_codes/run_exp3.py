...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        u_vec[info_idx] = info_bits
        x_vec = polar_encode(u_vec)
        llr = compute_llr(awgn_channel(bpsk_modulate(x_vec), sigma, rng), sigma)
        if not np.array_equal(sc_decode(llr, frozen_bits)[info_idx], info_bits):
            errors += 1
