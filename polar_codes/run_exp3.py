...
if __name__ == "__main__":
    ...
    MIN_ERRORS = int(os.environ.get("POLAR_MIN_ERRORS", "100" if not quick else "25"))
    EB_N0_RANGE = np.arange(1.0, 5.5, 0.25) if not quick else np.arange(2.0, 5.0, 0.5)
    ...
    for N in N_LIST:
        ...
        print(f"\nN={N} SC")
        r_sc = run_simulation(
            N, K, EB_N0_RANGE, sc_d, "sc", MAX_FRAMES, MIN_ERRORS, design_eb_n0_db=DESIGN_EBN0
        )
        all_results["SC"] = r_sc
        save_results_csv(r_sc, f"results/exp3_sc_N{N}_R0.5.csv")

        scl = SCLDecoder(N, frozen_bits.astype(bool), list_size=4)
        ...
        r_scl = run_simulation(
            N, K, EB_N0_RANGE, scl_d, "scl", MAX_FRAMES, MIN_ERRORS, design_eb_n0_db=DESIGN_EBN0
        )
        ...
        print(f"N={N} BP")
        r_bp = run_simulation(
            N, K, EB_N0_RANGE, bp_d, "bp", MAX_FRAMES, MIN_ERRORS, design_eb_n0_db=DESIGN_EBN0
        )
