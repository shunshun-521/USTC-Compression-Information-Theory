...
def _unit_tests():
    ...
    bp = BPDecoder(N, frozen_bits, max_iter=50, use_min_sum=False)
    errors = 0
    for _ in range(30):
        ...
        if not np.array_equal(u_hat[info_idx], u[info_idx]):
            errors += 1
    assert errors == 0, f"BP 高信噪比校验失败: {errors}"
...
def main():
    ...
    for N in N_LIST:
        ...
        bp_decoder = BPDecoder(N, frozen_bits, max_iter=MAX_ITER, use_min_sum=False)
