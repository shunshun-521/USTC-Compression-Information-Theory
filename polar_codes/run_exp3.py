...
N_LIST = [256] if os.environ.get('POLAR_FAST_SIM') == '1' else [256, 512]
RATE = 0.5
DESIGN_EBN0 = 2.5
MAX_ITER = 50
...
for N in N_LIST:
    ...
    r_sc = run_simulation(N, K, EB_N0_RANGE, sc_d, 'sc', info_indices=info_idx,
                           max_frames=sim['max_frames'], min_errors=sim['min_errors'])
    all_results['SC'] = r_sc
    save_results_csv(r_sc, f'results/exp3_sc_N{N}_R0.5.csv')

    def scl_d(llr_ch):
        u, pm = SCLDecoder(N, frozen_bits.astype(bool), list_size=4).decode(llr_ch)
        return u, None

    r_scl = run_simulation(N, K, EB_N0_RANGE, scl_d, 'scl', info_indices=info_idx,
                            max_frames=sim['max_frames'], min_errors=sim['min_errors'])
    ...
    r_bp = run_simulation(N, K, EB_N0_RANGE, bp_d, 'bp', info_indices=info_idx,
                           max_frames=sim['max_frames'], min_errors=sim['min_errors'])
