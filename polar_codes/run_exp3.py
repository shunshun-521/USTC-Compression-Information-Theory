...
EB_N0_RANGE = np.arange(1.0, 5.5, 0.5) if os.environ.get("POLAR_QUICK", "0") == "1" else np.arange(1.0, 5.5, 0.25)
...
for N in N_LIST:
    ...
    print(f"\nN={N} SC 仿真")
    r_sc = run_simulation(
        N, K, EB_N0_RANGE, sc_d, "sc", MAX_FRAMES, MIN_ERRORS, frozen_bits=frozen_bits, info_indices=info_idx
    )
    all_results["SC"] = r_sc
    ...
    r_scl = run_simulation(
        N, K, EB_N0_RANGE, scl_d, "scl", MAX_FRAMES, MIN_ERRORS, frozen_bits=frozen_bits, info_indices=info_idx
    )
    ...
    print(f"N={N} BP 仿真")
    r_bp = run_simulation(
        N, K, EB_N0_RANGE, bp_d, "bp", MAX_FRAMES, MIN_ERRORS, frozen_bits=frozen_bits, info_indices=info_idx
    )
