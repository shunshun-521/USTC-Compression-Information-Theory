...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        llr = compute_llr(bpsk_modulate(polar_encode(u_t)) + rng.normal(0, sigma, N), sigma)
        u_hat = sc_decode(llr, frozen)
        if not np.array_equal(u_t[info_idx], u_hat[info_idx]):
            errors += 1
    assert errors == 0, f"SC 校验失败: {errors}/100"
    print("单元测试通过。")
...
MAX_FRAMES = 100000
MIN_ERRORS = 100
EB_N0_RANGE = np.concatenate([np.arange(1.0, 5.5, 0.5), np.arange(6.0, 12.0, 1.0)])
