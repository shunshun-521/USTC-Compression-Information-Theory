...
validate.test_scl_l1_equals_sc()
validate.test_bp_noiseless()
...
for N in N_LIST:
    ...
    print(f"\n实验三 N={N}: SC")
    r_sc = run_simulation(
        N, K, EB_N0_RANGE, sc_d, "sc", info_indices=info_idx, verbose=True, **sim
    )
    all_results["SC"] = r_sc
    ...
    r_scl = run_simulation(
        N, K, EB_N0_RANGE, scl_d, "scl", info_indices=info_idx, verbose=True, **sim
    )
    ...
    print(f"\n实验三 N={N}: BP")
    r_bp = run_simulation(
        N, K, EB_N0_RANGE, bp_d, "bp", info_indices=info_idx, verbose=True, **sim
    )
