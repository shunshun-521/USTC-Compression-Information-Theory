...
def run_unit_tests():
    ...
    u_t[info_idx] = rng.integers(0, 2, K)
    x_t = polar_encode(u_t)
    llr_t = compute_llr(bpsk_modulate(x_t), 0.001)
    u_sc = sc_decode(llr_t, frozen)
    u_scl, _ = SCLDecoder(N, frozen, list_size=1).decode(llr_t)
...
if __name__ == "__main__":
    ...
    for N in N_LIST:
        ...
        def scl_d(llr_ch):
            u, pm = SCLDecoder(N, frozen, list_size=4).decode(llr_ch)
            return u, None
        ...
        bp_decoder = BPDecoder(N, frozen, max_iter=MAX_ITER)
