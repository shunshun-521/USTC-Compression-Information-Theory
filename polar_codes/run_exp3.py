...
def run_unit_tests():
    ...
    for _ in range(100):
        u_t = np.zeros(N, dtype=np.int8)
        u_t[info] = rng.integers(0, 2, K)
        llr = compute_llr(bpsk_modulate(polar_encode(u_t)), eb_n0_to_sigma(10.0, 0.5))
        assert np.array_equal(sc_decode(llr, fb), u_t)

    bp = BPDecoder(N, fb, max_iter=50)
    for _ in range(20):
        u_t = np.zeros(N, dtype=np.int8)
        u_t[info] = rng.integers(0, 2, K)
        llr = compute_llr(bpsk_modulate(polar_encode(u_t)), 1e-6)
        uh, _ = bp.decode(llr)
