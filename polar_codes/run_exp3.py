...
bp = BPDecoder(N_test, fb_t, max_iter=50)
ok = 0
for _ in range(50):
    ut = np.zeros(N_test, dtype=int)
    ut[info_t] = rng.integers(0, 2, K_test)
    llr_t = compute_llr(bpsk_modulate(polar_encode(ut)), eb_n0_to_sigma(8.0, 0.5))
    u_hat, _ = bp.decode(llr_t)
    if np.array_equal(u_hat, ut):
        ok += 1
...
N_LIST = [64] if QUICK else [256, 512]
RATE = 0.5
DESIGN_EBN0 = 2.5
MAX_ITER = 50
...
for N in N_LIST:
    ...
    r_sc = run_simulation(N, K, EB_N0_RANGE, sc_d, "sc", MAX_FRAMES, MIN_ERRORS)
    all_results["SC"] = r_sc
    ...
    r_scl = run_simulation(N, K, EB_N0_RANGE, scl_d, "scl", MAX_FRAMES, MIN_ERRORS)
    ...
    r_bp = run_simulation(N, K, EB_N0_RANGE, bp_d, "bp", MAX_FRAMES, MIN_ERRORS)
