...
def run_unit_tests():
    ...
    for _ in range(50):
        ...
        y = bpsk_modulate(x_t) + rng.normal(0, sigma, N)
        llr = compute_llr(y, sigma)
        uh, _ = BPDecoder(N, frozen_bits.astype(bool), max_iter=50).decode(llr)
        if not np.array_equal(uh[info_idx], u_t[info_idx]):
            errors += 1
    print(f"BP 高信噪比测试: {50 - errors}/50 帧正确")

    uh_scl, _ = SCLDecoder(N, frozen_bits.astype(bool), list_size=1).decode(
        compute_llr(bpsk_modulate(polar_encode(u_t)), sigma)
    )
    uh_sc = sc_decode(compute_llr(bpsk_modulate(polar_encode(u_t)), sigma), frozen_bits.astype(bool))
...
if __name__ == "__main__":
    ...
    for N in N_LIST:
        ...
        def scl_d(llr_ch):
            u, _ = SCLDecoder(N, frozen_bits.astype(bool), list_size=4).decode(llr_ch)
            return u, None
