...
def run_unit_tests():
    ...
    u_test[info_idx] = np.array([1, 0] * 16)
    llr = prepare_decoder_llr(
        compute_llr(bpsk_modulate(polar_encode(u_test)), 0.01), N
    )
    u_hat, _ = BPDecoder(N, fb_bool, max_iter=50).decode(llr)
    print(f"  BP 译码单帧测试完成 (max_iter=50)")
