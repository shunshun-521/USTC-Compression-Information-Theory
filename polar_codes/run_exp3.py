...
def _run_unit_tests():
    ...
    for _ in range(100):
        ...
        u64[info_idx] = payload
        llr = compute_llr(bpsk_modulate(polar_encode(u64)), eb_n0_to_sigma(10.0, 0.5))
        if not np.array_equal(sc_decode(llr, frozen_bits)[info_idx], payload):
            errors += 1
    assert errors == 0
    ...
    llr = compute_llr(bpsk_modulate(polar_encode(u_test)), eb_n0_to_sigma(4.0, 0.5))
...
if __name__ == '__main__':
    ...
    for N in N_LIST:
        ...
        def scl_d(llr_ch, _fb=frozen_bits):
            u, _ = SCLDecoder(N, _fb, list_size=4).decode(llr_ch)
            return u, None
