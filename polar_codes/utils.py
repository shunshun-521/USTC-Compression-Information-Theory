...
def compute_bpsk_capacity(eb_n0_db, rate):
    ...
    def integrand(y):
        val = 1.0 + np.exp(-snr * y ** 2)
        return np.log2(val) * np.exp(-y ** 2 / 2) / np.sqrt(2 * np.pi)
...
def plot_bler_curves(results_dict, title, save_path, shannon_limit_db=None,
    ...
    if shannon_limit_db is not None:
        ax.axvline(x=shannon_limit_db, color='gray', linestyle='--', label=f'Shannon limit ({shannon_limit_db:.2f} dB)')
