...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    def integrand(y, snr):
        return np.log2(1.0 + np.exp(-2.0 * snr * y)) * np.exp(-0.5 * y ** 2) / np.sqrt(2.0 * np.pi)

    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10.0 ** (eb_n0_db / 10.0))
        val, _ = integrate.quad(lambda y: integrand(y, snr), -np.inf, np.inf)
...
def plot_bler_curves(results_dict, title, save_path, shannon_limit_db=None,
    ...
    if shannon_limit_db is not None:
        ax.axvline(shannon_limit_db, color="gray", linestyle="--", label=f"Shannon R={title.split('R=')[-1].rstrip(')')}" if "R=" in title else "Shannon limit")
