...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    基于 LLR 形式：C = 1 - E[log2(1 + exp(-|LLR|))]
    其中 LLR = 4*R*Eb/N0_linear * y, y ~ N(0,1)
    """
    capacities = []
    y = np.linspace(-20, 20, 20000)
    w = np.exp(-y ** 2 / 2) / np.sqrt(2 * np.pi)
    dy = y[1] - y[0]
    for eb_n0_db in eb_n0_db_list:
        llr_scale = 4 * rate * 10 ** (eb_n0_db / 10)
        llr = np.abs(llr_scale * y)
        inner = np.log2(1 + np.exp(-np.clip(llr, 0, 80)))
        capacities.append(1 - np.sum(inner * w) * dy)
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-3, 6), num_points=1000):
