...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        p1 = np.exp(-(z - a) ** 2 / 2)
        py = 0.5 * (p0 + p1)
        norm = np.trapezoid(py, z)
        hy = -np.trapezoid((py / norm) * np.log2(py / norm + 1e-30), z)
        h_cond = 0.5 * np.log2(2 * np.pi * np.e)
        capacities.append(max(hy - h_cond, 0.0))
