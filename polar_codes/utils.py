...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK-AWGN 信道在二进制输入下的互信息（bits/channel use）。
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def p_x_given_y(x, y):
            return norm * np.exp(-((y - x) ** 2) / (2.0 * sigma ** 2))

        def p_y(y):
            return 0.5 * p_x_given_y(1.0, y) + 0.5 * p_x_given_y(-1.0, y)
        ...
        def conditional_entropy(y):
            ...
            h = 0.0
            for x in (1.0, -1.0):
                px_y = 0.5 * p_x_given_y(x, y) / py
                if px_y > 1e-300:
                    h -= px_y * np.log2(px_y)
            return py * h

        h_x_given_y, _ = integrate.quad(conditional_entropy, -12.0, 12.0)
...
def find_capacity_limit(rate, eb_n0_range=(-2, 10), num_points=200):
    ...
    raise ValueError("Could not bracket capacity limit in the given Eb/N0 range")
