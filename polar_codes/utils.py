...
def _bpsk_capacity_scalar(eb_n0_db, rate):
    ...
    def integrand(x):
        t = -4.0 * gamma * x * x
        if t < -50.0:
            return (-t / np.log(2.0)) * np.exp(-x * x)
        if t > 50.0:
            return 0.0
        return np.exp(-x * x) * np.log2(1.0 + np.exp(t))

    val, _ = integrate.quad(integrand, 0.0, 10.0)
...
def find_capacity_limit(rate, eb_n0_range=(-25, 20), num_points=2000):
    """
