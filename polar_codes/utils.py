...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E_y[log2(1 + exp(-|LLR(y)|))], LLR = snr * y
    """
    y = np.linspace(-12.0, 12.0, 5000)
    gauss = np.exp(-0.5 * y ** 2) / np.sqrt(2.0 * np.pi)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10.0 ** (eb_n0_db / 10.0))
        llr = snr * y
        t = -np.abs(llr)
        inner = np.where(t > -50, np.log2(1.0 + np.exp(t)), 0.0)
        val = np.trapezoid(inner * gauss, y)
