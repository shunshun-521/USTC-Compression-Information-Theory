...
def _bpsk_mi(eb_n0_db, rate=1.0):
    """BPSK 互信息（bits/channel use）。"""
    ebn0 = 10 ** (eb_n0_db / 10.0)
    snr_symbol = 2.0 * rate * ebn0

    def integrand(y):
        return np.exp(-(y ** 2)) * np.log2(
            1.0 + np.exp(-2.0 * snr_symbol - 2.0 * np.sqrt(snr_symbol) * y)
        )

    val, _ = integrate.quad(integrand, -30.0, 30.0, limit=200)
    return np.log2(2.0) - val / np.sqrt(np.pi)
