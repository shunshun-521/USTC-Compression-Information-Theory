...
def _bpsk_mutual_info(snr_linear):
    """BPSK 互信息（每信道使用），snr_linear = 2R * Eb/N0。"""
    def integrand(y):
        llr = 2.0 * snr_linear * y
        p_y = np.exp(-(y - np.sqrt(snr_linear)) ** 2) / np.sqrt(np.pi)
        p_y += np.exp(-(y + np.sqrt(snr_linear)) ** 2) / np.sqrt(np.pi)
        if p_y <= 0:
            return 0.0
        px0 = np.exp(-(y - np.sqrt(snr_linear)) ** 2) / np.sqrt(np.pi)
        px1 = np.exp(-(y + np.sqrt(snr_linear)) ** 2) / np.sqrt(np.pi)
        llr_val = 2.0 * snr_linear * y
        p0 = px0 / (px0 + px1)
        p1 = 1.0 - p0
        mi = 0.0
        if p0 > 0 and p1 > 0:
            mi = p0 * np.log2(1.0 / p0) + p1 * np.log2(1.0 / p1)
        return p_y * mi
