...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E_y[log2(1 + exp(-rho*y^2))]
  其中 rho = 2R * 10^(Eb/N0/10)，y ~ N(0,1)。
    """
    eb_n0_db_list = np.atleast_1d(eb_n0_db_list)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        rho = 2 * rate * (10 ** (eb_n0_db / 10))

        def integrand(y):
            return np.log2(1 + np.exp(-rho * y ** 2)) * np.exp(-y ** 2 / 2)

        val, _ = integrate.quad(integrand, -np.inf, np.inf)
        C = 1 - val / np.sqrt(2 * np.pi)
        capacities.append(C)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if caps[idx] == caps[idx - 1]:
        return float(eb_grid[idx])
    frac = (rate - caps[idx - 1]) / (caps[idx] - caps[idx - 1])
    return float(eb_grid[idx - 1] + frac * (eb_grid[idx] - eb_grid[idx - 1]))
