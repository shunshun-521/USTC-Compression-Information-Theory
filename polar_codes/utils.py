...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(t):
            ...
            return term * np.exp(-(t ** 2) / 2.0)

        val, _ = integrate.quad(integrand, 0.0, 30.0)
        capacities.append(1.0 - 2.0 * val / np.sqrt(2.0 * np.pi))
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(0, 5)):
    """找到使 BPSK 信道容量等于码率 R 的 Eb/N0（dB）。"""
    lo, hi = eb_n0_range
    for _ in range(50):
        mid = 0.5 * (lo + hi)
        if compute_bpsk_capacity(mid, rate).item() < rate:
            lo = mid
        else:
            hi = mid
    return 0.5 * (lo + hi)
