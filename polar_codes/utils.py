...
def bpsk_capacity(snr_linear):
    ...
    snr_linear = Es/N0 = 2*R*10^(Eb/N0/10)，符号能量归一化为 1。
    """
    sigma = 1.0 / np.sqrt(snr_linear)
    grid = np.linspace(-12.0, 12.0, 12001)
    dx = grid[1] - grid[0]
    py = np.zeros_like(grid)
    norm = sigma * np.sqrt(2.0 * np.pi)
    for x_sym in (1.0, -1.0):
        py += 0.5 * np.exp(-0.5 * ((grid - x_sym) / sigma) ** 2) / norm
    py = np.maximum(py, 1e-300)
    hy = -np.sum(py * np.log2(py)) * dx
    hn = 0.5 * np.log2(2.0 * np.pi * np.e * sigma ** 2)
    return max(hy - hn, 0.0)
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        capacities.append(bpsk_capacity(snr))
