...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    phi = np.exp(-0.5 * y_grid**2) / np.sqrt(2.0 * np.pi)

    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10 ** (eb_n0_db / 10.0))
        p0 = np.exp(-0.5 * (y_grid - 1.0) ** 2 * snr)
        p1 = np.exp(-0.5 * (y_grid + 1.0) ** 2 * snr)
        p_sum = p0 + p1
        mi = (
            p0 * np.log2(np.maximum(2.0 * p0 / p_sum, 1e-300))
            + p1 * np.log2(np.maximum(2.0 * p1 / p_sum, 1e-300))
        )
        capacities.append(float(np.trapezoid(mi * phi, y_grid)))
