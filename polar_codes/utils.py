...
def _bpsk_capacity_per_snr(ebn0_linear):
    """BPSK 输入约束下的每比特互信息（数值积分）"""
    from scipy import integrate

    def log2_1pexp(x):
        if x > 0:
            return x / np.log(2.0) + np.log1p(np.exp(-x)) / np.log(2.0)
        return np.log1p(np.exp(x)) / np.log(2.0)

    def integrand(y):
        return log2_1pexp(-2.0 * ebn0_linear * y * y) * np.exp(-0.5 * y * y)

    val, _ = integrate.quad(integrand, -np.inf, np.inf, limit=200)
    e_term = val / np.sqrt(2.0 * np.pi)
    return max(0.0, 1.0 - e_term)


def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算给定码率下等效 BPSK 信道容量（bits/channel use = rate * I_bit）。
    """
    eb_n0_db_list = np.asarray(eb_n0_db_list, dtype=np.float64)
    ebn0 = 10.0 ** (eb_n0_db_list / 10.0)
    i_bit = np.array([_bpsk_capacity_per_snr(e) for e in ebn0])
    return rate * i_bit


def find_capacity_limit(rate, eb_n0_range=(-1, 5)):
    """
    找到使 BPSK 每比特互信息 I(Eb/N0) = R 的 Eb/N0（dB）。
    """
    lo, hi = eb_n0_range[0], eb_n0_range[1]
    for _ in range(60):
        mid = (lo + hi) / 2.0
        i_mid = _bpsk_capacity_per_snr(10.0 ** (mid / 10.0))
        if i_mid < rate:
            lo = mid
        else:
            hi = mid
