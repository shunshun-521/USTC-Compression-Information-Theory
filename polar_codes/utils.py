...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        y = np.linspace(-8.0, 8.0, 32001)
        log_norm = -0.5 * np.log(2.0 * np.pi * sigma2)
        log_py0 = log_norm - ((y - 1.0) ** 2) / (2.0 * sigma2) + np.log(0.5)
        log_py1 = log_norm - ((y + 1.0) ** 2) / (2.0 * sigma2) + np.log(0.5)
        log_py = _logsumexp2(log_py0, log_py1)
        py = np.exp(log_py)
        px0y = np.exp(log_py0 - log_py)
        px1y = np.exp(log_py1 - log_py)
        h_cond = -np.trapezoid(
            py * (px0y * np.log2(px0y + 1e-300) + px1y * np.log2(px1y + 1e-300)),
            y,
        )
