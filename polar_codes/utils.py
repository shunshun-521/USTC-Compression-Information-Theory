...
def bpsk_capacity(snr_linear):
  ...
  def integrand(y):
    return np.log2(1.0 + np.exp(-2.0 * snr_linear * y**2)) * np.exp(-y**2) / np.sqrt(np.pi)
...
def find_capacity_limit(rate, eb_n0_range=(-15, 20), num_points=2000):
  ...
  vals = [cap_minus_r(e) for e in grid]
  for i in range(len(grid) - 1):
    if vals[i] * vals[i + 1] <= 0:
      a, b = grid[i], grid[i + 1]
      for _ in range(50):
        mid = (a + b) / 2
        if cap_minus_r(mid) >= 0:
          a = mid
        else:
          b = mid
      return (a + b) / 2
