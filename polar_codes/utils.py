...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = (1/sqrt(2π)) ∫ exp(-z²/2) [ln2 - ln(1+exp(-2ρ-2√ρ z))] dz / ln2
    其中 ρ = 2·Es/N0 = 2·R·(Eb/N0)_linear，Es=1。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        rho = 2.0 * rate * (10 ** (eb_n0_db / 10.0))

        def integrand(z):
            expo = -2.0 * rho - 2.0 * z * np.sqrt(rho)
            if expo > 20.0:
                inner = np.log(2.0)
            elif expo < -30.0:
                inner = -expo
            else:
                inner = np.log(2.0) - np.log(1.0 + np.exp(expo))
            return np.exp(-z ** 2 / 2.0) * inner

        val, _ = integrate.quad(integrand, -np.inf, np.inf, limit=200)
        capacities.append(val / (np.sqrt(2.0 * np.pi) * np.log(2.0)))
