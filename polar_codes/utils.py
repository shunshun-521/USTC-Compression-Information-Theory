...
def _bpsk_capacity_single(snr_linear):
    """BPSK 信道容量（bits/channel use），snr_linear = 2*R*Eb/N0 = Es/N0。"""

    gamma = snr_linear

    def integrand(y):
        p0 = np.exp(-0.5 * (y - np.sqrt(gamma)) ** 2) / np.sqrt(2.0 * np.pi)
        p1 = np.exp(-0.5 * (y + np.sqrt(gamma)) ** 2) / np.sqrt(2.0 * np.pi)
        py = 0.5 * (p0 + p1)
        if py < 1e-300:
            return 0.0
        return 0.5 * (p0 * np.log2(p0 / py) + p1 * np.log2(p1 / py))
