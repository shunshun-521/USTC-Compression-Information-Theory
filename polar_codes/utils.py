...
def _bpsk_capacity_per_es_n0(es_n0):
    """BPSK 容量，Es/N0 为每信道符号能量噪声比（线性）"""

    def integrand(y):
        return (
            np.log2(1.0 + np.exp(-2.0 * es_n0 * y ** 2))
            * np.exp(-y ** 2)
            / np.sqrt(np.pi)
        )
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """计算 BPSK 离散输入信道容量（bits/channel use），Es/N0 = R * Eb/N0"""
    eb_n0_db_list = np.atleast_1d(eb_n0_db_list)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        es_n0 = rate * (10.0 ** (eb_n0_db / 10.0))
        capacities.append(_bpsk_capacity_per_es_n0(es_n0))
