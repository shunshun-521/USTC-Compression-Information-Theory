    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            ...
            if z < -50.0:
                return 0.0
            if z > 50.0:
                return (z / np.log(2.0)) * np.exp(-y * y) / np.sqrt(np.pi)
            return np.log2(1.0 + np.exp(z)) * np.exp(-y * y) / np.sqrt(np.pi)

        val, _ = integrate.quad(integrand, 0.0, 20.0, limit=200)
        capacities.append(max(0.0, 1.0 - 2.0 * val))
...
def find_capacity_limit(rate, eb_n0_range=(-2, 8), num_points=1000):
    ...
    x0, x1 = eb_n0_vals[idx - 1], eb_n0_vals[idx + 1]
