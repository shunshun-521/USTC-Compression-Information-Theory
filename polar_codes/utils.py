        ...
        for row in results:
            writer.writerow(
                [
                    ...
                    "" if row["avg_iters"] is None else f"{row['avg_iters']:.4f}",
                ]
            )
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            return (
                np.exp(-y ** 2 / 2.0)
                / np.sqrt(2.0 * np.pi)
                * np.log2(1.0 + np.exp(-2.0 * snr * y ** 2))
            )

        entropy, _ = integrate.quad(integrand, -30.0, 30.0, limit=200)
...
def plot_bler_curves(
    ...
    for label, results in results_dict.items():
        ...
        plt.semilogy(eb, bler, marker="o", linewidth=1.5, label=label)

    if shannon_limit_db is not None:
        plt.axvline(shannon_limit_db, color="gray", linestyle="--", linewidth=1.0, label="Shannon limit")
