...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    通过条件熵积分：C = ∫ p(y) H(X|Y=y) dy
    """
    y = np.linspace(-8.0, 8.0, 80001)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        esno = (10 ** (eb_n0_db / 10.0)) * rate
        sigma2 = 1.0 / (2.0 * esno)
        sigma = np.sqrt(sigma2)
        p0 = np.exp(-((y - 1.0) ** 2) / (2.0 * sigma2)) / (sigma * np.sqrt(2.0 * np.pi))
        p1 = np.exp(-((y + 1.0) ** 2) / (2.0 * sigma2)) / (sigma * np.sqrt(2.0 * np.pi))
        py = 0.5 * (p0 + p1) + 1e-300
        p0y = np.clip(0.5 * p0 / py, 1e-300, 1.0)
        p1y = np.clip(0.5 * p1 / py, 1e-300, 1.0)
        h = -p0y * np.log2(p0y) - p1y * np.log2(p1y)
        capacities.append(np.trapezoid(h * py, y))
