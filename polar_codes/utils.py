...
def _log2_one_plus_exp(z):
    ...
    small = z < -30
    mid = ~large & ~small
    out[large] = z[large] / np.log(2)
    out[small] = 0.0
    out[mid] = np.log2(1.0 + np.exp(z[mid]))
    return out
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E_Z[log2(1 + exp(-2 * (Eb/N0 * R) * Z^2))], Z ~ N(0,1)
    """
    caps = []
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(z):
            val = _log2_one_plus_exp(-2.0 * es_n0 * z * z)
            return val * np.exp(-(z**2) / 2.0) / np.sqrt(2.0 * np.pi)

        ent, _ = integrate.quad(integrand, -15.0, 15.0, limit=200)
        caps.append(1.0 - ent)
