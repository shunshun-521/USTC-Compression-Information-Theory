...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            p_y = np.exp(-(y - 1.0) ** 2 / 2.0) / np.sqrt(2.0 * np.pi)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    sub_eb = eb_grid[lo : hi + 1]
    sub_cap = compute_bpsk_capacity(sub_eb, rate)
    idx2 = np.argmin(np.abs(sub_cap - rate))
    return float(sub_eb[idx2])
