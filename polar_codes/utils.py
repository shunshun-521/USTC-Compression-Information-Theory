...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        y = np.linspace(-10 / np.sqrt(snr), 10 / np.sqrt(snr), 2001)
        py = np.exp(-0.5 * snr * y ** 2) / np.sqrt(2 * np.pi / snr)
        integrand = np.log2(1.0 + np.exp(-2.0 * snr * y ** 2))
        if hasattr(np, "trapezoid"):
            cap = 1.0 - np.trapezoid(integrand * py, y)
        else:
            cap = 1.0 - np.trapz(integrand * py, y)
