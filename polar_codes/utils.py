...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    调制：0 -> +1, 1 -> -1；噪声方差 sigma^2 = 1/(2R*Eb/N0)。
    """
    caps = []
    y_grid = np.linspace(-12.0, 12.0, 12000)
    dy = y_grid[1] - y_grid[0]
    for eb_n0_db in eb_n0_db_list:
        sigma = 1.0 / np.sqrt(2.0 * rate * (10 ** (eb_n0_db / 10.0)))
        norm = sigma * np.sqrt(2.0 * np.pi)
        p0 = np.exp(-0.5 * ((y_grid - 1.0) / sigma) ** 2) / norm
        p1 = np.exp(-0.5 * ((y_grid + 1.0) / sigma) ** 2) / norm
        p_y = 0.5 * (p0 + p1)
        i0 = p0 * np.log2((p0 + 1e-300) / (p_y + 1e-300))
        i1 = p1 * np.log2((p1 + 1e-300) / (p_y + 1e-300))
        caps.append(float(0.5 * np.sum(i0 + i1) * dy))
