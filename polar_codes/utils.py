...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    return e0 + (rate - c0) * (e1 - e0) / (c1 - c0 + 1e-15)
...
def crc_encode(info_bits, crc_length=8):
    from decoder_scl import crc_encode_bits

    return crc_encode_bits(np.asarray(info_bits, dtype=int), crc_length)


def crc_check(bits, crc_length=8):
    from decoder_scl import crc_check_bits
