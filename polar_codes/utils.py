...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK-AWGN 信道容量（bits/channel use），数值积分互信息。
    """
    capacities = []
    ys = np.linspace(-12.0, 12.0, 2401)
    gauss = np.exp(-(ys ** 2) / 2.0) / np.sqrt(2.0 * np.pi)
    for eb_n0_db in eb_n0_db_list:
        ...
        capacities.append(1.0 - float(np.trapezoid(h * gauss, ys)))
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-5, 12), num_points=1000):
