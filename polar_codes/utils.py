...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK 离散输入信道容量（bits/channel use，高斯数值积分）。
    """
    eb_n0_db_list = np.atleast_1d(eb_n0_db_list)
    z = np.linspace(-10.0, 10.0, 4000)
    gauss = np.exp(-z ** 2 / 2.0) / np.sqrt(2.0 * np.pi)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10.0 ** (eb_n0_db / 10.0))
        llr = 2.0 * np.sqrt(snr) * z
        log2_term = np.logaddexp(0.0, -llr) / np.log(2.0)
        val = np.trapezoid(log2_term * gauss, z)
...
def find_capacity_limit(rate, eb_n0_range=(-2, 8), num_points=4000):
    ...
    若数值求解失败，回退到 AWGN 连续输入香农限 10*log10((2^R-1)/R)。
    """
    eb_grid = np.linspace(eb_n0_range[0], eb_n0_range[1], num_points)
    caps = compute_bpsk_capacity(eb_grid, rate)
    idx = np.where(caps >= rate)[0]
    if len(idx) == 0:
        fallback = 10.0 * np.log10((2.0 ** rate - 1.0) / rate)
