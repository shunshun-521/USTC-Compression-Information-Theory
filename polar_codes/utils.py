...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def entropy_y_integrand(y):
            p0 = np.exp(-0.5 * (y - 1.0) ** 2 / sigma2)
            p1 = np.exp(-0.5 * (y + 1.0) ** 2 / sigma2)
            py = 0.5 * (p0 + p1) / np.sqrt(2.0 * np.pi * sigma2)
            if py < 1e-300:
                return 0.0
            return -py * np.log2(py)

        h_y, _ = integrate.quad(entropy_y_integrand, -30.0, 30.0, limit=200)
