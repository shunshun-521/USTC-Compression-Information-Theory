...
def bpsk_capacity(snr_linear):
    ...
    y = np.linspace(-8.0 * sigma - 4.0, 8.0 * sigma + 4.0, 20001)
    py0 = np.exp(-0.5 * ((y - 1.0) / sigma) ** 2)
    py1 = np.exp(-0.5 * ((y + 1.0) / sigma) ** 2)
    py = 0.5 * py0 + 0.5 * py1
    px0 = 0.5 * py0 / np.maximum(py, 1e-300)
    px1 = 0.5 * py1 / np.maximum(py, 1e-300)
    h = -(px0 * np.log2(px0 + 1e-300) + px1 * np.log2(px1 + 1e-300))
    return float(np.trapezoid(h * py, y) / np.trapezoid(py, y))
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        es_no = 2.0 * rate * (10 ** (eb_n0_db / 10.0))
        capacities.append(bpsk_capacity(es_no))
...
def find_capacity_limit(rate, eb_n0_range=(-2, 6), num_points=1000):
    ...
    c0 = bpsk_capacity(2.0 * rate * (10 ** (e0 / 10.0)))
    c1 = bpsk_capacity(2.0 * rate * (10 ** (e1 / 10.0)))
    if abs(c1 - c0) < 1e-12:
        return float(eb_grid[idx])
...
def plot_bler_curves(
    ...
    if shannon_limit_db is not None:
        ax.axvline(shannon_limit_db, color="gray", linestyle="--", linewidth=1.2, label=f"Shannon limit ({shannon_limit_db:.2f} dB)")
