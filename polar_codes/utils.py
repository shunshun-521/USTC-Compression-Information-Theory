...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 AWGN 实信道容量（bits/channel use），C = 0.5*log2(1+SNR)。
    """
...
def find_capacity_limit(rate, eb_n0_range=(-2, 6), num_points=801):
    """找到使 AWGN 信道容量等于码率 R 的 Eb/N0（dB）。"""
    eb_grid = np.linspace(eb_n0_range[0], eb_n0_range[1], num_points)
    caps = compute_bpsk_capacity(eb_grid, rate)
    idx = np.argmin(np.abs(caps - rate))
    if idx in (0, len(eb_grid) - 1):
        snr_lin = 2.0 ** (2.0 * rate) - 1.0
        eb_lin = snr_lin / (2.0 * rate)
