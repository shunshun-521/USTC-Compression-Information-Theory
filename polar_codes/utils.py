...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if idx == 0 or idx == len(eb_grid) - 1:
        for _ in range(40):
            mid = (eb_n0_range[0] + eb_n0_range[1]) / 2
            cap_mid = compute_bpsk_capacity(mid, rate)
            if cap_mid < rate:
                eb_n0_range = (mid, eb_n0_range[1])
            else:
                eb_n0_range = (eb_n0_range[0], mid)
        return (eb_n0_range[0] + eb_n0_range[1]) / 2
...
def plot_bler_curves(
    ...
    if shannon_limit_db is not None:
        ax.axvline(shannon_limit_db, color="gray", linestyle="--", linewidth=1, label="Shannon limit")
