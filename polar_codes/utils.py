...
def _bpsk_mutual_info_from_sigma(sigma):
    """BPSK 互信息（bits/channel use），与 channel.py 的 sigma 定义一致。"""
    from scipy import integrate

    def integrand(y):
        t = (2.0 / (sigma ** 2)) * y
        log_term = np.logaddexp(0.0, -np.abs(t)) / np.log(2.0)
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb in eb_n0_db_list:
        ...
        capacities.append(_bpsk_mutual_info_from_sigma(sigma))
