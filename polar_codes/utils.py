...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - 2 ∫_0^∞ φ(y) log2(1+exp(-2ρy)) dy, ρ = 2R·10^(Eb/N0/10)
    """
    eb_n0_db_list = np.atleast_1d(eb_n0_db_list)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        rho = 2.0 * rate * (10.0 ** (eb_n0_db / 10.0))

        def integrand(y):
            val = np.log2(1.0 + np.exp(np.clip(-2.0 * rho * y, -100.0, 100.0)))
            return val * np.exp(-y ** 2 / 2.0) / np.sqrt(2.0 * np.pi)

        entropy, _ = integrate.quad(integrand, 0.0, np.inf)
        capacities.append(1.0 - 2.0 * entropy)
