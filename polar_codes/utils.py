...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        p0 = norm.pdf(ys, 1.0, sigma)
        p1 = norm.pdf(ys, -1.0, sigma)
        py = 0.5 * (p0 + p1)
        mask = py > 1e-300
        q0 = np.zeros_like(py)
        q1 = np.zeros_like(py)
        q0[mask] = 0.5 * p0[mask] / py[mask]
        q1[mask] = 0.5 * p1[mask] / py[mask]
        q0[~mask] = 0.5
        q1[~mask] = 0.5
        h = -(q0 * np.log2(q0 + 1e-300) + q1 * np.log2(q1 + 1e-300))
        h_xy = np.sum(h * py) * dy
        capacities.append(1.0 - h_xy)
