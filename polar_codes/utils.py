...
def _bpsk_capacity_at_eb_n0(eb_n0_db, rate):
    """计算给定 Eb/N0 下 BPSK 信道容量（bits/channel use）"""
...
def find_capacity_limit(rate, eb_n0_range=(0, 6), num_points=200):
    ...
    """
    lo, hi = eb_n0_range
    cap_lo = _bpsk_capacity_at_eb_n0(lo, rate)
    cap_hi = _bpsk_capacity_at_eb_n0(hi, rate)
    if not (cap_lo <= rate <= cap_hi):
        eb_grid = np.linspace(-5.0, 15.0, num_points)
