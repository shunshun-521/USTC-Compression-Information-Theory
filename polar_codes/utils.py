...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    通过 BPSK-AWGN 互信息数值积分：I(X;Y)，Es/N0 = R * Eb/N0。
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        sigma = 1.0 / np.sqrt(es_n0)
        p1 = np.exp(-0.5 * ((y - 1) / sigma) ** 2) / (sigma * np.sqrt(2 * np.pi))
        p2 = np.exp(-0.5 * ((y + 1) / sigma) ** 2) / (sigma * np.sqrt(2 * np.pi))
        denom = p1 + p2 + 1e-300
        mi1 = np.trapezoid(0.5 * p1 * np.log2(2 * p1 / denom), y)
        mi2 = np.trapezoid(0.5 * p2 * np.log2(2 * p2 / denom), y)
        capacities.append(mi1 + mi2)
