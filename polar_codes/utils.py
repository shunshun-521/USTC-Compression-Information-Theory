...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = (1/π) ∫_0^π log2(1 + (Eb/N0)·sin²(θ/2)) dθ
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(theta):
            return np.log2(1.0 + gamma * np.sin(theta / 2.0) ** 2) / np.pi

        c, _ = integrate.quad(integrand, 0.0, np.pi)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    i = idx[0]
    return float((eb_grid[i] + eb_grid[i - 1]) / 2.0)
