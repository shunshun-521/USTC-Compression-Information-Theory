...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            p_y = (1.0 / (np.sqrt(2 * np.pi) * sigma)) * np.exp(-y ** 2 / (2 * sigma ** 2))
            s = snr
            term = np.log2(1.0 + np.exp(-2 * s * y))
            return p_y * term

        expected, _ = integrate.quad(integrand, -10, 10, limit=200)
        capacities.append(1.0 - expected)
