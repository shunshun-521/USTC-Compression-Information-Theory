...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    通过离散化 AWGN 信道并用 Blahut-Arimoto 算法求互信息。
    """
    try:
        from commpy._informationTheory.formulas import channel_capacity_dmc
    except ImportError:
        channel_capacity_dmc = None

    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10.0 ** (eb_n0_db / 10.0))
        sigma = 1.0 / np.sqrt(snr)
        y_grid = np.linspace(-6.0, 6.0, 801)
        dy = y_grid[1] - y_grid[0]
        norm = 1.0 / (sigma * np.sqrt(2.0 * np.pi))
        p_y_given_0 = norm * np.exp(-0.5 * ((y_grid - 1.0) / sigma) ** 2) * dy
        p_y_given_1 = norm * np.exp(-0.5 * ((y_grid + 1.0) / sigma) ** 2) * dy
        p_y_given_0 /= np.sum(p_y_given_0)
        p_y_given_1 /= np.sum(p_y_given_1)
        transition = np.vstack([p_y_given_0, p_y_given_1])

        if channel_capacity_dmc is not None:
            cap, _ = channel_capacity_dmc(transition)
        else:
            cap = _bpsk_capacity_quad(snr)
        capacities.append(cap)
    return np.array(capacities)


def _bpsk_capacity_quad(snr):
    """BPSK 容量数值积分备用实现"""
