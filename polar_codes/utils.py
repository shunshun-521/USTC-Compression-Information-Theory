...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK-AWGN 信道互信息（bits/channel use），Es/N0 = R * Eb/N0。
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        esn0 = (10 ** (eb_n0_db / 10.0)) * rate
        sigma = 1.0 / np.sqrt(2.0 * esn0)
        x = rng.integers(0, 2, n)
        y = (1 - 2 * x) + rng.normal(0.0, sigma, n)
        p1 = np.exp(-0.5 * ((y - 1.0) / sigma) ** 2)
        p0 = np.exp(-0.5 * ((y + 1.0) / sigma) ** 2)
        py = 0.5 * p1 + 0.5 * p0
        mi = np.mean(
            0.5 * p1 / py * np.log2(p1 / py) + 0.5 * p0 / py * np.log2(p0 / py)
        )
        capacities.append(float(mi))
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-1, 3), num_points=400):
    """找到使 BPSK 互信息等于码率 R 的 Eb/N0（dB）。"""
