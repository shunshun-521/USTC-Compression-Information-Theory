...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    采用数值积分：C = 1 - (2/√(2π)) ∫₀^∞ log₂(1+e^{-γy²}) e^{-y²/2} dy
    其中 γ = 2R·10^{Eb/N0/10}。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            z = -gamma * y * y
            if z < -50:
                return (-z / np.log(2)) * np.exp(-(y ** 2) / 2)
            return np.log2(1.0 + np.exp(z)) * np.exp(-(y ** 2) / 2)

        val, _ = integrate.quad(integrand, 0, 50, limit=200)
        val = 2.0 * val / np.sqrt(2 * np.pi)
