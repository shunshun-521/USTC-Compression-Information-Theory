...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - (1/sqrt(pi)) * integral log2(1+exp(-2*gamma - 2*sqrt(gamma)*y)) * exp(-y^2) dy
    其中 gamma = R * 10^{Eb/N0/10}。
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        sqrt_g = np.sqrt(max(gamma, 1e-12))
        ...
        def integrand(y):
            return log2_1pexp(-2.0 * gamma - 2.0 * sqrt_g * y) * np.exp(-(y ** 2))
