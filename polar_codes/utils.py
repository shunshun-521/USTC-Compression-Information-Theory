...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E_y[log2(1 + exp(-|LLR|))], LLR = 4*R*Eb/N0*y
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        eb_lin = 10.0 ** (eb_n0_db / 10.0)
        scale = 4.0 * rate * eb_lin

        def integrand(y):
            llr = scale * abs(y)
            log_term = np.maximum(-llr, 0.0) + np.log1p(np.exp(-llr))
            return np.exp(-0.5 * y ** 2) / np.sqrt(2.0 * np.pi) * (log_term / np.log(2.0))
