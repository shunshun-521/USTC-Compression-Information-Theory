...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E_{y|x=0}[log2(1 + exp(-LLR(y)))]，x=0 对应发送 +1。
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            llr = 2.0 * y / (sigma ** 2)
            p = np.exp(-0.5 * ((y - 1.0) / sigma) ** 2) / (
                np.sqrt(2.0 * np.pi) * sigma
            )
            return p * _log2_one_plus_exp(-llr)

        val, _ = integrate.quad(integrand, -15.0 * sigma, 15.0 * sigma + 2.0, limit=200)
