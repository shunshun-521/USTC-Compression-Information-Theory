...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = (2/pi) * integral_0^{pi/2} log2(1 + SNR*sin^2(theta)) d(theta)
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(theta):
            return np.log2(1.0 + snr * np.sin(theta) ** 2)

        cap, _ = integrate.quad(integrand, 0.0, np.pi / 2.0)
        capacities.append(cap * 2.0 / np.pi)
