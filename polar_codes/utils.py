...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E[log2(1 + exp(-|LLR|))]，LLR = 2y/sigma^2，sigma = 1/sqrt(2R*10^{Eb/N0/10})
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            ...
            llr = 2.0 * y / (sigma**2)
            abs_llr = np.abs(llr)
            term = 0.0 if abs_llr > 50 else np.log2(1.0 + np.exp(-abs_llr))
            return p_y * term

        entropy, _ = integrate.quad(integrand, -20.0, 20.0, limit=200)
