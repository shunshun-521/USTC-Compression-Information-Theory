...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E_t[log2(1 + e^{-snr * t^2})]，t ~ N(0,1)，snr = 2R * 10^{Eb/N0/10}
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(t):
            log_term = np.logaddexp(0.0, -snr * t * t) / np.log(2.0)
            return log_term * np.exp(-(t ** 2) / 2.0) / np.sqrt(2.0 * np.pi)
...
def find_capacity_limit(rate, eb_n0_range=(0, 6), num_points=500):
    """
