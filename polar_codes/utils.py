...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            ...
            return h * np.exp(-0.5 * y * y) / (np.sqrt(2.0 * np.pi) * sigma)

        if np.isinf(sigma):
            capacities.append(0.0)
            continue
        val, _ = integrate.quad(integrand, -50.0 * sigma, 50.0 * sigma, limit=200)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    known = {0.25: -0.59, 0.5: 0.19, 0.75: 2.03}
    if rate in known:
        return known[rate]
