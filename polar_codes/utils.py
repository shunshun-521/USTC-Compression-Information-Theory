...
def compute_bpsk_capacity(eb_n0_db, rate):
    ...
    def integrand(y):
        return np.log2(1.0 + np.exp(-2.0 * snr * y)) * np.exp(-0.5 * y * y) / np.sqrt(2.0 * np.pi)

    capacity, _ = integrate.quad(integrand, -20.0, 20.0)
    return 1.0 - capacity
...
def plot_bler_curves(results_dict, title, save_path, shannon_limit_db=None,
    ...
    """绘制 BLER-Eb/N0 曲线，同时保存 PNG 和 PDF。"""
