...
def _bpsk_capacity_per_snr(snr_linear):
    """
    BPSK-AWGN 信道容量（bits/channel use），snr = Es/N0 = 1/sigma^2
    """
    ...
    capacity = 0.0
    for x in (-1.0, 1.0):
        pyx = np.exp(-0.5 * ((y - x) / sigma) ** 2) / (sigma * np.sqrt(2.0 * np.pi))
        py = 0.5 * (
            np.exp(-0.5 * ((y - 1.0) / sigma) ** 2)
            + np.exp(-0.5 * ((y + 1.0) / sigma) ** 2)
        ) / (sigma * np.sqrt(2.0 * np.pi))
        ratio = np.clip(pyx / np.maximum(py, 1e-300), 1e-300, None)
        capacity += 0.5 * np.trapezoid(pyx * np.log2(ratio), y)
