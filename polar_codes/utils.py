...
def compute_bpsk_capacity(eb_n0_db, rate):
    ...
    def integrand(y):
        p_y = (1.0 / np.sqrt(2.0 * np.pi)) * np.exp(-0.5 * (y - np.sqrt(snr)) ** 2)
        p_y += (1.0 / np.sqrt(2.0 * np.pi)) * np.exp(-0.5 * (y + np.sqrt(snr)) ** 2)
        term = np.log2(1.0 + np.exp(-2.0 * np.sqrt(snr) * y))
        return p_y * term
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    caps = np.array([compute_bpsk_capacity(e, rate) for e in eb_grid])
