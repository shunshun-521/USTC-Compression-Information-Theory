...
def _bpsk_capacity_mi(eb_n0_linear):
    """BPSK AWGN 互信息（bits/channel use）。"""
    if eb_n0_linear <= 0:
        return 0.0
    sigma = 1.0 / np.sqrt(2.0 * eb_n0_linear)
    y = np.linspace(-8.0, 8.0, 200001)
    dy = y[1] - y[0]
    p = 0.5 * np.exp(-0.5 * ((y - 1) / sigma) ** 2) + 0.5 * np.exp(
        -0.5 * ((y + 1) / sigma) ** 2
    )
    p /= sigma * np.sqrt(2.0 * np.pi)
    p = np.clip(p, 1e-300, None)
    hy = -np.sum(p * np.log2(p)) * dy
    hy_given_x = 0.5 * np.log2(2.0 * np.pi * np.e * sigma ** 2)
    return float(hy - hy_given_x)


def compute_bpsk_capacity(eb_n0_db_list, rate):
    """计算给定 Eb/N0 列表下的 BPSK 信道容量（bits/channel use）。"""
    del rate
    return np.array([_bpsk_capacity_mi(10 ** (eb / 10.0)) for eb in eb_n0_db_list])
...
def find_capacity_limit(rate, eb_n0_range=(-5, 15), num_points=1000):
    ...
    for _ in range(60):
        mid = (lo + hi) / 2.0
        cap = _bpsk_capacity_mi(10 ** (mid / 10.0))
        if cap > rate:
            hi = mid
        else:
            lo = mid
...
def save_frozen_set_info(N_list, K, design_eb_n0_db, save_path):
    ...
    with open(save_path, "w", encoding="utf-8") as f:
        for N in N_list:
            ...
            f.write(
                np.array2string(frozen_idx, threshold=frozen_idx.size, max_line_width=120)
            )
            f.write("\n")
            f.write("-" * 53 + "\n")
