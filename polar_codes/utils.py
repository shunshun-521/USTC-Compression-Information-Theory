...
def _bpsk_mi_integrand(y, sigma):
    """BPSK-AWGN 互信息积分被积函数"""
    s2 = sigma ** 2
    p0 = np.exp(-(y - 1.0) ** 2 / (2.0 * s2)) / np.sqrt(2.0 * np.pi * s2)
    p1 = np.exp(-(y + 1.0) ** 2 / (2.0 * s2)) / np.sqrt(2.0 * np.pi * s2)
    p = 0.5 * (p0 + p1)
    mi = 0.0
    if p0 > 1e-300:
        mi += 0.5 * p0 * np.log2(p0 / p)
    if p1 > 1e-300:
        mi += 0.5 * p1 * np.log2(p1 / p)
    return mi
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        mi, _ = integrate.quad(
            lambda y: _bpsk_mi_integrand(y, sigma), -50.0, 50.0, limit=200
        )
