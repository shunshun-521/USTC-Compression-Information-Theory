...
def bpsk_mutual_information(snr_linear):
    """BPSK-AWGN 互信息（bits/channel use），Es=1，snr_linear = Es/N0。"""
    noise_var = 1.0 / snr_linear

    def p_y(y):
        p0 = np.exp(-((y - 1.0) ** 2) / (2.0 * noise_var)) / np.sqrt(2.0 * np.pi * noise_var)
        p1 = np.exp(-((y + 1.0) ** 2) / (2.0 * noise_var)) / np.sqrt(2.0 * np.pi * noise_var)
        return 0.5 * (p0 + p1)

    def entropy_integrand(y):
        py = p_y(y)
        return -py * np.log2(py) if py > 1e-300 else 0.0

    h_y, _ = integrate.quad(entropy_integrand, -15.0, 15.0, limit=200)
    h_y_given_x = 0.5 * np.log2(2.0 * np.pi * np.e * noise_var)
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        capacities.append(bpsk_mutual_information(snr))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if idx == 0 or idx == len(eb_grid) - 1:
        for _ in range(50):
            mid = (lo + hi) / 2.0
            cap_mid = bpsk_mutual_information(2.0 * rate * (10 ** (mid / 10.0)))
            if cap_mid < rate:
                lo = mid
            else:
                hi = mid
