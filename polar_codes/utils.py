...
def _bpsk_capacity_per_snr(snr_linear):
    ...
    def integrand(y):
        return np.log2(1 + np.exp(-2 * snr_linear * y ** 2 * snr_linear / snr_linear))

    def integrand2(y):
        s = snr_linear
        return np.log2(1 + np.exp(-2 * s * y ** 2 / (1 / s))) / np.sqrt(2 * np.pi) * np.exp(-y ** 2 / 2)

    cap, _ = integrate.quad(integrand2, -10, 10)
    return 1 - cap
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            return np.log2(1 + np.exp(-snr * y ** 2)) * np.exp(-y ** 2 / 2) / np.sqrt(2 * np.pi)
