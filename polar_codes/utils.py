...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E[log2(1 + exp(-|L|))]，L 为 AWGN-BPSK 信道 LLR。
    """
    rng = np.random.default_rng(0)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        sigma = 1.0 / np.sqrt(2.0 * rate * (10 ** (eb_n0_db / 10.0)))
        x = rng.choice([-1.0, 1.0], size=200000)
        y = x + rng.normal(0.0, sigma, size=200000)
        llr = np.abs(2.0 * y / (sigma ** 2))
        log_term = np.log2(1.0 + np.exp(-llr))
        capacities.append(1.0 - np.mean(log_term))
