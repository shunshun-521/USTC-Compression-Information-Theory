...
def _bpsk_conditional_entropy(y, snr):
    """给定 y 时的条件熵 H(X|Y=y)（比特）"""
    sigma2 = 1.0 / snr
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            py = (
                np.exp(-((y - 1.0) ** 2) / (2.0 * sigma ** 2))
                + np.exp(-((y + 1.0) ** 2) / (2.0 * sigma ** 2))
            ) / (2.0 * np.sqrt(2.0 * np.pi * sigma ** 2))
            return py * _bpsk_conditional_entropy(y, snr)

        h_cond, _ = integrate.quad(integrand, -15.0, 15.0, limit=200)
