...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """计算 BPSK 实 AWGN 信道容量（bits/channel use）。"""
    eb_n0_db_list = np.atleast_1d(eb_n0_db_list, dtype=np.float64)
    gamma = 10.0 ** (eb_n0_db_list / 10.0)
    return 0.5 * np.log2(1.0 + 2.0 * gamma)


def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    """找到使 BPSK 信道容量等于码率 R 的 Eb/N0（dB）。"""
    gamma_req = (2.0 ** (2.0 * rate) - 1.0) / 2.0
    return float(10.0 * np.log10(gamma_req))
