        ...
        for r in results:
            writer.writerow(
                [
                    ...
                    "" if r.get("avg_iters") is None else f"{r['avg_iters']:.1f}",
                ]
            )
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            ...
            return np.log2(1.0 + np.exp(val)) * np.exp(-y ** 2 / 2.0) / np.sqrt(2.0 * np.pi)
...
def plot_bler_curves(
    ...
    if shannon_limit_db is not None:
        plt.axvline(shannon_limit_db, color="gray", linestyle="--", label=f"Capacity R (Eb/N0={shannon_limit_db:.2f} dB)")
