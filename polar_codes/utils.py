...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            return np.log2(1.0 + np.exp(-2.0 * snr * y ** 2)) * np.exp(-0.5 * y ** 2)
...
def save_frozen_set_info(N_list, K, design_eb_n0_db, save_path, rate=0.5):
    ...
    with open(save_path, "w", encoding="utf-8") as f:
        for N in N_list:
            ...
            r = k_val / N
            info_idx, frozen_idx, _ = ga_construction(N, k_val, design_eb_n0_db, rate=r)
            f.write("=" * 53 + "\n")
            f.write(f"N={N}, K={k_val}, design_Eb/N0={design_eb_n0_db} dB, R={r:.4f}\n")
            f.write("=" * 53 + "\n")
