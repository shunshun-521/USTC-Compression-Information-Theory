...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    采用标准 AWGN 下 BPSK 互信息公式，Eb/N0 为信息比特能量比。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            arg = np.clip(-2.0 * gamma * y * y, -700.0, 700.0)
            return np.log2(1.0 + np.exp(arg)) * np.exp(-0.5 * y * y)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    vals = [capacity_minus_rate(x) for x in grid]
