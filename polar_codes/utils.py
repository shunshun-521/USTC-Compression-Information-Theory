...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        C = 1 - integral / (np.sqrt(2 * np.pi) * sigma)
        capacities.append(C)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    capacities = compute_bpsk_capacity(eb_n0_vals, rate)

    for i in range(len(eb_n0_vals) - 1):
        if capacities[i] >= rate >= capacities[i + 1] or capacities[i] <= rate <= capacities[i + 1]:
            t = (rate - capacities[i]) / (capacities[i + 1] - capacities[i] + 1e-15)
            return eb_n0_vals[i] + t * (eb_n0_vals[i + 1] - eb_n0_vals[i])

    return eb_n0_vals[np.argmin(np.abs(capacities - rate))]
