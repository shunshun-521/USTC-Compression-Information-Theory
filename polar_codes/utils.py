...
def compute_bpsk_capacity(eb_n0_db, rate):
    ...
    def integrand(y):
        llr = np.clip(2 * snr_sym * y, -100, 100)
        p0 = 1.0 / (1.0 + np.exp(-llr))
        return h2(p0) * np.exp(-y ** 2 / 2) / np.sqrt(2 * np.pi)

    val, _ = integrate.quad(integrand, -15, 15, limit=200)
    return 1.0 - val
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    caps = np.array([compute_bpsk_capacity(eb, rate) for eb in eb_n0_vals])
    ...
    for i in range(len(caps) - 1):
        if (caps[i] - rate) * (caps[i + 1] - rate) <= 0:
            ...
            return float(eb_n0_vals[i] + t * (eb_n0_vals[i + 1] - eb_n0_vals[i]))
    return float(eb_n0_vals[idx])
