...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    通过 H(Y) - H(Y|X) 数值积分，噪声模型与 channel.py 一致。
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def py(y):
            f0 = np.exp(-((y - 1.0) ** 2) / (2.0 * sigma ** 2))
            f1 = np.exp(-((y + 1.0) ** 2) / (2.0 * sigma ** 2))
            return 0.5 * (f0 + f1) / norm
        ...
        hy, _ = integrate.quad(hy_integrand, -15.0, 15.0, limit=200)
    ...
    with open(save_path, "w", encoding="utf-8") as f:
        for N in N_list:
            if K is None:
                ...
                k_val = K if isinstance(K, int) else K
            rate = k_val / N
