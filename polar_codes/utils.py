...
def _bpsk_capacity_per_snr(gamma):
    """BPSK 信道容量（bits/channel use），gamma = R * Eb/N0（线性）。"""
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """计算各 Eb/N0 对应的 BPSK 离散输入信道容量。"""
    caps = []
    for eb_db in eb_n0_db_list:
        eb_lin = 10 ** (eb_db / 10.0)
        gamma = rate * eb_lin
        caps.append(_bpsk_capacity_per_snr(gamma))
