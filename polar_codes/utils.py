...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        y_samples = np.linspace(-10.0, 10.0, 20001)
        py = np.exp(-0.5 * y_samples ** 2) / np.sqrt(2.0 * np.pi)
        integrand = py * np.log2(1.0 + np.exp(-2.0 * gamma * y_samples ** 2))
        try:
            area = np.trapezoid(integrand, y_samples)
        except AttributeError:
            area = np.trapz(integrand, y_samples)
        capacities.append(float(1.0 - area))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if idx == 0 or idx == len(eb_grid) - 1:
        for _ in range(40):
            mid = 0.5 * (eb_n0_range[0] + eb_n0_range[1])
            cap_mid = compute_bpsk_capacity(mid, rate)[0]
            if cap_mid > rate:
                eb_n0_range = (eb_n0_range[0], mid)
            else:
                eb_n0_range = (mid, eb_n0_range[1])
        return 0.5 * sum(eb_n0_range)
