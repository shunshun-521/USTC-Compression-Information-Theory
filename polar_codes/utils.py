...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    return np.array([_bpsk_capacity_scalar(s) for s in snr])
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
  ...
  # refine with bisection around bracket
    lo, hi = eb_grid[max(0, idx - 1)], eb_grid[min(len(eb_grid) - 1, idx + 1)]
    for _ in range(50):
        mid = (lo + hi) / 2.0
        if _bpsk_capacity_scalar(2.0 * rate * (10 ** (mid / 10.0))) < rate:
            lo = mid
        else:
            hi = mid
    return (lo + hi) / 2.0
