...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    使用标准高斯信道容量 C = 0.5*log2(1 + 2*SNR) 并换算为每信道使用。
    """
    caps = []
    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10.0 ** (eb_n0_db / 10.0))
        caps.append(0.5 * np.log2(1.0 + 2.0 * snr))
...
def find_capacity_limit(rate, eb_n0_range=(-2, 10), num_points=200):
    ...
    由 R = 0.5*log2(1 + 2*R*Eb/N0) 得 Shannon 限：Eb/N0 = (2^{2R}-1)/(2R)。
    """
    linear = (2.0 ** (2.0 * rate) - 1.0) / (2.0 * rate)
