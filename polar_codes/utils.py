...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    使用 C = (2/pi) * integral_0^{pi/2} log2(1 + SNR*sin^2(theta)) d(theta)
    """
    capacities = []
    theta = np.linspace(0.0, np.pi / 2.0, 2000)
    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10.0 ** (eb_n0_db / 10.0))
        integrand = np.log2(1.0 + snr * np.sin(theta) ** 2)
        capacities.append(2.0 / np.pi * np.trapezoid(integrand, theta))
