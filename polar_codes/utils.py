...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E[log2(1 + exp(-|L|))]，L = 2y/sigma^2，y = +1 + n，n ~ N(0, sigma^2)
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(n):
            ...
            z = -np.abs(llr)
            if z > 50:
                log_term = z * np.log2(np.e)
            else:
                log_term = np.log2(1.0 + np.exp(z))
            return log_term * np.exp(-(n ** 2) / (2.0 * sigma ** 2)) / (
                np.sqrt(2.0 * np.pi) * sigma
            )

        expectation, _ = integrate.quad(integrand, -50.0, 50.0, limit=200)
        capacities.append(1.0 - expectation)
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-6, 8), num_points=2000):
    """
