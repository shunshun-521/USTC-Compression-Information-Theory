...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    def capacity_at_ebn0(eb_n0_db):
        ...
        def integrand(z):
            return (
                np.log2(1.0 + np.exp(-2.0 * snr - 2.0 * np.sqrt(snr) * z))
                * norm.pdf(z)
            )
    ...
    return [capacity_at_ebn0(e) for e in eb_n0_db_list]
...
def save_frozen_set_info(N_list, K, design_eb_n0_db, save_path):
    ...
    with open(save_path, "w", encoding="utf-8") as f:
        for N in N_list:
            ...
            rate = K_eff / N
            info_idx, frozen_idx, _ = ga_construction(N, K_eff, design_eb_n0_db, rate=rate)
            f.write("=" * 53 + "\n")
            f.write(
