...
def compute_bpsk_capacity(eb_n0_db_list, rate, num_samples=200000, seed=0):
    ...
    使用与 channel.py 一致的 LLR 定义进行蒙特卡洛估计：
    C = 1 - E_y[H(X|Y=y)]
    """
    rng = np.random.default_rng(seed)
    capacities = []
    for eb_db in np.asarray(eb_n0_db_list, dtype=np.float64):
        snr = 2.0 * rate * (10.0 ** (eb_db / 10.0))
        sigma = 1.0 / np.sqrt(snr)
        x = rng.choice([-1.0, 1.0], size=num_samples)
        y = x + rng.normal(0.0, sigma, size=num_samples)
        llr = 2.0 * y / (sigma ** 2)
        p0 = 1.0 / (1.0 + np.exp(-llr))
        p1 = 1.0 - p0
        eps = 1e-15
        h_cond = -p0 * np.log2(np.clip(p0, eps, 1.0)) - p1 * np.log2(np.clip(p1, eps, 1.0))
        capacities.append(1.0 - float(np.mean(h_cond)))
