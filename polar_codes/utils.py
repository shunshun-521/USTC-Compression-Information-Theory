...
def _bpsk_capacity_per_eb_n0(eb_n0_db, rate):
    ...
    使用标准公式 C = 1 - E[log2(1 + exp(-2*gamma*y^2))], gamma = Eb/N0 线性值。
    """
    gamma = 10.0 ** (eb_n0_db / 10.0)

    def integrand(y):
        z = -2.0 * gamma * y ** 2
        return _log2_1_plus_exp(z) * np.exp(-0.5 * y ** 2) / np.sqrt(2.0 * np.pi)
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    return np.array([_bpsk_capacity_per_eb_n0(eb, rate) for eb in eb_n0_db_list])
