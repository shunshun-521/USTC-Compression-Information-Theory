...
def bpsk_capacity_per_use(snr_linear, num_points=2000):
    """BPSK 离散输入信道容量（bits/channel use），高斯求积。"""
    y = np.linspace(-10.0, 10.0, num_points)
    dy = y[1] - y[0]
    integrand = np.log2(1.0 + np.exp(-2.0 * snr_linear * y)) * np.exp(-(y**2) / 2.0)
    trapz = getattr(np, "trapezoid", None) or np.trapz
    val = trapz(integrand, y) / np.sqrt(2.0 * np.pi)
    return 1.0 - val


def compute_bpsk_capacity(eb_n0_db_list, rate):
    """计算各 Eb/N0 点对应的 BPSK 信道容量（bits/channel use）。"""
    caps = []
    for eb in eb_n0_db_list:
        snr = (10.0 ** (eb / 10.0)) * 2.0 * rate
        caps.append(bpsk_capacity_per_use(snr))
    return np.array(caps)


def find_capacity_limit(rate, eb_n0_range=(0.0, 12.0), num_points=2000):
...
def plot_bler_curves(
    ...
    """绘制 BLER-Eb/N0 曲线（semilogy），保存 PNG 与 PDF。"""
    if plt is None:
    ...
    if shannon_limit_db is not None:
        ax.axvline(
            ...
            linestyle="--",
            linewidth=1.2,
            label=f"Shannon (R) @ {shannon_limit_db:.2f} dB",
        )
