...
def _bpsk_mutual_info(eb_n0_linear, rate):
    """BPSK 互信息（bits/channel use），考虑码率 R。"""
    a = np.sqrt(2.0 * rate * eb_n0_linear)

    def py(y):
        return 0.5 * (
            np.exp(-0.5 * (y - a) ** 2) + np.exp(-0.5 * (y + a) ** 2)
        ) / np.sqrt(2.0 * np.pi)
    ...
    h_y_given_x = 0.5 * np.log2(2.0 * np.pi * np.e)
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb in eb_n0_db_list:
        eb_lin = 10 ** (eb / 10.0)
        capacities.append(_bpsk_mutual_info(eb_lin, rate))
