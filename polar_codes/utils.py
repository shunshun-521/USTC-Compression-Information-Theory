...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        y = np.linspace(-12.0 * sigma, 12.0 * sigma, 80000)
        pdf = np.exp(-0.5 * (y / sigma) ** 2) / (sigma * np.sqrt(2.0 * np.pi))
        llr = 2.0 * y / sigma ** 2
        integrand = np.log2(1.0 + np.exp(-np.abs(llr)))
        capacities.append(1.0 - np.trapezoid(integrand * pdf, y))
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-6, 6)):
