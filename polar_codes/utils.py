...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            return np.log2(1.0 + np.exp(-2.0 * snr * y * y)) * np.exp(-0.5 * y * y)

        val, _ = integrate.quad(integrand, 0.0, np.inf)
        capacities.append(1.0 - (1.0 / np.sqrt(2.0 * np.pi)) * val)
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-20, 10), num_points=2000):
    ...
    if cap_hi < rate:
        return float(hi)
    if cap_lo > rate:
        for test_lo in np.linspace(lo - 20, lo, 40):
            if compute_bpsk_capacity(test_lo, rate)[0] <= rate:
                lo = test_lo
                cap_lo = compute_bpsk_capacity(lo, rate)[0]
                break
