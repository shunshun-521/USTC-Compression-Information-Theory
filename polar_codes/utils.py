            ...
            results.append(
                {
                    ...
                    "avg_iters": float(avg_iters) if avg_iters not in ("", None) else None,
                }
            )
    return results
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - (1/ln2) ∫ φ(x) ln(1 + exp(-2·(Eb/N0)·R·x²)) dx
    """
    caps = []
    for eb_db in eb_n0_db_list:
        gamma = (10.0 ** (eb_db / 10.0)) * rate

        def integrand(x):
            t = np.clip(-2.0 * gamma * (x**2), -80.0, 80.0)
            return np.log(1.0 + np.exp(t)) * np.exp(-(x**2) / 2.0) / np.sqrt(2.0 * np.pi)

        val, _ = integrate.quad(integrand, -8.0, 8.0, limit=200)
        caps.append(1.0 - val / np.log(2.0))
...
def plot_bler_curves(results_dict, title, save_path, shannon_limit_db=None, xlabel="Eb/N0 (dB)", ylabel="BLER"):
    ...
    if shannon_limit_db is not None:
        ax.axvline(shannon_limit_db, color="gray", linestyle="--", label=f"Shannon (C=R), {shannon_limit_db:.2f} dB")
