...
def _bpsk_mutual_info(eb_n0_db, rate, num_samples=200000):
    """BPSK-AWGN 互信息（bits/channel use），与信道模块 sigma 定义一致。"""
    from channel import eb_n0_to_sigma

    sigma = eb_n0_to_sigma(eb_n0_db, rate)
    rng = np.random.default_rng(0)
    x = rng.integers(0, 2, num_samples)
    s = 1.0 - 2.0 * x
    y = s + rng.normal(0.0, sigma, num_samples)
    llr = 2.0 * y / (sigma ** 2)
    p0 = 1.0 / (1.0 + np.exp(-llr))
    h2 = -p0 * np.log2(p0 + 1e-300) - (1.0 - p0) * np.log2(1.0 - p0 + 1e-300)
    return 1.0 - float(np.mean(h2))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    for _ in range(60):
        mid = (lo + hi) / 2.0
        if _bpsk_mutual_info(mid, rate) > rate:
            hi = mid
        else:
            lo = mid
    return float(hi)
