...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E_{y|x=+1}[log2(1 + exp(-2*snr*y))]
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            py = np.sqrt(snr / (2.0 * np.pi)) * np.exp(-0.5 * snr * (y - 1.0) ** 2)
            z = -2.0 * snr * y
            if z > 0:
                term = (z + np.log1p(np.exp(-z))) / np.log(2)
            else:
                term = np.log1p(np.exp(z)) / np.log(2)
            return py * term

        expectation, _ = integrate.quad(integrand, -15.0, 15.0, limit=200)
