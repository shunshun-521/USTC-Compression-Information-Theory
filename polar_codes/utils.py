...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK-AWGN 信道容量上界（bits/channel use）。
    采用高斯输入容量 C = log2(1 + Es/N0)，Es/N0 = Eb/N0 * R。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        es_n0 = 10 ** (eb_n0_db / 10.0) * rate
        capacities.append(np.log2(1.0 + es_n0))
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-2, 8), num_points=1000):
    """找到使 AWGN 容量等于码率 R 的 Eb/N0（dB），用作 BLER 图 Shannon 参考线"""
