...
def compute_bpsk_capacity(eb_n0_db, rate):
  ...
  C = 1 - E_y[log2(1 + exp(-2*s*y))]
    ...
    def integrand(y):
        p_y = (1 / np.sqrt(2 * np.pi)) * np.exp(-y ** 2 / 2)
        return p_y * np.log2(1 + np.exp(-2 * snr * y))

    capacity, _ = integrate.quad(integrand, -10, 10, limit=200)
    return 1 - capacity
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if idx > 0 and idx < len(eb_n0_vals) - 1:
        c0, c1 = capacities[idx - 1], capacities[idx + 1]
...
def plot_bler_curves(results_dict, title, save_path, shannon_limit_db=None,
    ...
    for label, results in results_dict.items():
        ...
        ax.semilogy(eb_n0, bler, 'o-', label=label, linewidth=2, markersize=5)
