...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        for x_bit in (0, 1):
            ...
            log_p_y_given_x = -((y - x) ** 2) / (2 * sigma ** 2)
            log_p_y_given_0 = -((y - 1.0) ** 2) / (2 * sigma ** 2) - np.log(2.0)
            log_p_y_given_1 = -((y + 1.0) ** 2) / (2 * sigma ** 2) - np.log(2.0)
            log_p_y = np.logaddexp(log_p_y_given_0, log_p_y_given_1)

            mi += 0.5 * np.mean(log_p_y_given_x - log_p_y)

        capacities.append(mi / np.log(2.0))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if idx == len(eb_grid) - 1:
        return float(eb_grid[-1])

    x0, x1 = eb_grid[idx - 1], eb_grid[idx]
    y0, y1 = caps[idx - 1] - rate, caps[idx] - rate
    if y1 == y0:
        return float(x1)
