...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    def capacity_at_eb_n0(eb_n0_db):
        ...
        log_term = np.where(
            t > 0.0,
            t / np.log(2.0) + np.log1p(np.exp(-t)) / np.log(2.0),
            np.log1p(np.exp(t)) / np.log(2.0),
        )
        pdf = np.exp(-0.5 * y * y) / np.sqrt(2.0 * np.pi)
        val = np.sum(log_term * pdf) * dy
...
def find_capacity_limit(rate, eb_n0_range=(-2, 8), num_points=400):
    ...
    lo, hi = eb_n0_range
    caps_lo = float(compute_bpsk_capacity([lo], rate)[0])
    caps_hi = float(compute_bpsk_capacity([hi], rate)[0])
    if caps_lo > rate:
        return float(lo)
    if caps_hi < rate:
        return float(hi)
    for _ in range(60):
        mid = 0.5 * (lo + hi)
        if float(compute_bpsk_capacity([mid], rate)[0]) >= rate:
            hi = mid
        else:
            lo = mid
