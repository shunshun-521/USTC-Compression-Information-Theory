...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    使用 LLR 域互信息：C = 1 - E[log2(1 + exp(-|LLR|))]，
    其中 LLR = 4 * gamma * y，gamma = 2R * Eb/N0（线性）。
    """
    eb_n0_db_list = np.atleast_1d(eb_n0_db_list)
    capacities = []
    y = np.linspace(-10.0, 10.0, 50000)
    w = np.exp(-y ** 2 / 2.0) / np.sqrt(2.0 * np.pi)
    dy = y[1] - y[0]
    for eb_n0_db in eb_n0_db_list:
        gamma = 2.0 * rate * (10.0 ** (eb_n0_db / 10.0))
        llr = 4.0 * gamma * y
        tclip = np.minimum(np.abs(llr), 80.0)
        mi = np.log2(1.0 + np.exp(-tclip))
        val = np.sum(mi * w * dy)
