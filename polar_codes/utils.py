...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK-AWGN 信道容量（bits/channel use），基于互信息数值积分。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ...
        def pdf_y(y):
            ...
            return 0.5 * norm * (np.exp(-(y + 1.0) ** 2 / (2.0 * sigma2)) +
        ...
        h_y, _ = integrate.quad(integrand, -20.0, 20.0)
        h_z = 0.5 * np.log2(2.0 * np.pi * np.e * sigma2)
        capacities.append(h_y - h_z)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    for i in range(1, len(eb_grid)):
        if caps[i] >= rate:
            if caps[i - 1] < rate:
                return float(np.interp(rate, [caps[i - 1], caps[i]], [eb_grid[i - 1], eb_grid[i]]))
