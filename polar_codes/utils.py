...
def _bpsk_mutual_information(eb_n0_db, rate, n_samples=200000):
    """BPSK-AWGN 互信息（bits/channel use），与 channel.py 噪声模型一致。"""
    eb_lin = 10.0 ** (eb_n0_db / 10.0)
    sigma = 1.0 / np.sqrt(2.0 * rate * eb_lin)
    rng = np.random.default_rng(int(abs(eb_n0_db * 1000) % (2**31 - 1)))
    x = 1.0 - 2.0 * rng.integers(0, 2, n_samples)
    y = x + rng.normal(0.0, sigma, n_samples)
    h_y_given_x = 0.5 * np.log2(2.0 * np.pi * np.e * sigma ** 2)
    hist, edges = np.histogram(y, bins=256, density=True)
    dx = edges[1] - edges[0]
    py = hist[hist > 0]
    h_y = -np.sum(py * np.log2(py)) * dx
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    采用与仿真一致的 BPSK-AWGN 互信息估计。
    """
    return np.array([_bpsk_mutual_information(eb, rate) for eb in eb_n0_db_list])
...
def find_capacity_limit(rate, eb_n0_range=(-2, 8)):
    ...
    for _ in range(40):
        mid = (lo + hi) / 2.0
        if _bpsk_mutual_information(mid, rate) < rate:
            lo = mid
        else:
            hi = mid
    return (lo + hi) / 2.0
