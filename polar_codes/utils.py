...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - ∫₀^∞ exp(-x) log2(1 + exp(-Eb/N0 * R * x)) dx
    """
    capacities = []
    x_grid = np.linspace(1e-4, 40.0, 8000)
    for eb_n0_db in eb_n0_db_list:
        ebn0 = 10 ** (eb_n0_db / 10.0)
        integrand = np.exp(-x_grid) * np.logaddexp(0.0, -ebn0 * rate * x_grid) / np.log(2.0)
        val = np.trapezoid(integrand, x_grid)
