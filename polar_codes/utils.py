...
def _bpsk_capacity_per_ebno(ebno_linear, rate):
    """BPSK 互信息（bits/channel use），ebno_linear 为线性 Eb/N0"""
    snr = 2.0 * rate * ebno_linear
    a = np.sqrt(snr)

    def integrand(y):
        p0 = np.exp(-0.5 * (y - a) ** 2)
        p1 = np.exp(-0.5 * (y + a) ** 2)
        py = 0.5 * (p0 + p1) / np.sqrt(2.0 * np.pi)
        py = max(py, 1e-300)
        return -py * np.log2(py)

    hy, _ = integrate.quad(integrand, -30.0, 30.0)
    h_noise = 0.5 * np.log2(2.0 * np.pi * np.e)
    return hy - h_noise
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ebno = 10.0 ** (eb_n0_db / 10.0)
        capacities.append(_bpsk_capacity_per_ebno(ebno, rate))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    for _ in range(80):
        mid = (lo + hi) / 2.0
        cap = _bpsk_capacity_per_ebno(10.0 ** (mid / 10.0), rate)
        if cap > rate:
            hi = mid
        else:
            lo = mid
