...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            val = np.clip(-snr * y, -500.0, 500.0)
            if val > 0:
                log_term = val / np.log(2) + np.log1p(np.exp(-val)) / np.log(2)
            else:
                log_term = np.log1p(np.exp(val)) / np.log(2)
            return log_term * np.exp(-y ** 2 / 2.0)

        val, _ = integrate.quad(integrand, 0.0, 10.0, limit=200)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    for i in range(len(eb_grid) - 1):
        if caps[i] >= rate and caps[i + 1] < rate:
            t = (rate - caps[i]) / (caps[i + 1] - caps[i])
            return eb_grid[i] + t * (eb_grid[i + 1] - eb_grid[i])
        if caps[i] <= rate and caps[i + 1] >= rate:
            t = (rate - caps[i]) / (caps[i + 1] - caps[i])
            return eb_grid[i] + t * (eb_grid[i + 1] - eb_grid[i])
    return eb_grid[np.argmin(np.abs(np.array(caps) - rate))]
