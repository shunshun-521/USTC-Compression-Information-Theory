...
def _bpsk_mutual_information(snr_linear):
    """
    BPSK 互信息（每信道使用，单位 bit）。
    snr_linear = 2 * R * Eb/N0（线性），对应 sigma^2 = 1/snr_linear。
    """
    if snr_linear <= 0:
        return 0.0
    ...
    def entropy_cond():
        h0, _ = integrate.quad(
            lambda y: -p_y_given_x(y, 1.0) * np.log2(max(p_y_given_x(y, 1.0), eps)),
            -8.0,
            8.0,
            limit=200,
        )
    ...
    def entropy_y():
        def integrand(y):
            py = 0.5 * (p_y_given_x(y, 1.0) + p_y_given_x(y, -1.0))
            return -py * np.log2(max(py, eps))

        val, _ = integrate.quad(integrand, -8.0, 8.0, limit=200)
        return val

    return max(0.0, entropy_y() - entropy_cond())
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        capacities.append(_bpsk_mutual_information(snr))
