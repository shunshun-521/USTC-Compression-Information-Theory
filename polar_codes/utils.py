...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    使用 C = 1 - H_b(0.5 * erfc(sqrt(Eb/N0))) 近似。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        eb_linear = 10.0 ** (eb_n0_db / 10.0)
        p_err = 0.5 * erfc(np.sqrt(eb_linear))
        p_err = np.clip(p_err, 1e-12, 1.0 - 1e-12)
        h_b = -p_err * np.log2(p_err) - (1.0 - p_err) * np.log2(1.0 - p_err)
        capacities.append(max(0.0, 1.0 - h_b))
