...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(t):
            return np.log2(1.0 + np.exp(-gamma * t ** 2)) * np.exp(-t ** 2) / np.sqrt(np.pi)
...
def find_capacity_limit(rate, eb_n0_range=(-35, 5), num_points=1000):
    """
    ...
    caps_lo, caps_hi = caps[idx - 1], caps[idx + 1]
    if abs(caps_hi - caps_lo) < 1e-12:
        return float(eb_grid[idx])
    t = (rate - caps_lo) / (caps_hi - caps_lo)
