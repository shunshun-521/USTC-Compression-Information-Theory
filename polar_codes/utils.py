...
def _bpsk_capacity_per_snr(eb_n0_linear):
    """BPSK 离散输入信道容量（bits/channel use）。"""

    def integrand(y):
        return (
            np.log2(1.0 + np.exp(-2.0 * eb_n0_linear * y ** 2))
            * np.exp(-0.5 * y ** 2)
            / np.sqrt(2.0 * np.pi)
        )
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        capacities.append(_bpsk_capacity_per_snr(eb_lin))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if len(idx) == 0:
        return eb_vals[np.argmin(np.abs(diff))]
    i = idx[0]
