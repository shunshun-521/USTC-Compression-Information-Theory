...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - H(X|Y)，对均匀先验 BPSK 数值积分。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ebno = 10.0 ** (eb_n0_db / 10.0)
        sigma2 = 1.0 / (2.0 * rate * ebno)

        def integrand(y):
            p1 = np.exp(-((y - 1.0) ** 2) / (2.0 * sigma2)) / np.sqrt(
                2.0 * np.pi * sigma2
            )
            pm1 = np.exp(-((y + 1.0) ** 2) / (2.0 * sigma2)) / np.sqrt(
                2.0 * np.pi * sigma2
            )
            py = 0.5 * p1 + 0.5 * pm1
            if py < 1e-300:
        ...
        cond_ent, _ = integrate.quad(integrand, -np.inf, np.inf, limit=200)
        capacities.append(max(0.0, 1.0 - cond_ent))
