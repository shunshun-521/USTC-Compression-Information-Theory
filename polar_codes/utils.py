...
def _log2_one_plus_exp_neg(x):
    ...
    out[~pos] = (-x[~pos] + np.log1p(np.exp(x[~pos]))) / np.log(2.0)
    return out
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        es_n0 = (10.0 ** (eb_n0_db / 10.0)) * rate

        def integrand(z):
            x = 2.0 * es_n0 + 2.0 * np.sqrt(2.0 * es_n0) * z
            return _log2_one_plus_exp_neg(x) * np.exp(-(z ** 2) / 2.0)
...
def find_capacity_limit(rate, eb_n0_range=(0, 12), num_points=2000):
    """
