...
def _j_function(x):
    ...
    def integrand(t):
        ...
        if tx > 20:
            ...
            term = np.log1p(np.exp(tx)) / np.log(2)
        return np.exp(-0.5 * (t - x) ** 2) / np.sqrt(2 * np.pi) * term

    val, _ = integrate.quad(integrand, -12, 12, limit=200)
    return 1.0 - val
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        caps.append(_j_function(np.sqrt(2.0 * snr)))
    return np.array(caps)


def find_capacity_limit(rate, eb_n0_range=(-5, 15), num_points=2000):
