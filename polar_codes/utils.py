...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E_y[log2(1 + exp(-2*snr*y))]，y ~ N(0,1)，snr = 2R*Eb/N0
    """
    capacities = []
...
def find_capacity_limit(rate, eb_n0_range=(-2, 12), num_points=2000):
    ...
    采用 Massey 近似：C(γ) = 1 - H₂(Φ(√γ))，γ = 2R·Eb/N0（线性）
    """
    from scipy.special import erfc

    def cap_db(eb_db):
        gamma = 2.0 * rate * (10.0 ** (eb_db / 10.0))
        p = 0.5 * erfc(np.sqrt(gamma / 2.0))
        p = np.clip(p, 1e-12, 1 - 1e-12)
        h2 = -p * np.log2(p) - (1 - p) * np.log2(1 - p)
        return 1.0 - h2

    eb_grid = np.linspace(eb_n0_range[0], eb_n0_range[1], num_points)
    caps = np.array([cap_db(eb) for eb in eb_grid])
