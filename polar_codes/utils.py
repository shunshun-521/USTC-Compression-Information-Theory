...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    采用数值积分：
        C = 1 - E[log2(1 + exp(-4 * Eb/N0 * R * Y^2))], Y ~ N(0,1)
    """
    eb_n0_db_list = np.atleast_1d(eb_n0_db_list).astype(float)
    capacities = np.empty_like(eb_n0_db_list)
    ...
    for idx, eb_n0_db in enumerate(eb_n0_db_list):
        ...
        def integrand(y):
            val = np.log2(1.0 + np.exp(-snr * y * y))
            val = np.where(-snr * y * y < -100, 0.0, val)
            return val * np.exp(-0.5 * y * y)

        val, _ = integrate.quad(integrand, -20.0, 20.0, limit=200)
        capacities[idx] = 1.0 - val / np.sqrt(2.0 * np.pi)

    return capacities[0] if capacities.size == 1 else capacities


def find_capacity_limit(rate, eb_n0_range=(-2, 6), num_points=2000):
    """找到使 BPSK 信道容量等于码率 R 的 Eb/N0（dB）"""
    lo, hi = eb_n0_range
    cap_lo = compute_bpsk_capacity(lo, rate)
    cap_hi = compute_bpsk_capacity(hi, rate)
