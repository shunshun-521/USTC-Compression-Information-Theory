...
def _capacity_at_eb(eb_n0_db, rate):
    ...
    def integrand(y):
        ...
        return log_term * np.exp(-y ** 2) / np.sqrt(np.pi)
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    return np.array([_capacity_at_eb(eb, rate) for eb in eb_n0_db_list])
...
def find_capacity_limit(rate, eb_n0_range=(-2, 12), num_points=200):
    ...
    if np.isclose(caps[idx], rate, atol=0.02):
        return float(eb_grid[idx])
    lo, hi = eb_n0_range[0], eb_n0_range[1]
    for _ in range(50):
        mid = (lo + hi) / 2.0
        if _capacity_at_eb(mid, rate) > rate:
            hi = mid
        else:
            lo = mid
