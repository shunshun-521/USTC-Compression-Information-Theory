        ...
        for row in results:
            writer.writerow(
                [
                    ...
                    row["num_errors"],
                    row["num_frames"],
                    f"{row['avg_decode_time'] * 1000.0:.6f}",
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for idx, eb_n0_db in enumerate(eb_n0_db_list):
        ...
        es_n0 = 2.0 * rate * eb_n0
        sigma2 = 1.0 / (2.0 * es_n0)
        hy_given_x = 0.5 * np.log2(2.0 * np.pi * np.e * sigma2)

        def py(y):
            norm = 1.0 / np.sqrt(2.0 * np.pi * sigma2)
            return 0.5 * norm * (
                np.exp(-((y - 1.0) ** 2) / (2.0 * sigma2))
                + np.exp(-((y + 1.0) ** 2) / (2.0 * sigma2))
            )
        ...
        hy, _ = integrate.quad(entropy_integrand, -10.0, 10.0, limit=200)
        capacities[idx] = hy - hy_given_x
    return capacities
