...
def _log2_1pexp(x):
    """数值稳定的 log2(1 + exp(x))。"""
    if x > 0:
        return x / np.log(2) + np.log1p(np.exp(-x)) / np.log(2)
...
def _bpsk_capacity_scalar(snr_linear):
    ...
    def integrand(y):
        # BPSK 互信息：对 y~N(0,1) 积分，指数项为 -2*SNR*y^2
        return _log2_1pexp(-2.0 * snr_linear * y * y) * np.exp(-0.5 * y * y)
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        capacities.append(_bpsk_capacity_scalar(snr_linear))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if idx == 0 or idx == num_points - 1:
        ...
        for _ in range(50):
            mid = (lo + hi) / 2.0
            cap = _bpsk_capacity_scalar(2.0 * rate * (10 ** (mid / 10.0)))
            if cap < rate:
                lo = mid
            else:
                hi = mid
