...
def _bpsk_mi_integrand(y, snr):
    """互信息密度被积函数（对标准高斯 y）"""
    s = snr
    p0 = np.exp(-((y - np.sqrt(s)) ** 2) / 2.0) / np.sqrt(2.0 * np.pi)
    p1 = np.exp(-((y + np.sqrt(s)) ** 2) / 2.0) / np.sqrt(2.0 * np.pi)
    py = 0.5 * (p0 + p1)
    with np.errstate(divide="ignore", invalid="ignore"):
        term0 = np.where(p0 > 0, p0 * np.log2(p0 / py), 0.0)
        term1 = np.where(p1 > 0, p1 * np.log2(p1 / py), 0.0)
    return term0 + term1
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = I(X;Y) 通过高斯数值积分计算，SNR = 2R * 10^{Eb/N0/10}
    """
    ...
    for eb in eb_n0_db_list:
        ...
        mi, _ = integrate.quad(
            lambda y: _bpsk_mi_integrand(y, snr),
            -20.0,
            20.0,
            limit=200,
        )
