...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    capacities = []
    y_max = 10.0
    y = np.linspace(-y_max, y_max, 20001)
    dy = y[1] - y[0]
    norm = np.sqrt(2.0 * np.pi)

    for s in snr:
        integrand_vals = integrand(y, s)
        expectation = np.sum(integrand_vals * np.exp(-0.5 * y * y)) * dy / norm
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if idx == 0 or idx == len(eb_grid) - 1:
        for _ in range(40):
            ...
            cap_mid = compute_bpsk_capacity(np.array([mid]), rate)[0]
            if cap_mid > rate:
                eb_n0_range = (eb_n0_range[0], mid)
            else:
                eb_n0_range = (mid, eb_n0_range[1])
        return 0.5 * (eb_n0_range[0] + eb_n0_range[1])
