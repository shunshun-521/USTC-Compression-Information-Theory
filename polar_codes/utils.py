...
def _bpsk_capacity_scalar(snr_linear):
    """单点 BPSK 容量（bits/channel use），蒙特卡洛估计。"""
    if snr_linear <= 0:
        return 0.0
    rng = np.random.default_rng(0)
    n_mc = 200000
    x = rng.choice([-1.0, 1.0], size=n_mc)
    y = np.sqrt(snr_linear) * x + rng.normal(size=n_mc)
    hist, edges = np.histogram(y, bins=256, density=True)
    dx = edges[1] - edges[0]
    p = hist[hist > 0]
    hy = -np.sum(p * np.log2(p)) * dx
    hy_given_x = 0.5 * np.log2(2 * np.pi * np.e)
    return max(0.0, hy - hy_given_x)
