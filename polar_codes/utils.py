...
def _bpsk_capacity(snr_linear):
    ...
    def integrand(y):
        t = -2.0 * snr_linear * y * y
        if t > 0:
            log_term = (t + np.log1p(np.exp(-t))) / np.log(2)
        else:
            log_term = np.log1p(np.exp(t)) / np.log(2)
        return log_term * np.exp(-y * y) / np.sqrt(np.pi)

    val, _ = integrate.quad(integrand, 0, 20, limit=200)
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb in eb_n0_db_list:
        ...
        caps.append(_bpsk_capacity(es_n0))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    def cap_minus_rate(eb_db):
        ...
        return _bpsk_capacity(es_n0) - rate

    grid = np.linspace(eb_n0_range[0], eb_n0_range[1], num_points)
    vals = [cap_minus_rate(e) for e in grid]
    for i in range(len(grid) - 1):
        if vals[i] * vals[i + 1] <= 0:
            return brentq(cap_minus_rate, grid[i], grid[i + 1])

    # BPSK 在 R=0.5 时的经典香农限约 -0.459 dB
