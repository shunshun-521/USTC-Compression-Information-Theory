...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            return _log2_one_plus_exp(-gamma * y * y) * np.exp(-y * y)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    采用 BPSK 在 AWGN 下的经典 Shannon 限近似。
    """
    ebno_linear = (2.0 ** (2.0 * rate) - 1.0) / (2.0 * rate)
    return 10.0 * np.log10(ebno_linear)
