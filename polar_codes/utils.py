...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK 离散输入信道互信息（bits/channel use）。
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            p1 = np.exp(-0.5 * (y - np.sqrt(gamma)) ** 2)
            p2 = np.exp(-0.5 * (y + np.sqrt(gamma)) ** 2)
            p = 0.5 * (p1 + p2)
            if p < 1e-300:
                return 0.0
            return -p * np.log2(p)

        hy, _ = integrate.quad(integrand, -20.0, 20.0, limit=200)
        hn = 0.5 * np.log2(2.0 * np.pi * np.e)
        capacities.append(max(0.0, hy - hn))
