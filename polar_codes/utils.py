...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK-AWGN 互信息（bits/channel use）。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ...
        p0 = np.exp(-0.5 * ((grid - 1.0) / sigma) ** 2)
        p1 = np.exp(-0.5 * ((grid + 1.0) / sigma) ** 2)
        py = 0.5 * (p0 + p1)
        norm = np.trapezoid(py, grid)
        px0 = p0 / (p0 + p1 + 1e-300)
        px1 = p1 / (p0 + p1 + 1e-300)
        h_cond = -np.trapezoid(
            py * (px0 * np.log2(px0 + 1e-300) + px1 * np.log2(px1 + 1e-300)),
            grid,
        ) / norm
