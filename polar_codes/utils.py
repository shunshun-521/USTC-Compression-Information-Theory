...
def _bpsk_capacity_single(snr_linear):
    """计算给定 SNR（线性）下 BPSK 信道容量（bits/channel use）。"""
    y = np.linspace(0.0, 10.0, 4000)
    log_term = np.log2(1.0 + np.exp(-np.sqrt(2.0 * snr_linear) * y))
    pdf = np.exp(-y ** 2 / 2.0) / np.sqrt(2.0 * np.pi)
    integral = 2.0 * np.trapezoid(log_term * pdf, y)
    return 1.0 - integral
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        capacities.append(_bpsk_capacity_single(snr))
...
def plot_bler_curves(results_dict, title, save_path, shannon_limit_db=None,
    ...
    if shannon_limit_db is not None:
        ax.axvline(shannon_limit_db, color="gray", linestyle="--", linewidth=1.0,
