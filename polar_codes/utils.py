...
def _bpsk_mutual_info(snr_linear):
    """BPSK 信道互信息（bits/channel use），snr_linear = Es/N0."""

    def integrand(z):
        pdf = np.exp(-0.5 * z ** 2) / np.sqrt(2.0 * np.pi)
        y = np.sqrt(snr_linear) + z
        llr = 2.0 * np.sqrt(snr_linear) * y
        if llr >= 0:
            p0 = np.exp(-llr) / (1.0 + np.exp(-llr))
        else:
    ...
    return float(np.clip(1.0 - cond_entropy, 0.0, 1.0))
...
def find_capacity_limit(rate, eb_n0_range=(-2, 10), num_points=2000):
    ...
    return float(e0 - c0 * (e1 - e0) / (c1 - c0))
