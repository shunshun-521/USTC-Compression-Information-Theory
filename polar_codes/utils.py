...
def bpsk_capacity(eb_n0_db, rate):
    ...
    integrand = np.log2(1.0 + es_n0 * np.sin(phi) ** 2)
    return float(np.sum(integrand) * (phi[1] - phi[0]) / np.pi)


def compute_bpsk_capacity(eb_n0_db_list, rate):
    """计算多个 Eb/N0 点的 BPSK 容量"""
...
def find_capacity_limit(rate, eb_n0_range=(-2, 8), num_points=500):
    ...
    caps = [bpsk_capacity(eb, rate) for eb in eb_vals]
