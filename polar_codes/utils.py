...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            return np.log2(1.0 + np.exp(-2.0 * snr * np.abs(y))) * np.exp(-(y ** 2) / 2.0)

        val, _ = integrate.quad(integrand, 0.0, np.inf)
        val2, _ = integrate.quad(integrand, -np.inf, 0.0)
        ey = val + val2
        capacities.append(1.0 - ey / np.sqrt(2.0 * np.pi))
