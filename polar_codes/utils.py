...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            term0 = np.where(p0 > 0, p0 * np.log2(np.where(py > 0, py / p0, 1.0)), 0.0)
            term1 = np.where(p1 > 0, p1 * np.log2(np.where(py > 0, py / p1, 1.0)), 0.0)
        dy = y[1] - y[0]
        caps.append(float(-0.5 * np.trapezoid(term0 + term1, dx=dy)))
