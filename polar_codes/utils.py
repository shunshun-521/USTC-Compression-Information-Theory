...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - (2/sqrt(2*pi)) * integral_0^inf log2(1+exp(-2*gamma*x^2)) * exp(-x^2/2) dx
    其中 gamma = (Eb/N0)_linear * R
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        gamma = (10 ** (eb_n0_db / 10)) * rate

        def integrand(x):
            t = -2 * gamma * x * x
            log_term = np.log2(1 + np.exp(np.clip(t, -700, 700)))
            return log_term * np.exp(-x ** 2 / 2)

        val, _ = integrate.quad(integrand, 0, 20, limit=200)
        val = 2 * val / np.sqrt(2 * np.pi)
