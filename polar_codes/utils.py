...
def _bpsk_mi_integrand(y, snr):
    """BPSK 互信息积分被积函数（bits/channel use）。"""
    gauss = np.exp(-0.5 * y * y) / np.sqrt(2.0 * np.pi)
    inner = _log2_one_plus_exp(-np.sqrt(2.0 * snr) * np.abs(y))
    return gauss * inner
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E[log2(1 + exp(-2*sqrt(SNR)*Z))], Z~N(0,1)
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10.0 ** (eb_n0_db / 10.0))
        integral, _ = integrate.quad(
            lambda y: _bpsk_mi_integrand(y, snr), -50.0, 50.0
        )
