...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def p0(y):
            return np.exp(-(y - 1.0) ** 2 / (2.0 * sigma ** 2)) / (sigma * np.sqrt(2.0 * np.pi))
        ...
        def integrand(y):
            ...
            if py < 1e-300:
                return 0.0
            return 0.5 * p0y * np.log2(p0y / py) + 0.5 * p1y * np.log2(p1y / py)
...
def save_frozen_set_info(N_list, K, design_eb_n0_db, save_path):
    ...
    with open(save_path, 'w') as f:
        for N in N_list:
            ...
            f.write(np.array2string(info_idx, threshold=1000, max_line_width=120) + '\n')
            f.write(f'Frozen indices (all {len(frozen_idx)}):\n')
            f.write(np.array2string(frozen_idx, threshold=1000, max_line_width=120) + '\n')
            f.write('-' * 53 + '\n')
