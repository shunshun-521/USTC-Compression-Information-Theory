...
def bpsk_capacity(snr_linear):
    ...
    def integrand(y):
        return np.log2(1.0 + np.exp(-np.sqrt(2.0 * snr_linear) * np.abs(y))) * np.exp(-y ** 2 / 2.0)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    for i in range(len(eb_vals) - 1):
        if caps[i] <= rate <= caps[i + 1] or caps[i] >= rate >= caps[i + 1]:
            ...
            t = (rate - caps[i]) / (caps[i + 1] - caps[i]) if caps[i + 1] != caps[i] else 0
...
def save_frozen_set_info(N_list, K_func, design_eb_n0_db, save_path):
    ...
    with open(save_path, 'w') as f:
        for N in N_list:
            K = K_func(N) if callable(K_func) else (N // 2 if K_func is None else K_func)
            rate = K / N
