...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - (1/√π) ∫₀^∞ log₂(1 + exp(-η x²)) dx, η = 2R·10^(Eb/N0/10)
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        eta = 2.0 * rate * (10 ** (eb_n0_db / 10.0))

        def integrand(x, _eta=eta):
            z = np.clip(-_eta * x * x, -700.0, 700.0)
            return np.log2(1.0 + np.exp(z)) / np.sqrt(np.pi)
