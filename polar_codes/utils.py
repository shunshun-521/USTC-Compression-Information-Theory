...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    capacities = []
    y = np.linspace(-10.0, 10.0, 200000)
    eps = 1e-300
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        py = 0.5 * p0 + 0.5 * p1 + eps
        hxy = 0.0
        for px_y in (0.5 * p0 / py, 0.5 * p1 / py):
            term = np.zeros_like(y)
            mask = px_y > eps
            term[mask] = -px_y[mask] * np.log2(px_y[mask])
            hxy += np.trapezoid(term, y)
        capacities.append(max(0.0, 1.0 - hxy))
