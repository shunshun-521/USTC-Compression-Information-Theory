...
def _bpsk_capacity_scalar(snr_linear):
    """单点 BPSK 互信息（bits/channel use），高斯数值积分。"""
    sigma = 1.0 / np.sqrt(snr_linear)
    norm = sigma * np.sqrt(2.0 * np.pi)

    def p_y_given_x(y, x):
        mean = 1.0 if x == 0 else -1.0
        return np.exp(-((y - mean) ** 2) / (2.0 * sigma ** 2)) / norm

    def entropy_y(y):
        p0 = p_y_given_x(y, 0)
        p1 = p_y_given_x(y, 1)
        py = 0.5 * (p0 + p1)
        return -py * np.log2(py + 1e-300)

    def entropy_y_given_x(y):
        p0 = p_y_given_x(y, 0)
        p1 = p_y_given_x(y, 1)
        return -0.5 * (p0 * np.log2(p0 + 1e-300) + p1 * np.log2(p1 + 1e-300))

    hy, _ = integrate.quad(entropy_y, -20.0, 20.0, limit=200)
    hyx, _ = integrate.quad(entropy_y_given_x, -20.0, 20.0, limit=200)
    return hy - hyx
...
def plot_bler_curves(
    ...
    for label, results in results_dict.items():
        ...
        ax.semilogy(eb, bler, marker="o", label=label)
