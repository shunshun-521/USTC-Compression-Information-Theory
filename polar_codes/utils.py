  ...
  C = 1 - E_{y}[log2(1 + e^{-2*s*y})]，其中 s = SNR = 2R * 10^{Eb/N0/10}
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            ...
            llr = 2.0 * y / (sigma ** 2)
            return pdf * np.log2(1.0 + np.exp(-llr))
        ...
        c_pos, _ = integrate.quad(integrand, -np.inf, np.inf, limit=200)
        c_neg, _ = integrate.quad(integrand_neg, -np.inf, np.inf, limit=200)
        capacities.append(1.0 - 0.5 * (c_pos + c_neg))
