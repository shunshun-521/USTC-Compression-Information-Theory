...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = (1/pi) * int_0^{pi/2} log2(1 + (Eb/N0 * R) / sin^2(theta)) d(theta)
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr = rate * (10 ** (eb_n0_db / 10.0))

        def integrand(theta):
            s = np.sin(theta)
            return np.log2(1.0 + snr / (s * s)) / np.pi

        cap, _ = integrate.quad(integrand, 1e-10, np.pi / 2.0)
        capacities.append(cap)
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-12, 20), num_points=2000):
