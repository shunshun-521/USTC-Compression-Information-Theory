...
def _bpsk_capacity_per_ebn0(eb_n0_db, rate):
    ...
    def integrand(y):
        return np.log2(1.0 + np.exp(-snr * y ** 2)) * np.exp(-y ** 2 / 2.0)

    val, _ = integrate.quad(integrand, 0.0, np.inf, limit=200)
    return 1.0 - (2.0 / np.sqrt(2.0 * np.pi)) * val
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    return np.array([_bpsk_capacity_per_ebn0(e, rate) for e in eb_n0_db_list])
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if idx == 0 or idx == len(eb_vals) - 1:
        ...
        for _ in range(50):
            mid = (lo + hi) / 2.0
            if _bpsk_capacity_per_ebn0(mid, rate) < rate:
                lo = mid
            else:
                hi = mid
        return (lo + hi) / 2.0
