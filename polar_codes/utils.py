...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """计算 BPSK 离散输入信道容量列表。"""
    return [_bpsk_capacity_scalar(eb, rate) for eb in eb_n0_db_list]


def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    """找到使 BPSK 信道容量等于码率 R 的 Eb/N0（dB）。"""
    eb_vals = np.linspace(eb_n0_range[0], eb_n0_range[1], num_points)
    caps = np.array([_bpsk_capacity_scalar(eb, rate) for eb in eb_vals])
    idx = np.argmin(np.abs(caps - rate))
    if idx > 0 and idx < len(eb_vals) - 1:
        e0, e1 = eb_vals[idx - 1], eb_vals[idx + 1]
        c0 = _bpsk_capacity_scalar(e0, rate)
        c1 = _bpsk_capacity_scalar(e1, rate)
        if abs(c1 - c0) > 1e-12:
