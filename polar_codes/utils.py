...
def save_results_csv(results, filepath):
    ...
    fieldnames = ['eb_n0_db', 'bler', 'ber', 'num_errors', 'num_frames',
...
def compute_bpsk_capacity(eb_n0_db, rate):
    ...
    C = 1 - 2 * ∫_0^∞ φ(y) log2(1 + exp(-2*SNR*y)) dy
    ...
    def integrand(y):
        ...
        return log_term * np.exp(-y ** 2 / 2) / np.sqrt(2.0 * np.pi)

    val, _ = integrate.quad(integrand, 0, 30)
    return 1.0 - 2.0 * val


def find_capacity_limit(rate, eb_n0_range=(-3, 10), num_points=200):
