...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        sigma = 1.0 / np.sqrt(2.0 * rate * ebn0)
        scale = np.sqrt(2.0 * np.pi) * sigma

        def p_y_given_x(y, x):
            s = 1.0 - 2.0 * x
            return np.exp(-((y - s) ** 2) / (2.0 * sigma ** 2)) / scale

        def p_y(y):
            return 0.5 * p_y_given_x(y, 0) + 0.5 * p_y_given_x(y, 1)

        def conditional_entropy_integrand(y):
            py = p_y(y)
            if py < 1e-300:
                return 0.0
            h = 0.0
            for x in (0, 1):
                px = 0.5 * p_y_given_x(y, x)
                if px > 0.0:
                    h -= (px / py) * np.log2(px / py)
            return h * py

        h_x_given_y, _ = integrate.quad(
            conditional_entropy_integrand,
            -10.0 * sigma,
            10.0 * sigma,
            limit=200,
        )
        capacities.append(1.0 - h_x_given_y)
...
def plot_bler_curves(
    ...
    for label, results in results_dict.items():
        ...
        ax.semilogy(eb, bler, marker="o", linewidth=1.5, label=label)
