...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(z):
            llr = 2.0 * gamma + 2.0 * np.sqrt(gamma) * z
            log_term = np.log1p(np.exp(-llr)) / np.log(2.0)
            return log_term * np.exp(-0.5 * z * z)

        val, _ = integrate.quad(integrand, -15.0, 15.0, limit=100)
