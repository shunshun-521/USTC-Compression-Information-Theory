...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK-AWGN 香农容量近似（bits/channel use）。
    使用连续输入高斯近似：C = 0.5 * log2(1 + SNR)，SNR = 2*R*10^(Eb/N0/10)。
    """
...
def find_capacity_limit(rate, eb_n0_range=(-2, 12), num_points=2000):
    ...
    # 解析解：R = 0.5*log2(1+2*R*10^(Eb/N0/10)) => Eb/N0 = 10*log10((2^(2R)-1)/(2R))
    snr_req = 2.0 ** (2.0 * rate) - 1.0
    eb_linear = snr_req / (2.0 * rate)
