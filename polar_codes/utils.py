...
def compute_bpsk_capacity(eb_n0_db_list, rate, n_samples=200000, seed=0):
    """
    计算 BPSK-AWGN 信道互信息（bits/channel use），蒙特卡洛估计。
    I = 1 - E[log2(1 + exp(-2*x*y/sigma^2))]
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        mi = 1.0 - np.mean(np.log2(1.0 + np.exp(z)))
...
def find_capacity_limit(rate, eb_n0_range=(-2, 8)):
    ...
    for _ in range(30):
        mid = (lo + hi) / 2.0
        cap = compute_bpsk_capacity([mid], rate, n_samples=100000)[0]
        if cap > rate:
            hi = mid
        else:
            lo = mid
