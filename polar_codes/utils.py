...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(x):
            return np.exp(-(x ** 2)) * np.log2(1.0 + np.exp(-2.0 * np.sqrt(snr) * x))

        val, _ = integrate.quad(integrand, 0.0, np.inf)
        c = 1.0 - (2.0 / np.sqrt(np.pi)) * val
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    lo = eb_grid[max(idx - 1, 0)]
    hi = eb_grid[min(idx + 1, len(eb_grid) - 1)]
    for _ in range(50):
        mid = (lo + hi) / 2.0
        cap = compute_bpsk_capacity(mid, rate)
        cap = float(np.atleast_1d(cap)[0])
        if cap > rate:
            lo = mid
        else:
            hi = mid
