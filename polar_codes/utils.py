            ...
            results.append(
                {
                    ...
                    "avg_iters": float(avg_iters) if avg_iters.strip() else None,
                }
            )
    return results
...
def bpsk_capacity(eb_n0_db, rate):
    ...
    def integrand(y):
        t = -2.0 * snr * y
        if t > 0:
            log_term = t + np.log2(1.0 + np.exp(-t))
        else:
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """返回各 Eb/N0 点的 BPSK 容量。"""
    return [bpsk_capacity(eb, rate) for eb in eb_n0_db_list]


def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    """找到使 BPSK 信道容量等于码率 R 的 Eb/N0（dB）。"""
    eb_lo, eb_hi = eb_n0_range
    eb_grid = np.linspace(eb_lo, eb_hi, num_points)
    caps = [bpsk_capacity(eb, rate) for eb in eb_grid]
    ...
    idx = int(np.argmin(np.abs(np.array(caps) - rate)))
    return eb_grid[idx]
