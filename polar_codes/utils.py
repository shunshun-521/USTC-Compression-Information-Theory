  ...
  使用对称 BPSK 输入分布的精确互信息公式。
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            ...
            pm = np.exp(-0.5 * ((y + 1.0) / sigma) ** 2)
            t1 = np.logaddexp(0.0, -4.0 * y * inv_var) / np.log(2.0)
            t2 = np.logaddexp(0.0, 4.0 * y * inv_var) / np.log(2.0)
            return 0.5 * (p1 * t1 + pm * t2) / (np.sqrt(2.0 * np.pi) * sigma)
