...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK-AWGN 对称信道互信息（bits/channel use）。
    I = 1 - E[log2(1 + exp(-|LLR|))], LLR = 2y/sigma^2, y ~ N(+1, sigma^2)
    """
    eb_n0_db_list = np.atleast_1d(eb_n0_db_list)
    capacities = []
    y_grid = np.linspace(-8.0, 8.0, 4001)

    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10.0 ** (eb_n0_db / 10.0))
        sigma = 1.0 / np.sqrt(snr)
        pdf0 = np.exp(-((y_grid - 1.0) ** 2) / (2.0 * sigma ** 2))
        pdf1 = np.exp(-((y_grid + 1.0) ** 2) / (2.0 * sigma ** 2))
        llr = 2.0 * y_grid / (sigma ** 2)
        metric = np.log2(1.0 + np.exp(-np.abs(llr)))
        term0 = np.trapezoid(metric * pdf0, y_grid) / np.trapezoid(pdf0, y_grid)
        term1 = np.trapezoid(metric * pdf1, y_grid) / np.trapezoid(pdf1, y_grid)
        mi = 1.0 - 0.5 * (term0 + term1)
        capacities.append(float(np.clip(mi, 0.0, 1.0)))
