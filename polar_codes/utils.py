...
def _stable_log2_one_plus_exp_neg(z):
    """数值稳定的 log2(1 + exp(-z))"""
    return np.logaddexp(0.0, -z) / np.log(2)


def _bpsk_capacity_single(eb_n0_db, rate):
    """单点 BPSK 容量（bits/channel use），Es/N0 = rate * Eb/N0"""
    gamma = rate * (10.0 ** (eb_n0_db / 10.0))

    def integrand(y):
        z = 4.0 * gamma * y * y
        return _stable_log2_one_plus_exp_neg(z) * np.exp(-0.5 * y ** 2) / np.sqrt(2.0 * np.pi)
