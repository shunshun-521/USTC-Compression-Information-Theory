...
def _log2_1_plus_exp(x):
    ...
    return np.where(
        x > 0,
        x / np.log(2) + np.log2(1.0 + np.exp(-x)),
        np.log2(1.0 + np.exp(x)),
    )
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    capacities = []
    y_grid = np.linspace(-12, 12, 4000)
    pdf = np.exp(-0.5 * y_grid ** 2) / np.sqrt(2 * np.pi)

    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10.0 ** (eb_n0_db / 10.0))
        integrand = _log2_1_plus_exp(-2.0 * snr * np.abs(y_grid))
        c = 1.0 - _trapz(integrand * pdf, y_grid)
        capacities.append(c)
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-2, 4)):
    """找到使 BPSK 信道容量等于码率 R 的 Eb/N0（dB）。"""
    lo, hi = eb_n0_range
    for _ in range(60):
        mid = (lo + hi) / 2.0
        c = float(compute_bpsk_capacity(np.array([mid]), rate)[0])
        if c > rate:
            hi = mid
        else:
            lo = mid
