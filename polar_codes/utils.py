...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    估计 BPSK-AWGN 信道容量（bits/channel use）。

    采用 AWGN 经典容量公式 C = 0.5 * log2(1 + 2*R*Eb/N0) 作为参考曲线，
    用于标注香农限（与极化码文献中常用参考一致）。
    """
    eb_n0_db_list = np.atleast_1d(eb_n0_db_list)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10 ** (eb_n0_db / 10.0))
        capacities.append(0.5 * np.log2(1.0 + snr))
    return np.array(capacities)


def _capacity_scalar(eb_n0_db, rate):
    return float(compute_bpsk_capacity(eb_n0_db, rate).reshape(-1)[0])
...
def find_capacity_limit(rate, eb_n0_range=(-2, 12), num_points=2000):
    ...
    """
    lo, hi = eb_n0_range
    caps_lo = _capacity_scalar(lo, rate)
    caps_hi = _capacity_scalar(hi, rate)
    if caps_lo >= rate:
        return lo
    if caps_hi <= rate:
        return hi
    for _ in range(40):
        mid = (lo + hi) / 2.0
        if _capacity_scalar(mid, rate) < rate:
            lo = mid
        else:
            hi = mid
    return (lo + hi) / 2.0
