...
def _bpsk_mutual_info(snr_linear):
    """BPSK 信道互信息 I(X;Y) for SNR (linear)."""

    def integrand(y):
        p_y = 0.5 * np.exp(-(y - np.sqrt(snr_linear)) ** 2 / 2) / np.sqrt(2 * np.pi)
        p_y += 0.5 * np.exp(-(y + np.sqrt(snr_linear)) ** 2 / 2) / np.sqrt(2 * np.pi)
        if p_y < 1e-300:
            return 0.0
        px0 = 0.5 * np.exp(-(y - np.sqrt(snr_linear)) ** 2 / 2) / np.sqrt(2 * np.pi) / p_y
        px1 = 0.5 * np.exp(-(y + np.sqrt(snr_linear)) ** 2 / 2) / np.sqrt(2 * np.pi) / p_y
        mi = 0.0
        if px0 > 1e-300:
            mi += px0 * np.log2(px0)
        if px1 > 1e-300:
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        capacities.append(_bpsk_mutual_info(snr))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if idx == 0 or idx == num_points - 1:
        ...
        for _ in range(50):
            mid = (lo + hi) / 2
            cap_mid = _bpsk_mutual_info(2 * rate * 10 ** (mid / 10))
            if cap_mid < rate:
                lo = mid
            else:
                hi = mid
