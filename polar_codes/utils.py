...
def _bpsk_capacity_scalar(eb_n0_db, rate):
    ...
    def integrand(y):
        t = -snr * y ** 2
        if t > 0:
            log2_term = t / np.log(2.0) + np.log1p(np.exp(-t)) / np.log(2.0)
        else:
            log2_term = np.log1p(np.exp(t)) / np.log(2.0)
        return log2_term * np.exp(-0.5 * y ** 2)
...
def find_capacity_limit(rate, eb_n0_range=(0, 6), num_points=200):
    """找到使 BPSK 信道容量等于码率 R 的 Eb/N0（dB）"""
    lo, hi = eb_n0_range
    caps_lo = _bpsk_capacity_scalar(lo, rate) - rate
    caps_hi = _bpsk_capacity_scalar(hi, rate) - rate
