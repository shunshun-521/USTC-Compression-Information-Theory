...
def compute_bpsk_capacity(eb_n0_db_list, rate=None):
    ...
    使用经典积分形式：
    C(γ) = (1/π) ∫₀^{π/2} log₂(1 + γ / sin²θ) dθ
    其中 γ = Eb/N0（线性）。
    """
    eb_n0_db_list = np.asarray(eb_n0_db_list, dtype=np.float64)
    capacities = np.zeros_like(eb_n0_db_list)
    eps = 1e-9
    for i, eb_n0_db in enumerate(eb_n0_db_list):
        gamma = 10.0 ** (eb_n0_db / 10.0)

        def integrand(theta):
            s = np.sin(theta)
            return np.log2(1.0 + gamma / (s * s))

        val, _ = integrate.quad(integrand, eps, np.pi / 2.0 - eps, limit=200)
        capacities[i] = val / np.pi
    return capacities


def find_capacity_limit(rate, eb_n0_range=(-12, 8), num_points=500):
    """
    ...
    if k == 0:
        return float(eb_grid[0])
    lo, hi = eb_grid[k - 1], eb_grid[k]
    c_lo, c_hi = caps[k - 1], caps[k]
    if c_hi == c_lo:
        return float(lo)
    frac = (rate - c_lo) / (c_hi - c_lo)
