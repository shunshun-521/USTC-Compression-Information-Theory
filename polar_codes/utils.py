    ...
    with open(filepath, 'r') as f:
        ...
        for row in reader:
            results.append({
                ...
                'avg_iters': None if not row['avg_iters'] else float(row['avg_iters']),
            })
    return results
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        integrand = lambda y: np.log2(1.0 + np.exp(-2.0 * snr * y)) * np.exp(-y ** 2 / 2.0)
        val, _ = integrate.quad(integrand, -np.inf, np.inf)
        val = 1.0 - val / np.sqrt(2.0 * np.pi)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    for i in range(1, len(eb_grid)):
        if caps[i] >= rate and caps[i - 1] < rate:
            ...
            return eb_grid[i - 1] + t * (eb_grid[i] - eb_grid[i - 1])
    return eb_grid[-1]
