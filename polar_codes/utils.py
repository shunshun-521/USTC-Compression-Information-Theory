...
def _bpsk_mi(eb_n0_db, rate):
    """BPSK 互信息（蒙特卡洛估计，用于容量限搜索）。"""
    eb_n0_lin = 10.0 ** (eb_n0_db / 10.0)
    sigma2 = 1.0 / (2.0 * rate * eb_n0_lin)
    sigma = np.sqrt(sigma2)
    rng = np.random.default_rng(0)
    n = 200000
    x = rng.integers(0, 2, n) * 2 - 1
    y = x + rng.normal(0.0, sigma, n)
    llr = 2.0 * y / sigma2
    t = np.logaddexp(0.0, -np.abs(llr)) / np.log(2.0)
    return 1.0 - np.mean(t)


def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK 离散输入信道容量（bits/channel use）。
    使用与仿真一致的 AWGN 模型进行互信息估计。
    """
    eb_n0_db_list = np.atleast_1d(eb_n0_db_list)
    return np.array([_bpsk_mi(eb, rate) for eb in eb_n0_db_list])


def find_capacity_limit(rate, eb_n0_range=(-8, 4)):
    """
