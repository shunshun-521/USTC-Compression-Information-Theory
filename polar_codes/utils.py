...
def compute_bpsk_capacity(eb_n0_db_list, rate=None):
    ...
    I(X;Y) = H(Y) - H(N)，符号为 ±1，Eb/N0 为每信息比特能量比。
    """
    _ = rate  # 保留接口兼容，BPSK 每符号 1 比特时与码率无关
    eb_n0_db_list = np.atleast_1d(eb_n0_db_list)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        gamma = 10.0 ** (eb_n0_db / 10.0)
        var = 1.0 / (2.0 * gamma)
        noise_entropy = 0.5 * np.log2(2.0 * np.pi * np.e * var)

        def output_pdf(y):
            return (
                0.5
                * (
                    np.exp(-((y - 1.0) ** 2) / (2.0 * var))
                    + np.exp(-((y + 1.0) ** 2) / (2.0 * var))
                )
        ...
        output_entropy, _ = integrate.quad(integrand, -15.0, 15.0)
        capacities.append(output_entropy - noise_entropy)
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-3, 5), num_points=1000):
