...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            # BPSK 容量标准公式：C = 1 - E[log2(1 + exp(-2*SNR*y^2))]
            t = np.clip(-2.0 * snr * y ** 2, -700.0, 700.0)
            log2_term = np.logaddexp(0.0, t) / np.log(2.0)
            return log2_term * np.exp(-0.5 * y ** 2)

        val, _ = integrate.quad(integrand, -50.0, 50.0, limit=200)
        capacities.append(1.0 - val / np.sqrt(2 * np.pi))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if idx >= len(eb_grid):
        return eb_grid[-1]
    return float(np.interp(rate, [caps[idx - 1], caps[idx]], [eb_grid[idx - 1], eb_grid[idx]]))
