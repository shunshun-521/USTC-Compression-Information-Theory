    ...
    with open(filepath, "w", newline="", encoding="utf-8") as f:
        ...
        for r in results:
            ...
            writer.writerow([
                ...
                f"{avg_iters:.4f}" if avg_iters is not None else "",
            ])
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E_y[log2(1 + exp(-|LLR(y)|))]，其中 LLR(y) = 2y/sigma^2。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ebn0 = 10 ** (eb_n0_db / 10.0)
        esn0 = ebn0 * rate
        sigma = 1.0 / np.sqrt(2.0 * esn0)

        def integrand(y):
            llr = 2.0 * y / (sigma ** 2)
            pdf = np.exp(-y ** 2 / (2.0 * sigma ** 2)) / (np.sqrt(2.0 * np.pi) * sigma)
            return pdf * np.log2(1.0 + np.exp(-np.abs(llr)))

        ent, _ = integrate.quad(integrand, -np.inf, np.inf, limit=200)
        capacities.append(1.0 - ent)
    return np.asarray(capacities)


def find_capacity_limit(rate, eb_n0_range=(-3, 10), num_points=1000):
    """
