...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        py0 = norm * np.exp(-(y_grid - 1.0) ** 2 / (2.0 * sigma ** 2))
        py1 = norm * np.exp(-(y_grid + 1.0) ** 2 / (2.0 * sigma ** 2))
        py = 0.5 * py0 + 0.5 * py1
        py = np.maximum(py, 1e-300)
        py0 = np.maximum(py0, 1e-300)
        py1 = np.maximum(py1, 1e-300)
        mi = 0.5 * np.sum(py0 * np.log2(py0 / py)) * dy
        mi += 0.5 * np.sum(py1 * np.log2(py1 / py)) * dy
