...
def _bpsk_capacity_per_eb(eb_n0_db, rate):
    """给定 Eb/N0 (dB)，计算 BPSK 信道容量 (bits/channel use)"""
    snr = 10.0 ** (eb_n0_db / 10.0)

    def integrand(y):
        val = np.log2(1.0 + np.exp(-2.0 * snr * y * y))
        return val * np.exp(-0.5 * y ** 2)

    integral, _ = integrate.quad(integrand, -15.0, 15.0, limit=200)
    return 1.0 - integral / np.sqrt(2.0 * np.pi)


def compute_bpsk_capacity(eb_n0_db_list, rate):
    """计算 BPSK 离散输入信道容量（bits/channel use）"""
    eb_n0_db_list = np.atleast_1d(eb_n0_db_list)
    return np.array([_bpsk_capacity_per_eb(eb, rate) for eb in eb_n0_db_list])


def find_capacity_limit(rate, eb_n0_range=(-2, 8), num_points=200):
    """找到使 BPSK 信道容量等于码率 R 的 Eb/N0（dB）"""
    lo, hi = eb_n0_range
    cap_lo = _bpsk_capacity_per_eb(lo, rate) - rate
    cap_hi = _bpsk_capacity_per_eb(hi, rate) - rate
    if cap_lo * cap_hi > 0:
    ...
    for _ in range(60):
        mid = (lo + hi) / 2.0
        if _bpsk_capacity_per_eb(mid, rate) < rate:
            lo = mid
        else:
            hi = mid
    return (lo + hi) / 2.0
