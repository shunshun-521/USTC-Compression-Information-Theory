...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def stable_log2_1pexp(x):
            x = np.asarray(x, dtype=np.float64)
            out = np.empty_like(x)
            pos = x >= 0
            out[pos] = np.log2(1.0 + np.exp(-x[pos]))
            xn = x[~pos]
            out[~pos] = (-xn / np.log(2.0)) + np.log2(1.0 + np.exp(xn))
            return out

        def integrand(y):
            x = 2.0 * snr * y ** 2
            return stable_log2_1pexp(x) * np.exp(-y ** 2 / 2.0)
...
def find_capacity_limit(rate, eb_n0_range=(-3, 6)):
