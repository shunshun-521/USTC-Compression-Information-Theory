...
def _log2_1pexp(z):
    """数值稳定的 log2(1 + exp(z))"""
    return np.maximum(z, 0.0) / np.log(2.0) + np.log1p(np.exp(-np.abs(z))) / np.log(2.0)


def _bpsk_mi(snr_lin):
    """BPSK 互信息（bits/channel use），Monte Carlo 估计"""
    rng = np.random.default_rng(0)
    y = rng.standard_normal(200000)
    log_terms = _log2_1pexp(-np.abs(2.0 * snr_lin * y))
    return max(0.0, 1.0 - np.mean(log_terms))


def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK 离散输入信道容量（bits/channel use）。
    C = 1 - E_y[log2(1 + exp(-2*snr*y))], snr = 2*R*Eb/N0
    """
    capacities = []
