...
def _bpsk_mi_integrand(y, snr):
  """互信息密度被积函数"""
  p0 = np.exp(-0.5 * (y - np.sqrt(snr)) ** 2) / np.sqrt(2 * np.pi)
  p1 = np.exp(-0.5 * (y + np.sqrt(snr)) ** 2) / np.sqrt(2 * np.pi)
  py = 0.5 * (p0 + p1)
  with np.errstate(divide="ignore", invalid="ignore"):
    term = np.where(py > 0, py * np.log2(py), 0.0)
  return -term
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = I(X;Y) = H(Y) - H(Y|X)，通过高斯数值积分计算。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10 ** (eb_n0_db / 10.0))
        h_y, _ = integrate.quad(
            lambda y: _bpsk_mi_integrand(y, snr), -20.0, 20.0, limit=200
        )
        h_y_given_x = 0.5 * np.log2(2 * np.pi * np.e)
...
def save_frozen_set_info(N_list, K, design_eb_n0_db, save_path):
    ...
    with open(save_path, "w") as f:
        for N in N_list:
            ...
            f.write(
                np.array2string(frozen_idx, threshold=N, max_line_width=120) + "\n"
            )
            f.write("-" * 53 + "\n")
