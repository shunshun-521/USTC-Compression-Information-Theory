...
def compute_bpsk_capacity(eb_n0_db, rate):
    """
    计算 BPSK-AWGN 信道每信道使用的互信息（bits/channel use）。
    与 channel.py 中 sigma = 1/sqrt(2*R*Eb/N0) 的归一化一致。
    """
    eb_lin = 10 ** (eb_n0_db / 10.0)
    snr = 2.0 * rate * eb_lin
    sigma = 1.0 / np.sqrt(snr)

    y_max = 12.0 * sigma + 2.0
    y = np.linspace(-y_max, y_max, 200000)
    dy = y[1] - y[0]

    p_y_given_0 = np.exp(-(y - 1) ** 2 / (2 * sigma ** 2)) / (sigma * np.sqrt(2 * np.pi))
    p_y_given_1 = np.exp(-(y + 1) ** 2 / (2 * sigma ** 2)) / (sigma * np.sqrt(2 * np.pi))
    p_y = 0.5 * p_y_given_0 + 0.5 * p_y_given_1
    p0_y = 0.5 * p_y_given_0 / p_y
    p1_y = 0.5 * p_y_given_1 / p_y

    cond_entropy = -np.sum(
        (p0_y * np.log2(p0_y + 1e-300) + p1_y * np.log2(p1_y + 1e-300)) * p_y
    ) * dy
    return 1.0 - cond_entropy
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    return eb_vals[np.argmin(np.abs(np.array(caps) - rate))]
