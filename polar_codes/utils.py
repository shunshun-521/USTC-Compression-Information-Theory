...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_db in eb_n0_db_list:
        ...
        caps.append(np.log2(1.0 + snr))
    return np.array(caps)


def find_capacity_limit(rate, eb_n0_range=(-5, 5), num_points=2000):
    """
    使信道容量等于码率 R 的 Eb/N0（dB）。
    基于 C = log2(1 + 2R·Eb/N0) 的 BPSK-AWGN 香农限。
    """
    eb_lin = (2.0 ** rate - 1.0) / (2.0 * rate)
    return float(10.0 * np.log10(eb_lin))
...
def plot_bler_curves(
    ...
    if shannon_limit_db is not None:
        ax.axvline(shannon_limit_db, color="gray", linestyle="--", label=f"Capacity R (Eb/N0={shannon_limit_db:.2f} dB)")
