...
def _bpsk_capacity_scalar(eb_n0_db, rate):
    ...
    def integrand(y):
        p_y = (1.0 / (np.sqrt(2 * np.pi) * sigma)) * np.exp(-((y - 1.0) ** 2) / (2 * sigma ** 2))
        llr = 2.0 * y / (sigma ** 2)
        # 数值稳定：log2(1 + exp(-llr))
        if llr >= 0:
            term = np.log2(1.0 + np.exp(-llr))
        else:
            term = np.log2(1.0 + np.exp(llr)) - llr / np.log(2)
        return p_y * term

    cap, _ = integrate.quad(integrand, -10 * sigma - 5, 10 * sigma + 5, limit=200)
    return 1.0 - cap
