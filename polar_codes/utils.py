...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E_t[log2(1 + exp(-SNR * t^2))], t ~ N(0,1),
    SNR = Es/N0 = 2R * 10^(Eb/N0/10)。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(t):
            z = -snr * t ** 2
            return _log2_one_plus_exp(z) * np.exp(-0.5 * t ** 2) / np.sqrt(2.0 * np.pi)
...
def plot_bler_curves(
    ...
    """绘制 BLER-Eb/N0 曲线（对数纵轴），并保存 PNG/PDF。"""
