...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            p_y = 0.5 * norm.pdf(y, 1, sigma) + 0.5 * norm.pdf(y, -1, sigma)
            llr_mag = 2.0 * np.abs(y) / (sigma ** 2)
            log_term = np.log1p(np.exp(-llr_mag)) / np.log(2.0)
            return p_y * log_term

        h_cond, _ = integrate.quad(integrand, -30.0, 30.0)
        capacities.append(max(0.0, 1.0 - h_cond))
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-6, 10), num_points=1000):
