...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - 1/sqrt(pi) * int exp(-y^2) log2(1+exp(-4*snr*y^2)) dy
  """
    capacities = []
...
def find_capacity_limit(rate, eb_n0_range=(-3, 8), num_points=500):
    """找到使 BPSK 信道容量等于码率 R 的 Eb/N0（dB）"""
    eb_grid = np.linspace(eb_n0_range[0], eb_n0_range[1], num_points)
    caps = compute_bpsk_capacity(eb_grid, rate)
    for i in range(len(eb_grid) - 1):
        c0, c1 = caps[i], caps[i + 1]
        if (c0 - rate) * (c1 - rate) <= 0 and not (np.isnan(c0) or np.isnan(c1)):
            t = (rate - c0) / (c1 - c0 + 1e-15)
            return float(eb_grid[i] + t * (eb_grid[i + 1] - eb_grid[i]))
    idx = int(np.nanargmin(np.abs(caps - rate)))
