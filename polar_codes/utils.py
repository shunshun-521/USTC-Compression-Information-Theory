...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK 离散输入信道容量（bits/channel use），通过互信息数值积分。
    """
    from scipy.stats import norm

    eb_n0_db_list = np.atleast_1d(eb_n0_db_list)
    capacities = []
    ys = np.linspace(-10.0, 10.0, 20000)
    dy = ys[1] - ys[0]
    xs = np.array([-1.0, 1.0])
    px = np.array([0.5, 0.5])
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        py = sum(p * norm.pdf(ys, x, sigma) for x, p in zip(xs, px))
        cap = 0.0
        for x, p in zip(xs, px):
            p_yx = norm.pdf(ys, x, sigma)
            ratio = p_yx / np.maximum(py, 1e-300)
            with np.errstate(divide="ignore", invalid="ignore"):
                term = np.where(ratio > 0, p_yx * np.log2(ratio), 0.0)
            cap += p * np.nansum(term) * dy
