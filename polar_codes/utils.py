            ...
            results.append(
                {
                    ...
                    "avg_iters": (
                        float(row["avg_iters"]) if row.get("avg_iters") and row["avg_iters"] else None
                    ),
                }
            )
    return results
...
def _bpsk_capacity_at_snr(snr_linear):
    """BPSK 离散输入信道容量（bits/channel use）。"""
    from scipy import integrate

    def integrand(y):
        t = np.clip(-2.0 * snr_linear * y ** 2, -500.0, 500.0)
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        capacities.append(_bpsk_capacity_at_snr(snr))
...
def find_capacity_limit(rate, eb_n0_range=(-2, 5), num_points=200):
    ...
    cap_lo = _bpsk_capacity_at_snr(2.0 * rate * 10 ** (lo / 10.0))
    cap_hi = _bpsk_capacity_at_snr(2.0 * rate * 10 ** (hi / 10.0))

    if cap_lo < rate < cap_hi:
        for _ in range(80):
            mid = (lo + hi) / 2.0
            cap_mid = _bpsk_capacity_at_snr(2.0 * rate * 10 ** (mid / 10.0))
            if cap_mid > rate:
                hi = mid
            else:
                lo = mid
