...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """计算 BPSK-AWGN 信道容量（bits/channel use），基于互信息"""
    grid = np.linspace(-10, 10, 8000)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10 ** (eb_n0_db / 10.0))
        sigma = 1.0 / np.sqrt(snr)
        p_y = (
            0.5 * np.exp(-((grid - 1) ** 2) / (2 * sigma ** 2))
            + 0.5 * np.exp(-((grid + 1) ** 2) / (2 * sigma ** 2))
        ) / (np.sqrt(2 * np.pi) * sigma)
        hy = -np.trapezoid(
            np.where(p_y > 1e-300, p_y * np.log2(p_y), 0.0), grid
        )
        hy_given_x = 0.5 * np.log2(2 * np.pi * np.e * sigma ** 2)
        capacities.append(hy - hy_given_x)
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-1, 4), num_points=2000):
    ...
    return float(eb_grid[idx - 1] + t * (eb_grid[idx] - eb_grid[idx - 1]))
