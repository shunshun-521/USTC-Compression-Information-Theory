...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - (1/√π) ∫ log2(1 + exp(-4*Es/N0*x²)) exp(-x²) dx
    其中 Es/N0 = 2*R*Eb/N0（线性）
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        integrand = np.log2(1.0 + np.exp(-4.0 * esn0 * (x ** 2)))
        val = np.trapezoid(integrand * weight, x)
        capacities.append(1.0 - val)
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-2, 6), num_points=4000):
    """找到使 BPSK 信道容量等于码率 R 的 Eb/N0（dB）"""
    eb_grid = np.linspace(eb_n0_range[0], eb_n0_range[1], num_points)
    caps = compute_bpsk_capacity(eb_grid, rate)
    for i in range(len(eb_grid) - 1):
        if (caps[i] - rate) * (caps[i + 1] - rate) <= 0:
            t = abs(caps[i] - rate) / (abs(caps[i] - rate) + abs(caps[i + 1] - rate))
