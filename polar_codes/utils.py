...
def _log2_one_plus_exp_neg(z):
    ...
    out = np.empty_like(z)
    pos = z >= 0
    out[pos] = np.log2(1.0 + np.exp(-z[pos]))
    out[~pos] = (-z[~pos] / np.log(2)) + np.log2(1.0 + np.exp(z[~pos]))
    return out
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E_y[log2(1 + exp(-2*s*y^2))], s = 2R * 10^{Eb/N0/10}, y ~ N(0,1)
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            return _log2_one_plus_exp_neg(2.0 * snr * y * y) * np.exp(-(y ** 2) / 2.0) / np.sqrt(2.0 * np.pi)
...
def find_capacity_limit(rate, eb_n0_range=(-2, 6), num_points=200):
    ...
    try:
        return float(brentq(cap_diff, eb_n0_range[0], eb_n0_range[1]))
    except ValueError:
