...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        pdf0 = np.exp(-0.5 * ((y - 1.0) / sigma) ** 2) / (sigma * np.sqrt(2.0 * np.pi))
        pdf1 = np.exp(-0.5 * ((y + 1.0) / sigma) ** 2) / (sigma * np.sqrt(2.0 * np.pi))
        llr = 2.0 * y / (sigma ** 2)
        val = np.trapezoid(
            np.log2(1.0 + np.exp(-llr)) * pdf0 + np.log2(1.0 + np.exp(llr)) * pdf1,
            y,
        )
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    i = idx[0]
    try:
        func = lambda eb: compute_bpsk_capacity([eb], rate)[0] - rate
        return float(brentq(func, eb_grid[i], eb_grid[i + 1]))
    except ValueError:
