...
def compute_bpsk_capacity(eb_n0_db, rate):
    ...
    def integrand(y):
        ...
        return np.log2(1 + np.exp(val)) * np.exp(-y ** 2 / 2) / np.sqrt(2 * np.pi)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=200):
    ...
    for i in range(1, len(eb_vals)):
        if caps[i - 1] < rate <= caps[i] or caps[i - 1] > rate >= caps[i]:
            return float(np.interp(rate, [caps[i - 1], caps[i]], [eb_vals[i - 1], eb_vals[i]]))
    return float(eb_vals[np.argmin(np.abs(np.array(caps) - rate))])
...
def save_frozen_set_info(N_list, K, design_eb_n0_db, save_path):
    ...
    with open(save_path, 'w') as f:
        for N in N_list:
            K_n = (N // 2) if K is None else int(K)
            rate = K_n / N
