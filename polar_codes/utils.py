        ...
        for r in results:
            writer.writerow(
                [
                    ...
                    "" if r.get("avg_iters") is None else f"{r['avg_iters']:.3f}",
                ]
            )
            ...
            results.append(
                {
                    ...
                    if row.get("avg_iters") not in (None, "", "nan")
                    else None,
                }
            )
    return results
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        val, _ = integrate.quad(integrand, -np.inf, np.inf, args=(snr,))
...
def plot_bler_curves(
    ...
    for label, results in results_dict.items():
        ...
        ax.semilogy(x, y, "o-", linewidth=2, markersize=5, label=label)
