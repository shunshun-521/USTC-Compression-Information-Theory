...
def bpsk_capacity(snr_linear):
    ...
    snr_linear = 2*R*Eb/N0，对应 sigma = 1/sqrt(snr_linear)。
    """
    def integrand(x, y):
        # x in {+1,-1} 等概，y = x + n, n~N(0, 1/snr_linear)
        sigma2 = 1.0 / snr_linear
        llr_factor = 2.0 / sigma2
        z = llr_factor * x * y
        if z > 40:
            log_term = -z / np.log(2)
        else:
            log_term = np.log2(1.0 + np.exp(-z))
        return 0.5 * np.exp(-(y - x) ** 2 / (2 * sigma2)) / np.sqrt(2 * np.pi * sigma2) * log_term

    def integrand_y(y):
        return integrand(1.0, y) + integrand(-1.0, y)

    val, _ = integrate.quad(integrand_y, -20.0, 20.0, limit=200)
    return 1.0 - val
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    def objective(eb_n0_db):
        snr = 2.0 * rate * (10 ** (eb_n0_db / 10.0))
        return bpsk_capacity(snr) - rate

    grid = np.linspace(eb_n0_range[0], eb_n0_range[1], num_points)
    vals = [objective(x) for x in grid]
    for i in range(len(grid) - 1):
        if vals[i] * vals[i + 1] <= 0:
            return brentq(objective, grid[i], grid[i + 1])
    raise ValueError("Capacity limit not found in range")
