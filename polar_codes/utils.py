...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    caps_fine = compute_bpsk_capacity(np.linspace(e0, e1, 200), rate)
    fine_grid = np.linspace(e0, e1, 200)
    idx2 = np.argmin(np.abs(caps_fine - rate))
    return float(fine_grid[idx2])
