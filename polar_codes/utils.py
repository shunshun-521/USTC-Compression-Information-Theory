...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 AWGN 信道下 BPSK 的 Shannon 容量（bits/channel use）。
    采用 C = 0.5 * log2(1 + 2*R*Eb/N0) 近似（用于绘图参考）。
    """
    eb_lin = 10.0 ** (np.asarray(eb_n0_db_list, dtype=float) / 10.0)
    snr = 2.0 * rate * eb_lin
    return 0.5 * np.log2(1.0 + snr)


def find_capacity_limit(rate, eb_n0_range=(-5, 15)):
    """找到使 Shannon 容量等于码率 R 的 Eb/N0（dB）。"""

    def cap_at(eb_db):
        return float(compute_bpsk_capacity([eb_db], rate)[0])
