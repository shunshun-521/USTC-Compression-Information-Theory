...
def _bpsk_mutual_info(eb_n0_db, rate):
    """
    BPSK-AWGN 互信息（bits/channel use），与 channel.py 的 sigma 约定一致。
    """
    ...
    def integrand(y):
        def pdf(y_val, x):
            return np.exp(-((y_val - x) ** 2) / (2.0 * sigma2)) / np.sqrt(
                2.0 * np.pi * sigma2
            )

        p_y = 0.5 * pdf(y, 1.0) + 0.5 * pdf(y, -1.0)
        h = 0.0
        for x in (1.0, -1.0):
            p_x_y = 0.5 * pdf(y, x) / p_y
            if p_x_y > 1e-300:
                h -= p_x_y * np.log2(p_x_y)
        return p_y * h

    h_xy, _ = integrate.quad(integrand, -20.0, 20.0, limit=200)
    return 1.0 - h_xy
