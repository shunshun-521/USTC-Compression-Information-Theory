...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        p0 = np.exp(-0.5 * ((ys - 1) / sigma) ** 2) / (np.sqrt(2 * np.pi) * sigma)
        p1 = np.exp(-0.5 * ((ys + 1) / sigma) ** 2) / (np.sqrt(2 * np.pi) * sigma)
        py = 0.5 * p0 + 0.5 * p1
        mi = 0.0
        for p_yx in (p0, p1):
            ratio = np.maximum(p_yx / (py + 1e-300), 1e-300)
            mi += 0.5 * np.sum(p_yx * np.log2(ratio)) * dy
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if idx == 0 or idx == len(eb_grid) - 1:
        for _ in range(40):
            mid = (eb_grid[0] + eb_grid[-1]) / 2.0
            cap_mid = compute_bpsk_capacity(mid, rate)
            if cap_mid > rate:
                eb_grid[0] = mid
            else:
                eb_grid[-1] = mid
        return float(np.mean(eb_grid))
