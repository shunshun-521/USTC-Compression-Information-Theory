...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 0.5 * log2(1 + 2*R*10^{Eb/N0/10})，实数 BPSK 经典容量公式。
    """
    eb = np.asarray(eb_n0_db_list, dtype=np.float64)
    snr_linear = 2.0 * rate * (10 ** (eb / 10.0))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 10), num_points=1000):
    ...
    解析解：R = 0.5*log2(1 + 2*R*gamma) => gamma = 2^1 - 1 = 1（当 R=0.5）
    一般地：Eb/N0 = 10*log10((2^(2R)-1)/(2*R))
    """
    gamma_needed = 2.0 ** (2.0 * rate) - 1.0
    eb_n0_linear = gamma_needed / (2.0 * rate)
