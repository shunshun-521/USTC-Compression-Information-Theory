...
def save_frozen_set_info(N_list, K, design_eb_n0_db, save_path):
    ...
    with open(save_path, "w", encoding="utf-8") as f:
        for N in N_list:
            ...
            f.write(np.array2string(info_idx, threshold=info_idx.size, max_line_width=100) + "\n")
            f.write(f"Frozen indices (all {len(frozen_idx)}):\n")
            f.write(np.array2string(frozen_idx, threshold=frozen_idx.size, max_line_width=100) + "\n")
            f.write("-" * 53 + "\n")
