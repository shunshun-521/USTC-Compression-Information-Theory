...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            # 标准 BPSK-AWGN 互信息数值积分（y^2 型）
            return np.log2(1.0 + np.exp(-snr * y ** 2)) * np.exp(-y ** 2) / np.sqrt(np.pi)

        cap, _ = integrate.quad(integrand, 0, np.inf, limit=200)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 5), num_points=1000):
    ...
    使用 BPSK 二进制输入 AWGN 互信息：I = 1 - H2(Q(sqrt(2*SNR)))。
    """
    ...
    def mutual_info(eb_n0_db):
        snr = 2.0 * rate * (10 ** (eb_n0_db / 10.0))
        p_err = 0.5 * erfc(np.sqrt(snr))
        return 1.0 - h2(p_err)

    lo, hi = eb_n0_range
    try:
        return brentq(lambda x: mutual_info(x) - rate, lo, hi)
    except ValueError:
        grid = np.linspace(lo, hi, num_points)
        vals = [mutual_info(x) for x in grid]
        idx = int(np.argmin(np.abs(np.array(vals) - rate)))
