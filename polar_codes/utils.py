...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        grid = np.linspace(-1.0 - 6.0 * sigma, 1.0 + 6.0 * sigma, 4000)
        pdf = np.exp(-0.5 * (grid - 1.0) ** 2 / sigma ** 2) / (
            np.sqrt(2.0 * np.pi) * sigma
        )
        integrand = np.log2(1.0 + np.exp(-2.0 * snr * grid))
        cap = np.trapezoid(integrand * pdf, grid)
