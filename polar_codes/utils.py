...
def _bpsk_mutual_information(snr_linear):
    ...
    def integrand(y):
        ...
        if llr > 50:
            ...
            inner = np.log2(1.0 + np.exp(-llr))
        pdf = np.exp(-0.5 * (y - 1.0) ** 2) / np.sqrt(2.0 * np.pi)
        return pdf * inner

    mi, _ = integrate.quad(integrand, -15.0, 15.0, limit=200)
    return float(mi)
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        capacities.append(_bpsk_mutual_information(snr_linear))
