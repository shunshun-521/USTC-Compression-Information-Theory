...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK 约束 AWGN 信道容量（bits/channel use）。
    C = 1 - (1/√π) ∫ exp(-x²) log2(1 + exp(-2γx²)) dx, γ = (Eb/N0)*R
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        gamma = (10 ** (eb_n0_db / 10.0)) * rate

        def integrand(x):
            z = -2.0 * gamma * (x ** 2)
            log_term = logsumexp([0.0, z]) / np.log(2.0)
            return log_term * np.exp(-(x ** 2)) / np.sqrt(np.pi)
...
def find_capacity_limit(rate, eb_n0_range=(-2, 12), num_points=2000):
    ...
    for i in range(1, len(eb_grid)):
        if (caps[i - 1] - rate) * (caps[i] - rate) <= 0 and caps[i] != caps[i - 1]:
