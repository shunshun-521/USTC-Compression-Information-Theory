...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK-AWGN 信道容量（bits/channel use），与 channel.py 的 LLR 约定一致。
    """
    eb_n0_db_list = np.atleast_1d(eb_n0_db_list)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr_lin = 2.0 * rate * (10.0 ** (eb_n0_db / 10.0))
        sigma = 1.0 / np.sqrt(snr_lin)

        def bit_mi(symbol):
            def integrand(y):
                llr = 2.0 * y / (sigma**2)
                sign = 1.0 if symbol == 1 else -1.0
                return _log2_one_plus_exp(-sign * llr) * np.exp(
                    -((y - symbol) ** 2) / (2.0 * sigma**2)
                )

            val, _ = integrate.quad(integrand, -20.0, 20.0, limit=200)
            return val / (sigma * np.sqrt(2.0 * np.pi))

        mi = 0.5 * (bit_mi(1.0) + bit_mi(-1.0))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if idx == 0 or idx == num_points - 1:
        ...
        for _ in range(50):
            mid = 0.5 * (a + b)
            c = compute_bpsk_capacity(mid, rate)
            if c < rate:
                a = mid
            else:
                b = mid
