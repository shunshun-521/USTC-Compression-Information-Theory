...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK 信道容量（bits/channel use），基于积分公式。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(phi):
            return np.log2(1.0 + np.exp(-es_no * np.sin(phi) ** 2))

        val, _ = integrate.quad(integrand, 0.0, np.pi / 2.0, limit=200)
...
def find_capacity_limit(rate, eb_n0_range=(-2, 12), num_points=200):
    ...
    if c1 == c0:
        return float(e0)
    return float(e0 + (rate - c0) / (c1 - c0) * (e1 - e0))
