...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    capacities = []
    ys = np.linspace(-12, 12, 24001)
    for eb_n0_db in eb_n0_db_list:
        esn0 = rate * (10 ** (eb_n0_db / 10.0))
        amp = np.sqrt(2.0 * esn0)
        py1 = np.exp(-(ys - amp) ** 2 / 2.0) / np.sqrt(2.0 * np.pi)
        pym = np.exp(-(ys + amp) ** 2 / 2.0) / np.sqrt(2.0 * np.pi)
        py = 0.5 * (py1 + pym)
        integrand = np.zeros_like(ys)
        mask = py > 1e-300
        integrand[mask] = 0.5 * (
            py1[mask] * np.log2(py1[mask] / py[mask])
            + pym[mask] * np.log2(pym[mask] / py[mask])
        )
        capacities.append(float(np.trapezoid(integrand, ys)))
