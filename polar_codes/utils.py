...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        if es_n0 <= 0.0:
            ...
            continue
        scale = np.sqrt(2.0 * es_n0)

        def integrand(t, sc=scale):
            x = sc * t
            if x >= 0.0:
                log_term = np.log2(1.0 + np.exp(-x))
            else:
                log_term = -x / np.log(2.0) + np.log2(1.0 + np.exp(x))
            return log_term * np.exp(-t ** 2 / 2.0)

        val, _ = integrate.quad(integrand, 0.0, 50.0)
        capacities.append(1.0 - (2.0 / np.sqrt(2.0 * np.pi)) * val)
...
def save_frozen_set_info(N_list, K, design_eb_n0_db, save_path):
    ...
    """
    if K is None:
        K = None  # computed per N as N//2 in caller context
