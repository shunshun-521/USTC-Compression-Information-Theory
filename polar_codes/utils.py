...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    通过互信息 I(X;Y) 数值积分，X ∈ {+1,-1}，Es=1，Eb/N0 给定。
    """
    y = np.linspace(-10.0, 10.0, 20001)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        eb_lin = 10.0 ** (eb_n0_db / 10.0)
        es_no = rate * eb_lin * 2.0
        sigma = 1.0 / np.sqrt(es_no)
        p_y = 0.5 * np.exp(-0.5 * ((y - 1.0) / sigma) ** 2) + 0.5 * np.exp(
            -0.5 * ((y + 1.0) / sigma) ** 2
        )
        p_y /= sigma * np.sqrt(2.0 * np.pi)
        p_y = np.clip(p_y, 1e-300, None)
        h_y = -np.trapezoid(p_y * np.log2(p_y), y)
        h_n = 0.5 * np.log2(2.0 * np.pi * np.e * sigma ** 2)
        capacities.append(max(h_y - h_n, 0.0))
