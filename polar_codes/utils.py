            ...
            results.append(
                {
                    ...
                    if row["avg_iters"] not in ("", "None", None)
                    else None,
                }
            )
    return results
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = H(Y) - H(N)，X ∈ {+1,-1}，AWGN 方差 σ² = 1/(2R·10^(Eb/N0/10))。
    """
    eb_n0_db_list = np.atleast_1d(eb_n0_db_list).astype(float)
    y_grid = np.linspace(-12.0, 12.0, 24001)
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        var = sigma ** 2
        p_y = 0.5 * np.exp(-0.5 * ((y_grid - 1.0) ** 2) / var) / np.sqrt(
            2.0 * np.pi * var
        ) + 0.5 * np.exp(-0.5 * ((y_grid + 1.0) ** 2) / var) / np.sqrt(
            2.0 * np.pi * var
        )
        p_y = np.maximum(p_y, 1e-300)
        h_y = -np.sum(p_y * np.log2(p_y)) * dy
        h_n = 0.5 * np.log2(2.0 * np.pi * np.e * var)
        capacities.append(h_y - h_n)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    x0, x1 = eb_vals[idx - 1], eb_vals[idx + 1]
