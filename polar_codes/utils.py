...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            log_term = np.log2(1.0 + np.exp(-2.0 * snr * y))
...
def _bpsk_capacity_scalar(eb_n0_db, rate):
    ...
    采用 Es/N0 = R * Eb/N0 与 BPSK 互信息数值积分。
    """
    es_n0 = rate * (10 ** (eb_n0_db / 10.0))
    sigma = 1.0 / np.sqrt(2.0 * es_n0) if es_n0 > 0 else 1e6
    y = np.linspace(-10.0, 10.0, 30000)
    llr = 2.0 * y / (sigma ** 2)
    ...
    return float(np.trapezoid(h * pdf, y))


def find_capacity_limit(rate, eb_n0_range=(-3, 10)):
    """找到使 BPSK 信道容量等于码率 R 的 Eb/N0（dB）。"""
    lo, hi = eb_n0_range
    if _bpsk_capacity_scalar(lo, rate) < rate:
        lo = -5.0
    if _bpsk_capacity_scalar(hi, rate) < rate:
        hi = 20.0
