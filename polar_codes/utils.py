...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    使用互信息 I(X;Y) 的数值积分，sigma 与 channel.py 一致。
    """
    ...
    for eb in eb_n0_db_list:
        ...
        def integrand(y):
            ...
            py = 0.5 * (p0 + p1)
            val = 0.0
            if p0 > 1e-300 and py > 1e-300:
                val += 0.5 * p0 * np.log2(p0 / py)
            if p1 > 1e-300 and py > 1e-300:
                val += 0.5 * p1 * np.log2(p1 / py)
            return val

        mi, _ = integrate.quad(integrand, -30.0, 30.0, limit=200)
        caps.append(max(0.0, min(1.0, mi)))
