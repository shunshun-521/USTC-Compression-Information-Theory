...
def _bpsk_capacity_per_ebn0(eb_n0_db, rate):
    sigma = 1.0 / np.sqrt(2.0 * rate * (10.0 ** (eb_n0_db / 10.0)))
    y = np.linspace(-20.0, 20.0, 8000)
    pdf = np.exp(-0.5 * y ** 2) / np.sqrt(2.0 * np.pi)
    llr_mag = np.abs(2.0 * y / (sigma ** 2))
    integrand = np.log2(1.0 + np.exp(-llr_mag))
    if hasattr(np, "trapezoid"):
        integral = np.trapezoid(integrand * pdf, y)
    else:
        integral = np.trapz(integrand * pdf, y)
    return 1.0 - integral
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    return np.array([_bpsk_capacity_per_ebn0(eb, rate) for eb in eb_n0_db_list])
