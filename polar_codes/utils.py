...
def _bpsk_capacity_per_snr(snr_linear):
  ...
  def integrand(y):
    return _log2_one_plus_exp(-2.0 * snr_linear * (y ** 2)) * np.exp(-(y ** 2) / 2.0)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 5), num_points=2000):
  ...
  lo, hi = eb_n0_range
  flo = _bpsk_capacity_per_snr(2.0 * rate * (10.0 ** (lo / 10.0))) - rate
  fhi = _bpsk_capacity_per_snr(2.0 * rate * (10.0 ** (hi / 10.0))) - rate
  if flo * fhi > 0:
