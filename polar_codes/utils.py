...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            val = -2.0 * snr * y
            log_term = np.log2(1.0 + np.exp(np.clip(val, -500, 500)))
            return log_term * np.exp(-y ** 2 / 2.0)

        cap, _ = integrate.quad(integrand, -20.0, 20.0, limit=200)
        cap = 1.0 - cap / np.sqrt(2.0 * np.pi)
        capacities.append(max(0.0, min(1.0, cap)))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    for i in range(len(caps) - 1):
        if caps[i] <= rate <= caps[i + 1]:
            f_lo = compute_bpsk_capacity([eb_vals[i]], rate)[0] - rate
            f_hi = compute_bpsk_capacity([eb_vals[i + 1]], rate)[0] - rate
            if f_lo * f_hi <= 0:
                return brentq(
                    lambda x: compute_bpsk_capacity([x], rate)[0] - rate,
                    eb_vals[i], eb_vals[i + 1],
                )

    try:
        return brentq(
            lambda x: compute_bpsk_capacity([x], rate)[0] - rate,
            eb_n0_range[0], eb_n0_range[1],
        )
    except ValueError:
        return 1.0
