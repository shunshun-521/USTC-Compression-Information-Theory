...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    通过 BPSK-AWGN 混合高斯输出的微分熵数值计算。
    """
    ys = np.linspace(-12.0, 12.0, 20001)
    capacities = []
    for eb_n0_db in np.atleast_1d(eb_n0_db_list):
        snr = 2.0 * rate * (10 ** (eb_n0_db / 10.0))
        sigma = 1.0 / np.sqrt(snr)
        norm = np.sqrt(2.0 * np.pi * sigma ** 2)
        p0 = np.exp(-((ys - 1.0) ** 2) / (2.0 * sigma ** 2)) / norm
        p1 = np.exp(-((ys + 1.0) ** 2) / (2.0 * sigma ** 2)) / norm
        py = 0.5 * p0 + 0.5 * p1
        h_y = -np.trapezoid(py * np.log2(np.clip(py, 1e-300, None)), ys)
