...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        snr_linear = 10 ** (eb_n0_db / 10.0) * rate
        amplitude = np.sqrt(2.0 * snr_linear)
        ys = np.linspace(-20.0, 20.0, 40001)
        p0 = np.exp(-0.5 * (ys - amplitude) ** 2) / np.sqrt(2.0 * np.pi)
        p1 = np.exp(-0.5 * (ys + amplitude) ** 2) / np.sqrt(2.0 * np.pi)
        py = 0.5 * (p0 + p1)
        py = np.clip(py, 1e-300, None)
        h_y = -np.trapezoid(py * np.log2(py), ys)
        h_y_given_x = 0.0
        for px in (p0, p1):
            px = np.clip(px, 1e-300, None)
            h_y_given_x -= 0.5 * np.trapezoid(px * np.log2(px), ys)
