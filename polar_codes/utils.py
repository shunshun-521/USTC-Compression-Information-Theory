...
def compute_bpsk_capacity(eb_n0_db, rate, n_samples=80000, seed=0):
    """
    ...
    x = rng.integers(0, 2, n_samples)
    s = 1 - 2 * x
    y = s + rng.normal(0.0, sigma, n_samples)
    llr = 2.0 * y / (sigma ** 2)
    t = -(1 - 2 * x) * llr
    max_t = np.maximum(t, 0.0)
    log_term = (max_t + np.log1p(np.exp(t - max_t))) / np.log(2.0)
    return float(1.0 - np.mean(log_term))


def find_capacity_limit(rate, eb_n0_range=(-2, 8), num_points=80):
