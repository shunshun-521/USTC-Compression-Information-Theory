        ...
        for r in results:
            ...
            writer.writerow(
                [
                    ...
                    f"{avg_iters:.1f}" if avg_iters is not None else "",
                ]
            )
            ...
            results.append(
                {
                    ...
                    "avg_iters": float(avg_iters) if avg_iters else None,
                }
            )
    return results
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    I(X;Y) = H(Y) - H(Y|X)，±1 调制，噪声方差 sigma^2 = 1/SNR。
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def p_y(y):
            p0 = norm * np.exp(-0.5 * ((y - 1.0) / sigma) ** 2)
            p1 = norm * np.exp(-0.5 * ((y + 1.0) / sigma) ** 2)
        ...
        def neg_entropy_cond(y, center):
            p = norm * np.exp(-0.5 * ((y - center) / sigma) ** 2)
            if p < 1e-300:
                return 0.0
            return -p * np.log2(p)

        hy, _ = integrate.quad(neg_entropy_y, y_lo, y_hi, limit=200)
        hy0, _ = integrate.quad(lambda y: neg_entropy_cond(y, 1.0), y_lo, y_hi, limit=200)
        hy1, _ = integrate.quad(lambda y: neg_entropy_cond(y, -1.0), y_lo, y_hi, limit=200)
        capacities.append(hy - 0.5 * (hy0 + hy1))
...
def plot_bler_curves(
    ...
    for label, results in results_dict.items():
        ...
        bler = [max(r["bler"], 1e-10) for r in results]
