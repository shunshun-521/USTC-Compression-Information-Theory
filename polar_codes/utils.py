...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    使用 LLR 域公式：
        C = 1 - E[log2(1 + exp(-|L|))]
    其中 L = 2y/sigma^2，sigma^2 = 1/(2*R*Eb/N0)。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            ...
            log_term = np.where(
                abs_llr > 30,
                abs_llr / np.log(2),
                np.log2(1.0 + np.exp(-abs_llr)),
            )
            return log_term * np.exp(-0.5 * (y / sigma) ** 2) / (sigma * np.sqrt(2.0 * np.pi))

        val, _ = integrate.quad(integrand, -20.0 * sigma, 20.0 * sigma, limit=200)
