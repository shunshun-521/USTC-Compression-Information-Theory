...
def compute_bpsk_capacity(eb_n0_db_list, rate, num_samples=200_000, seed=0):
    """
    计算 BPSK 离散输入信道容量（bits/channel use），蒙特卡洛估计。
    C ≈ I(X;Y)，X∈{+1,-1}，Y = X + n，n~N(0,σ²)，σ 由 Eb/N0 与码率确定。
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        x = rng.choice([-1.0, 1.0], num_samples)
        y = x + sigma * rng.standard_normal(num_samples)
        py0 = np.exp(-((y - 1.0) ** 2) / (2.0 * sigma ** 2))
        py1 = np.exp(-((y + 1.0) ** 2) / (2.0 * sigma ** 2))
        py = 0.5 * (py0 + py1)
        pyx = np.where(x > 0, py0, py1)
        mi = np.mean(np.log2(pyx / np.maximum(py, 1e-300)))
        capacities.append(float(mi))
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-1, 8), num_points=400):
    """
