...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def p_y_given_x(y, x):
            return np.exp(-(y - x) ** 2 / (2.0 * sigma2)) / np.sqrt(2.0 * np.pi * sigma2)
        ...
        def h_cond(x):
            ...
            return integrate.quad(integrand, -12.0, 12.0, limit=200)[0]

        def mix(y):
            p = 0.5 * p_y_given_x(y, 1.0) + 0.5 * p_y_given_x(y, -1.0)
            if p < 1e-300:
                return 0.0
            return -p * np.log2(p)

        h_y = integrate.quad(mix, -12.0, 12.0, limit=200)[0]
        capacities.append(max(0.0, h_y - 0.5 * (h_cond(1.0) + h_cond(-1.0))))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    c0, c_mid, c1 = caps[idx - 1], caps[idx], caps[idx + 1]
    if (c0 - rate) * (c_mid - rate) <= 0:
        t = (rate - c0) / (c_mid - c0 + 1e-15)
        return float(e0 + t * (eb_grid[idx] - e0))
    t = (rate - c_mid) / (c1 - c_mid + 1e-15)
    return float(eb_grid[idx] + t * (e1 - eb_grid[idx]))
