...
def _crc_bits(data, poly, crc_len):
    reg = 0
    for bit in data:
    ...
    return [(reg >> (crc_len - 1 - i)) & 1 for i in range(crc_len)]
...
def crc_encode(info_bits, crc_length=8):
    ...
    if crc_length == 8:
        crc = _crc_bits(info_bits, _CRC8_POLY, 8)
    elif crc_length == 16:
        crc = _crc_bits(info_bits, _CRC16_POLY, 16)
    else:
