...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - (2/sqrt(2π)) ∫_0^∞ log2(1 + exp(-γ y²)) exp(-y²/2) dy
    其中 γ = Eb/N0（线性，BPSK 每信道使用 1 比特）。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            ...
            if x < -50.0:
                return 0.0
            if x > 50.0:
                return (x / np.log(2.0)) * np.exp(-y * y / 2.0) / np.sqrt(2.0 * np.pi)
            return np.log2(1.0 + np.exp(x)) * np.exp(-y * y / 2.0) / np.sqrt(2.0 * np.pi)
