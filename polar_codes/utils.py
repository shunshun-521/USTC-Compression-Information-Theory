...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    y = np.linspace(-10.0, 10.0, 5000)
    pdf = np.exp(-0.5 * y * y) / np.sqrt(2.0 * np.pi)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10.0 ** (eb_n0_db / 10.0))
        integrand = _log2_one_plus_exp(-2.0 * snr * y * y)
        val = np.trapezoid(integrand * pdf, y)
...
def find_capacity_limit(rate, eb_n0_range=(-2, 10), num_points=1000):
    ...
    try:
        return brentq(capacity_minus_rate, eb_n0_range[0], eb_n0_range[1])
    except ValueError:
