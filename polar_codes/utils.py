...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    通过数值积分计算互信息 I(X;Y)，Es/N0 = R * Eb/N0。
    """
    capacities = []
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        s = np.sqrt(2.0 * es_n0)
        p0 = np.exp(-0.5 * (y - s) ** 2) / np.sqrt(2.0 * np.pi)
        p1 = np.exp(-0.5 * (y + s) ** 2) / np.sqrt(2.0 * np.pi)
        py = 0.5 * (p0 + p1)
        cap = 0.5 * np.sum(p0 * np.log2((p0 + eps) / (py + eps)) * dy)
        cap += 0.5 * np.sum(p1 * np.log2((p1 + eps) / (py + eps)) * dy)
        capacities.append(cap)
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-1, 4), num_points=200):
