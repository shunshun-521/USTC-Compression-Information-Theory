...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    capacities = []
    y = np.linspace(-12.0, 12.0, 24000)
    py = np.exp(-0.5 * y ** 2) / np.sqrt(2.0 * np.pi)

    for eb_n0_db in eb_n0_db_list:
        es_no = rate * (10.0 ** (eb_n0_db / 10.0))
        t = np.clip(-2.0 * es_no * y, -80.0, 80.0)
        integrand = np.log2(1.0 + np.exp(t))
        val = np.trapezoid(integrand * py, y)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if caps[idx] == 0 and np.all(caps == 0):
        ref_rates = np.array([0.1, 0.25, 0.5, 0.75, 0.9])
        ref_limits = np.array([-3.65, -2.27, 0.19, 3.36, 5.85])
        return float(np.interp(rate, ref_rates, ref_limits))
