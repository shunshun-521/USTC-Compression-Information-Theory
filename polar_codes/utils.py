...
def compute_bpsk_capacity(eb_n0_db, rate):
    ...
    C = 1 - E_y[log2(1 + exp(-2*gamma_b*y^2))], gamma_b = 2R*Eb/N0
    """
    gamma_b = 2.0 * rate * 10.0 ** (eb_n0_db / 10.0)

    def integrand(y):
        a = -2.0 * gamma_b * y ** 2
        log_term = np.logaddexp(0.0, a) / np.log(2.0)
...
def find_capacity_limit(rate, eb_n0_range=(-2, 10), num_points=1000):
    ...
    capacities = np.array([compute_bpsk_capacity(eb, rate) for eb in eb_n0_vals])
    idx = np.argmin(np.abs(capacities - rate))
    if idx == 0 or idx == len(eb_n0_vals) - 1:
        for i in range(len(eb_n0_vals) - 1):
            c0, c1 = capacities[i], capacities[i + 1]
...
def plot_bler_curves(results_dict, title, save_path, shannon_limit_db=None,
    ...
    for idx, (label, results) in enumerate(results_dict.items()):
        ...
        ax.semilogy(
            eb, bler, marker=markers[idx % len(markers)],
