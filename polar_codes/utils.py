...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - (1/ln2) E[log(1 + exp(-2*SNR*y^2))], y ~ N(0,1)
    """
    y = np.linspace(-12, 12, 4001)
    pdf = np.exp(-y ** 2 / 2) / np.sqrt(2 * np.pi)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10 ** (eb_n0_db / 10.0))
        integrand = np.log1p(np.exp(-2.0 * snr * y ** 2)) * pdf
        cap = 1.0 - np.trapezoid(integrand, y) / np.log(2)
