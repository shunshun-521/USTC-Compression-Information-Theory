...
def _log2_one_plus_exp(x):
    ...
    out = np.empty_like(x)
    pos = x >= 0
    out[pos] = x[pos] / np.log(2.0) + np.log2(1.0 + np.exp(-x[pos]))
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    采用 BPSK-AWGN 对称信道互信息数值积分。
    """
    eb_n0_db_list = np.atleast_1d(eb_n0_db_list)
    y = np.linspace(-10.0, 10.0, 8000)
    phi = np.exp(-0.5 * y ** 2) / np.sqrt(2.0 * np.pi)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10.0 ** (eb_n0_db / 10.0))
        integrand = _log2_one_plus_exp(-snr * y ** 2)
        val = np.trapezoid(integrand * phi, y)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    return float(brentq(objective, eb_n0_range[0], eb_n0_range[1]))
