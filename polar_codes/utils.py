...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    def capacity_at_eb_n0(eb_n0_db):
        ...
        def integrand(y):
            llr = 2.0 * y / (sigma ** 2)
            term = np.logaddexp(0.0, -llr) / np.log(2.0)
            return term * np.exp(-0.5 * (y - 1.0) ** 2 / (sigma ** 2))

        val, _ = integrate.quad(integrand, -np.inf, np.inf, limit=200)
        val /= sigma * np.sqrt(2.0 * np.pi)
        return 1.0 - val

    eb_n0_db_list = np.asarray(eb_n0_db_list, dtype=float)
    return np.array([capacity_at_eb_n0(x) for x in eb_n0_db_list])
