...
def _bpsk_mutual_info(eb_n0_db, rate):
    """BPSK-AWGN 互信息（bits/channel use），与信道模型 sigma 定义一致。"""
    sigma = 1.0 / np.sqrt(2.0 * rate * (10.0 ** (eb_n0_db / 10.0)))
    inv_sigma2 = 1.0 / (sigma ** 2)

    def p_y_given_x(y, x):
        return np.exp(-0.5 * inv_sigma2 * (y - x) ** 2) / np.sqrt(2.0 * np.pi * sigma ** 2)

    def integrand(y):
        p0 = p_y_given_x(y, 1.0)
        p1 = p_y_given_x(y, -1.0)
        py = 0.5 * (p0 + p1)
        if py < 1e-300:
            return 0.0
        term = 0.0
        if p0 > 1e-300:
            term += 0.5 * p0 * np.log2(p0 / py)
        if p1 > 1e-300:
            term += 0.5 * p1 * np.log2(p1 / py)
        return term

    mi, _ = integrate.quad(integrand, -20.0 * sigma - 5.0, 20.0 * sigma + 5.0, limit=200)
    return mi
