...
def compute_bpsk_capacity(eb_n0_db, rate):
    ...
    C = 1 - E_N[log2(1 + exp(-2 * SNR * N^2))], N ~ N(0,1), SNR = 2R * Eb/N0
    """
    snr = 2.0 * rate * (10.0 ** (eb_n0_db / 10.0))

    def integrand(y):
        t = -2.0 * snr * (y ** 2)
        t = np.clip(t, -700.0, 0.0)
        return np.log2(1.0 + np.exp(t)) * np.exp(-0.5 * y ** 2) / np.sqrt(2.0 * np.pi)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if caps[idx] >= rate:
        ...
        i0, i1 = idx, idx + 1
    t = (rate - caps[i0]) / (caps[i1] - caps[i0])
    return float(eb_grid[i0] + t * (eb_grid[i1] - eb_grid[i0]))
