    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            ...
            if val < -50:
                ...
                term = np.log2(1.0 + np.exp(val))
            return p_y * term

        cap, _ = integrate.quad(integrand, -10.0, 10.0, limit=200)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    try:
        ...
        caps = _compute_bpsk_capacity_numpy(eb_grid, rate)
    ...
    for _ in range(50):
        ...
        try:
            ...
            cap = _compute_bpsk_capacity_numpy([mid], rate)[0]
        if cap < rate:
            lo = mid
        else:
            hi = mid
...
def _compute_bpsk_capacity_numpy(eb_n0_db_list, rate):
    """无 SciPy 时的数值积分后备实现"""
    y = np.linspace(-8, 8, 2000)
    p_y = np.exp(-0.5 * y ** 2) / np.sqrt(2.0 * np.pi)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10.0 ** (eb_n0_db / 10.0))
        integrand = p_y * np.log2(1.0 + np.exp(-2.0 * snr * y))
        capacities.append(1.0 - np.trapz(integrand, y))
    return np.array(capacities)
