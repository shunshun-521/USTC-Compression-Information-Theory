...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    capacities = []
    y = np.linspace(-12, 12, 24001)
    dy = y[1] - y[0]

    for eb_n0_db in eb_n0_db_list:
        sigma = eb_n0_to_sigma(float(eb_n0_db), rate)
        norm = sigma * np.sqrt(2.0 * np.pi)
        py0 = np.exp(-((y - 1.0) ** 2) / (2.0 * sigma ** 2)) / norm
        py1 = np.exp(-((y + 1.0) ** 2) / (2.0 * sigma ** 2)) / norm
        py = 0.5 * (py0 + py1)
        safe = py > 1e-12
        mi = 0.5 * np.sum(
            py0[safe] * np.log2(py0[safe] / py[safe])
        ) * dy
        mi += 0.5 * np.sum(
            py1[safe] * np.log2(py1[safe] / py[safe])
        ) * dy
