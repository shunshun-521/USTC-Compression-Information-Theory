...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    BPSK 离散输入信道容量（bits/channel use），高斯数值积分。
    """
    eb_n0_db_list = np.atleast_1d(eb_n0_db_list)
    capacities = []
    y = np.linspace(0.0, 6.0, 2000)
    w = np.exp(-y ** 2) / np.sqrt(np.pi)
    for eb in eb_n0_db_list:
        snr = 2.0 * rate * (10.0 ** (eb / 10.0))
        integrand = np.log2(1.0 + np.exp(-2.0 * snr * y ** 2)) * w
        val = np.trapezoid(integrand, y)
        capacities.append(1.0 - val)
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-2.0, 12.0), num_points=1000):
