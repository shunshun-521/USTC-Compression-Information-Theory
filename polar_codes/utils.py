...
def _bpsk_mi_integrand(y, eb_n0_linear):
    """互信息被积函数（高斯积分变量 y，使用 |y| 对称形式）。"""
    val = -2.0 * eb_n0_linear * abs(y)
    val = np.clip(val, -700.0, 700.0)
    inner = np.log2(1.0 + np.exp(val))
    return np.exp(-y ** 2 / 2.0) / np.sqrt(2.0 * np.pi) * inner


def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK 离散输入信道容量（bits/channel use）。
    C = 1 - 2 * ∫_0^∞ φ(y) log2(1 + e^{-2*Eb/N0*|y|}) dy
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        mi, _ = integrate.quad(
            lambda y: _bpsk_mi_integrand(y, eb_n0), 0.0, 20.0, limit=200
        )
        capacities.append(1.0 - 2.0 * mi)
