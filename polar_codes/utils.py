...
def _log2_one_plus_exp(x):
    ...
    mask_hi = x > 30
    mask_mid = (x >= -30) & (x <= 30)
    out[mask_hi] = x[mask_hi] / np.log(2.0)
    out[mask_mid] = np.logaddexp(0.0, x[mask_mid]) / np.log(2.0)
    return out
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            term = _log2_one_plus_exp(-2.0 * snr * y * y)
            return term * np.exp(-(y ** 2) / 2.0) / np.sqrt(2.0 * np.pi)
...
def find_capacity_limit(rate, eb_n0_range=(-25, 2), num_points=10000):
    """
    ...
    for i in range(1, len(eb_grid)):
        if (caps[i - 1] - rate) * (caps[i] - rate) <= 0:
            return float((eb_grid[i - 1] + eb_grid[i]) / 2.0)
