...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E_{x}[log2(1 + exp(-2*Eb/N0*x^2))]，x ~ N(0,1)，Eb/N0 为能量比（线性）。
  通过高斯数值积分计算。
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def log2_one_plus_exp_neg(t):
            return np.where(
                t > 30.0,
                0.0,
                np.where(t < -30.0, -t / np.log(2.0), np.log1p(np.exp(-t)) / np.log(2.0)),
            )

        def integrand(x):
            t = 2.0 * gamma * (x ** 2)
            return log2_one_plus_exp_neg(t) * np.exp(-(x ** 2) / 2.0)

        val, _ = integrate.quad(integrand, -12.0, 12.0, limit=200)
        capacities.append(max(0.0, 1.0 - val / np.sqrt(2.0 * np.pi)))
