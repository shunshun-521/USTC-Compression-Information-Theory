...
def _stable_log2_one_plus_exp(t):
    """数值稳定的 log2(1 + exp(t))。"""
    t = np.asarray(t, dtype=np.float64)
    out = np.empty_like(t)
    pos = t > 0
    out[pos] = t[pos] / np.log(2.0) + np.log2(1.0 + np.exp(-t[pos]))
    out[~pos] = np.log2(1.0 + np.exp(t[~pos]))
    return out
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for idx, eb_n0_db in enumerate(eb_n0_db_list):
        ...
        def integrand(y):
            return _stable_log2_one_plus_exp(-2.0 * snr * y * y) * np.exp(-(y ** 2) / 2.0)
