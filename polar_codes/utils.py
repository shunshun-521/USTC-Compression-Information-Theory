...
def compute_bpsk_capacity(eb_n0_db_list, rate):
  ...
  C = 1 - E[log2(1 + exp(-2*snr*|Y|))], snr = 2*R*Eb/N0, Y~N(0,1)
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            ...
            if arg > 700:
                ...
                inner = np.log2(1.0 + np.exp(arg))
            return inner * np.exp(-y ** 2 / 2.0) / np.sqrt(2.0 * np.pi)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 15), num_points=500):
