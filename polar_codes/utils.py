...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(t):
            p = np.exp(-0.5 * t * t) / np.sqrt(2.0 * np.pi)
            ll = np.clip(-2.0 * snr * np.abs(t), -500, 500)
            return p * np.log2(1.0 + np.exp(ll))
...
def plot_bler_curves(
    ...
    if shannon_limit_db is not None:
        plt.axvline(shannon_limit_db, color="gray", linestyle="--", label=f"Shannon R (Eb/N0={shannon_limit_db:.2f} dB)")
