...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            p0 = np.exp(-(y - 1) ** 2 / (2 * snr)) / np.sqrt(2 * np.pi * snr)
            p1 = np.exp(-(y + 1) ** 2 / (2 * snr)) / np.sqrt(2 * np.pi * snr)
            py = 0.5 * (p0 + p1)
            h_y = -py * np.log2(np.maximum(py, 1e-300))
            h_y0 = -p0 * np.log2(np.maximum(p0, 1e-300))
            h_y1 = -p1 * np.log2(np.maximum(p1, 1e-300))
            return h_y - 0.5 * (h_y0 + h_y1)
