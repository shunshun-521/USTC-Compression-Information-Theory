...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        p0 = norm.pdf(y, -1.0, sigma)
        py = 0.5 * p1 + 0.5 * p0
        mi = 0.0
        for pxy in (p1, p0):
            ratio = np.divide(pxy, py, out=np.ones_like(pxy), where=py > 1e-300)
            integrand = np.where(pxy > 1e-300, 0.5 * pxy * np.log2(np.maximum(ratio, 1e-300)), 0.0)
