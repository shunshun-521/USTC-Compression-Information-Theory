...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        y_grid = np.linspace(-6.0 * sigma - 1.0, 6.0 * sigma + 1.0, 4000)
        dy = y_grid[1] - y_grid[0]

        def cond_entropy(x_sign):
            pdf = np.exp(-((y_grid - x_sign) ** 2) / (2.0 * sigma ** 2))
            pdf /= np.sum(pdf) * dy
            llr = 2.0 * y_grid / (sigma ** 2)
            p1 = 1.0 / (1.0 + np.exp(llr))
            p1 = np.clip(p1, 1e-12, 1.0 - 1e-12)
            h = -p1 * np.log2(p1) - (1.0 - p1) * np.log2(1.0 - p1)
            return np.sum(h * pdf) * dy

        c = 1.0 - 0.5 * (cond_entropy(1.0) + cond_entropy(-1.0))
        capacities.append(c)
