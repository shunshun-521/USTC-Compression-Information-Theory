...
def _bpsk_mutual_info(snr_linear):
    """BPSK 互信息 I(X;Y) for SNR = snr_linear."""

    def integrand(y):
        p0 = np.exp(-((y - 1.0) ** 2) / (2.0 * snr_linear)) / np.sqrt(2.0 * np.pi * snr_linear)
        p1 = np.exp(-((y + 1.0) ** 2) / (2.0 * np.pi * snr_linear)) / np.sqrt(2.0 * np.pi * snr_linear)
        py = 0.5 * (p0 + p1)
        px0 = 0.5 * p0 / py
        px1 = 0.5 * p1 / py
        with np.errstate(divide="ignore", invalid="ignore"):
            h = -(px0 * np.log2(px0) + px1 * np.log2(px1))
        h = np.where(py > 1e-300, h, 0.0)
        return h * py

    mi, _ = integrate.quad(integrand, -20.0, 20.0, limit=200)
    return mi
