...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    采用 Massey 1967 数值积分（与 SNR=2R*Eb/N0 约定一致）。
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(theta):
            # C = 1 - (1/pi) * integral_0^{pi/2} log2(1 + sin^2(theta)/snr) d theta
            val = np.sin(theta) ** 2 / snr
            return np.log1p(val) / np.log(2)

        val, _ = integrate.quad(integrand, 0.0, 0.5 * np.pi, limit=200)
        capacities.append(max(0.0, min(1.0, 1.0 - val / np.pi)))
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-6, 6), num_points=600):
    ...
    for i in range(len(diff) - 1):
        if diff[i] * diff[i + 1] <= 0:
            bracket = (eb_grid[i], eb_grid[i + 1])
            break
    if bracket is None:
        return float(eb_grid[np.argmin(np.abs(diff))])
    a, b = bracket
    fa = compute_bpsk_capacity(a, rate)[0] - rate
    for _ in range(60):
        mid = 0.5 * (a + b)
        fm = compute_bpsk_capacity(mid, rate)[0] - rate
        if fa * fm <= 0:
            b = mid
        else:
            a, fa = mid, fm
...
def plot_bler_curves(
    ...
    """绘制 BLER-Eb/N0 曲线（semilogy，PNG + PDF）。"""
