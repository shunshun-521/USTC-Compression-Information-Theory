...
def _stable_log2_one_plus_exp(x):
    ...
    out = np.empty_like(x)
    mask = x > 0
    out[mask] = x[mask] / np.log(2.0) + np.log1p(np.exp(-x[mask])) / np.log(2.0)
    out[~mask] = np.log1p(np.exp(x[~mask])) / np.log(2.0)
    return out
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - (1/ln2) ∫ φ(t) ln(1 + exp(-2*γ*t²)) dt, γ = 10^(Eb/N0/10)
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(t):
            arg = -2.0 * gamma * t * t
            return np.exp(-0.5 * t * t) / np.sqrt(2.0 * np.pi) * _stable_log2_one_plus_exp(arg)
