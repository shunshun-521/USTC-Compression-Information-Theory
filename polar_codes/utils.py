...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E[log2(1+exp(-2√γ y))], y~N(√γ,1), γ=2R·Eb/N0
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            py = np.exp(-0.5 * (y - s) ** 2) / np.sqrt(2.0 * np.pi)
            x = -2.0 * s * y
            if x > 30:
                log_term = x / np.log(2)
            elif x < -30:
                ...
                log_term = np.log1p(np.exp(x)) / np.log(2)
            return py * log_term

        cap, _ = integrate.quad(integrand, -30.0, 30.0, limit=200)
        capacities.append(max(0.0, 1.0 - cap))
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-5, 10), num_points=500):
