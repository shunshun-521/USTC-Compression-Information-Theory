...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E_{y}[log2(1 + e^{-2*s*y^2})]，其中 s = SNR = 2R * 10^{Eb/N0/10}
    通过高斯数值积分计算。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            return np.log2(1.0 + np.exp(-2.0 * snr * y ** 2))

        cap, _ = integrate.quad(
            lambda y: integrand(y) * np.exp(-y ** 2 / 2.0) / np.sqrt(2.0 * np.pi),
            -np.inf,
            np.inf,
        )
