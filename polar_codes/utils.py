        ...
        for r in results:
            writer.writerow(
                [
                    ...
                    (
                        f"{r['avg_iters']:.1f}"
                        if r.get("avg_iters") is not None
                        else ""
                    ),
                ]
            )
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            val = -2.0 * snr * y
            log_inner = np.log1p(np.exp(val)) / log2
            return log_inner * np.exp(-0.5 * y ** 2) / np.sqrt(2.0 * np.pi)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    """
    找到使 BPSK 信道容量等于码率 R 的 Eb/N0（dB）（香农限）。
    由 R = 0.5*log2(1 + 2*SNR) 且 SNR = 2R*Eb/N0 推得。
    """
    eb_n0_linear = (2 ** (2 * rate) - 1) / (2 * rate)
    return 10.0 * np.log10(eb_n0_linear)
