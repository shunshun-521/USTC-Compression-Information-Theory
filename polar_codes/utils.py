...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        a = np.sqrt(10.0 ** (eb_n0_db / 10.0) * rate)
        t1 = np.logaddexp(0.0, -2.0 * a * y) / ln2
        t2 = np.logaddexp(0.0, 2.0 * a * y) / ln2
        penalty = 0.5 * np.trapezoid((t1 + t2) * gauss, y)
        capacities.append(float(np.clip(penalty - 1.0, 0.0, 1.0)))
...
def plot_bler_curves(
    ...
    for label, results in results_dict.items():
        ...
        ax.semilogy(eb, bler, "-o", markersize=4, linewidth=1.5, label=label)
...
def save_frozen_set_info(N_list, K, design_eb_n0_db, save_path):
    ...
    with open(save_path, "w", encoding="utf-8") as f:
        for N in N_list:
            ...
            rate = k_val / N
            info, frozen, _ = ga_construction(N, k_val, design_eb_n0_db)
            f.write("=" * 53 + "\n")
            f.write(
