...
def _log2_one_plus_exp_neg(x):
    ...
    return (-x + np.log1p(np.exp(x))) / np.log(2)
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E_{y|x=+1}[log2(1 + exp(-2y/σ²))]
    其中 y = x + n, x=+1 (BPSK 符号 0), n~N(0,σ²)
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            return _log2_one_plus_exp_neg(2.0 * y / (sigma ** 2)) * np.exp(
                -((y - 1.0) ** 2) / (2.0 * sigma ** 2)
            )

        val, _ = integrate.quad(integrand, -10.0, 10.0)
        val /= np.sqrt(2.0 * np.pi * sigma ** 2)
