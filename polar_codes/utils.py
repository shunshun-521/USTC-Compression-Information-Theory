...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            return (
                np.log2(1.0 + np.exp(-4.0 * gamma * y * y))
                * np.exp(-y * y)
                / np.sqrt(np.pi)
            )
...
def find_capacity_limit(rate, eb_n0_range=(-10, 10), num_points=4000):
...
def save_frozen_set_info(N_list, K, design_eb_n0_db, save_path):
    ...
    if K is None:
        ...
        k_val = int(K)
        k_func = lambda n, kv=k_val: kv if kv <= n else n // 2
    else:
        k_func = K
