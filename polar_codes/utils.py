...
def save_results_csv(results, filepath):
    ...
    with open(filepath, "w", newline="") as f:
        ...
        for r in results:
            avg_ms = r["avg_decode_time"] * 1000.0
...
def bpsk_capacity(snr_linear):
    ...
    def integrand(x):
        z = -snr_linear * x * x
        return np.logaddexp(0.0, z) / np.log(2.0)

    val, _ = integrate.quad(integrand, 0.0, 30.0, limit=200)
    return max(0.0, 1.0 - val / np.pi)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 5), num_points=200):
    ...
    """
    lo, hi = eb_n0_range
    cap_lo = bpsk_capacity(2.0 * rate * (10.0 ** (lo / 10.0)))
    cap_hi = bpsk_capacity(2.0 * rate * (10.0 ** (hi / 10.0)))
    if cap_lo > rate:
        return float(lo)
    if cap_hi < rate:
        return float(hi)
    for _ in range(60):
        mid = (lo + hi) / 2.0
        snr = 2.0 * rate * (10.0 ** (mid / 10.0))
        if bpsk_capacity(snr) > rate:
            hi = mid
        else:
            lo = mid
    return (lo + hi) / 2.0
