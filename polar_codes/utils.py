...
def _bpsk_mi_integrand(y, es_n0):
    """BPSK 互信息积分的被积函数（Es/N0 为符号信噪比）"""
    return np.log2(1.0 + np.exp(-4.0 * es_n0 * y ** 2)) * np.exp(-y ** 2)
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - (1/sqrt(pi)) * E_y[log2(1 + exp(-4 * Es/N0 * y^2))]
    其中 Es/N0 = R * 10^{Eb/N0/10}
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        es_n0 = rate * (10.0 ** (eb_n0_db / 10.0))
        integral, _ = integrate.quad(
            lambda y: _bpsk_mi_integrand(y, es_n0), -np.inf, np.inf, limit=200
        )
