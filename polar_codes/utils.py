...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        amplitude = np.sqrt(snr)

        def py(y, a=amplitude):
            return 0.5 * (
                np.exp(-0.5 * (y - a) ** 2) + np.exp(-0.5 * (y + a) ** 2)
            ) * inv_sqrt_2pi

        hy, _ = integrate.quad(
            lambda y: -py(y) * np.log2(np.maximum(py(y), 1e-300)),
            -30.0,
            30.0,
            limit=200,
        )
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    for i in range(len(caps) - 1):
        if (caps[i] - rate) * (caps[i + 1] - rate) <= 0:
            return brentq(lambda eb: compute_bpsk_capacity([eb], rate)[0] - rate, eb_grid[i], eb_grid[i + 1])
