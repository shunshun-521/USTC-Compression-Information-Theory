...
def _bpsk_capacity_per_snr(snr_linear):
    ...
    def integrand(y):
        p0 = np.exp(-((y - np.sqrt(snr_linear)) ** 2) / 2.0) / np.sqrt(2 * np.pi)
        p1 = np.exp(-((y + np.sqrt(snr_linear)) ** 2) / 2.0) / np.sqrt(2 * np.pi)
        p_sum = p0 + p1
        h = np.zeros_like(y)
        mask = p_sum > 0
        h[mask] = -(
            p0[mask] * np.log2(p0[mask] / p_sum[mask])
            + p1[mask] * np.log2(p1[mask] / p_sum[mask])
        )
        return h
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if idx == 0 or idx == len(eb_grid) - 1:
        ...
        for _ in range(50):
            ...
            cap = _bpsk_capacity_per_snr(snr)
            if cap < rate:
                lo = mid
            else:
                hi = mid
