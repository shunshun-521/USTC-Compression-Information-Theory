...
def _bpsk_mutual_info(eb_n0_db, rate):
    ...
    def p_y(y):
        ...
        return 0.5 * np.exp(-0.5 * ((y - 1) / sigma) ** 2) / norm + 0.5 * np.exp(
            -0.5 * ((y + 1) / sigma) ** 2
        ) / norm
    ...
    hy, _ = integrate.quad(entropy_integrand, -50, 50, limit=200)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    for i in range(len(grid) - 1):
        if (caps[i] - rate) * (caps[i + 1] - rate) <= 0:
            a, b = grid[i], grid[i + 1]
            ca, cb = caps[i] - rate, caps[i + 1] - rate
            if ca == cb:
                return a
            return a - ca * (b - a) / (cb - ca)
