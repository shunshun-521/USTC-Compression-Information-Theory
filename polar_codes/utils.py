    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            ...
            log1pe = np.where(
                x >= 0,
                np.log1p(np.exp(-ax)),
                x + np.log1p(np.exp(-ax)),
            )
            return (log1pe / np.log(2.0)) * np.exp(-0.5 * y ** 2)
...
def find_capacity_limit(rate, eb_n0_range=(-10, 5)):
    """找到使 BPSK 信道容量等于码率 R 的 Eb/N0（dB）。"""
    lo, hi = eb_n0_range
    if compute_bpsk_capacity(hi, rate).item() < rate:
        return hi
    if compute_bpsk_capacity(lo, rate).item() > rate:
        return lo
    for _ in range(80):
        mid = (lo + hi) / 2.0
        cap = compute_bpsk_capacity(mid, rate).item()
        if cap < rate:
            lo = mid
        else:
            hi = mid
