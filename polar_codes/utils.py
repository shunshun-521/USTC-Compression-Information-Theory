...
def _bpsk_capacity_per_snr(snr_linear):
    ...
    snr_linear = 2*R*10^(Eb/N0/10)，对应 sigma = 1/sqrt(snr_linear)。
    """
    sigma = 1.0 / np.sqrt(snr_linear)

    def integrand(y):
        py1 = np.exp(-0.5 * ((y - 1.0) / sigma) ** 2)
        pym1 = np.exp(-0.5 * ((y + 1.0) / sigma) ** 2)
        denom = py1 + pym1
        p1 = np.clip(py1 / denom, 1e-15, 1.0 - 1e-15)
        h_y = -(p1 * np.log2(p1) + (1.0 - p1) * np.log2(1.0 - p1))
        return h_y * denom

    norm = 1.0 / (2.0 * np.sqrt(2.0 * np.pi) * sigma)
    upper = 10.0 * sigma + 5.0
    val, _ = integrate.quad(integrand, -upper, upper, limit=200)
    entropy = norm * val
