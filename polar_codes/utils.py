...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    使用互信息数值积分：Y = sqrt(SNR)*X + N, X in {+1,-1}, N~N(0,1)。
  其中 SNR = 2*R*10^(Eb/N0/10)。
    """
    capacities = []
    y = np.linspace(-20.0, 20.0, 200000)
    eps = 1e-300
    for eb_n0_db in eb_n0_db_list:
        ...
        py = 0.5 * (p0 + p1)
        hy = -np.trapezoid(py * np.log2(py + eps), y)
        hy_x = -0.5 * (
            np.trapezoid(p0 * np.log2(p0 + eps), y)
            + np.trapezoid(p1 * np.log2(p1 + eps), y)
        )
        capacities.append(hy - hy_x)
