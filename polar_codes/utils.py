...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    def log2_one_plus_exp_neg(z):
        z = np.abs(z)
        if z >= 50.0:
            return 0.0
        if z >= 0:
            return float(np.log2(1.0 + np.exp(-z)))
        return float(-z / np.log(2) + np.log2(1.0 + np.exp(z)))

    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10 ** (eb_n0_db / 10.0))

        def integrand(y):
            return log2_one_plus_exp_neg(2.0 * snr * y) * np.exp(-(y**2) / 2.0)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if idx == 0 or idx == len(eb_grid) - 1:
        for eb in np.linspace(eb_n0_range[0], eb_n0_range[1], 10000):
            cap = compute_bpsk_capacity(eb, rate)
            if cap >= rate:
                return float(eb)
