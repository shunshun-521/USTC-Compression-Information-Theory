...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - (1/ln2) E[log2(1 + exp(-gamma - 2*sqrt(gamma)*U))], U~N(0,1)
    其中 gamma = 2*R*10^(Eb/N0/10) 为 Es/N0（线性）。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(u):
            val = -gamma - 2.0 * np.sqrt(gamma) * u
            val = np.clip(val, -500.0, 500.0)
            return np.exp(-u * u / 2.0) / np.sqrt(2.0 * np.pi) * np.log2(1.0 + np.exp(val))

        mi, _ = integrate.quad(integrand, -30.0, 30.0, limit=300)
