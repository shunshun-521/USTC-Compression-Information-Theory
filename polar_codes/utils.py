...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    采用 I(X;Y) = H(Y) - H(Y|X)，其中 X∈{+1,-1}，Y = sqrt(2*R*Eb/N0)*X + N(0,1)。
    """
    y = np.linspace(-20.0, 20.0, 200001)
    dy = y[1] - y[0]
    h_cond = 0.5 * np.log2(2.0 * np.pi * np.e)
    capacities = []

    for eb_n0_db in eb_n0_db_list:
        eb_lin = 10.0 ** (eb_n0_db / 10.0)
        amp = np.sqrt(2.0 * rate * eb_lin)
        py = (
            0.5 * np.exp(-0.5 * (y - amp) ** 2) / np.sqrt(2.0 * np.pi)
            + 0.5 * np.exp(-0.5 * (y + amp) ** 2) / np.sqrt(2.0 * np.pi)
        )
        hy = -np.sum(py * np.log2(py + 1e-300)) * dy
        capacities.append(hy - h_cond)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    for i in range(len(eb_grid) - 1):
        ...
        if (c0 - rate) * (c1 - rate) <= 0:
            t = (rate - c0) / (c1 - c0) if c1 != c0 else 0.0
