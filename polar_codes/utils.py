...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            n0 = np.exp(-0.5 * ((y - 1.0) / sigma) ** 2) / (np.sqrt(2.0 * np.pi) * sigma)
            n1 = np.exp(-0.5 * ((y + 1.0) / sigma) ** 2) / (np.sqrt(2.0 * np.pi) * sigma)
            llr = 2.0 * y / (sigma ** 2)
            ...
            if p1 > 1e-15:
                h -= p1 * np.log2(p1)
            return h * (0.5 * n0 + 0.5 * n1)

        val, _ = integrate.quad(integrand, -8.0 * sigma - 2.0, 8.0 * sigma + 2.0, limit=200)
        capacities.append(max(0.0, min(1.0, val)))
