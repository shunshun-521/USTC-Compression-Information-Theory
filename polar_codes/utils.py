...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - (1/sqrt(pi)) * integral exp(-y^2) log2(1 + exp(-2*Eb/N0*|y|)) dy
    """
    y = np.linspace(-12, 12, 8000)
    pdf = np.exp(-y ** 2) / np.sqrt(np.pi)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr = 10.0 ** (eb_n0_db / 10.0)
        t = 2.0 * snr * np.abs(y)
        log_term = np.where(t > 50, 0.0, np.log2(1.0 + np.exp(-t)))
        cap = 1.0 - np.trapezoid(log_term * pdf, y)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    lo = max(eb_n0_range[0], eb_grid[max(0, idx - 1)])
    hi = min(eb_n0_range[1], eb_grid[min(num_points - 1, idx + 1)])
    fine = np.linspace(lo, hi, 200)
    fine_caps = compute_bpsk_capacity(fine, rate)
    return float(fine[np.argmin(np.abs(fine_caps - rate))])
