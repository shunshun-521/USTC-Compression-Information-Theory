...
def _bpsk_mi(snr_linear):
    """BPSK 互信息（每信道使用），snr_linear = Es/N0 = 2R * 10^(Eb/N0/10)"""
    def integrand(y):
        t = -2 * np.sqrt(snr_linear) * y - snr_linear
        t = np.clip(t, -50, 50)
        return np.log2(1 + np.exp(t)) * np.exp(-y ** 2 / 2)
    val, _ = integrate.quad(integrand, -10, 10, limit=200)
    return 1.0 - val / np.sqrt(2 * np.pi)
...
def find_capacity_limit(rate, eb_n0_range=(-2, 12), num_points=1000):
    ...
    lo, hi = eb_n0_range
    ebn0s = np.linspace(lo, hi, num_points)
    caps = compute_bpsk_capacity(ebn0s, rate)
