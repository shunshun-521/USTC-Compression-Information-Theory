...
def _log2_one_plus_exp(neg_x):
    """数值稳定的 log2(1 + exp(-x))。"""
    if neg_x > 0:
        return np.log1p(np.exp(-neg_x)) / np.log(2)
    return np.log1p(np.exp(neg_x)) / np.log(2) - neg_x / np.log(2)


def compute_bpsk_capacity(eb_n0_db_list, rate=None):
    """
    计算 BPSK 离散输入信道容量（bits/channel use）。
    C = 1 - (1/pi) ∫_0^pi log2(1 + exp(-2*(Eb/N0)*sin^2(phi))) dphi
    """
    del rate
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(phi):
            return _log2_one_plus_exp(2.0 * snr * (np.sin(phi) ** 2))

        val, _ = integrate.quad(integrand, 0.0, np.pi, limit=200)
        capacities.append(1.0 - val / np.pi)
