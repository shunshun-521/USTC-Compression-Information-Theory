...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK-AWGN 信道容量（bits/channel use）的近似。
    使用连续输入 AWGN 香农公式 C = 0.5*log2(1 + 2*R*Eb/N0) 作为参考。
    """
    eb_n0_db_list = np.atleast_1d(eb_n0_db_list, dtype=np.float64)
    snr_linear = 2.0 * rate * (10.0 ** (eb_n0_db_list / 10.0))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    使用 C = 0.5*log2(1 + 2*R*Eb/N0) = R 的闭式解。
    """
    eb_no_linear = (2.0 ** (2.0 * rate) - 1.0) / (2.0 * rate)
    return float(10.0 * np.log10(eb_no_linear))
