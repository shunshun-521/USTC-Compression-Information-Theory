...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - ∫ log2(1+exp(-snr*y²)) φ(y) dy, snr = 2R·10^(Eb/N0/10)
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            ...
            if arg < -50.0:
                ...
                log_term = arg / np.log(2.0)
            else:
                log_term = np.log1p(np.exp(arg)) / np.log(2.0)
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if (c1 - rate) * (c0 - rate) < 0:
        return float(np.interp(rate, [c0, c1], [x0, eb_grid[idx]]))
    return float(np.interp(rate, [c1, c2], [eb_grid[idx], x1]))
