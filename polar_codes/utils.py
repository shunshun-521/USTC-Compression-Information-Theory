...
def _bpsk_mutual_info(eb_n0_db, rate):
    """BPSK-AWGN 互信息（bits/channel use），与 channel.eb_n0_to_sigma 一致"""
    ...
    def p_y(y):
        return (
            0.5 * norm * np.exp(-(y - 1.0) ** 2 / (2.0 * sigma ** 2))
            + 0.5 * norm * np.exp(-(y + 1.0) ** 2 / (2.0 * sigma ** 2))
        )
    ...
    def integrand(y):
        ...
        mi = 0.0
        for x in (1.0, -1.0):
            pxy = 0.5 * norm * np.exp(-(y - x) ** 2 / (2.0 * sigma ** 2))
            mi += pxy * np.log2(pxy / (0.5 * py))
        return mi
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    return [_bpsk_mutual_info(e, rate) for e in eb_n0_db_list]
