...
def bpsk_capacity_mc(eb_n0_db, rate, samples=200000, seed=0):
    """蒙特卡洛估计 BPSK-AWGN 信道容量（bits/channel use）。"""
...
def bpsk_capacity(snr_linear):
    ...
    C = 1 - E_y[log2(1 + exp(-2*snr*y^2))], y ~ N(0,1)
    """
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    return np.array([bpsk_capacity_mc(eb, rate) for eb in eb_n0_db_list])
