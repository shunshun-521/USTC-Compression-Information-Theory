...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            p0 = 0.5 * stats.norm.pdf(y, loc=1.0, scale=sigma)
            p1 = 0.5 * stats.norm.pdf(y, loc=-1.0, scale=sigma)
            py = p0 + p1
            px0 = p0 / py
            px0 = np.clip(px0, 1e-15, 1.0 - 1e-15)
            return -(px0 * np.log2(px0) + (1.0 - px0) * np.log2(1.0 - px0)) * py

        h_cond, _ = integrate.quad(integrand, -30.0, 30.0, limit=200)
