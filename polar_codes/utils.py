...
def compute_bpsk_capacity(eb_n0_db, rate):
    ...
    def integrand(y):
        return np.log2(1.0 + np.exp(-2.0 * s * y)) * np.exp(-y ** 2 / 2.0) / np.sqrt(2.0 * np.pi)

    capacity, _ = integrate.quad(integrand, -10.0, 10.0)
    return 1.0 - capacity
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    for i in range(len(eb_n0_vals) - 1):
        if capacities[i] >= rate >= capacities[i + 1] or capacities[i] <= rate <= capacities[i + 1]:
            if abs(capacities[i + 1] - capacities[i]) < 1e-12:
                return eb_n0_vals[i]
...
def save_frozen_set_info(N_list, K, design_eb_n0_db, save_path):
    ...
    with open(save_path, "w") as f:
        for N in N_list:
            ...
            f.write("=" * 53 + "\n")
            f.write(f"N={N}, K={k_val}, design_Eb/N0={design_eb_n0_db:.1f} dB, R={rate:.4f}\n")
            f.write("=" * 53 + "\n")
