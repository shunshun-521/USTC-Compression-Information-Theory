...
def bpsk_mutual_information(snr_linear):
    """BPSK-AWGN 信道互信息（bits/channel use）。"""

    def integrand(y):
        p_y = np.sqrt(snr_linear / np.pi) * np.exp(-snr_linear * (y - 1.0) ** 2)
        llr = 2.0 * snr_linear * y
        p0 = 1.0 / (1.0 + np.exp(-llr))
        p1 = 1.0 - p0
        with np.errstate(divide="ignore", invalid="ignore"):
            term = p0 * np.log2(p0) + p1 * np.log2(p1)
        term = np.where(np.isfinite(term), term, 0.0)
        return -p_y * term

    value, _ = integrate.quad(integrand, -10.0, 10.0, limit=200)
    return value
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        capacities.append(bpsk_mutual_information(snr_linear))
