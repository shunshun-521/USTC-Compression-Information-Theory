...
def _bpsk_capacity_single(eb_n0_db, rate):
    """单点 BPSK 离散输入信道容量（bits/channel use）"""
    eps = rate * (10.0 ** (eb_n0_db / 10.0))

    def integrand(y):
        return np.log2(1.0 + np.exp(-2.0 * eps * y * y)) * np.exp(-y ** 2 / 2.0)
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
  ...
  返回与 eb_n0_db_list 对应的容量数组。
    """
    return np.array([_bpsk_capacity_single(e, rate) for e in eb_n0_db_list])
...
def find_capacity_limit(rate, eb_n0_range=(-2, 12), num_points=1000):
  ...
  """
    ...
    for _ in range(60):
        mid = (lo + hi) / 2.0
        if _bpsk_capacity_single(mid, rate) < rate:
            lo = mid
        else:
            hi = mid
    return (lo + hi) / 2.0
