...
def _bpsk_capacity_scalar(eb_n0_db, rate):
    """
    BPSK-AWGN 互信息 I(X;Y)（bits/channel use），I(X;Y)=H(Y)-H(Y|X)。
    """
    from channel import eb_n0_to_sigma

    sigma_n = eb_n0_to_sigma(eb_n0_db, rate)
    norm = 1.0 / (np.sqrt(2.0 * np.pi) * sigma_n)

    def hy_integrand(y):
        p0 = norm * np.exp(-((y - 1.0) ** 2) / (2.0 * sigma_n**2))
        p1 = norm * np.exp(-((y + 1.0) ** 2) / (2.0 * sigma_n**2))
        py = 0.5 * (p0 + p1)
        if py <= 0.0:
            return 0.0
    ...
    hy, _ = integrate.quad(hy_integrand, -30.0, 30.0, limit=200)
    hyx, _ = integrate.quad(hyx_integrand, -30.0, 30.0, limit=200)
    return hy - hyx
