...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        es_n0 = 2 * rate * (10 ** (eb_n0_db / 10.0))
        sigma = 1 / np.sqrt(2 * es_n0)

        def py(y):
            norm = 1 / (np.sqrt(2 * np.pi) * sigma)
            return 0.5 * norm * np.exp(-(y - 1) ** 2 / (2 * sigma ** 2)) + \
                   0.5 * norm * np.exp(-(y + 1) ** 2 / (2 * sigma ** 2))

        hy, _ = integrate.quad(
            lambda y: -py(y) * np.log2(max(py(y), 1e-300)), -10, 10
        )
