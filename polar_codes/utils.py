...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E_{y}[log2(1 + exp(-2*s*y))]，s = 2R * 10^{Eb/N0/10}
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10 ** (eb_n0_db / 10.0))
        grid = np.linspace(-12.0, 12.0, 20001)
        integrand = np.log2(1.0 + np.exp(-2.0 * snr * grid)) * np.exp(-grid ** 2 / 2.0)
        integrand /= np.sqrt(2.0 * np.pi)
        capacities.append(1.0 - np.trapezoid(integrand, grid))
