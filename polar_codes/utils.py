...
def save_results_csv(results, filepath):
    ...
    with open(filepath, "w", newline="", encoding="utf-8") as f:
        ...
        for row in results:
            writer.writerow(
                {
                    "eb_n0_db": f"{row['eb_n0_db']:.2f}",
                    "bler": f"{row['bler']:.6e}",
                    "ber": f"{row['ber']:.6e}",
                    "num_errors": row["num_errors"],
                    "num_frames": row["num_frames"],
                    "avg_decode_time_ms": f"{row['avg_decode_time'] * 1000:.6f}",
                    "avg_iters": "" if row["avg_iters"] is None else f"{row['avg_iters']:.2f}",
                }
            )
...
def _bpsk_mutual_info(eb_n0_linear):
    """BPSK-AWGN 互信息（每信道使用 bit）。"""

    def integrand(y):
        return (
            np.exp(-y ** 2 / 2.0)
            / np.sqrt(2.0 * np.pi)
            * np.log2(1.0 + np.exp(-2.0 * eb_n0_linear - 2.0 * np.sqrt(eb_n0_linear) * y))
        )
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        capacities.append(_bpsk_mutual_info(eb_n0_linear))
...
def save_frozen_set_info(N_list, K, design_eb_n0_db, save_path):
    ...
    with open(save_path, "w", encoding="utf-8") as f:
        for N in N_list:
            ...
            f.write("=" * 53 + "\n")
            f.write(
                f"N={N}, K={k_val}, design_Eb/N0={design_eb_n0_db:.1f} dB, R={rate:.4f}\n"
            )
            f.write("=" * 53 + "\n")
