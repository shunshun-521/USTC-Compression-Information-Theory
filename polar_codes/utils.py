...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(t):
            return np.exp(-(t**2)) * np.log2(1.0 + np.exp(-snr * (t**2)))

        integral, _ = integrate.quad(integrand, 0, 40)
        caps.append(1.0 - integral / np.sqrt(np.pi))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    return float(10.0 * np.log10((2.0 ** (2.0 * rate) - 1.0) / (2.0 * rate)))
