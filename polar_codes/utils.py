...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        py = (
            np.exp(-0.5 * ((y - 1.0) / sigma) ** 2) + np.exp(-0.5 * ((y + 1.0) / sigma) ** 2)
        ) / (2.0 * np.sqrt(2.0 * np.pi) * sigma)
        py = np.maximum(py, 1e-300)
        hy = -np.trapezoid(py * np.log2(py), y)
        hn = 0.5 * np.log2(2.0 * np.pi * np.e * sigma2)
        capacities.append(hy - hn)
