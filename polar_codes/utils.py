...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    采用积分形式：C = 1 - (1/sqrt(pi)) ∫ log2(1+exp(-γ x²)) exp(-x²) dx，
    其中 γ = 2R · 10^(Eb/N0/10)。
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(x):
            ...
            if z < -700.0:
                ...
                val = np.log1p(np.exp(z)) / np.log(2.0)
            return val * np.exp(-x * x)

        val, _ = integrate.quad(integrand, 0.0, np.inf, limit=200)
        capacities.append(max(0.0, 1.0 - val / np.sqrt(np.pi)))
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    """
    找到使 AWGN 信道容量等于码率 R 的 Eb/N0（dB）参考值。
    使用 SNR = 2R·10^(Eb/N0/10) 与 C = 0.5·log2(1+SNR)（BPSK 对齐的常用参考限）。
    """
    target_snr = 2.0 ** (2.0 * rate) - 1.0
    target_eb_linear = target_snr / (2.0 * rate)
    return float(10.0 * np.log10(target_eb_linear))
