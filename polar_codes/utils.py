...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK-AWGN 信道互信息（bits/channel use），即对称 BPSK 输入容量。
    使用 I(X;Y) = H(Y) - H(Y|X)，其中 SNR = 2R * 10^(Eb/N0/10)。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ...
        hy, _ = integrate.quad(hy_integrand, -12.0, 12.0, limit=200)
        hy_given_x = 0.5 * np.log2(2.0 * np.pi * np.e * var)
        capacities.append(max(0.0, hy - hy_given_x))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=200):
    ...
    def f(x):
        return compute_bpsk_capacity([x], rate)[0] - rate

    try:
        return brentq(f, eb_grid[idx - 1], eb_grid[idx])
    except ValueError:
        return eb_grid[idx]
