...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    capacities = []
    y_grid = np.linspace(-12, 12, 2401)
    dy = y_grid[1] - y_grid[0]

    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10 ** (eb_n0_db / 10.0))
        amp = np.sqrt(snr)
        p_y_given_0 = norm.pdf(y_grid, amp, 1.0)
        p_y_given_1 = norm.pdf(y_grid, -amp, 1.0)
        p_y = 0.5 * (p_y_given_0 + p_y_given_1)
        mi = 0.0
        for p_xy in (p_y_given_0, p_y_given_1):
            mask = (p_xy > 0) & (p_y > 0)
            mi += 0.5 * np.sum(p_xy[mask] * np.log2(p_xy[mask] / p_y[mask])) * dy
