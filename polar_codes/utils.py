...
def _bpsk_mutual_info(es_n0_linear):
    """BPSK 互信息 I(X;Y)，符号能量 Es=1，Es/N0 = es_n0_linear。"""
    if es_n0_linear <= 0:
        return 0.0
    sigma2 = 1.0 / (2.0 * es_n0_linear)
    ...
    def integrand(y):
        ...
        for x in (1.0, -1.0):
            ...
            ratio = p_xy / (0.5 * p_y)
            if ratio > 0:
                mi += p_xy * np.log2(ratio)
        return mi

    mi, _ = integrate.quad(integrand, -30, 30, limit=200)
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        capacities.append(_bpsk_mutual_info(es_n0))
