...
def _bpsk_mi_per_use(eb_n0_db, rate):
    """BPSK 互信息 (bits/channel use) via numerical integration."""
    snr = 2.0 * rate * (10 ** (eb_n0_db / 10.0))
    sigma_n = 1.0 / np.sqrt(snr)

    def integrand(y):
        p0 = (1.0 / (np.sqrt(2 * np.pi) * sigma_n)) * np.exp(-((y - 1) ** 2) / (2 * sigma_n ** 2))
        p1 = (1.0 / (np.sqrt(2 * np.pi) * sigma_n)) * np.exp(-((y + 1) ** 2) / (2 * sigma_n ** 2))
        denom = p0 + p1
        denom = np.maximum(denom, 1e-300)
        return p0 * np.log2(2 * p0 / denom) + p1 * np.log2(2 * p1 / denom)

    mi, _ = integrate.quad(integrand, -20, 20, limit=200)
    return mi
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    """
    return np.array([_bpsk_mi_per_use(eb, rate) for eb in eb_n0_db_list])
