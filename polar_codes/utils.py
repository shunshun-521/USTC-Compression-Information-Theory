...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        mask0 = bits == 0
        mask1 = ~mask0
        h[mask0] = -np.log2(np.clip(p0[mask0], 1e-12, 1.0))
        h[mask1] = -np.log2(np.clip(p1[mask1], 1e-12, 1.0))
        capacities.append(float(np.mean(h)))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=200):
    ...
    if idx == 0 or idx == len(eb_grid) - 1:
        for eb in np.linspace(eb_n0_range[0], eb_n0_range[1], 2000):
