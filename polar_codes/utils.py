...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - (1/sqrt(pi)) * integral exp(-t^2) log2(1 + exp(-4*gamma_s*t^2)) dt
    其中 gamma_s = Eb/N0 * R（线性）。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        gamma_s = (10.0 ** (eb_n0_db / 10.0)) * rate

        def integrand(t):
            t2 = t * t
            log_term = np.log2(1.0 + np.exp(-4.0 * gamma_s * t2))
            if not np.isfinite(log_term):
                log_term = 0.0
            return np.exp(-t2) * log_term
