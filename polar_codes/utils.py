...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    y = np.linspace(0.0, 12.0, 8001)
    weight = np.exp(-y ** 2 / 2.0) / np.sqrt(2.0 * np.pi)
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10 ** (eb_n0_db / 10.0))
        log_term = _log2_one_plus_exp(-2.0 * snr * y)
        val = 2.0 * np.trapezoid(log_term * weight, y)
