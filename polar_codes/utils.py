...
def _bpsk_mi_integrand(y, snr):
    """BPSK 互信息损失项（对称数值稳定形式）"""
    ay = np.abs(y)
    log_term = np.log2(1.0 + np.exp(-2.0 * snr * ay))
    return (1.0 / np.sqrt(2 * np.pi)) * np.exp(-0.5 * y ** 2) * log_term
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        capacities.append(max(0.0, 1.0 - mi))
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-2, 2), num_points=2000):
