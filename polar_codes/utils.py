...
def compute_bpsk_capacity(eb_n0_db, rate):
    ...
    def integrand(y):
        p_y = (1.0 / np.sqrt(2 * np.pi * sigma ** 2)) * np.exp(-((y - 1) ** 2) / (2 * sigma ** 2))
        return p_y * np.log2(1.0 + np.exp(-2.0 * snr * y))

    y_max = 1.0 + 8.0 * sigma
    capacity, _ = integrate.quad(integrand, -y_max, y_max)
    capacity += integrate.quad(
        lambda y: ((1.0 / np.sqrt(2 * np.pi * sigma ** 2))
                   * np.exp(-((y + 1) ** 2) / (2 * sigma ** 2))
                   * np.log2(1.0 + np.exp(2.0 * snr * y))),
        -y_max,
        y_max,
    )[0]
    return 1.0 - capacity
...
def plot_bler_curves(
    ...
    if shannon_limit_db is not None:
        plt.axvline(shannon_limit_db, color="gray", linestyle="--", label=f"Shannon limit ({shannon_limit_db:.2f} dB)")
