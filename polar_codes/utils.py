...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    采用标准 BPSK-AWGN 互信息公式（数值积分）。
    """
    from scipy import integrate

    capacities = []
    for eb_n0_db in eb_n0_db_list:
        ebno = 10.0 ** (eb_n0_db / 10.0) * rate

        def integrand(t):
            z = -4.0 * ebno * t
            l2 = _log2_one_plus_exp(z)
            return np.exp(-t) * l2

        integral, _ = integrate.quad(integrand, 0.0, np.inf, limit=200)
        c = 1.0 - 2.0 / np.sqrt(np.pi) * integral
        capacities.append(max(float(c), 0.0))
