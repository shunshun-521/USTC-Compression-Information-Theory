...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = ∫ p(y) [1 - H2(p(y|x))] dy，其中 p(y|0) ∝ exp(-(y-1)^2/2σ^2) 等效为 LLR 形式。
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        def integrand(y):
            logit = np.logaddexp(0.0, -2.0 * snr * y)
            p0 = np.exp(-logit)
            hy = binary_entropy(p0)
            return np.exp(-(y ** 2) / 2.0) / np.sqrt(2.0 * np.pi) * (1.0 - hy)
...
def find_capacity_limit(rate, eb_n0_range=(-2, 6), num_points=1000):
    ...
    fine = np.linspace(e0, e1, 200)
    fine_caps = compute_bpsk_capacity(fine, rate)
    idx2 = np.argmin(np.abs(fine_caps - rate))
    return float(fine[idx2])
