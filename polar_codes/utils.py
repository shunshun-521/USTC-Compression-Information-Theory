...
def compute_bpsk_capacity(eb_n0_db_list, rate, num_samples=200000, seed=42):
    """
    计算 BPSK 离散输入信道容量（bits/channel use）。
    使用与信道模型一致的蒙特卡洛估计：
    C = 1 - E[log2(1 + exp(-|LLR|))], LLR = 2y/sigma^2
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        y = rng.normal(0.0, sigma, num_samples)
        llr = 2.0 * y / (sigma ** 2)
        mi_penalty = np.mean(np.log2(1.0 + np.exp(-np.abs(llr))))
        capacities.append(1.0 - mi_penalty)
    return np.array(capacities)


def find_capacity_limit(rate, eb_n0_range=(-6, 6), num_points=200):
    """找到使 BPSK 信道容量等于码率 R 的 Eb/N0（dB）。"""
    eb_grid = np.linspace(eb_n0_range[0], eb_n0_range[1], num_points)
    caps = compute_bpsk_capacity(eb_grid, rate, num_samples=100000)
