            ...
            writer.writerow(
                {
                    ...
                    "avg_decode_time_ms": f"{r['avg_decode_time'] * 1000.0:.6f}",
...
def bpsk_capacity(eb_n0_db, rate):
    ...
    y_grid = np.linspace(-8.0 * sigma, 8.0 * sigma, 4000)
    pdf = (1.0 / (np.sqrt(2.0 * np.pi) * sigma)) * np.exp(-((y_grid - 1.0) ** 2) / (2.0 * sigma ** 2))
    integrand = np.log2(1.0 + np.exp(-2.0 * snr * y_grid))
    return 1.0 - np.trapezoid(pdf * integrand, y_grid)


def compute_bpsk_capacity(eb_n0_db_list, rate):
    """返回各 Eb/N0 点对应的 BPSK 信道容量。"""
...
def save_frozen_set_info(N_list, K, design_eb_n0_db, save_path, rate=0.5):
    ...
    with open(save_path, "w", encoding="utf-8") as f:
        for N in N_list:
            ...
            f.write(np.array2string(info_idx, threshold=len(info_idx), max_line_width=120) + "\n")
            f.write(f"Frozen indices (all {len(frozen_idx)}):\n")
            f.write(np.array2string(frozen_idx, threshold=len(frozen_idx), max_line_width=120) + "\n")
            f.write("-" * 53 + "\n")
