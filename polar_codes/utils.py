...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    """
    计算 BPSK-AWGN 二进制输入信道容量（bits/channel use）。
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        gamma = 2 * rate * (10 ** (eb_n0_db / 10.0))
        sigma = 1.0 / np.sqrt(gamma)
        norm = sigma * np.sqrt(2 * np.pi)

        def integrand(y):
            p0 = np.exp(-0.5 * ((y - 1.0) / sigma) ** 2) / norm
            p1 = np.exp(-0.5 * ((y + 1.0) / sigma) ** 2) / norm
            py = 0.5 * (p0 + p1)
            if py < 1e-300:
                return 0.0
            val = 0.0
            for p_cond in (p0, p1):
                if p_cond > 1e-300:
                    val += 0.5 * p_cond * np.log2(p_cond / py)
            return val

        mi, _ = integrate.quad(integrand, -12, 12, limit=200)
