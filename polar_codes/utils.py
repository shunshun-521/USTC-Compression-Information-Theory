...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    C = 1 - E_y[log2(1 + e^{-2*s*y})]，s = SNR = 2R * 10^{Eb/N0/10}
    """
    capacities = []
    for eb_n0_db in eb_n0_db_list:
        snr = 2.0 * rate * (10.0 ** (eb_n0_db / 10.0))
        # E_y over y ~ N(0,1)
        integrand = lambda y: np.log2(1.0 + np.exp(-2.0 * snr * y)) * np.exp(-0.5 * y * y)
    ...
    with open(save_path, "w") as f:
        for N in N_list:
            if K is None:
                ...
                K_local = K if isinstance(K, int) else K
            rate = K_local / N
            info_idx, frozen_idx, _ = ga_construction(N, K_local, design_eb_n0_db)
            f.write("=" * 53 + "\n")
            f.write(
                f"N={N}, K={K_local}, design_Eb/N0={design_eb_n0_db} dB, R={rate:.4f}\n"
            )
            f.write("=" * 53 + "\n")
