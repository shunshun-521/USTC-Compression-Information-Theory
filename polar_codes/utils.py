...
def _bpsk_mi(snr_linear):
    """BPSK-AWGN 互信息（bits/channel use），snr = Es/N0。"""
    sigma = 1.0 / np.sqrt(snr_linear)
    y = np.linspace(-1.0 - 8.0 * sigma, 1.0 + 8.0 * sigma, 20000)
    dy = y[1] - y[0]
    norm = 1.0 / (np.sqrt(2.0 * np.pi) * sigma)
    p0 = norm * np.exp(-((y - 1.0) ** 2) / (2.0 * sigma ** 2))
    p1 = norm * np.exp(-((y + 1.0) ** 2) / (2.0 * sigma ** 2))
    p_y = 0.5 * (p0 + p1)
    with np.errstate(divide="ignore", invalid="ignore"):
        hy = -np.sum(np.where(p_y > 0, p_y * np.log2(p_y), 0.0)) * dy
    hy_given_x = 0.5 * np.log2(2.0 * np.pi * np.e * sigma ** 2)
    return float(max(hy - hy_given_x, 0.0))
...
def find_capacity_limit(rate, eb_n0_range=(-5, 20), num_points=1000):
    ...
    if idx == 0 or idx == len(eb_grid) - 1:
        ...
        for _ in range(50):
            mid = (lo + hi) / 2.0
            if _bpsk_mi(2.0 * rate * (10 ** (mid / 10.0))) < rate:
                lo = mid
            else:
                hi = mid
        return (lo + hi) / 2.0
