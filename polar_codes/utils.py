...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        span = max(8.0 * gamma, 8.0)
        u = np.linspace(-span, span, 20001)
        pdf = np.exp(-(u - gamma) ** 2 / (2.0 * gamma)) / np.sqrt(2.0 * np.pi * gamma)
        integrand = pdf * np.log2(1.0 + np.exp(np.clip(-2.0 * u, -700, 700)))
        capacities.append(float(np.trapezoid(integrand, u)))
