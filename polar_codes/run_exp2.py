...
from decoder_sc import sc_decode_channel
...
if os.environ.get("POLAR_FAST_SIM", "0") == "1":
    EB_N0_RANGE = np.arange(2.0, 4.5, 0.5)
    L_LIST = [2, 4]
    RUN_CASCL = False
else:
...
def sc_decoder(llr_ch):
    return sc_decode_channel(llr_ch, frozen_bits), None

results_sc = run_simulation(
    N, K, EB_N0_RANGE, sc_decoder, "sc",
    crc_length=0, info_indices=info_idx, frozen_bits=frozen_bits,
    design_eb_n0_db=DESIGN_EBN0, verbose=True, **sim_kw,
)
