...
if __name__ == '__main__':
    ...
    EB_N0_RANGE = (
        np.arange(2.0, 4.5, 0.5) if QUICK else np.arange(1.0, 5.5, 0.25)
    )
    CASCL_L = 2 if QUICK else 8
    if QUICK:
        print(f'POLAR_QUICK=1: MAX_FRAMES={MAX_FRAMES}, MIN_ERRORS={MIN_ERRORS}')
    ...
    for L in L_LIST:
        ...
        def make_scl_decoder(list_size):
            def scl_decoder(llr_ch):
                u_hat, pm = SCLDecoder(
                    N, frozen_bits, list_size=list_size, crc_length=0
                ).decode(llr_ch)
                return u_hat, None
            return scl_decoder

        results = run_simulation(
            N, K, EB_N0_RANGE, make_scl_decoder(L), 'scl',
            MAX_FRAMES, MIN_ERRORS, info_indices=info_idx,
        )
        label = f'SCL (L={L})'
    ...
    def cascl_decoder(llr_ch):
        u_hat, pm = SCLDecoder(
            N, frozen_bits, list_size=CASCL_L, crc_length=CRC_LENGTH
        ).decode(llr_ch)
        return u_hat, None
    ...
    save_results_csv(
        results_cascl, f'results/exp2_cascl_L{CASCL_L}_N{N}_R0.5.csv'
    )
