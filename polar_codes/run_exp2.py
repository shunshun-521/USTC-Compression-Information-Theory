...
def run_unit_tests():
    from validate import test_encoder, test_sc_lossless, test_scl_equiv_sc
...
MAX_FRAMES = int(os.environ.get("POLAR_MAX_FRAMES", "500" if _QUICK else "100000"))
MIN_ERRORS = int(os.environ.get("POLAR_MIN_ERRORS", "20" if _QUICK else "100"))
EB_N0_RANGE = np.arange(1.0, 3.5, 0.5) if _QUICK else np.arange(1.0, 5.5, 0.25)
...
if __name__ == "__main__":
    ...
    def cascl_decoder(llr_ch):
        dec = SCLDecoder(
            N, frozen_bits.astype(bool), list_size=8,
            crc_length=CRC_LENGTH, info_indices=info_idx,
        )
        return dec.decode(llr_ch)[0], None

    results_cascl = run_simulation(
        N, K, EB_N0_RANGE, cascl_decoder, "scl", MAX_FRAMES, MIN_ERRORS,
        crc_length=CRC_LENGTH, info_indices=info_idx, frozen_bits=frozen_bits,
