...
EB_N0_RANGE = np.arange(1.0, 3.0, 0.5) if FAST else np.arange(1.0, 5.5, 0.25)


def run_unit_tests():
    u = np.array([1, 0, 1, 1])
    x = polar_encode(u)
    assert np.array_equal(x, polar_encode(u))

    N_t, K_t = 64, 32
    info_idx, _, _ = ga_construction(N_t, K_t, 2.5)
    frozen_bits = np.ones(N_t, dtype=int)
    frozen_bits[info_idx] = 0
    rng = np.random.default_rng(1)
    sigma = eb_n0_to_sigma(8.0, 0.5)
    for _ in range(10):
        u_test = np.zeros(N_t, dtype=np.int8)
        u_test[info_idx] = rng.integers(0, 2, K_t)
        llr = compute_llr(
            awgn_channel(bpsk_modulate(polar_encode(u_test)), sigma, rng), sigma
        )
        u_sc = sc_decode(llr, frozen_bits)
        u_scl, _ = SCLDecoder(N_t, frozen_bits, list_size=1).decode(llr)
        assert np.array_equal(u_sc, u_scl)
    print("[PASS] run_exp2 单元测试")
