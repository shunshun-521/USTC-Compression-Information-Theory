...
from exp_common import run_unit_tests, sim_params
...
CRC_LENGTH = 8
L_LIST = [2, 4, 8]
MAX_FRAMES, MIN_ERRORS, EB_N0_RANGE = sim_params(
    default_eb=np.arange(1.0, 5.5, 0.25)
)
...
for L in L_LIST:
    print(f"\nSCL 仿真: N={N}, K={K}, L={L}")
    def scl_decoder(llr, _L=L):
        u_hat, pm = SCLDecoder(N, frozen_bits, list_size=_L, crc_length=0).decode(llr)
        return u_hat, None
...
def cascl_decoder(llr):
    u_hat, pm = SCLDecoder(N, frozen_bits, list_size=8, crc_length=CRC_LENGTH).decode(llr)
    return u_hat, None
