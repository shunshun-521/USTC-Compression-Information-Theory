...
def run_unit_tests():
    ...
    for _ in range(30):
        ...
        llr_t = 2 * (1 - 2 * x_t) / 1e-12
        u_sc = sc_decode(llr_t, frozen_bits.astype(bool))
        u_scl, _ = SCLDecoder(N, frozen_bits, list_size=1, crc_length=0).decode(
            llr_t
        )
        if not np.array_equal(u_sc, u_scl):
            mismatches += 1
    assert mismatches == 0, "L=1 的 SCL 应与 SC 等价"
    print("单元测试通过。")
...
MAX_FRAMES = 5000
MIN_ERRORS = 40
EB_N0_RANGE = np.arange(2.0, 6.5, 0.5)
