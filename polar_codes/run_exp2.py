...
N_test, K_test = 64, 32
info_idx, _, _ = ga_construction(N_test, K_test, 2.5)
fb = np.ones(N_test, dtype=bool)
fb[info_idx] = False
...
for _ in range(100):
    ...
    assert np.array_equal(sc_decode(llr, fb)[info_idx], u[info_idx])

u_test = np.zeros(32, dtype=int)
u_test[info_idx[:16]] = rng.integers(0, 2, 16)
llr = compute_llr(bpsk_modulate(polar_encode(u_test[:32] if len(u_test)==32 else u_test)), 0.01)
u_sc = sc_decode(llr, fb[:32])
u_scl, _ = SCLDecoder(32, fb[:32], list_size=1).decode(llr)
...
results_cascl = run_simulation(
    ...
    verbose=True, info_indices=info_idx,
)
