...
def _sim_params():
  fast = os.environ.get("POLAR_FAST_SIM", "")
  if fast:
    return 2000, 20, np.arange(2.0, 4.5, 0.5)
  return 100000, 100, np.arange(1.0, 5.5, 0.25)


def run_unit_tests():
  from validate import test_encoder, test_sc_noiseless, test_scl_l1_equals_sc
  test_encoder()
  test_sc_noiseless()
...
if __name__ == "__main__":
  ...
  def sc_decoder(llr_ch):
    return sc_decode_with_llr_reversal(llr_ch, frozen_bits), None
  ...
  results_cascl = run_simulation(
    ...
    crc_encode_fn=crc_encode, verbose=True,
  )
