...
def run_unit_tests():
    from decoder_scl import SCLDecoder as SCL
    ...
    llr = compute_llr(bpsk_modulate(polar_encode(u_t)), eb_n0_to_sigma(12.0, 0.5))
    u_sc = sc_decode(llr, fb)
    u_scl, _ = SCL(N, fb, list_size=1).decode(llr)
...
results_sc = run_simulation(
    N, K, EB_N0_RANGE, sc_decoder, "sc", MAX_FRAMES, MIN_ERRORS, verbose=True, info_idx=info_idx
)
...
for L in L_LIST:
    ...
    results = run_simulation(
        N, K, EB_N0_RANGE, scl_decoder, "scl", MAX_FRAMES, MIN_ERRORS, verbose=True, info_idx=info_idx
    )
