...
from simulation import fast_sim_enabled, run_simulation
...
if __name__ == "__main__":
    ...
    CRC_LENGTH = 8
    L_LIST = [2, 4, 8]
    MAX_FRAMES = 500 if fast_sim_enabled() else 100000
    MIN_ERRORS = 10 if fast_sim_enabled() else 100
    EB_N0_RANGE = (
        np.arange(2.0, 4.5, 0.5) if fast_sim_enabled() else np.arange(1.0, 5.5, 0.25)
    )
    ...
    for L in L_LIST:
        ...
        def scl_decoder(llr_ch, _L=L):
            u_hat, _ = SCLDecoder(N, frozen_bool, list_size=_L, crc_length=0).decode(
                llr_ch
            )
            return u_hat, None
    ...
    def cascl_decoder(llr_ch):
        u_hat, _ = SCLDecoder(
            N, frozen_bool, list_size=8, crc_length=CRC_LENGTH
        ).decode(llr_ch)
        return u_hat, None
