...
def run_unit_tests():
    ...
    for _ in range(50):
        ...
        llr = compute_llr(
            bpsk_modulate(polar_encode(u)) + rng.normal(0, sigma, N),
            sigma,
        )
...
def main():
    ...
    results_sc = run_simulation(
        ...
        info_indices=info_idx, frozen_bits=frozen_bool, verbose=True,
    )
    ...
    for L in L_LIST:
        ...
        def scl_decoder(llr_ch, list_size=L):
            u_hat, _ = SCLDecoder(N, frozen_bool, list_size=list_size).decode(llr_ch)
            return u_hat, None
    ...
    results_cascl = run_simulation(
        ...
        crc_length=CRC_LENGTH, info_indices=info_idx, frozen_bits=frozen_bool, verbose=True,
    )
