...
from verify import test_encoder, test_sc_decoder, test_scl_l1_equals_sc
...
MAX_FRAMES = 200 if QUICK else 100000
MIN_ERRORS = 10 if QUICK else 100
EB_N0_RANGE = np.arange(2.0, 4.0, 1.0) if QUICK else np.arange(1.0, 5.5, 0.25)
...
for L in L_LIST:
    ...
    def make_scl_decoder(list_size):
        def scl_decoder(llr_ch):
            u_hat, pm = SCLDecoder(N, frozen_bool, list_size=list_size, crc_length=0).decode(
                llr_ch
            )
            return u_hat, None
...
if QUICK:
    results_cascl = {"eb_n0_db": 0, "bler": 0, "ber": 0, "num_errors": 0, "num_frames": 0, "avg_decode_time": 0, "avg_iters": None}
else:
