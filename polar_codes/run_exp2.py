...
from channel import bpsk_modulate, compute_llr, prepare_decoder_llr
...
def run_unit_tests():
    ...
    for _ in range(20):
        ...
        llr = prepare_decoder_llr(
            compute_llr(bpsk_modulate(polar_encode(u)), 0.01), N
        )
...
if __name__ == "__main__":
    ...
    for L in L_LIST:
        print(f"\n{'=' * 60}")
        print(f"SCL 仿真: N={N}, K={K}, L={L}")
        print(f"{'=' * 60}")

        def scl_decoder(llr_ch, _L=L):
            u_hat, pm = SCLDecoder(N, fb_bool, list_size=_L, crc_length=0).decode(llr_ch)
            return u_hat, None
    ...
    def cascl_decoder(llr_ch):
        u_hat, pm = SCLDecoder(N, fb_bool, list_size=8, crc_length=CRC_LENGTH).decode(llr_ch)
        return u_hat, None
