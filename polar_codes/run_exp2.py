...
def run_unit_tests():
    ...
    for _ in range(30):
        ...
        us, _ = SCLDecoder(N, frozen, list_size=1).decode(llr)
        uc = sc_decode(llr, frozen)
        assert np.array_equal(us, uc), "单路径 SCL 应等价于 SC"
    print("SCL 单元测试通过。")
...
results_sc = run_simulation(
    ...
    info_indices=info_idx, frozen_bits=frozen_bits, verbose=True,
)
...
for L in L_LIST:
    ...
    def make_scl_decoder(list_size):
        def scl_decoder(llr_ch):
            u_hat, _ = SCLDecoder(N, frozen_bool, list_size=list_size, crc_length=0).decode(llr_ch)
            return u_hat, None
        return scl_decoder
