...
from simulation import run_simulation, get_sim_params
...
for L in L_LIST:
    print(f"\nSCL 仿真: N={N}, K={K}, L={L}")
    scl_obj = SCLDecoder(N, frozen_bits, list_size=L, crc_length=0)

    def scl_decoder(llr_ch, _obj=scl_obj):
        u_hat, _ = _obj.decode(llr_ch)
        return u_hat, None
...
cascl_obj = SCLDecoder(N, frozen_bits, list_size=8, crc_length=CRC_LENGTH)


def cascl_decoder(llr_ch):
    u_hat, _ = cascl_obj.decode(llr_ch)
    return u_hat, None
