...
EB_N0_RANGE = np.arange(2.0, 6.5, 0.5) if QUICK else np.arange(1.0, 5.5, 0.25)
...
for L in L_LIST:
    ...
    save_results_csv(
        results, os.path.join(RESULTS_DIR, f"exp2_scl_L{L}_N{N}_R0.5.csv")
    )
...
def cascl_decoder(llr_ch):
    u_hat, _ = SCLDecoder(N, frozen_bits, list_size=8, crc_length=CRC_LENGTH).decode(
        llr_ch
    )
    return u_hat, None
...
save_results_csv(
    results_cascl, os.path.join(RESULTS_DIR, f"exp2_cascl_L8_N{N}_R0.5.csv")
)
