...
def run_unit_tests():
    u = np.array([1, 0, 1, 1])
    x = polar_encode(u)
    assert np.array_equal(x, polar_encode(u)), f"编码器错误: {x}"
    verify_sc_decoders(N=64, K=32, num_frames=30, eb_n0_db=15.0)
    verify_scl_equals_sc(N=64, K=32, num_frames=20, eb_n0_db=8.0)
    print("单元测试通过。")
