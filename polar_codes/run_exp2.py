...
def run_unit_tests():
    ...
    llr = compute_llr(bpsk_modulate(x_t), sigma)
...
if __name__ == '__main__':
    ...
    results_sc = run_simulation(
        ...
        frozen_bits=frozen_bits, info_indices=info_idx, verbose=True,
    )
    ...
    for L in L_LIST:
        print(f"\nSCL 仿真: N={N}, K={K}, L={L}")
        scl = SCLDecoder(N, frozen_bits, list_size=L, info_indices=info_idx)
    ...
    cascl = SCLDecoder(N, frozen_bits, list_size=8, crc_length=CRC_LENGTH, info_indices=info_idx)
