...
fb_t[frozen_t] = True
llr_t = __import__('channel').compute_llr(
    __import__('channel').bpsk_modulate(polar_encode(np.zeros(N_test, dtype=int))), 0.01
)
u_sc = sc_decode(llr_t, fb_t)
u_scl, _ = SCLDecoder(N_test, fb_t, list_size=1).decode(llr_t)
...
EB_N0_RANGE = np.arange(1.0, 8.5, 0.5) if FAST else np.arange(1.0, 5.5, 0.25)
