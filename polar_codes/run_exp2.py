...
if __name__ == "__main__":
    ...
    results_sc = run_simulation(
        ...
        info_indices=info_idx, frozen_bits=frozen_bits.astype(bool),
    )
    ...
    for L in L_LIST:
        ...
        def scl_decoder(llr_ch, _L=L):
            u_hat, pm = SCLDecoder(N, frozen_bits.astype(bool), list_size=_L, crc_length=0).decode(llr_ch)
            return u_hat, None
    ...
    results_cascl = run_simulation(
        ...
        crc_length=CRC_LENGTH, info_indices=info_idx, frozen_bits=frozen_bits.astype(bool),
    )
