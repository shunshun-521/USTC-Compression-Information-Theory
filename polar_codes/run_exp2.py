...
def run_unit_tests():
    ...
    for _ in range(50):
        ...
        if not np.array_equal(uh_sc[info_idx], u_test[info_idx]):
            errors_sc += 1
        if not np.array_equal(uh_scl[info_idx], u_test[info_idx]):
            errors_scl += 1
        assert np.array_equal(uh_sc, uh_scl), "单路径 SCL 应等价于 SC"
    assert errors_sc == 0 and errors_scl == 0
    print("单元测试通过。")
