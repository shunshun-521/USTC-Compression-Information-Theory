...
def run_unit_tests():
    ...
    for _ in range(50):
        ...
        u_sent[info] = np.random.randint(0, 2, K)
        llr = compute_llr(
            bpsk_modulate(polar_encode(u_sent)) + np.random.normal(0, sigma, N), sigma
        )
...
results_sc = run_simulation(
    ...
    max_frames=MAX_FRAMES, min_errors=MIN_ERRORS,
...
for L in L_LIST:
    ...
    def scl_decoder(llr_ch, _scl=scl_dec):
