...
for _ in range(100):
    ...
    llr_t = compute_llr(awgn_channel(bpsk_modulate(polar_encode(ut)), sigma_t, rng), sigma_t)
    if not np.array_equal(sc_decode(llr_t, frozen_t)[info_t], ut[info_t]):
        err += 1
assert err == 0, f"SC 高 SNR 测试失败: {err} 帧错误"
...
for _ in range(20):
    ...
    ut[info_t] = rng.integers(0, 2, K_t)
    llr_t = compute_llr(awgn_channel(bpsk_modulate(polar_encode(ut)), sigma_t, rng), sigma_t)
    us, _ = scl1.decode(llr_t)
    assert np.array_equal(sc_decode(llr_t, frozen_t), us), "L=1 SCL 与 SC 不一致"
print("单元测试通过。")
...
EB_N0_RANGE = np.arange(1.5, 5.0, 0.5) if QUICK else np.arange(1.0, 5.5, 0.25)
...
results_sc = run_simulation(
    ...
    MAX_FRAMES, MIN_ERRORS, frozen_bits=frozen_bits, info_indices=info_idx,
)
