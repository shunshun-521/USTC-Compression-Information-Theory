...
def run_unit_tests():
    ok, _, _, _ = verify_encoder(4)
    assert ok, "编码器校验失败"
    assert verify_sc_decoders(64, 32, 100, 10.0), "SC 译码校验失败"
    assert verify_scl_equals_sc(64, 32, 20), "SCL(L=1) 与 SC 不等价"
    print("单元测试通过。")
...
if __name__ == "__main__":
    ...
    for L in L_LIST:
        print(f"\nSCL 仿真: N={N}, K={K}, L={L}")
        scl_dec = SCLDecoder(N, frozen_bits, list_size=L, crc_length=0)
    ...
    cascl_dec = SCLDecoder(N, frozen_bits, list_size=8, crc_length=CRC_LENGTH)
