...
def run_validations():
    ...
    from decoder_scl import scl_decode_equivalent_to_sc

    llr_test = compute_llr(bpsk_modulate(x), 0.5)
    frozen4 = np.array([False, True, False, False])
    assert scl_decode_equivalent_to_sc(4, frozen4, llr_test)

    info = np.array([1, 0, 1, 1, 0, 0, 1, 1])
    assert crc_check(crc_encode(info, 8), 8)
    print("单元测试通过。\n")
