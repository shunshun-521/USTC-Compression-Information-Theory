...
def _unit_tests():
    from run_exp1 import _unit_tests as t1
...
if __name__ == "__main__":
    ...
    fbool = frozen_bits.astype(bool)

    all_results = {}

    def sc_decoder(llr_ch):
        return sc_decode(llr_ch, fbool), None
    ...
    for L in L_LIST:
        print(f"\nSCL 仿真: N={N}, K={K}, L={L}")
        dec = SCLDecoder(N, fbool, list_size=L, crc_length=0)
    ...
    cascl = SCLDecoder(N, fbool, list_size=8, crc_length=CRC_LENGTH)
    ...
    results_cascl = run_simulation(
        ...
        crc_length=CRC_LENGTH, verbose=True,
        info_indices=info_idx, frozen_bits=frozen_bits,
    )
