...
from channel import eb_n0_to_sigma, prepare_channel_llr, compute_llr, bpsk_modulate, awgn_channel
...
def validate_modules():
    ...
    for _ in range(50):
        ...
        llr = prepare_channel_llr(compute_llr(bpsk_modulate(x_test), eb_n0_to_sigma(8.0, K / N)))
        u_sc = sc_decode(llr, frozen_bits)
        u_scl, _ = SCLDecoder(N, frozen_bits, list_size=1).decode(llr)
        if not np.array_equal(u_sc, u_scl):
            mismatches += 1
    assert mismatches == 0, f"SCL L=1 与 SC 不一致: {mismatches}/50"
    print("单元测试通过：编码器 + CRC + SCL(L=1)=SC")
