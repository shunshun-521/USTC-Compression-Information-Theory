...
from encoder import polar_encode, polar_encode_verify
...
def run_unit_tests():
    u = np.array([1, 0, 1, 1])
    x = polar_encode(u)
    assert np.array_equal(x, polar_encode_verify(u))
    ...
    for seed in range(50):
        ...
        assert np.array_equal(sc_decode(llr, frozen_bits), SCLDecoder(N, frozen_bits, 1).decode(llr)[0])
    print("所有单元测试通过。")
