...
def run_validation_tests():
    ...
    for _ in range(100):
        ...
        assert np.array_equal(sc_decode(llr, frozen_bits)[info_idx], u_test[info_idx])

    scl = SCLDecoder(N, frozen_bits, list_size=1)
    for _ in range(20):
        u_test = np.zeros(N, dtype=int)
        u_test[info_idx] = rng.integers(0, 2, K)
        sigma = eb_n0_to_sigma(5, 0.5)
        llr = compute_llr(awgn_channel(bpsk_modulate(polar_encode(u_test)), sigma, rng), sigma)
...
if __name__ == "__main__":
    ...
    results_sc = run_simulation(
        ...
        MAX_FRAMES, MIN_ERRORS, info_indices=info_idx, design_eb_n0_db=DESIGN_EBN0,
    )
    ...
    print(f"\nCA-SCL 仿真: N={N}, K={K}, L={CA_SCL_LIST_SIZE}, CRC={CRC_LENGTH}")

    def cascl_decoder(llr_ch):
        u_hat, _ = SCLDecoder(N, frozen_bits, list_size=CA_SCL_LIST_SIZE, crc_length=CRC_LENGTH).decode(llr_ch)
        return u_hat, None
