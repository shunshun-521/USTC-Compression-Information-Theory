...
for _ in range(30):
    ...
    uu[info_t] = rng.integers(0, 2, K_t)
    sigma = eb_n0_to_sigma(10.0, 0.5)
    llr = compute_llr(
        awgn_channel(bpsk_modulate(polar_encode(uu)), sigma, rng=rng), sigma
    )
    assert np.array_equal(
        sc_decode(llr, frozen_t), SCLDecoder(N_t, frozen_t, list_size=1).decode(llr)[0]
    ), "L=1 SCL 应等价于 SC"
print("单元测试通过。")
...
EB_N0_RANGE = np.arange(1.0, 5.5, 0.5) if QUICK else np.arange(1.0, 5.5, 0.25)
...
results_sc = run_simulation(
    N, K, EB_N0_RANGE, sc_decoder, "sc", MAX_FRAMES, MIN_ERRORS, frozen_bits=frozen_bool
)
...
def cascl_decoder(llr_ch):
    u_hat, _ = SCLDecoder(N, frozen_bool, list_size=8, crc_length=CRC_LENGTH).decode(
        llr_ch
    )
    return u_hat, None
