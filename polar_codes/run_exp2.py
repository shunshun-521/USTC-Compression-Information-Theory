...
def run_scl_unit_tests():
    ...
    sigma = eb_n0_to_sigma(5.0, K_test / N_test)
    for _ in range(20):
        bits = rng.integers(0, 2, size=K_test)
        ...
        u_scl, _ = SCLDecoder(N_test, frozen_bits, list_size=1).decode(llr)
        assert np.array_equal(u_sc, u_scl), "L=1 SCL 与 SC 不一致"

    payload = crc_encode(np.array([1, 0, 1, 1, 0, 1, 0, 1]), 8)
