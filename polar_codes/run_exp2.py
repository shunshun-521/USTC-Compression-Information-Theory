...
verify_sc_decoder(num_frames=30, N=64, K=32, eb_n0_db=12.0)
...
results_sc = run_simulation(
    ...
    frozen_bits=fb, info_indices=info_idx, verbose=True,
)
...
for L in L_LIST:
    print(f"\nSCL 仿真: N={N}, K={K}, L={L}")
    scl = SCLDecoder(N, fb, list_size=L, crc_length=0)
...
cascl = SCLDecoder(N, fb, list_size=8, crc_length=CRC_LENGTH)
...
results_cascl = run_simulation(
    ...
    crc_length=CRC_LENGTH, frozen_bits=fb, info_indices=info_idx, verbose=True,
)
