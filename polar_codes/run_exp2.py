...
def main():
    ...
    def sc_decoder(llr_ch, _fb=frozen_bits):
    ...
    for L in L_LIST:
        ...
        def scl_decoder(llr_ch, _L=L, _fb=frozen_bits):
            u_hat, _ = SCLDecoder(N, _fb, list_size=_L, crc_length=0).decode(llr_ch)
            return u_hat, None
    ...
    def cascl_decoder(llr_ch, _fb=frozen_bits):
        u_hat, _ = SCLDecoder(
            N, _fb, list_size=8, crc_length=CRC_LENGTH
        ).decode(llr_ch)
        return u_hat, None
