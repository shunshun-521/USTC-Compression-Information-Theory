...
for L in L_LIST:
    ...
    def make_scl(llr, _L=L):
        u_hat, _ = SCLDecoder(N, frozen_bits, list_size=_L, crc_length=0).decode(llr)
        return u_hat, None

    results = run_simulation(
        N, K, EB_N0_RANGE, make_scl, 'scl',
...
def cascl_decoder(llr):
    u_hat, _ = SCLDecoder(N, frozen_bits, list_size=8, crc_length=CRC_LENGTH).decode(llr)
    return u_hat, None
