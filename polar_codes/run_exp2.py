...
from encoder import verify_encoder
...
fb_chk = np.ones(N_chk, dtype=int)
fb_chk[info_chk] = 0
...
for _ in range(20):
    u = np.zeros(N_chk, dtype=int)
    u[info_chk] = rng_chk.integers(0, 2, K_chk)
    llr = compute_llr(bpsk_modulate(polar_encode(u)), sigma_chk)
    u_sc = sc_decode(llr, fb_chk)
    u_scl, _ = SCLDecoder(N_chk, fb_chk, list_size=1).decode(llr)
    assert np.array_equal(u_sc, u_scl), "SCL L=1 与 SC 不等价"
print("单元测试通过：SCL L=1 等价 SC")
