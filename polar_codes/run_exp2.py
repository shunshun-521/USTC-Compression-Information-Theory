...
EB_N0_RANGE = np.arange(2.0, 3.5, 0.5) if FAST else np.arange(1.0, 5.5, 0.25)
...
def sc_decoder(llr_ch):
    return sc_decode(llr_ch, fb, info_idx), None
...
for L in L_LIST:
    ...
    def make_scl_decoder(list_size):
        def scl_decoder(llr_ch):
            u_hat, pm = SCLDecoder(N, fb, list_size=list_size).decode(llr_ch)
            return u_hat, None
        return scl_decoder
...
if not FAST:
    ...
    def cascl_decoder(llr_ch):
        u_hat, pm = SCLDecoder(
            N, fb, list_size=8, crc_length=CRC_LENGTH, info_indices=info_idx,
        ).decode(llr_ch)
        return u_hat, None
