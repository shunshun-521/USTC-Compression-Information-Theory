...
def run_unit_tests():
    ...
    verify_sc_decoders(64, 32, 50, 10.0)
    verify_scl_equals_sc(32, 16, 8.0, 20)
    print("单元测试通过。")
...
if __name__ == "__main__":
    ...
    for L in L_LIST:
        print(f"\nSCL 仿真: N={N}, K={K}, L={L}")
        scl = SCLDecoder(N, fb_bool, list_size=L, crc_length=0)
    ...
    cascl = SCLDecoder(N, fb_bool, list_size=8, crc_length=CRC_LENGTH)
    ...
    # 汇总 SCL 曲线（与任务说明一致的主输出文件）
    save_results_csv(
        results_cascl, f"results/exp2_scl_N{N}_R0.5.csv"
    )
