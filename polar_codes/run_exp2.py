...
from simulation import quick_mode_enabled, run_simulation
...
def run_unit_tests():
    ...
    info = np.array([1, 1, 0, 0, 0, 1, 1, 1])
    enc = crc_encode(info[:4], 8)
    assert crc_check(enc, 8), "CRC 编码/校验失败"
...
def main():
    ...
    results_cascl = run_simulation(
        ...
        crc_length=CRC_LENGTH, frozen_bits=frozen_bits, info_indices=info_idx,
    )
