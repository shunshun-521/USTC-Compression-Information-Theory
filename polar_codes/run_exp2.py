...
from tests_unit import run_all_tests
...
N = 128 if _QUICK else 512
RATE = 0.5
K = N // 2
DESIGN_EBN0 = 2.5
CRC_LENGTH = 8
L_LIST = [2, 4] if _QUICK else [2, 4, 8]
MAX_FRAMES = 500 if _QUICK else 100000
MIN_ERRORS = 10 if _QUICK else 100
EB_N0_RANGE = np.arange(1.0, 3.0, 0.5) if _QUICK else np.arange(1.0, 5.5, 0.25)
...
results_sc = run_simulation(
    N, K, EB_N0_RANGE, sc_decoder, "sc", MAX_FRAMES, MIN_ERRORS, verbose=True, info_indices=info_idx
)
...
for L in L_LIST:
    ...
    results = run_simulation(
        N, K, EB_N0_RANGE, scl_decoder, "scl", MAX_FRAMES, MIN_ERRORS, verbose=True, info_indices=info_idx
    )
    label = f"SCL (L={L})"
...
L_CASCL = 4 if _QUICK else 8


def cascl_decoder(llr_ch):
    u_hat, pm = SCLDecoder(N, frozen_bits, list_size=L_CASCL, crc_length=CRC_LENGTH).decode(llr_ch)
    return u_hat, None
...
all_results[f"CA-SCL (L={L_CASCL}, CRC={CRC_LENGTH})"] = results_cascl
save_results_csv(results_cascl, f"results/exp2_cascl_L{L_CASCL}_N{N}_R0.5.csv")
