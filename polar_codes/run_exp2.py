...
DESIGN_EBN0 = 2.5
CRC_LENGTH = 8
L_LIST = [2, 4] if os.environ.get('POLAR_FAST_SIM') == '1' else [2, 4, 8]
EB_N0_RANGE = np.arange(1.5, 4.5 + sim['eb_step'], sim['eb_step'])
...
results_sc = run_simulation(N, K, EB_N0_RANGE, sc_decoder, 'sc', info_indices=info_idx,
                             max_frames=sim['max_frames'], min_errors=sim['min_errors'])
...
for L in L_LIST:
    ...
    def make_scl(llr_ch, _L=L):
        u_hat, pm = SCLDecoder(N, frozen_bits.astype(bool), list_size=_L).decode(llr_ch)
        return u_hat, None
    results = run_simulation(N, K, EB_N0_RANGE, make_scl, 'scl', info_indices=info_idx,
                              max_frames=sim['max_frames'], min_errors=sim['min_errors'])
    all_results[f'SCL (L={L})'] = results
    save_results_csv(results, f'results/exp2_scl_L{L}_N{N}_R0.5.csv')

def cascl_decoder(llr_ch):
    u_hat, pm = SCLDecoder(N, frozen_bits.astype(bool), list_size=8, crc_length=CRC_LENGTH).decode(llr_ch)
    return u_hat, None

results_cascl = run_simulation(N, K, EB_N0_RANGE, cascl_decoder, 'scl', crc_length=CRC_LENGTH,
                                info_indices=info_idx, max_frames=sim['max_frames'],
