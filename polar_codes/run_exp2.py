...
from simulation import _env_int, run_simulation
...
def run_unit_tests():
    ...
    llr = compute_llr(awgn_channel(bpsk_modulate(x_test), sigma, rng), sigma)
...
CRC_LENGTH = 8
L_LIST = [2, 4, 8]
MAX_FRAMES = _env_int("POLAR_MAX_FRAMES", 100000 if not os.environ.get("POLAR_QUICK") else 1500)
MIN_ERRORS = _env_int("POLAR_MIN_ERRORS", 100 if not os.environ.get("POLAR_QUICK") else 15)
EB_N0_RANGE = np.arange(1.0, 5.5, 0.5 if os.environ.get("POLAR_QUICK") else 0.25)
