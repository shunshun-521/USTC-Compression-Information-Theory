...
from validate import run_all_tests
from construction import ga_construction
from decoder_sc import sc_decode
from decoder_scl import SCLDecoder
from simulation import fast_sim_params, run_simulation
...
EB_N0_RANGE = sim["eb_n0_range"] if sim["eb_n0_range"] is not None else np.arange(1.0, 5.5, 0.25)
...
print(f"\nCA-SCL 仿真: N={N}, K={K}, L={CRC_CASCL_L}, CRC={CRC_LENGTH}")


def cascl_decoder(llr_ch):
    u_hat, _ = SCLDecoder(N, frozen_bits, list_size=CRC_CASCL_L, crc_length=CRC_LENGTH).decode(llr_ch)
    return u_hat, None
...
all_results[f"CA-SCL (L={CRC_CASCL_L}, CRC={CRC_LENGTH})"] = results_cascl
save_results_csv(results_cascl, f"results/exp2_cascl_L{CRC_CASCL_L}_N{N}_R0.5.csv")
