...
assert np.array_equal(polar_encode(u), (u @ G4) % 2)
verify_sc_decoders(N=64, K=32, num_frames=50, eb_n0_db=10.0)
...
def cascl_decoder(llr_ch):
    u_hat, pm = SCLDecoder(
        N, frozen_bool, list_size=8, crc_length=CRC_LENGTH, info_indices=info_idx
    ).decode(llr_ch)
    return u_hat, None
