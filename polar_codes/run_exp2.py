...
MAX_FRAMES = 30 if FAST else 100000
MIN_ERRORS = 5 if FAST else 100
EB_N0_RANGE = np.array([2.0, 3.0]) if FAST else np.arange(1.0, 5.5, 0.25)
RUN_CASCL = not FAST
...
for L in L_LIST:
    ...
    def make_scl_decoder(list_size):
        def scl_decoder(llr_ch):
            u_hat, _ = SCLDecoder(N, frozen_bits, list_size=list_size, crc_length=0).decode(llr_ch)
            return u_hat, None
        return scl_decoder
