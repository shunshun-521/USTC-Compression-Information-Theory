...
MAX_FRAMES = int(os.environ.get("POLAR_MAX_FRAMES", "40" if QUICK else "100000"))
MIN_ERRORS = int(os.environ.get("POLAR_MIN_ERRORS", "8" if QUICK else "100"))
...
L_LIST = [2, 4, 8]
if QUICK:
    EB_N0_RANGE = np.array([10.0, 12.0])
    L_LIST = [2, 4]
else:
...
if __name__ == "__main__":
    ...
    scl_list = L_LIST if not QUICK else [2, 4]
    for L in scl_list:
        print(f"\nSCL 仿真: L={L}")
