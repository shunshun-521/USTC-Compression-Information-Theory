...
def run_unit_tests():
    ...
    for _ in range(50):
        ...
        if not np.array_equal(uh_sc, u):
            errs_sc += 1
        if not np.array_equal(uh_scl, uh_sc):
            errs_scl += 1
    assert errs_sc == 0, "SC 基线失败"
    assert errs_scl == 0, "L=1 SCL 应等价于 SC"
    print("单元测试通过。")
...
if __name__ == "__main__":
    ...
    save_results_csv(
        all_results.get("SCL (L=8)", results_cascl),
