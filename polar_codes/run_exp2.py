...
from decoder_sc import run_sc_self_test, sc_decode
from decoder_scl import SCLDecoder, run_scl_self_test
...
def _verify_modules():
    ...
    GN = (B @ G) % 2
    u = np.array([1, 0, 1, 1])
    assert np.array_equal(polar_encode(u), (u @ GN) % 2)
    run_sc_self_test()
    run_scl_self_test()
    print("实验二模块校验通过。")
...
results_sc = run_simulation(
    ...
    info_indices=info_idx, design_eb_n0_db=DESIGN_EBN0, verbose=True,
)
...
results_cascl = run_simulation(
    ...
    crc_length=CRC_LENGTH, info_indices=info_idx, design_eb_n0_db=DESIGN_EBN0, verbose=True,
)
