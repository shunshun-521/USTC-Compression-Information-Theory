...
from validate import run_all as run_unit_tests
...
EB_N0_RANGE = np.arange(2.0, 4.5, 0.5) if FAST else np.arange(1.0, 5.5, 0.25)
...
if __name__ == "__main__":
    ...
    for L in L_LIST:
        ...
        def make_scl_decoder(list_size):
            def scl_decoder(llr_ch):
                u_hat, pm = SCLDecoder(N, frozen_bits, list_size=list_size, crc_length=0).decode(llr_ch)
                return u_hat, None
            return scl_decoder
    ...
    results_cascl = run_simulation(
        ...
        MAX_FRAMES, MIN_ERRORS, crc_length=CRC_LENGTH, info_indices=info_idx,
    )
