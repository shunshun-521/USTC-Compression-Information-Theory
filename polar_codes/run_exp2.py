...
def main():
    ...
    results_sc = run_simulation(
        ...
        info_indices=info_idx, frozen_bits=frozen_bool, design_eb_n0_db=DESIGN_EBN0,
    )
    ...
    for L in L_LIST:
        print(f"\nSCL 仿真: N={N}, K={K}, L={L}")
        dec = SCLDecoder(N, frozen_bool, list_size=L, crc_length=0)

        def scl_decoder(llr_ch, _d=dec):
            u_hat, _ = _d.decode(llr_ch)
            return u_hat, None
    ...
    cascl = SCLDecoder(N, frozen_bool, list_size=8, crc_length=CRC_LENGTH)
    ...
    results_cascl = run_simulation(
        ...
        crc_length=CRC_LENGTH, info_indices=info_idx, frozen_bits=frozen_bool,
