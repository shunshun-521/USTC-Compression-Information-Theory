...
from simulation import get_fast_sim_params, run_simulation
from utils import find_capacity_limit, plot_bler_curves, save_results_csv

if __name__ == "__main__":
    from validate import test_scl_l1_equals_sc, test_crc
    ...
    CRC_LENGTH = 8
    L_LIST = [2, 4, 8]
    fast = get_fast_sim_params()
    MAX_FRAMES = fast["max_frames"]
    MIN_ERRORS = fast["min_errors"]
    EB_N0_RANGE = np.arange(1.0, 5.5, 0.5) if MAX_FRAMES <= 2000 else np.arange(1.0, 5.5, 0.25)
