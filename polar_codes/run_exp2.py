...
EB_N0_RANGE = (
    np.arange(1.0, 4.0, 0.5) if FAST else np.arange(1.0, 5.5, 0.25)
)
...
scl_instances = {}
for L in L_LIST:
    scl_instances[L] = SCLDecoder(N, frozen_bits, list_size=L, crc_length=0)
    print(f"\nSCL 仿真: N={N}, K={K}, L={L}")

    def scl_decoder(llr_ch, _scl=scl_instances[L]):
