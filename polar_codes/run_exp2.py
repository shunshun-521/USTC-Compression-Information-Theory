...
_validate.test_encoder()
_validate.test_sc_noiseless()
_validate.test_scl_equals_sc()
_validate.test_crc()
...
EB_N0_RANGE = np.arange(1.5, 3.5, 0.5) if FAST else np.arange(1.0, 5.5, 0.25)
...
for L in L_LIST:
    ...
    def make_scl_decoder(list_size):
        def scl_decoder(llr_ch):
            u_hat, pm = SCLDecoder(N, frozen_bits, list_size=list_size, crc_length=0).decode(
                llr_ch
            )
            return u_hat, None
        return scl_decoder
