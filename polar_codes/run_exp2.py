...
u_t = np.zeros(N_t, dtype=int); u_t[info_t] = rng.integers(0, 2, K_t)
llr_t = compute_llr(bpsk_modulate(polar_encode(u_t)), 1e-6)
assert np.array_equal(sc_decode(llr_t, frozen_t), SCLDecoder(N_t, frozen_t, 1).decode(llr_t)[0])
print("单元测试通过。\n")
...
MAX_FRAMES = 500 if QUICK else int(os.environ.get('POLAR_MAX_FRAMES', 100000))
MIN_ERRORS = 20 if QUICK else int(os.environ.get('POLAR_MIN_ERRORS', 100))
