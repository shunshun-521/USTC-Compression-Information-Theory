...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        llr = compute_llr(bpsk_modulate(polar_encode(u_test)) + rng.normal(0, sigma, N), sigma)
    ...
    for _ in range(50):
        ...
        assert np.array_equal(u_scl, u_test)

    payload = crc_encode(np.array([1, 0, 1, 1, 0, 0, 1, 0]), 8)
...
results_cascl = run_simulation(
    ...
    MAX_FRAMES, MIN_ERRORS, crc_length=CRC_LENGTH, verbose=True, info_indices=info_idx,
)
