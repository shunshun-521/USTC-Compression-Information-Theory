...
def _run_unit_tests():
    ...
    for _ in range(30):
        ...
        llr = compute_llr(
            awgn_channel(bpsk_modulate(polar_encode(u)), 0.3, rng), 0.3
        )
...
for L in L_LIST:
    print(f"\nSCL 仿真: N={N}, K={K}, L={L}")
    scl = SCLDecoder(N, frozen_mask, list_size=L, crc_length=0)
...
cascl = SCLDecoder(N, frozen_mask, list_size=8, crc_length=CRC_LENGTH)
