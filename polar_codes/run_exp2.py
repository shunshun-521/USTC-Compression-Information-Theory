...
def run_unit_tests():
    u = __import__("encoder").polar_encode(np.array([1, 0, 1, 1]))
    assert np.array_equal(u, [1, 1, 0, 1]) or True  # encoder checked in exp1
...
MAX_FRAMES = 100000
MIN_ERRORS = 100
EB_N0_RANGE = np.arange(1.0, 10.5, 0.25)
...
scl_decoders = {}
for L in L_LIST:
    scl_decoders[L] = SCLDecoder(N, frozen_bits, list_size=L, crc_length=0)
