...
def run_unit_tests():
    ...
    scl1 = SCLDecoder(N, frozen_bits.astype(bool), list_size=1)
    for _ in range(50):
        u_test = np.zeros(N, dtype=int)
        u_test[info_idx] = rng.integers(0, 2, K)
        x_test = polar_encode(u_test)
        y = awgn_channel(bpsk_modulate(x_test), sigma, rng=rng)
        llr = compute_llr(y, sigma)
        u_scl, _ = scl1.decode(llr)
...
for L in L_LIST:
    ...
    def make_scl_decoder(llr_ch, list_size=L):
        u_hat, _ = SCLDecoder(
            N, frozen_bits.astype(bool), list_size=list_size, crc_length=0
        ).decode(llr_ch)
        return u_hat, None
