...
def run_unit_tests():
    ...
    frozen_bits = np.ones(N_test, dtype=int)
    ...
    for _ in range(50):
        ...
        y_t = awgn_channel(bpsk_modulate(x_t), sigma, rng)
        llr_t = compute_llr(y_t, sigma)
        u_sc = sc_decode(llr_t, frozen_bits)
        u_scl, _ = SCLDecoder(N_test, frozen_bits, list_size=1).decode(llr_t)
...
EB_N0_RANGE = np.arange(2.0, 4.0, 0.5) if FAST else np.arange(1.0, 5.5, 0.25)
