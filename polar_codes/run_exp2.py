...
def main():
    ...
    results_sc = run_simulation(
        N, K, info_idx, EB_N0_RANGE, sc_decoder, "sc",
    ...
    for L in L_LIST:
        ...
        results = run_simulation(
            N, K, info_idx, EB_N0_RANGE,
            lambda llr, _L=L: scl_decoder(llr, _L), "scl",
            max_frames, min_errors, verbose=True,
        )
        label = f"SCL (L={L})"
    ...
    results_cascl = run_simulation(
        N, K, info_idx, EB_N0_RANGE, cascl_decoder, "scl",
    ...
    plot_bler_curves(
        all_results,
        title=f"SCL vs SC BLER (N={N}, R={RATE})",
        save_path="results/fig2_scl_bler.png",
