...
def run_unit_tests():
    ...
    info_idx, _, _ = ga_construction(64, 32, DESIGN_EBN0)
    frozen_bits = np.ones(64, dtype=int)
    frozen_bits[info_idx] = 0
    from channel import bpsk_modulate, compute_llr

    u64 = np.zeros(64, dtype=int)
    u64[info_idx] = np.random.default_rng(1).integers(0, 2, 32)
    llr = compute_llr(bpsk_modulate(polar_encode(u64)), 1e-3)
    u_sc = sc_decode(llr, frozen_bits)
    u_scl, _ = SCLDecoder(64, frozen_bits, list_size=1).decode(llr)
