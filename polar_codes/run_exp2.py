...
def run_unit_tests():
    ...
    for _ in range(30):
        ...
        llr = np.where(polar_encode(u_sent) == 0, 30.0, -30.0)
...
for L in L_LIST:
    ...
    def scl_decoder(llr_ch, _L=L):
        u_hat, _pm = SCLDecoder(N, fb, list_size=_L, crc_length=0).decode(llr_ch)
        return u_hat, None
...
def cascl_decoder(llr_ch):
    u_hat, _pm = SCLDecoder(N, fb, list_size=8, crc_length=CRC_LENGTH).decode(llr_ch)
    return u_hat, None
