...
from channel import compute_llr, eb_n0_to_sigma, awgn_channel, bpsk_modulate
...
# ========== 单元测试 ==========
u = np.array([0, 0, 1, 1])
assert np.array_equal(polar_encode(u), [0, 0, 1, 1])

N_t = 32
info_idx, _, _ = ga_construction(N_t, 16, 2.5)
fb = np.ones(N_t, dtype=int)
fb[info_idx] = 0
llr = compute_llr(bpsk_modulate(polar_encode(np.random.randint(0, 2, N_t))), 0.01)
uh_sc = sc_decode(llr, fb)
uh_scl, _ = SCLDecoder(N_t, fb, list_size=1).decode(llr)
...
for L in L_LIST:
    ...
    def scl_decoder(llr_ch, _dec=dec):
...
results_cascl = run_simulation(
    ...
    crc_length=CRC_LENGTH, design_eb_n0_db=DESIGN_EBN0, verbose=True,
)
