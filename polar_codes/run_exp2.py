...
def run_unit_tests():
    ...
    from simulation import make_frozen_bits, prepare_llr_for_decode

    N_t, K_t = 64, 32
    frozen_bits, info_idx = make_frozen_bits(N_t, K_t, 2.5)
    sigma = eb_n0_to_sigma(4.0, K_t / N_t)
    rng = np.random.default_rng(1)
    for _ in range(20):
        payload = rng.integers(0, 2, size=K_t, dtype=np.int8)
        u = np.zeros(N_t, dtype=np.int8)
        u[info_idx] = payload
        llr = prepare_llr_for_decode(
            compute_llr(awgn_channel(bpsk_modulate(polar_encode(u)), sigma, rng=rng), sigma),
            N_t,
        )
