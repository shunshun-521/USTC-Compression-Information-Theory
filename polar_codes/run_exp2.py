...
def run_unit_tests():
    ...
    for _ in range(50):
        ...
        if not np.array_equal(u_sc, u_scl):
            mismatches += 1
    assert mismatches == 0, f"路径度量校验失败: SCL(L=1) 与 SC 不一致 {mismatches} 次"

    bits = crc_encode(np.array([1, 0, 1, 0, 1, 1, 0, 0]), 8)
    from decoder_scl import crc_check

    assert crc_check(bits, 8), "CRC 编解码校验失败"
    print("单元测试通过。")
...
if __name__ == "__main__":
    ...
    for L in L_LIST:
        if L == 4:
            save_results_csv(all_results[f"SCL (L={L})"], "results/exp2_scl_N512_R0.5.csv")
            break
    print("\n实验二完成。")
