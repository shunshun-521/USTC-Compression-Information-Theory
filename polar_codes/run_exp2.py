...
def run_unit_tests():
    ...
    for _ in range(30):
        ...
        payload[info_idx] = rng.integers(0, 2, K)
        llr = compute_llr(
            awgn_channel(bpsk_modulate(polar_encode(payload)), sigma, rng), sigma
        )
...
def main():
    ...
    results_sc = run_simulation(
        ...
        verbose=True, info_indices=info_idx, frozen_bits=frozen_bits,
    )
    ...
    for L in L_LIST:
        ...
        def scl_decoder(llr_ch, list_size=L):
            u_hat, pm = SCLDecoder(N, frozen_bits, list_size=list_size).decode(llr_ch)
            return u_hat, None
