...
sigma_t = eb_n0_to_sigma(5.0, K_test / N_test)
mismatches = 0
for _ in range(20):
    u_t = np.zeros(N_test, dtype=int)
    u_t[info_idx_t] = rng.integers(0, 2, K_test)
    llr_t = compute_llr(bpsk_modulate(polar_encode(u_t)), sigma_t)
    if not np.array_equal(sc_decode(llr_t, frozen_t),
                         SCLDecoder(N_test, frozen_t, list_size=1).decode(llr_t)[0]):
        mismatches += 1
assert mismatches == 0, "L=1 SCL 应与 SC 等价"
print("单元测试通过")
