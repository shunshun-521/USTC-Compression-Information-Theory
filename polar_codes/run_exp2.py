...
def run_unit_tests():
    ...
    assert verify_sc_decoder(64, 32, 50, 12.0)
    ...
    for _ in range(20):
        ...
        llr = 500 * bpsk_modulate(polar_encode(u))
...
for L in L_LIST:
    ...
    def make_scl_decoder(llr_ch, _L=L):
        u_hat, pm = SCLDecoder(N, frozen_bool, list_size=_L, crc_length=0).decode(llr_ch)
        return u_hat, None

    results = run_simulation(
        N, K, EB_N0_RANGE, make_scl_decoder, "scl", MAX_FRAMES, MIN_ERRORS, verbose=True,
        info_indices=info_idx, frozen_bits=frozen_bits,
    )
...
results_cascl = run_simulation(
    ...
    crc_length=CRC_LENGTH, verbose=True, info_indices=info_idx, frozen_bits=frozen_bits,
)
