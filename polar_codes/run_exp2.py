...
for _ in range(20):
    u0 = np.zeros(N_t, dtype=np.int8)
    u0[info_t] = rng.integers(0, 2, K_t)
    x0 = polar_encode(u0)
    y0 = bpsk_modulate(x0) + rng.normal(0, sigma_t, N_t)
    llr0 = compute_llr(y0, sigma_t)
    u_sc = sc_decode(llr0, frozen_t)
    u_scl, _ = SCLDecoder(N_t, frozen_t, list_size=1).decode(llr0)
