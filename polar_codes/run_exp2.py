...
from polar_config import MAX_FRAMES, MIN_ERRORS, QUICK
...
def run_unit_tests():
    ...
    for _ in range(50):
        ...
        if not np.array_equal(uh_sc[info_idx], u[info_idx]):
            err_sc += 1
        if not np.array_equal(uh_scl[info_idx], u[info_idx]):
            err_scl += 1
    assert err_sc == 0 and err_scl == 0, (
        f"L=1 SCL 应等价于 SC (Eb/N0=12dB): sc_err={err_sc}, scl_err={err_scl}"
    )
    print("单元测试通过。")
...
def main():
    ...
    if QUICK:
        print(f"[POLAR_QUICK] MAX_FRAMES={MAX_FRAMES}, MIN_ERRORS={MIN_ERRORS}")
