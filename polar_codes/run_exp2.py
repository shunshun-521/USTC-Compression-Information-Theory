...
CRC_LENGTH = 8
L_LIST = [2, 4, 8]
EB_N0_RANGE = np.arange(EB_MIN, EB_MAX + EB_STEP * 0.5, EB_STEP)
...
results_sc = run_simulation(
    N, K, EB_N0_RANGE,
    lambda llr: (sc_decode(llr, frozen_bool), None),
    "sc", MAX_FRAMES, MIN_ERRORS,
...
for L in L_LIST:
    print(f"\nSCL 仿真: N={N}, K={K}, L={L}")
    scl_decoder_obj = SCLDecoder(N, frozen_bool, list_size=L, crc_length=0)

    def scl_decoder(llr_ch, _dec=scl_decoder_obj):
        u_hat, _ = _dec.decode(llr_ch)
        return u_hat, None
...
cascl_decoder = SCLDecoder(N, frozen_bool, list_size=8, crc_length=CRC_LENGTH)

def cascl_decoder_fn(llr_ch):
    u_hat, _ = cascl_decoder.decode(llr_ch)
    return u_hat, None

results_cascl = run_simulation(
    N, K, EB_N0_RANGE, cascl_decoder_fn, "scl",
