...
sim_cfg = fast_sim_settings()
N = 512
RATE = 0.5
K = N // 2
DESIGN_EBN0 = 2.5
CRC_LENGTH = 8
L_LIST = sim_cfg.get("l_list_exp2", [2, 4, 8])
MAX_FRAMES_SCL = sim_cfg.get("scl_max_frames") or MAX_FRAMES
RUN_CASCL = sim_cfg.get("run_cascl", True)
MAX_FRAMES = sim_cfg["max_frames"]
MIN_ERRORS = sim_cfg["min_errors"]
EB_N0_RANGE = np.arange(1.0, 5.5, sim_cfg["eb_n0_step"])
...
for L in L_LIST:
    ...
    if L == 8 or (L == max(L_LIST) and not RUN_CASCL):
