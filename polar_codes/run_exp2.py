...
from verify import run_all as run_unit_tests
...
save_results_csv(results_sc, os.path.join(os.path.dirname(__file__), f'results/exp2_sc_N{N}_R0.5.csv'))
...
for L in L_LIST:
    ...
    save_results_csv(results, os.path.join(os.path.dirname(__file__), f'results/exp2_scl_L{L}_N{N}_R0.5.csv'))
...
save_results_csv(results_cascl, os.path.join(os.path.dirname(__file__), f'results/exp2_cascl_L8_N{N}_R0.5.csv'))

shannon_db = find_capacity_limit(RATE)
plot_bler_curves(
    all_results,
    f'SCL vs SC BLER (N={N}, R={RATE})',
    os.path.join(os.path.dirname(__file__), 'results/fig2_scl_bler.png'),
...
plt.tight_layout()
out = os.path.join(os.path.dirname(__file__), 'results/fig2_decode_time')
