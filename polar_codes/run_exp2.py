...
def run_unit_tests():
    ...
    s_sigma = eb_n0_to_sigma(10.0, K / N)
    for _ in range(20):
        u_sent = np.zeros(N, dtype=int)
        u_sent[info_idx] = rng.integers(0, 2, K)
        x = polar_encode(u_sent)
        y = awgn_channel(bpsk_modulate(x), s_sigma, rng)
        llr = compute_llr(y, s_sigma)
...
for L in L_LIST:
    ...
    def scl_decoder(llr_ch, _L=L):
        u_hat, _pm = SCLDecoder(N, frozen, list_size=_L, crc_length=0).decode(llr_ch)
        return u_hat, None
...
def cascl_decoder(llr_ch):
    u_hat, _pm = SCLDecoder(N, frozen, list_size=8, crc_length=CRC_LENGTH).decode(llr_ch)
    return u_hat, None
