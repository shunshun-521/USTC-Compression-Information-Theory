...
def run_unit_tests():
    ...
    for _ in range(30):
        ...
        llr = 2 * (1 - 2 * polar_encode(u_t)) / (sigma**2)
...
if os.environ.get("POLAR_QUICK"):
    MAX_FRAMES = 2000
    MIN_ERRORS = 20
    EB_N0_RANGE = np.arange(2.0, 5.0, 0.5)
    L_LIST = [2, 4]
else:
    MAX_FRAMES = 100000
    MIN_ERRORS = 100
...
if __name__ == "__main__":
    ...
    for L in L_LIST:
        ...
        def scl_decoder(llr_ch, _L=L):
            u_hat, _ = SCLDecoder(N, fbool, list_size=_L, crc_length=0).decode(llr_ch)
            return u_hat, None
    ...
    def cascl_decoder(llr_ch):
        u_hat, _ = SCLDecoder(N, fbool, list_size=8, crc_length=CRC_LENGTH).decode(
            llr_ch
        )
        return u_hat, None
