...
N_v, K_v = 64, 32
info_v, _, _ = ga_construction(N_v, K_v, 2.5)
frozen_v = np.ones(N_v, dtype=int)
frozen_v[info_v] = 0
from channel import bpsk_modulate, compute_llr

u_v = np.zeros(N_v, dtype=int)
u_v[info_v] = [1, 0, 1, 0] * 8
llr_v = compute_llr(bpsk_modulate(polar_encode(u_v)), 0.001)
u_sc = sc_decode(llr_v, frozen_v)
u_scl, _ = SCLDecoder(N_v, frozen_v, list_size=1).decode(llr_v)
