...
from validate import test_encoder, test_sc_noiseless, test_scl_equals_sc
...
DESIGN_EBN0 = 2.5
CRC_LENGTH = 8
L_LIST = [2, 4] if FAST else [2, 4, 8]
MAX_FRAMES = 2000 if FAST else 100000
MIN_ERRORS = 20 if FAST else 100
EB_N0_RANGE = np.arange(2.0, 7.0, 0.5) if FAST else np.arange(1.0, 5.5, 0.25)
...
for L in L_LIST:
    ...
    def make_scl_decoder(list_size):
        def scl_decoder(llr_ch):
            u_hat, pm = SCLDecoder(N, frozen_bool, list_size=list_size, crc_length=0).decode(llr_ch)
            return u_hat, None
        return scl_decoder

    results = run_simulation(
        N, K, EB_N0_RANGE, make_scl_decoder(L), info_idx, "scl",
        MAX_FRAMES, MIN_ERRORS, verbose=True,
    )
...
def cascl_decoder(llr_ch):
    u_hat, pm = SCLDecoder(N, frozen_bool, list_size=8, crc_length=CRC_LENGTH).decode(llr_ch)
    return u_hat, None


results_cascl = run_simulation(
    N, K, EB_N0_RANGE, cascl_decoder, info_idx, "scl",
