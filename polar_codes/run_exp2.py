...
from validate import test_encoder, test_scl_equals_sc
...
CRC_LENGTH = 8
L_LIST = [2, 4, 8]
if os.environ.get("POLAR_FAST", "").lower() in ("1", "true", "yes"):
    L_LIST = [2, 4]
MAX_FRAMES = 100000
MIN_ERRORS = 100

eb_min = float(os.environ.get("POLAR_EB_MIN", "4.0"))
eb_max = float(os.environ.get("POLAR_EB_MAX", "12.0"))
eb_step = float(os.environ.get("POLAR_EB_STEP", "0.25"))
EB_N0_RANGE = np.arange(eb_min, eb_max + eb_step / 2, eb_step)
...
results_cascl = run_simulation(
    ...
    crc_length=CRC_LENGTH, info_indices=info_idx, frozen_bits=frozen_bits, verbose=True,
)
