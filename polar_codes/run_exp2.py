...
if os.environ.get("POLAR_FAST", "0") == "1":
    EB_N0_RANGE = np.arange(
        float(os.environ.get("POLAR_EB_MIN", "1.5")),
        float(os.environ.get("POLAR_EB_MAX", "4.0")),
        float(os.environ.get("POLAR_EB_STEP", "0.5")),
    )
...
def cascl_decoder(llr_ch):
    list_size = 4 if os.environ.get("POLAR_FAST", "0") == "1" else 8
    u_hat, pm = SCLDecoder(
        N, frozen_bits, list_size=list_size, crc_length=CRC_LENGTH
    ).decode(llr_ch)
    return u_hat, None
