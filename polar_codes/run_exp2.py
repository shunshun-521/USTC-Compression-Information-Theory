...
from simulation import fast_mode, run_simulation
...
CRC_LENGTH = 8
L_LIST = [2, 4, 8]
MAX_FRAMES = 1500 if fast_mode() else 100000
MIN_ERRORS = 10 if fast_mode() else 100
EB_N0_RANGE = np.arange(1.5, 4.5, 0.5) if fast_mode() else np.arange(1.0, 5.5, 0.25)
...
for L in L_LIST:
    ...
    def scl_decoder(llr_ch, _L=L):
        u_hat, pm = SCLDecoder(N, frozen_bits, list_size=_L, crc_length=0, info_indices=info_idx).decode(
            llr_ch
        )
        return u_hat, None
