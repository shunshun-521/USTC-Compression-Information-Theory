...
for L in L_LIST:
    ...
    def scl_decoder(llr_ch, _L=L):
        u_hat, pm = SCLDecoder(
            N, frozen_bits, list_size=_L, crc_length=0, info_indices=info_idx
        ).decode(llr_ch)
        return u_hat, None
...
try:
    ...
    with open("results/fig2_decode_time.csv", "w", encoding="utf-8") as f:
        f.write("decoder,avg_decode_time_ms\n")
        for label, t in zip(labels, avg_times):
