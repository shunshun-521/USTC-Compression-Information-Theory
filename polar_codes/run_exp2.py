...
if not run_validation(verbose=True):
    raise SystemExit("单元测试未通过，终止实验二。")
...
EB_N0_RANGE = np.arange(
    float(os.environ.get("POLAR_EB_MIN", "1.0")),
...
for L in L_LIST:
    ...
    def scl_decoder(llr_ch, _L=L):
        u_hat, _ = SCLDecoder(N, frozen_bits.astype(bool), list_size=_L, crc_length=0).decode(llr_ch)
        return u_hat, None
...
def cascl_decoder(llr_ch):
    u_hat, _ = SCLDecoder(N, frozen_bits.astype(bool), list_size=8, crc_length=CRC_LENGTH).decode(llr_ch)
    return u_hat, None
