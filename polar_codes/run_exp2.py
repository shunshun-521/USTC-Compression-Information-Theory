...
for L in L_LIST:
    ...
    def make_scl_decoder(list_size=L):
        def scl_decoder(llr_ch):
            u_hat, pm = SCLDecoder(N, frozen_mask, list_size=list_size, crc_length=0).decode(llr_ch)
            return u_hat, None
        return scl_decoder
...
def cascl_decoder(llr_ch):
    u_hat, pm = SCLDecoder(N, frozen_mask, list_size=8, crc_length=CRC_LENGTH).decode(llr_ch)
    return u_hat, None
