...
def _crc_process_bits(bits, poly, width):
    reg = 0
    mask = (1 << width) - 1
    for bit in bits:
        feedback = int(bit) ^ ((reg >> (width - 1)) & 1)
...
def crc_encode(info_bits, crc_length=8):
    ...
    remainder = _crc_process_bits(info_bits, poly, crc_length)
    crc_bits = np.array(
...
def crc_check(bits, crc_length=8):
    ...
    poly = CRC8_POLY if crc_length == 8 else CRC16_POLY
    return _crc_process_bits(bits, poly, crc_length) == 0
...
class SCLDecoder:
    ...
    def decode(self, llr_ch):
        ...
        if self.crc_length > 0:
            ...
            crc_paths = [p for p in paths if crc_check(p.u_hat[info_mask], self.crc_length)]
            if crc_paths:
