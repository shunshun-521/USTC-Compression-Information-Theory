...
def _crc_remainder(info_bits, poly, crc_length):
    """GF(2) CRC 余数（标准移位寄存器实现）"""
    reg = 0
    for bit in info_bits.astype(int):
        ...
        for phi in range(self.N):
            ...
            for path in paths:
                ...
                if self.frozen_bits[phi]:
                    new_path = _SCLPath(self.N, self.n, llr_ch)
                    new_path.pm = path.pm + self._pm_penalty(llr_bit, 0)
                    new_path.u_hat[:phi] = path.u_hat[:phi]
                    ...
                    for u in (0, 1):
                        new_path = _SCLPath(self.N, self.n, llr_ch)
                        new_path.pm = path.pm + self._pm_penalty(llr_bit, u)
                        new_path.u_hat[:phi] = path.u_hat[:phi]
