...
class SCLDecoder:
    ...
    def __init__(self, N, frozen_bits, list_size=4, crc_length=0):
        ...
        if crc_length > 0:
            ...
            self.payload_positions = np.where(~self.frozen_bits)[0]
    ...
    def decode(self, llr_ch):
        ...
        if self.crc_length > 0:
            valid = []
            for path in paths:
                payload = path.u_hat[self.payload_positions]
                crc_part = path.u_hat[self.crc_positions]
                if np.array_equal(
                    crc_part, crc_encode(payload, self.crc_length)[-self.crc_length:]
                ):
                    valid.append(path)
