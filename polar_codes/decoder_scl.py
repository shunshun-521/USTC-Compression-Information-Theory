...
class SCLDecoder:
    """
    SCL 译码器（Lazy Copy：路径分裂时复制对象，适用于中等列表大小）。
    """
        ...
        for i in range(self.N):
            ...
            for path in paths:
                ...
                if l in self.frozen_set:
                    new_path = _Path(self.N, self.n, llr_perm)
                    new_path.L = path.L.copy()
                    new_path.B = path.B.copy()
                    new_path.u_hat = path.u_hat.copy()
                    new_path.pm = path.pm + self._llr_penalty(llr_val, 0)
                    ...
                    for u_bit in (0, 1):
                        ...
                        new_path.B = path.B.copy()
                        new_path.u_hat = path.u_hat.copy()
                        new_path.pm = path.pm + self._llr_penalty(llr_val, u_bit)
