...
class _Path:
    ...
    def __init__(self, L, B, pm=0.0):
        ...
        self.B = B
        self.pm = pm
        self.u_hat = np.zeros(L.shape[0], dtype=np.int8)
...
class SCLDecoder:
    ...
    def _new_path(self, llr_ch):
        ...
        L[:, 0] = llr_ch
        return _Path(L, B, 0.0)

    def _copy_path(self, path):
        new_path = _Path(path.L.copy(), path.B.copy(), path.pm)
        ...
        for step, l in enumerate(self.decode_order):
            ...
            for path in paths:
                ...
                if l in self.frozen_set:
                    ...
                    np_path.pm += self._path_metric_penalty(llr, 0)
                    ...
                    for u_bit in (0, 1):
                        np_path = self._copy_path(path)
                        np_path.pm += self._path_metric_penalty(llr, u_bit)
