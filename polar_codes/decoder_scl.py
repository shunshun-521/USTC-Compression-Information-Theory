...
def crc_check(bits, crc_length=8):
    ...
    crc_received = bits[-crc_length:]
    crc_computed = crc_encode(info, crc_length)[-crc_length:]
    return np.array_equal(crc_received, crc_computed)
...
class SCLDecoder:
    ...
    def __init__(self, N, frozen_bits, list_size=4, crc_length=0):
        ...
        self.frozen_set = set(np.where(frozen)[0])
        self.info_indices = np.where(~frozen)[0]
        self.L = list_size
