...
from decoder_sc import (
    _bit_reversed, _upper_llr, _lower_llr,
...
class SCLDecoder:
    ...
    def decode(self, llr_ch):
        ...
        for l in self._decode_order:
            ...
            for path in paths:
                ...
                if self.frozen_bits[l]:
                    ...
                    for u_bit in (0, 1):
                        preferred = 0 if cur_llr >= 0 else 1
                        penalty = 0.0 if u_bit == preferred else abs(cur_llr)
                        p = _Path(self.N, self.n)
                        p.L = path.L.copy()
                        p.B = path.B.copy()
                        p.pm = path.pm + penalty
        ...
        best = min(
            crc_paths if crc_paths else paths,
            key=lambda p: p.pm,
        )
