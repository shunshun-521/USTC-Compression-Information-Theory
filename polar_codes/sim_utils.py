...
def _bpsk_capacity_scalar(eb_n0_db):
    """BPSK-AWGN 信道容量（bits/channel use），Es/N0 = Eb/N0"""
    es_n0 = 10.0 ** (eb_n0_db / 10.0)

    def integrand(y):
        val = np.clip(-2.0 * es_n0 * y ** 2, -500, 500)
        return np.log2(1.0 + np.exp(val)) * np.exp(-y ** 2 / 2.0)
...
def compute_bpsk_capacity(eb_n0_db_list, rate):
    ...
    return np.array([_bpsk_capacity_scalar(eb) for eb in eb_n0_db_list])
