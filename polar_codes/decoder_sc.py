...
def active_llr_level(i, n):
    """找到索引 i 二进制表示（MSB 优先）中第一个 1 的位置"""
    mask = 2 ** (n - 1)
    count = 1
    for _ in range(n):
        if (mask & i) == 0:
...
def active_bit_level(i, n):
    """找到索引 i 二进制表示（MSB 优先）中第一个 0 的位置"""
    mask = 2 ** (n - 1)
    count = 1
    for _ in range(n):
        if (mask & i) > 0:
...
def precompute_sc_indices(N):
    ...
    for _ in range(n):
        layer_size *= 2
        lambda_offset.append(lambda_offset[-1] + layer_size // 2)
...
def sc_decode(llr_ch, frozen_bits):
    """
    非递归 SC 译码（参考 polarcodes.SCD 实现）。
    信道 LLR 按自然顺序输入，内部按比特倒序相位处理。
    """
