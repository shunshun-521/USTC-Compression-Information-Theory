...
def sc_decode(llr_ch, frozen_bits):
    """
    非递归 SC 译码（SCD 算法）。
    llr_ch: 信道 LLR，应与编码器输出位序一致（自然序 x[0..N-1]）。
    内部对 LLR 做比特倒序以匹配译码树。
    """
...
def sc_decode_recursive(llr, frozen_bits):
    """递归 SC 译码（调用 SCD 实现作为参考）"""
    return sc_decode(llr, frozen_bits)


def precompute_sc_indices(N):
    """预计算非递归 SC 译码辅助向量（供报告/接口兼容）"""
    n = int(np.log2(N))
    ...
    for phi in range(N):
        ...
        bit_layer_vec.append(list(range(layer + 1)) if phi % 2 == 1 else list(range(layer)))
