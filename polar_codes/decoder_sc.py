...
def _lower_llr(l_bottom, l_top, bit):
    """对数域 g 运算（l_bottom 为下分支，l_top 为上分支）"""
    if bit == 0:
...
def sc_decode(llr_ch, frozen_bits):
    """
    非递归置换 SC 译码（Vangala et al., 2014）。
    信道 LLR 按自然码字顺序输入，无需额外置换。
    """
