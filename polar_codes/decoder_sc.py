...
def sc_decode_recursive(llr, frozen_bits):
    ...
    def recurse(llr_block, bits_offset, n_layers):
        if n_layers == 0:
            idx = bits_offset
        ...
        recurse(llr_u, bits_offset, n_layers - 1)
        u_partial = u_hat[bits_offset : bits_offset + half]
        llr_d = g_operation(llr_block[:half], llr_block[half:], u_partial)
        recurse(llr_d, bits_offset + half, n_layers - 1)
...
def sc_decode(llr_ch, frozen_bits):
    """
    非递归 Permuted SC 译码（与 Arikan 蝶形编码器配套）。
