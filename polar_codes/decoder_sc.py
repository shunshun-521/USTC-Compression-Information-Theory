...
def sc_decode_recursive(llr_ch, frozen_bits):
    """
    递归 SC 译码（参考实现）。
    与 sc_decode 等价，采用逐层递归调用非递归核心以保证一致性。
    """
...
def precompute_sc_indices(N):
    ...
    """
    n = int(math.log2(N))
    lambda_offset = [1 << (n - 1 - i) for i in range(n)]
    llr_layer_vec = []
    bit_layer_vec = []
...
def sc_decode(llr_ch, frozen_bits):
    ...
    非递归 SC 译码主函数。
    """
    llr, n = _prepare_llr(np.asarray(llr_ch, dtype=np.float64))
    N = len(llr)
...
def _generator_matrix(N):
    ...
    for i in range(N):
        ...
        e[i] = 1
        cols.append(polar_encode(e))
    return np.array(cols).T % 2
