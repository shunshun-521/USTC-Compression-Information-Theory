...
def _sc_decode_layered(llr_ch, frozen_bits, N, n):
    """分层 SC 译码（与 mcba1n SCD 一致）。"""
...
def sc_decode_recursive(llr, frozen_bits):
    ...
    N = len(llr)
    n = int(math.log2(N))
    return _sc_decode_layered(llr, frozen_bits, N, n)
...
def sc_decode(llr_ch, frozen_bits):
    ...
    N = len(llr_ch)
    n = int(math.log2(N))
    return _sc_decode_layered(llr_ch, frozen_bits, N, n)
