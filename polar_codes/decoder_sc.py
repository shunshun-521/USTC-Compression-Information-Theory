"""
极化码 SC（串行抵消）译码器
提供递归版本（参考实现）和非递归版本（高效实现，与蝶形编码匹配）
"""
import math
import numpy as np
...
def precompute_sc_indices(N):
    """
    预计算非递归 SC 译码所需的辅助向量：
      - lambda_offset: 各层 LLR 块大小（2^s）
      - llr_layer_vec[phi]: 第 phi 个比特需要执行 LLR 运算的层列表
      - bit_layer_vec[phi]: 第 phi 个比特需要执行比特回传的层列表
    """
    n = int(math.log2(N))
    lambda_offset = [1 << s for s in range(n + 1)]
    llr_layer_vec = []
    bit_layer_vec = []
    decode_order = [bit_reversal_index(i, n) for i in range(N)]
    for phi in range(N):
        l = decode_order[phi]
...
def sc_decode(llr_ch, frozen_bits):
    """
    非递归 SC 译码（按比特倒序相位更新，与 encoder.polar_encode 配套）。
    """
