...
def sc_decode_recursive(llr, frozen_bits):
    """
    递归 SC 译码（参考实现）。
    按 Permuted SCD 顺序逐比特递归计算 LLR 并判决。
    """
    ...
    def update_llrs(l, s_start):
        ...
        return update_llrs(l, s_start + 1)

    def update_bits(l, s_end):
        if l < N // 2:
            return
        if s_end < n - _active_bit_level(l, n):
            return
        block_size = 2 ** s_end
        branch_size = block_size // 2
        for j in range(l, -1, -block_size):
            if j % block_size >= branch_size:
                B[j - branch_size, s_end - 1] = (B[j, s_end] + B[j - branch_size, s_end]) % 2
                B[j, s_end - 1] = B[j, s_end]
        update_bits(l, s_end - 1)

    decode_order = [bit_reversal_permutation(N)[i] for i in range(N)]

    def decode_bit(order_idx):
        if order_idx >= N:
            return
        l = decode_order[order_idx]
...
def precompute_sc_indices(N):
    """
    预计算非递归 SC 译码辅助向量（Permuted SCD 顺序）。
    """
    n = int(np.log2(N))
