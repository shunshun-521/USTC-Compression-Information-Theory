...
def _update_bits(B, l, n):
    ...
    for s in range(n, n - _active_bit_level(l, n), -1):
        ...
        for j in range(l, -1, -block_size):
            if j % block_size >= branch_size:
                B[j - branch_size, s - 1] = (int(B[j, s]) + int(B[j - branch_size, s])) % 2
                B[j, s - 1] = B[j, s]


def precompute_sc_indices(N):
    """
    预计算非递归 SC 译码所需的辅助向量（按自然顺序 phi 译码）。
    """
    n = int(math.log2(N))
    ...
    return lambda_offset, llr_layer_vec, bit_layer_vec, [bit_reversed(i, n) for i in range(N)]


def sc_decode(llr_ch, frozen_bits):
    """
    非递归 SC 译码主函数（与 polar_encode 配套）。
    """
...
if __name__ == "__main__":
    ...
    assert np.array_equal(u_rec, u_nr), "Recursive and non-recursive SC mismatch"
    print("SC recursive/non-recursive consistency passed.")
