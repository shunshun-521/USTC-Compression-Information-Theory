...
def sc_decode_recursive(llr, frozen_bits):
    ...
    def decode(llr_vec, offset, length):
        if length == 1:
            idx = offset
            u_hat[idx] = 0 if frozen_bits[idx] or llr_vec[0] >= 0 else 1
            return
        half = length // 2
        decode(f_operation(llr_vec[:half], llr_vec[half:]), offset, half)
        decode(
            g_operation(llr_vec[:half], llr_vec[half:], u_hat[offset:offset + half]),
            offset + half,
            half,
        )

    decode(np.asarray(llr, dtype=np.float64), 0, N)
    return u_hat
        ...
        for s in range(n - active_llr_level(l, n), n):
            ...
            for j in range(l, N, block_size):
                if j % block_size < branch_size:
                    ...
                    L[j, s + 1] = g_operation(
                        np.array([L[j - branch_size, s]]),
                        np.array([L[j, s]]),
                        np.array([B[j - branch_size, s + 1]]),
                    )[0]
