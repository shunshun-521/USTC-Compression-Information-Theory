...
def sc_decode_recursive(llr, frozen_bits):
    ...
    采用与编码端 bit-reversal 一致的 L/B 矩阵更新流程。
    """
...
def sc_decode(llr_ch, frozen_bits):
    ...
    br_indices = [bit_reversal_permutation(N)[i] for i in range(N)]

    for idx in br_indices:
        _update_llrs(L, B, idx, n, N)
        if frozen_bits[idx]:
            B[idx, n] = 0
        else:
            B[idx, n] = 0 if L[idx, n] >= 0 else 1
        _update_bits(B, idx, n, N)
