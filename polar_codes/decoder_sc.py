...
def _reorder_channel_llr(llr_ch, N):
    """将信道 LLR 重排为译码树所需顺序（与含比特倒序的编码器配套）"""
    n = int(math.log2(N))
    inv_br = np.array([bit_reversed_index(i, n) for i in range(N)], dtype=int)
...
def precompute_sc_indices(N):
    ...
    for phi in range(N):
        ...
        if l < n:
            layers_llr.append(l)
            for ll in range(l + 1, n):
                layers_llr.append(ll)

        layers_bit = []
        t = phi
        ll = 0
        while (t & 1) == 0 and ll < n:
            layers_bit.append(ll)
            t >>= 1
            ll += 1
