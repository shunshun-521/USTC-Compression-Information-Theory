...
def sc_decode(llr_ch, frozen_bits):
    """非递归 SC 译码主函数。"""
    return _sc_decode_core(_reorder_channel_llr(llr_ch), frozen_bits)


def sc_decode_recursive(llr, frozen_bits):
    """递归 SC 译码（参考实现）。"""
    return _sc_decode_core(_reorder_channel_llr(llr), frozen_bits)
...
def precompute_sc_indices(N):
    ...
    for phi in range(N):
        llr_layer_vec.append(
            list(range(n - _active_llr_level(phi, n), n))
        )
        if phi % 2 == 0:
            bit_layer_vec.append(list(range(_active_bit_level(phi, n))))
        else:
            bit_layer_vec.append(list(range(_active_bit_level(phi, n), n)))
