...
def sc_decode_recursive(llr, frozen_bits):
    ...
    def decode_node(llr_node, pos):
        n = len(llr_node)
        if n == 1:
            if frozen_bits[pos]:
                u_hat[pos] = 0
            else:
                u_hat[pos] = 0 if llr_node[0] >= 0 else 1
            return
        half = n // 2
        llr_left = f_operation(llr_node[:half], llr_node[half:])
        decode_node(llr_left, pos)
        u_left = u_hat[pos:pos + half]
        llr_right = g_operation(llr_node[:half], llr_node[half:], u_left)
        decode_node(llr_right, pos + half)
...
def precompute_sc_indices(N):
    ...
    for phi in range(N):
        l = _bit_reversed_index(phi, n)
        if l >= N / 2:
            layers = list(range(n, n - _active_bit_level(l, n), -1))
        else:
            layers = []
