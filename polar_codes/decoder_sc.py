"""
极化码 SC（串行抵消）译码器
提供递归版本（参考实现）和非递归版本（高效实现，比特倒序相位）
"""
import numpy as np
...
def precompute_sc_indices(N):
    """预计算非递归 SC 辅助向量（自然相位顺序，保留接口）"""
    n = int(np.log2(N))
    lambda_offset = [1 << i for i in range(n + 1)]
    llr_layer_vec, bit_layer_vec = [], []
    for phi in range(N):
        phi_bin = format(phi, f"0{n}b")
        llr_layer_vec.append([layer for layer in range(n) if phi_bin[layer] == "0"])
        bit_layer_vec.append([layer for layer in range(n) if phi_bin[layer] == "1"])
    return lambda_offset, llr_layer_vec, bit_layer_vec


def sc_decode(llr_ch, frozen_bits):
    """
    非递归 SC 译码（比特倒序相位，与蝶形编码配套）。
