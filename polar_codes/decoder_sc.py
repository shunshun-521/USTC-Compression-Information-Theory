...
def prepare_channel_llr(llr_ch, N):
    """
    将信道 LLR 重排为 SC 译码器所需顺序。
    编码器含比特倒序置换，因此信道 LLR 需做相同倒序。
    """
...
def precompute_sc_indices(N):
    """
    预计算非递归 SC 译码辅助向量（层索引形式）。
  """
    n = int(math.log2(N))
    llr_layer_vec = []
    bit_layer_vec = []
    for phi in range(N):
        br = _bit_reversed_index(phi, n)
        llr_layer_vec.append(list(range(n - _active_llr_level(br, n), n)))
        if br < N / 2:
