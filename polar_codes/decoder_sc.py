...
def _to_tree_frozen_mask(frozen_bits, N):
    ...
    for i in range(N):
        tree_mask[_bit_reversed_index(i, n)] = frozen_bits[i]
    return tree_mask


def _sc_decode_core(llr_ch, frozen_bits):
    """基于 mcba1n SCD 结构的 min-sum SC 译码核心。"""
    llr_ch = np.asarray(llr_ch, dtype=np.float64)
    N = len(llr_ch)
    n = int(math.log2(N))
    tree_frozen = _to_tree_frozen_mask(frozen_bits, N)
    frozen_set = set(np.where(tree_frozen)[0])
...
def sc_decode_recursive(llr, frozen_bits):
    ...
    return _sc_decode_core(llr_perm, frozen_bits)


def precompute_sc_indices(N):
    """
    预计算非递归 SC 译码所需的辅助向量（与 mcba1n active level 对应）。
    """
    n = int(math.log2(N))
...
def sc_decode(llr_ch, frozen_bits):
    """
    非递归 SC 译码主函数。
    信道 LLR 先按比特倒序重排，以匹配 B_N F^otimes n 编码约定。
    """
    N = len(llr_ch)
    rev = bit_reversal_permutation(N)
    llr_perm = np.asarray(llr_ch, dtype=np.float64)[rev]
    return _sc_decode_core(llr_perm, frozen_bits)
