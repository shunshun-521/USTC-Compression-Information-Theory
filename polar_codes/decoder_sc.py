...
def _active_llr_level(i, n):
    """二进制表示中自高位起第一个 1 的位置计数（用于 LLR 更新层数）"""
    mask = 2 ** (n - 1)
    count = 1
    for _ in range(n):
        if (mask & i) == 0:
...
def _active_bit_level(i, n):
    """二进制表示中自高位起第一个 0 的位置计数（用于比特回传层数）"""
    mask = 2 ** (n - 1)
    count = 1
    for _ in range(n):
        if (mask & i) > 0:
...
def precompute_sc_indices(N):
    ...
    for i in range(1, n + 1):
        lambda_offset[i] = lambda_offset[i - 1] + 2 ** (n - i)

    llr_layer_vec = []
    bit_layer_vec = []
    for phi in range(N):
        l = _bit_reverse_index(phi, n)
...
def sc_decode(llr_ch, frozen_bits):
    """
    非递归 SC 译码（层叠 LLR/比特数组）。
    按比特倒序依次译码，与 polar_encode 配套。
    """
    ...
    decode_order = [_bit_reverse_index(i, n) for i in range(N)]
