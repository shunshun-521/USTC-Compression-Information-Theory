"""
极化码 SC（串行抵消）译码器
提供递归版本（参考实现）和非递归 Permuted SCD 实现（高效）
"""
import math
import numpy as np
...
def sc_decode_recursive(llr, frozen_bits):
    ...
    def decode_node(llr_node, frozen_node):
        ...
        half = n // 2
        u_left = decode_node(f_operation(llr_node[:half], llr_node[half:]), frozen_node[:half])
        u_right = decode_node(
            g_operation(llr_node[:half], llr_node[half:], u_left), frozen_node[half:]
        )
...
def precompute_sc_indices(N):
    ...
    for phi in range(N):
        ...
        bit_start = n - _active_bit_level(br_phi, n)
