...
class _SCDCore:
    """Permuted SC 译码核心（与 polar-codes 库 SCD 一致）"""

    def __init__(self, N, n, llr, frozen_indices):
        self.N = N
        self.n = n
...
def sc_decode_recursive(llr, frozen_bits):
    """
    递归 SC 译码（参考实现）。
    注：与蝶形编码器配套时使用 Permuted SCD 等价实现。
    """
...
def sc_decode(llr_ch, frozen_bits):
    ...
    return _SCDCore(N, n, llr_ch, frozen_indices).decode()
