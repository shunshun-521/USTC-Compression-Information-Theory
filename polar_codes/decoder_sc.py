...
def sc_decode_recursive(llr, frozen_bits):
    """
    递归 SC 译码（与 Permuted SCD 等价的递归比特序实现）。
    """
    ...
    decode_order = _get_decode_order(N)
...
def _get_decode_order(N):
    if N not in _SC_CACHE:
        n = int(math.log2(N))
        _SC_CACHE[N] = [_bit_reversed(i, n) for i in range(N)]
    return _SC_CACHE[N]
