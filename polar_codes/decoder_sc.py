...
if __name__ == "__main__":
    ...
    x = polar_encode(u)
    F = np.array([[1, 0], [1, 1]])
    assert np.array_equal(x, np.mod(u @ np.kron(F, F), 2))
    ...
    for name, dec in [("recursive", sc_decode_recursive), ("iterative", sc_decode)]:
        errors = 0
        for _ in range(100):
            ...
            if not np.array_equal(u_hat[info_idx], u_sent[info_idx]):
                errors += 1
        assert errors == 0, f"{name} SC 译码在 15dB 下有 {errors} 帧错误"
