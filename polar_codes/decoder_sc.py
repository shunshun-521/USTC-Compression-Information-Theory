...
def precompute_sc_indices(N):
    ...
    for phi in range(N):
        ...
        if phi % 2 == 0:
            if phi == 0:
                blayers = list(range(n))
            else:
                blayers = []
                psi = phi
                while psi % 2 == 0:
                    blayers.append(int(math.log2(psi & -psi)))
                    psi >>= 1
        else:
            blayers = [0]
    ...
    for phi in range(N):
        ...
        offset += len(llr_layer_vec[phi]) + 1
    lambda_offset[N] = offset
