"""
极化码 SC（串行抵消）译码器
提供递归版本（参考实现）和非递归版本（高效实现，基于 mcba1n SCD 结构）
"""
import math
import numpy as np
...
def sc_decode(llr_ch, frozen_bits):
    """
    非递归 SC 译码主函数。
    信道 LLR 按自然顺序输入，内部按比特倒序处理。
    """
    N = len(llr_ch)
    n = int(math.log2(N))
    frozen_nat = np.asarray(frozen_bits, dtype=bool)
    frozen_set = set(np.where(frozen_nat)[0])
