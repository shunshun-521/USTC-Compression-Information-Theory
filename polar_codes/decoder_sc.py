...
def _bit_reversed_index(i, n):
    """单索引比特倒序"""
    br = bit_reversal_permutation(1 << n)
    return int(br[i])


def _active_llr_level(i, n):
    """llr 更新起始层（Vangala active_llr_level）"""
    mask = 1 << (n - 1)
    count = 1
    for _ in range(n):
        if (mask & i) == 0:
...
def _active_bit_level(i, n):
    """比特回传起始层（Vangala active_bit_level）"""
    mask = 1 << (n - 1)
    count = 1
    for _ in range(n):
        if (mask & i) > 0:
...
def sc_decode(llr_ch, frozen_bits):
    """
    非递归 SC 译码（Vangala 2014 置换 SC，与含比特倒序的编码器配套）
    """
...
def precompute_sc_indices(N):
    """预计算辅助向量（供 SCL 使用，与 Vangala 层索引对应）"""
    n = int(math.log2(N))
    ...
    for phi in range(N):
        ...
        bit_layer_vec.append(list(range(n, bit_start, -1)) if bit_start < n else [])
