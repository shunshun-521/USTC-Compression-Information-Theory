...
def f_operation(La, Lb):
    """
    f 运算（log-domain 精确 box-plus，标量输入）。
    对小信号幅值退化为 min-sum 近似。
    """
    if np.isscalar(La) and np.isscalar(Lb):
        if abs(La) > 50 or abs(Lb) > 50:
            return float(
...
def precompute_sc_indices(N):
    ...
    for phi in range(N):
        ...
        bit_layer_vec.append(list(range(i)))
