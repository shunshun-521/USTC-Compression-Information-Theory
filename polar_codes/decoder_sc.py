"""
极化码 SC（串行抵消）译码器
提供递归版本（参考实现）和非递归版本（高效实现，基于置换 SC 算法）
"""
import math
import numpy as np
...
def _upper_llr(l1, l2):
    """f 运算（对数域精确形式，与 min-sum 在高 SNR 下等价）"""
...
def precompute_sc_indices(N):
    ...
    for l in phase_order:
        ...
        start_s_bit = n - _active_bit_level(l, n)
        bit_layer_vec.append(list(range(n, start_s_bit, -1)))
    return lambda_offset, llr_layer_vec, bit_layer_vec, phase_order


def sc_decode(llr_ch, frozen_bits):
    """
    非递归 SC 译码（置换 SC，与极化码蝶形编码 + 比特倒序输出配套）。

    输入 llr_ch 为信道侧码字顺序 LLR（与 polar_encode 输出比特顺序一致），
    内部自动做比特倒序重排以匹配译码树。
    """
