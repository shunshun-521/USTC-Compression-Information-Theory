...
def sc_decode_recursive(llr, frozen_bits):
    ...
    参数：
        llr: 长度 N 的信道 LLR 数组
        frozen_bits: 长度 N 的 bool/int 数组，非零表示冻结位（置 0）
    返回：
        u_hat: 长度 N 的估计源序列
    """
...
def precompute_sc_indices(N):
    ...
    for phi in decode_order:
        layers_llr = list(range(n - _active_llr_level(phi, n), n))
        llr_layer_vec.append(layers_llr)

        if phi < N // 2:
            layers_bit = []
        else:
            layers_bit = list(range(n, n - _active_bit_level(phi, n), -1))
