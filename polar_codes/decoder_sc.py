...
def sc_decode_recursive(llr, frozen_bits):
    ...
    def recur(L, depth, pos):
        if depth == n:
            if pos in frozen_set:
                u_hat[pos] = 0
            else:
                u_hat[pos] = 0 if L[0] >= 0 else 1
            return

        half = len(L) // 2
        L_left = f_operation(L[:half], L[half:])
        recur(L_left, depth + 1, pos)
        u_partial = u_hat[pos:pos + half]
        L_right = g_operation(L[:half], L[half:], u_partial)
        recur(L_right, depth + 1, pos + half)
...
def precompute_sc_indices(N):
    ...
    for phi in range(N):
        ...
        llr_layer_vec.append(
            list(range(n - _active_llr_level(br_phi, n), n))
        )
        bit_layer_vec.append(
            list(range(n, n - _active_bit_level(br_phi, n), -1))
        )
...
def sc_decode(llr_ch, frozen_bits):
    """
    非递归 SC 译码主函数（逐相位更新，与编码比特倒序约定一致）。
    """
