"""
极化码 SC（串行抵消）译码器
提供递归版本（参考实现）和非递归版本（高效实现，mcba1n 调度）
"""
import numpy as np
from encoder import bit_reversal_permutation, polar_transform
...
def sc_decode(llr_ch, frozen_bits):
    """
    非递归 SC 译码（Permuted SCD，与蝶形+比特倒序编码配套）。
    内部将信道 LLR 按比特倒序重排后译码。
    """
    N = len(llr_ch)
