...
def _update_llrs(L, B, n):
    """按 polarcodes 索引方式更新 LLR 树。"""

    def update_for_index(l):
    ...
    return update_for_index


def _update_bits(B, n):
    """比特回传。"""
...
def sc_decode(llr_ch, frozen_bits):
    """
    非递归 SC 译码（Permuted SC，与左支 XOR 编码器配套）。
    ...
    update_llrs = _update_llrs(L, B, n)
    update_bits = _update_bits(B, n)
