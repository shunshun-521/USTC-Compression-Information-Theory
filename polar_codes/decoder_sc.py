...
def sc_decode_recursive(llr, frozen_bits):
    ...
    def compute_L(j, s):
        ...
        if j % block_size < branch_size:
            L[j, s] = f_operation(compute_L(j, prev), compute_L(j + branch_size, prev))
        else:
            L[j, s] = g_operation(
                compute_L(j - branch_size, prev),
                compute_L(j, prev),
                B[j - branch_size, s],
            )
        return L[j, s]
    ...
    for l in [_bit_reversed(i, n) for i in range(N)]:
        ...
        if frozen_bits[l]:
            ...
            u_hat[l] = 0 if compute_L(l, n) >= 0 else 1
        B[l, n] = u_hat[l]
