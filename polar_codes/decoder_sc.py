...
def sc_decode_recursive(llr_ch, frozen_bits):
    """
    递归 SC 译码（按比特索引递归，与 sc_decode 等价，供交叉验证）。
    """
...
if __name__ == "__main__":
    ...
    u = np.array([1, 0, 1, 1, 0, 0, 1, 0])
    x = polar_encode(u)
    llr = compute_llr(bpsk_modulate(x), 0.01)
    frozen = np.zeros(8, dtype=bool)
    frozen[[1, 2, 4, 7]] = True
    print("u:", u)
    print("sc:", sc_decode(llr, frozen))
    print("recursive:", sc_decode_recursive(llr, frozen))
