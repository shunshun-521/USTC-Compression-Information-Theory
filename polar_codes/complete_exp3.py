...
for N in N_LIST:
    ...
    if not os.path.exists(f"results/exp3_sc_N{N}_R0.5.csv"):
        def sc_d(llr_ch):
        ...
        r_sc = load_results_csv(f"results/exp3_sc_N{N}_R0.5.csv")

    all_results["SC"] = r_sc

    if not os.path.exists(f"results/exp3_scl_N{N}_R0.5.csv"):
        def scl_d(llr_ch):
        ...
        from utils import load_results_csv
        r_scl = load_results_csv(f"results/exp3_scl_N{N}_R0.5.csv")

    all_results["SCL (L=4)"] = r_scl

    bp_decoder = BPDecoder(N, frozen_bits, max_iter=BP_MAX_ITER)
    ...
    all_results[f"BP (max_iter={BP_MAX_ITER})"] = r_bp
    ...
    ax.set_title(f"BP Average Iterations (N={N}, max_iter={BP_MAX_ITER})")
