    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            if crc_length > 0:
                ...
                u[info_indices[: len(info_with_crc)]] = info_with_crc
                u_sent_info = info_payload
                compare_indices = info_indices[:k_info]
            else:
                u = np.zeros(N, dtype=int)
                payload = rng.integers(0, 2, K)
                u[info_indices] = payload
                u_sent_info = payload
                compare_indices = info_indices
            ...
            if not np.array_equal(u_hat[compare_indices], u_sent_info):
                num_errors += 1
                num_bit_errors += np.count_nonzero(u_hat[compare_indices] != u_sent_info)
...
def fast_mode():
    return os.environ.get("POLAR_FAST", "0") == "1"
