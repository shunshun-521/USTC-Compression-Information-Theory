...
def _fast_sim_enabled():
    return os.environ.get("POLAR_FAST_SIM", "").strip() in ("1", "true", "yes")
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                frame_ok = crc_check_payload(decoded_info, info_bits, crc_length)
                bit_err = np.sum(decoded_info[:K_info] != info_bits)
            else:
                frame_ok = np.array_equal(u_hat[info_indices], payload)
...
def crc_check_payload(decoded, info_bits, crc_length):
    from decoder_scl import crc_check
    return crc_check(decoded, crc_length) and np.array_equal(decoded[: len(info_bits)], info_bits)
