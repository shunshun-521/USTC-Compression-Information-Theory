"""
蒙特卡洛仿真主循环
"""
import time
import numpy as np

from channel import bpsk_modulate, awgn_channel, compute_llr, eb_n0_to_sigma, prepare_channel_llr
from encoder import polar_encode, bit_reversal_permutation
from construction import ga_construction


def build_code(N, K, design_eb_n0_db=2.5):
    """构造极化码并返回 frozen_bits 与 info_indices。"""
    info_idx, frozen_idx, _ = ga_construction(N, K, design_eb_n0_db)
    frozen_bits = np.ones(N, dtype=int)
    frozen_bits[info_idx] = 0
    return frozen_bits, info_idx


def run_simulation(
    N,
    K,
    eb_n0_db_list,
    decoder,
    decoder_type="sc",
    max_frames=100000,
    min_errors=100,
    crc_length=0,
    verbose=True,
    seed=42,
    frozen_bits=None,
    info_indices=None,
    design_eb_n0_db=2.5,
    permute_llr=True,
):
    """
    蒙特卡洛仿真。
    """
    rng = np.random.default_rng(seed)
    rate = K / N
    if frozen_bits is None or info_indices is None:
        frozen_bits, info_indices = build_code(N, K, design_eb_n0_db)

    k_info = K - crc_length
    results = []

    for eb_n0_db in eb_n0_db_list:
        sigma = eb_n0_to_sigma(eb_n0_db, rate)
        num_errors = 0
        num_bit_errors = 0
        num_frames = 0
        total_decode_time = 0.0
        total_iters = 0.0

        while num_frames < max_frames and num_errors < min_errors:
            info_bits = rng.integers(0, 2, size=k_info)
            if crc_length > 0:
                from decoder_scl import crc_encode

                payload = crc_encode(info_bits, crc_length)
            else:
                payload = info_bits

            u = np.zeros(N, dtype=int)
            u[info_indices] = payload

            x = polar_encode(u)
            s = bpsk_modulate(x)
            y = awgn_channel(s, sigma, rng)
            llr_ch = compute_llr(y, sigma)
            if permute_llr and decoder_type in ("sc", "scl"):
                llr_in = prepare_channel_llr(llr_ch)
            else:
                llr_in = llr_ch

            t0 = time.perf_counter()
            u_hat, aux = decoder(llr_ch if decoder_type == "bp" else llr_in)
            total_decode_time += time.perf_counter() - t0

            if aux is not None and decoder_type == "bp":
                total_iters += aux

            if crc_length > 0:
                from decoder_scl import crc_check

                frame_error = not crc_check(u_hat[info_indices], crc_length)
            else:
                frame_error = not np.array_equal(u_hat[info_indices[:k_info]], info_bits)

            bit_err = np.sum(u_hat[info_indices[:k_info]] != info_bits)
            num_bit_errors += bit_err
            num_errors += int(frame_error)
            num_frames += 1

        bler = num_errors / num_frames
        ber = num_bit_errors / (num_frames * k_info) if k_info > 0 else 0.0
        avg_time = total_decode_time / num_frames
        avg_iters = (total_iters / num_frames) if decoder_type == "bp" else None

        result = {
            "eb_n0_db": float(eb_n0_db),
            "bler": bler,
            "ber": ber,
            "num_errors": num_errors,
            "num_frames": num_frames,
            "avg_decode_time": avg_time,
            "avg_iters": avg_iters,
        }
        results.append(result)

        if verbose:
            msg = (
                f"  Eb/N0={eb_n0_db:.2f}dB | BLER={bler:.4e} | BER={ber:.4e} "
                f"| Errors={num_errors} | Frames={num_frames} "
                f"| AvgTime={avg_time * 1000:.2f}ms"
            )
            if avg_iters is not None:
                msg += f" | AvgIter={avg_iters:.1f}"
            print(msg)

    return results
