    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if info_indices is None:
                u[: len(payload)] = payload
            else:
                u[np.array(info_indices[: len(payload)])] = payload
            ...
            if info_indices is None:
                info_hat = u_hat[: len(payload)]
            else:
                info_hat = u_hat[np.array(info_indices[: len(payload)])]

            frame_error = not np.array_equal(info_hat, payload)
            bit_errors = int(np.sum(info_hat != payload))
