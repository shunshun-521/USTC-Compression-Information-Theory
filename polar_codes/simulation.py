...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            if crc_length > 0:
                info_payload = rng.integers(0, 2, size=k_info, dtype=np.int_)
                payload = crc_encode(info_payload, crc_length)
            else:
                payload = rng.integers(0, 2, size=K, dtype=np.int_)
            ...
            if crc_length > 0:
                ...
                bit_errors = np.count_nonzero(decoded_payload != payload)
