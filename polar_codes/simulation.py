...
def run_simulation(
    ...
    payload_indices = info_indices[:k_info]
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            sent_payload = u[payload_indices]
            decoded_payload = u_hat[payload_indices]
            bit_err = np.sum(sent_payload != decoded_payload)
            num_bit_errors += bit_err
            if bit_err > 0:
                num_errors += 1
            num_frames += 1

            if verbose and num_frames % 5000 == 0:
                print(
                    f"    Eb/N0={eb_n0_db:.2f}dB progress: "
                    f"frames={num_frames}, errors={num_errors}"
                )
