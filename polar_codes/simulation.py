    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                ref_info = rng.integers(0, 2, size=K)
                u[info_indices] = ref_info
            ...
            if crc_length > 0:
                decoded_info = u_hat[info_indices][:-crc_length]
            else:
                decoded_info = u_hat[info_indices]

            frame_error = not np.array_equal(decoded_info, ref_info)
            if frame_error:
                num_errors += 1
                num_bit_errors += np.sum(decoded_info != ref_info)
            num_frames += 1

            if verbose and num_frames % max(1, max_frames // 10) == 0 and os.getenv('POLAR_FAST_SIM'):
                print(f"    progress: {num_frames} frames, {num_errors} errors")
