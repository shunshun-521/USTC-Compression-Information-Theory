...
def run_simulation(
    ...
    if fast:
        max_frames = int(os.environ.get('POLAR_MAX_FRAMES', min(max_frames, 500)))
        min_errors = int(os.environ.get('POLAR_MIN_ERRORS', min(min_errors, 10)))
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if not frame_ok:
                num_errors += 1
                num_bit_errors += np.count_nonzero(u_hat[info_indices[:k_info]] != info_payload)
