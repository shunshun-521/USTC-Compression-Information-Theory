...
def run_simulation(
    ...
    参数：
        info_indices: 信息位索引数组，用于 BER 统计
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            if info_indices is not None:
                ...
                u_sent_info = rng.integers(0, 2, K_info)
                u = np.zeros(N, dtype=int)
                if crc_length > 0:
                    from decoder_scl import crc_encode
                    payload = crc_encode(u_sent_info, crc_length)
                    idx = np.arange(K)
                    u[idx] = payload
                else:
                    u[:K] = u_sent_info
            ...
            if info_indices is not None:
                if crc_length > 0:
                    frame_err = not np.array_equal(u_hat[info_indices[:K]], u[info_indices[:K]])
                ...
                if crc_length > 0:
                    frame_err = not np.array_equal(u_hat[:K], u[:K])
                    bit_err = np.sum(u_hat[:K_info] != u[:K_info])
                else:
                    frame_err = not np.array_equal(u_hat[:K], u[:K])
                    bit_err = np.sum(u_hat[:K_info] != u[:K_info])
