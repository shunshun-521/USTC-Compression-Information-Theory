...
def run_simulation(
    ...
    if max_frames is None or min_errors is None:
        default_max, default_min = _sim_params()
    ...
    if info_indices is None:
        info_indices = None  # will compare all non-frozen via mask
        ...
        while num_frames < max_frames and num_errors < min_errors:
            if crc_length > 0:
                ...
                if info_indices is not None:
                    ...
                    raise ValueError("info_indices required for CRC simulation")
            else:
                info_bits = rng.integers(0, 2, size=K_info)
                u = np.zeros(N, dtype=int)
                if info_indices is not None:
