...
def run_simulation(
    ...
    if info_indices is None:
        info_indices = np.where(np.arange(N) < K)[0]  # placeholder, caller should pass
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                info_bits = rng.integers(0, 2, size=K, dtype=int)
                payload = info_bits
            ...
            if crc_length > 0:
                ...
                if not crc_check(u_hat_info, crc_length):
                    frame_err = True
                else:
                    frame_err = not np.array_equal(info_bits, u_hat_info[:k_info])
            else:
                frame_err = not np.array_equal(payload, u_hat_info)

            if frame_err:
                num_errors += 1
                num_bit_errors += np.sum(info_bits != u_hat_info[: len(info_bits)])

            num_frames += 1

        bler = num_errors / num_frames if num_frames else 1.0
        denom_bits = num_frames * (K - crc_length) if num_frames else 1
        ber = num_bit_errors / denom_bits
