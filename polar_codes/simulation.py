    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                if not np.array_equal(check_bits, payload):
                    num_errors += 1
                num_bit_errors += np.sum(check_bits != payload)
            else:
                err_mask = u_hat[info_indices] != u[info_indices]
                if np.any(err_mask):
                    num_errors += 1
                num_bit_errors += np.sum(err_mask)
