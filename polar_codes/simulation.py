...
def run_simulation(
    ...
    返回每个 Eb/N0 点的结果列表。
    """
    if os.environ.get("POLAR_FAST") == "1":
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                compare_len = len(sent_info)

            frame_err = not np.array_equal(sent_info[:compare_len], recv_info[:compare_len])
            if frame_err:
                num_errors += 1
                num_bit_errors += np.sum(sent_info[:compare_len] != recv_info[:compare_len])
