...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                sent_info = info_bits
                recv_info = u_hat[info_indices][:K_info] if info_indices is not None else u_hat[:K_info]
            else:
                sent_info = u[info_indices] if info_indices is not None else payload
                recv_info = u_hat[info_indices] if info_indices is not None else u_hat[:K]

            frame_error = not np.array_equal(sent_info, recv_info)
            bit_errors = int(np.sum(sent_info != recv_info))
