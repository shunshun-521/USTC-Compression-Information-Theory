...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            x = polar_encode(u)
            s = bpsk_modulate(x)
            y = s + rng.normal(0.0, sigma, size=N)
            ...
            if crc_length > 0:
                frame_error = not np.array_equal(u_hat[info_idx][:k_info], info_bits)
                bit_errors = np.count_nonzero(u_hat[info_idx][:k_info] != info_bits)
            else:
                frame_error = not np.array_equal(u_hat[info_idx], payload)
                bit_errors = np.count_nonzero(u_hat[info_idx] != payload)
