"""
蒙特卡洛仿真主循环
"""
import os
import time

import numpy as np

from channel import awgn_channel, bpsk_modulate, compute_llr, eb_n0_to_sigma
from construction import ga_construction
from encoder import polar_encode


def run_simulation(
    N,
    K,
    eb_n0_db_list,
    decoder,
    decoder_type="sc",
    max_frames=100000,
    min_errors=100,
    crc_length=0,
    verbose=True,
    seed=42,
    info_indices=None,
    frozen_bits=None,
):
    """
    蒙特卡洛仿真。
    """
    rng = np.random.default_rng(seed)
    rate = K / N
    k_info = K - crc_length

    if info_indices is None or frozen_bits is None:
        info_indices, _, _ = ga_construction(N, K, 2.5, rate=rate)
        frozen_bits = np.ones(N, dtype=int)
        frozen_bits[info_indices] = 0

    info_indices = np.asarray(info_indices, dtype=int)
    frozen_bits = np.asarray(frozen_bits, dtype=bool)

    env_max = os.environ.get("POLAR_MAX_FRAMES")
    env_min = os.environ.get("POLAR_MIN_ERRORS")
    if env_max is not None:
        max_frames = int(env_max)
    if env_min is not None:
        min_errors = int(env_min)

    results = []

    for eb_n0_db in eb_n0_db_list:
        sigma = eb_n0_to_sigma(eb_n0_db, rate)
        num_errors = 0
        num_bit_errors = 0
        num_frames = 0
        total_decode_time = 0.0
        total_iters = 0.0

        while num_frames < max_frames and num_errors < min_errors:
            if crc_length > 0:
                from decoder_scl import crc_encode

                info_payload = rng.integers(0, 2, size=k_info)
                payload_with_crc = crc_encode(info_payload, crc_length)
                u = np.zeros(N, dtype=int)
                u[info_indices] = payload_with_crc
            else:
                u = np.zeros(N, dtype=int)
                u[info_indices] = rng.integers(0, 2, size=K)

            x = polar_encode(u)
            symbols = bpsk_modulate(x)
            received = awgn_channel(symbols, sigma, rng)
            llr = compute_llr(received, sigma)

            t0 = time.perf_counter()
            u_hat, aux = decoder(llr)
            total_decode_time += time.perf_counter() - t0

            if decoder_type == "bp" and aux is not None:
                total_iters += aux

            if crc_length > 0:
                frame_error = not np.array_equal(u_hat[info_indices], u[info_indices])
                bit_err = np.count_nonzero(u_hat[info_indices][:k_info] != u[info_indices][:k_info])
            else:
                frame_error = not np.array_equal(u_hat[info_indices], u[info_indices])
                bit_err = np.count_nonzero(u_hat[info_indices] != u[info_indices])

            num_frames += 1
            num_errors += int(frame_error)
            num_bit_errors += bit_err

        bler = num_errors / num_frames
        ber = num_bit_errors / (num_frames * k_info)
        avg_time = total_decode_time / num_frames
        avg_iters = (total_iters / num_frames) if decoder_type == "bp" else None

        result = {
            "eb_n0_db": float(eb_n0_db),
            "bler": bler,
            "ber": ber,
            "num_errors": num_errors,
            "num_frames": num_frames,
            "avg_decode_time": avg_time,
            "avg_iters": avg_iters,
        }
        results.append(result)

        if verbose:
            print(
                f"  Eb/N0={eb_n0_db:.2f}dB | BLER={bler:.4e} | BER={ber:.4e} "
                f"| Errors={num_errors} | Frames={num_frames} "
                f"| AvgTime={avg_time * 1000:.2f}ms"
                + (f" | AvgIter={avg_iters:.1f}" if avg_iters is not None else "")
            )

    return results
