...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            decoded_payload = decoded_info[:compare_len]
            frame_error = not np.array_equal(decoded_payload, ref_bits)
            if frame_error:
                num_errors += 1
            num_bit_errors += np.count_nonzero(decoded_payload != ref_bits)
            num_frames += 1
