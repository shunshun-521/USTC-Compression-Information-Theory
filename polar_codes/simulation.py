...
def run_simulation(
    ...
    if info_indices is None or frozen_bits is None:
        info_idx, frozen_idx, _ = construct_frozen_ga(N, K, design_eb_n0_db)
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                sent_info = info_payload
                rcv_info = u_hat[info_indices][:K_info]
            else:
                sent_info = info_payload
                rcv_info = u_hat[info_indices]

            frame_err = not np.array_equal(sent_info, rcv_info)
            if frame_err:
                num_errors += 1
            num_bit_errors += np.sum(sent_info != rcv_info)
            num_frames += 1
