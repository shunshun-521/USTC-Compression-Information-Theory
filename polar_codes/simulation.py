...
def run_simulation(
    ...
    """
    fast = _fast_sim_params()
    max_frames = fast.get("max_frames", max_frames)
    min_errors = fast.get("min_errors", min_errors)
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if frame_error:
                ...
                if crc_length > 0:
                    ...
                    num_bit_errors += np.sum(u_hat[info_indices] != info_bits)

            num_frames += 1

        bler = num_errors / num_frames if num_frames > 0 else 0.0
        ber = (
            num_bit_errors / (num_frames * K_info) if num_frames > 0 else 0.0
        )
