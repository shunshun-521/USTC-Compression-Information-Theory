...
def run_simulation(
    ...
    参数：
        N: 码长
        K: 信息位数（含 CRC 时包含 CRC 位）
        eb_n0_db_list: Eb/N0 列表（dB）
        decoder: 译码器 callable，返回 (u_hat, aux)
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            user_sent = info_bits
            user_recv = u_hat[info_indices[:k_info]]
            frame_error = not np.array_equal(user_sent, user_recv)
            if frame_error:
                num_errors += 1
            num_bit_errors += np.sum(user_sent != user_recv)
            num_frames += 1
