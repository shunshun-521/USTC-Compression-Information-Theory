    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                frame_ok = crc_check_payload(info_bits, payload_hat, crc_length)
            else:
                frame_ok = np.array_equal(u_hat[info_indices][:K_info], info_bits)
...
def crc_check_payload(info_bits, payload_hat, crc_length):
    """检查译码后的 payload 是否与原始信息比特一致（含 CRC 校验）。"""
    from decoder_scl import crc_check
    if not np.array_equal(payload_hat[: len(info_bits)], info_bits):
        return False
    return crc_check(payload_hat, crc_length)
