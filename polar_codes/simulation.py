    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                frame_error = not crc_check_bits(decoded_info, info_bits, crc_length)
                bit_error_count = np.sum(decoded_info[:k_info] != info_bits)
            else:
                frame_error = not np.array_equal(u_hat[info_indices], payload)
                bit_error_count = np.sum(u_hat[info_indices] != payload)

            num_bit_errors += bit_error_count
...
def crc_check_bits(decoded_payload, info_bits, crc_length):
    """检查译码后的信息位与 CRC 是否一致"""
    from decoder_scl import crc_check
    return np.array_equal(decoded_payload[:len(info_bits)], info_bits) and crc_check(
        decoded_payload, crc_length
    )
