    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            if crc_length > 0:
                ...
                u_info_crc = crc_encode(info_bits, crc_length)
                u = np.zeros(N, dtype=np.int8)
                u[info_indices] = u_info_crc
            else:
            ...
            if crc_length > 0:
                ...
                frame_error = not np.array_equal(payload_hat, info_bits)
                bit_errors = np.sum(payload_hat != info_bits)
            else:
...
def fast_sim_enabled():
    return os.environ.get("POLAR_FAST_SIM", "").lower() in ("1", "true", "yes")
