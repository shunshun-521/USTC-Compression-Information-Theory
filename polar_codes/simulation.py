"""
蒙特卡洛仿真主循环
"""
import os
import time
import numpy as np

from channel import bpsk_modulate, awgn_channel, compute_llr, eb_n0_to_sigma
from encoder import polar_encode, bit_reversal_permutation


def permute_llr_for_decode(llr_ch):
    """将信道 LLR 重排为 SC/SCL 译码器所需的顺序（配合含比特倒序的编码器）。"""
    llr_ch = np.asarray(llr_ch, dtype=np.float64)
    N = len(llr_ch)
    brp = bit_reversal_permutation(N)
    return llr_ch[brp]


def run_simulation(
    N,
    K,
    eb_n0_db_list,
    decoder,
    decoder_type="sc",
    max_frames=100000,
    min_errors=100,
    crc_length=0,
    verbose=True,
    seed=42,
    frozen_bits=None,
    info_indices=None,
):
    """
    蒙特卡洛仿真。
    """
    if os.environ.get("POLAR_FAST_SIM") == "1":
        max_frames = min(max_frames, 500)
        min_errors = min(min_errors, 20)

    rng = np.random.default_rng(seed)
    rate = K / N
    results = []
    k_info = K - crc_length

    for eb_n0_db in eb_n0_db_list:
        sigma = eb_n0_to_sigma(eb_n0_db, rate)
        num_errors = 0
        num_bit_errors = 0
        num_frames = 0
        total_decode_time = 0.0
        total_iters = 0.0

        while num_frames < max_frames and num_errors < min_errors:
            if crc_length > 0:
                from decoder_scl import crc_encode

                info = rng.integers(0, 2, size=k_info, dtype=np.int_)
                payload = crc_encode(info, crc_length)
                u = np.zeros(N, dtype=np.int_)
                u[info_indices] = payload
            else:
                info = rng.integers(0, 2, size=K, dtype=np.int_)
                u = np.zeros(N, dtype=np.int_)
                u[info_indices] = info

            x = polar_encode(u)
            s = bpsk_modulate(x)
            y = awgn_channel(s, sigma, rng)
            llr_ch = compute_llr(y, sigma)
            llr_dec = permute_llr_for_decode(llr_ch)

            t0 = time.perf_counter()
            u_hat, aux = decoder(llr_dec)
            total_decode_time += time.perf_counter() - t0

            if decoder_type == "bp" and aux is not None:
                total_iters += aux

            if crc_length > 0:
                frame_err = not np.array_equal(u_hat[info_indices], u[info_indices])
                bit_err = np.sum(u_hat[info_indices][:k_info] != info)
            else:
                frame_err = not np.array_equal(u_hat[info_indices], u[info_indices])
                bit_err = np.sum(u_hat[info_indices] != info)

            num_frames += 1
            if frame_err:
                num_errors += 1
            num_bit_errors += bit_err

        bler = num_errors / num_frames if num_frames else 1.0
        ber = num_bit_errors / (num_frames * k_info) if num_frames else 1.0
        avg_time = total_decode_time / num_frames if num_frames else 0.0
        avg_iters = (total_iters / num_frames) if decoder_type == "bp" and num_frames else None

        result = {
            "eb_n0_db": eb_n0_db,
            "bler": bler,
            "ber": ber,
            "num_errors": num_errors,
            "num_frames": num_frames,
            "avg_decode_time": avg_time,
            "avg_iters": avg_iters,
        }
        results.append(result)

        if verbose:
            msg = (
                f"  Eb/N0={eb_n0_db:.2f}dB | BLER={bler:.4e} | BER={ber:.4e} "
                f"| Errors={num_errors} | Frames={num_frames} "
                f"| AvgTime={avg_time * 1000:.2f}ms"
            )
            if avg_iters is not None:
                msg += f" | AvgIter={avg_iters:.1f}"
            print(msg)

    return results
