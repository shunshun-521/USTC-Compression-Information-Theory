...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                frame_error = not np.array_equal(u_hat[info_indices][:K_info], u_sent[info_indices][:K_info])
                bit_err = np.sum(u_hat[info_indices][:K_info] != u_sent[info_indices][:K_info])
            else:
                frame_error = not np.array_equal(u_hat[info_indices], u_sent[info_indices])
