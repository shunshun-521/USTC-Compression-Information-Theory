"""
蒙特卡洛仿真主循环
"""
import os
import time

import numpy as np

from channel import awgn_channel, bpsk_modulate, compute_llr, eb_n0_to_sigma
from construction import ga_construction
from encoder import polar_encode
from decoder_scl import crc_encode


def _build_codeword(N, K, info_indices, crc_length, rng):
    """构造发送码字 u 及有效信息比特。"""
    u = np.zeros(N, dtype=int)
    k_info = K - crc_length

    if crc_length > 0:
        info_bits = rng.integers(0, 2, size=k_info, dtype=int)
        payload = crc_encode(info_bits, crc_length)
    else:
        info_bits = rng.integers(0, 2, size=K, dtype=int)
        payload = info_bits

    u[info_indices] = payload
    return u, info_bits


def run_simulation(
    N,
    K,
    eb_n0_db_list,
    decoder,
    decoder_type="sc",
    max_frames=100000,
    min_errors=100,
    crc_length=0,
    verbose=True,
    seed=42,
    design_eb_n0_db=2.5,
):
    """蒙特卡洛仿真。"""
    rng = np.random.default_rng(seed)
    rate = K / N
    results = []

    info_indices, _, _ = ga_construction(N, K, design_eb_n0_db, rate=rate)
    k_info = K - crc_length

    for eb_n0_db in eb_n0_db_list:
        sigma = eb_n0_to_sigma(eb_n0_db, rate)
        num_errors = 0
        num_bit_errors = 0
        num_frames = 0
        total_decode_time = 0.0
        total_iters = 0.0

        while num_frames < max_frames and num_errors < min_errors:
            u, info_bits = _build_codeword(N, K, info_indices, crc_length, rng)
            x = polar_encode(u)
            s = bpsk_modulate(x)
            y = awgn_channel(s, sigma, rng)
            llr = compute_llr(y, sigma)

            t0 = time.perf_counter()
            u_hat, aux = decoder(llr)
            total_decode_time += time.perf_counter() - t0

            if aux is not None and decoder_type == "bp":
                total_iters += aux

            decoded_info = u_hat[info_indices]
            if crc_length > 0:
                frame_error = not np.array_equal(decoded_info[:k_info], info_bits)
                bit_err = np.count_nonzero(decoded_info[:k_info] != info_bits)
            else:
                frame_error = not np.array_equal(decoded_info, info_bits)
                bit_err = np.count_nonzero(decoded_info != info_bits)

            num_errors += int(frame_error)
            num_bit_errors += bit_err
            num_frames += 1

        bler = num_errors / num_frames
        ber = num_bit_errors / (num_frames * k_info) if k_info > 0 else 0.0
        avg_time = total_decode_time / num_frames
        avg_iters = (total_iters / num_frames) if decoder_type == "bp" else None

        result = {
            "eb_n0_db": eb_n0_db,
            "bler": bler,
            "ber": ber,
            "num_errors": num_errors,
            "num_frames": num_frames,
            "avg_decode_time": avg_time,
            "avg_iters": avg_iters,
        }
        results.append(result)

        if verbose:
            msg = (
                f"  Eb/N0={eb_n0_db:.2f}dB | BLER={bler:.4e} | BER={ber:.4e} "
                f"| Errors={num_errors} | Frames={num_frames} "
                f"| AvgTime={avg_time * 1000:.2f}ms"
            )
            if avg_iters is not None:
                msg += f" | AvgIter={avg_iters:.1f}"
            print(msg)

    return results


def fast_sim_settings():
    """自动化/快速测试用缩减参数。"""
    if os.environ.get("POLAR_FAST_SIM", "0") == "1":
        return {
            "max_frames": 300,
            "min_errors": 15,
            "eb_n0_sc": np.arange(1.0, 4.5, 0.5),
            "eb_n0_other": np.arange(2.0, 4.5, 0.5),
        }
    return {
        "max_frames": 100000,
        "min_errors": 100,
        "eb_n0_sc": np.arange(0.0, 5.5, 0.25),
        "eb_n0_other": np.arange(1.0, 5.5, 0.25),
    }
