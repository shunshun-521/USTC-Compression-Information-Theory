...
def run_simulation(
    ...
    参数：
        info_indices: 信息位索引数组；若为 None 则假定前 K 个最可靠位（由调用方传入）
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if decoder_type == 'bp':
                llr_for_dec = compute_llr(y, sigma)
            else:
                llr_for_dec = prepare_channel_llr(compute_llr(y, sigma), N)

            t0 = time.perf_counter()
            u_hat, aux = decoder(llr_for_dec)
