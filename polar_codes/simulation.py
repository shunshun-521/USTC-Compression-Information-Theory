...
def run_simulation(
    ...
    参数：
        info_indices: 信息位索引（可选，与 frozen_bits 二选一）
        frozen_bits: 冻结位标记数组，1=冻结，0=信息
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if not np.array_equal(u_hat[info_idx][:K_info], u[info_idx][:K_info]):
                num_errors += 1
                num_bit_errors += np.sum(u_hat[info_idx][:K_info] != u[info_idx][:K_info])
