...
def run_simulation(
    ...
    返回：dict 列表，每个 Eb/N0 点包含 bler, ber, num_errors 等字段。
    """
    ...
    if info_indices is None:
        info_indices = np.where(frozen_bits == 0)[0] if frozen_bits is not None else None
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if frame_error:
                num_errors += 1
                num_bit_errors += np.sum(sent_info != decoded_info)
