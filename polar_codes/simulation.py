...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                err = not np.array_equal(u_hat[info_indices][:K_info], info_raw)
            else:
                err = not np.array_equal(u_hat[info_indices], info_raw)

            if err:
                num_errors += 1
                num_bit_errors += np.sum(u_hat[info_indices][:K_info] != info_raw)
