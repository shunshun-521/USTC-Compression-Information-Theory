...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            if crc_length > 0:
                msg = rng.integers(0, 2, size=K_info, dtype=int)
            ...
            if crc_length > 0:
                frame_err = not np.array_equal(u_hat[info_indices][:K_info], msg)
            else:
            ...
            if frame_err:
                num_errors += 1
                if crc_length > 0:
                    num_bit_errors += np.sum(u_hat[info_indices][:K_info] != msg)
                else:
