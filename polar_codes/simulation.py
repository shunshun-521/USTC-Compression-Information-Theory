...
def _quick_mode():
  return os.environ.get("POLAR_QUICK", "").strip().lower() in {"1", "true", "yes"}
  ...
  for eb_n0_db in eb_n0_db_list:
    ...
    while num_frames < max_frames and num_errors < min_errors:
      ...
      if crc_length > 0:
        ...
        frame_error = not np.array_equal(info_bits, decoded_info)
        bit_err = np.count_nonzero(info_bits != decoded_info)
      else:
