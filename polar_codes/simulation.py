    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            if crc_length > 0:
                ...
                payload = crc_encode(info_bits, crc_length=crc_length)
            else:
            ...
            frame_error = not np.array_equal(recv_info, sent_info)
            bit_errors = np.count_nonzero(recv_info != sent_info)
...
def get_sim_params():
    ...
    if fast:
        max_frames = min(max_frames, int(os.environ.get('POLAR_MAX_FRAMES', '200')))
        min_errors = min(min_errors, int(os.environ.get('POLAR_MIN_ERRORS', '8')))
