...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            sent_payload = u[info_indices]

            if crc_length > 0:
                frame_error = not np.array_equal(decoded_payload[:K_info], sent_payload[:K_info])
                bit_errors = int(np.sum(decoded_payload[:K_info] != sent_payload[:K_info]))
            else:
                frame_error = not np.array_equal(decoded_payload, sent_payload)
                bit_errors = int(np.sum(decoded_payload != sent_payload))
        ...
        ber = num_bit_errors / (num_frames * K_info) if num_frames > 0 and K_info > 0 else 0.0
