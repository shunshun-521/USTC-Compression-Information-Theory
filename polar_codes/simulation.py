"""
蒙特卡洛仿真主循环
"""
import os
import time

import numpy as np

from channel import awgn_channel, bpsk_modulate, compute_llr, eb_n0_to_sigma
from decoder_scl import crc_encode
from encoder import compute_channel_permutation, polar_encode


def run_simulation(
    N,
    K,
    eb_n0_db_list,
    decoder,
    decoder_type="sc",
    max_frames=100000,
    min_errors=100,
    crc_length=0,
    verbose=True,
    seed=42,
):
    """
    蒙特卡洛仿真。
    """
    fast = os.environ.get("POLAR_FAST_SIM", "0") == "1"
    if fast:
        max_frames = min(max_frames, 50)
        min_errors = min(min_errors, 5)

    rng = np.random.default_rng(seed)
    rate = K / N
    K_info = K - crc_length
    info_count = K_info if crc_length > 0 else K

    from construction import ga_construction

    info_idx, _, _ = ga_construction(N, K, 2.5)
    frozen_bits = np.ones(N, dtype=bool)
    frozen_bits[info_idx] = False
    info_positions = np.where(~frozen_bits)[0]

    perm = compute_channel_permutation(N)
    results = []

    for eb_n0_db in eb_n0_db_list:
        sigma = eb_n0_to_sigma(eb_n0_db, rate)
        num_errors = 0
        num_bit_errors = 0
        num_frames = 0
        total_decode_time = 0.0
        total_iters = 0

        while num_frames < max_frames and num_errors < min_errors:
            if crc_length > 0:
                info_bits = rng.integers(0, 2, size=K_info)
                payload = crc_encode(info_bits, crc_length)
                u = np.zeros(N, dtype=int)
                u[info_positions] = payload
            else:
                u = np.zeros(N, dtype=int)
                info_bits = rng.integers(0, 2, size=K)
                u[info_positions] = info_bits

            x = polar_encode(u)
            s = bpsk_modulate(x)
            y = awgn_channel(s, sigma, rng)
            llr_ch = compute_llr(y, sigma)
            llr_tree = llr_ch[perm]

            t0 = time.perf_counter()
            u_hat, aux = decoder(llr_tree)
            total_decode_time += time.perf_counter() - t0

            if decoder_type == "bp" and aux is not None:
                total_iters += aux

            if crc_length > 0:
                frame_err = not np.array_equal(u_hat[info_positions], u[info_positions])
                bit_err = np.sum(u_hat[info_positions][:K_info] != info_bits)
            else:
                frame_err = not np.array_equal(u_hat[info_positions], u[info_positions])
                bit_err = np.sum(u_hat[info_positions] != info_bits)

            num_errors += int(frame_err)
            num_bit_errors += bit_err
            num_frames += 1

        bler = num_errors / num_frames
        ber = num_bit_errors / (num_frames * info_count)
        avg_time = total_decode_time / num_frames
        avg_iters = (total_iters / num_frames) if decoder_type == "bp" else None

        result = {
            "eb_n0_db": eb_n0_db,
            "bler": bler,
            "ber": ber,
            "num_errors": num_errors,
            "num_frames": num_frames,
            "avg_decode_time": avg_time,
            "avg_iters": avg_iters,
        }
        results.append(result)

        if verbose:
            print(
                f"  Eb/N0={eb_n0_db:.2f}dB | BLER={bler:.4e} | BER={ber:.4e} "
                f"| Errors={num_errors} | Frames={num_frames} "
                f"| AvgTime={avg_time * 1000:.2f}ms"
                + (f" | AvgIter={avg_iters:.1f}" if avg_iters else "")
            )

    return results
