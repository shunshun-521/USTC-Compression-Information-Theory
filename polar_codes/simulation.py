    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            if crc_length > 0:
                ...
                info_raw = rng.integers(0, 2, size=K)
                u = np.zeros(N, dtype=int)
                u[info_indices] = info_raw
            ...
            if crc_length > 0:
                frame_error = not np.array_equal(u_hat[info_indices][:k_info], info_raw)
                bit_errors = np.sum(u_hat[info_indices][:k_info] != info_raw)
            else:
                frame_error = not np.array_equal(u_hat[info_indices], info_raw)
                bit_errors = np.sum(u_hat[info_indices] != info_raw)
