...
def _fast_sim_limits(max_frames, min_errors):
    if os.environ.get("POLAR_FAST_SIM", "").strip() in {"1", "true", "yes"}:
        return min(max_frames, 500), min(min_errors, 20)
...
def run_simulation(
    ...
    蒙特卡洛仿真。
    """
    max_frames, min_errors = _fast_sim_limits(max_frames, min_errors)
    ...
    if info_indices is None:
        info_indices = np.where(np.arange(N) >= 0)[0][:K]
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            u[info_indices] = info_bits if crc_length == 0 else info_bits
            ...
            if crc_length > 0:
                recv_payload = u_hat[info_indices][:-crc_length]
            else:
                recv_payload = u_hat[info_indices]

            bit_err = np.count_nonzero(recv_payload != sent_payload)
