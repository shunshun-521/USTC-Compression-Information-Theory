...
def run_simulation(
    ...
    if os.environ.get("POLAR_QUICK", "").strip() in ("1", "true", "yes"):
        max_frames, min_errors = env_max, env_min
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                err = not np.array_equal(u_hat[info_idx[:k_info]], u[info_idx[:k_info]])
                bit_err = np.sum(u_hat[info_idx[:k_info]] != u[info_idx[:k_info]])
            else:
                err = not np.array_equal(u_hat[info_idx], u[info_idx])
