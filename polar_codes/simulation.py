    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                frame_ok = np.array_equal(decoded_info, info_with_crc)
                bit_err = np.count_nonzero(decoded_info != info_with_crc)
            else:
                decoded_info = u_hat[info_indices]
                frame_ok = np.array_equal(decoded_info, info_payload)
                bit_err = np.count_nonzero(decoded_info != info_payload)
