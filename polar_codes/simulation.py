...
def run_simulation(
    ...
    if info_indices is None:
        info_indices, _, _ = ga_construction(N, K, design_eb_n0_db=2.5)
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            msg_bits = rng.integers(0, 2, K_info, dtype=np.int8)
            if crc_length > 0:
            ...
            decoded_msg = u_hat[info_indices][:K_info]
            bit_err = int(np.sum(decoded_msg != msg_bits))
