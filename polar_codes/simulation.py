...
def run_simulation(
    ...
    if os.environ.get("POLAR_QUICK") == "1":
        max_frames = _env_int("POLAR_MAX_FRAMES", 2000)
        min_errors = _env_int("POLAR_MIN_ERRORS", 20)
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                payload_idx = info_indices[:K_info]
                frame_error = not np.array_equal(u_hat[payload_idx], u[payload_idx])
                bit_errors = np.count_nonzero(u_hat[payload_idx] != u[payload_idx])
            else:
                payload_idx = info_indices[:K]
                frame_error = not np.array_equal(u_hat[payload_idx], u[payload_idx])
                bit_errors = np.count_nonzero(u_hat[payload_idx] != u[payload_idx])
