...
def run_simulation(
    ...
    if fast:
        max_frames = int(os.environ.get("POLAR_MAX_FRAMES", min(max_frames, 5000)))
        min_errors = int(os.environ.get("POLAR_MIN_ERRORS", min(min_errors, 20)))
    ...
    if info_indices is None:
        info_indices = np.sort(np.arange(N)[-K:])
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                frame_error = not np.array_equal(decoded_info, payload_with_crc)
                bit_errors = np.sum(decoded_info != payload_with_crc)
            else:
                decoded_info = u_hat[info_indices]
