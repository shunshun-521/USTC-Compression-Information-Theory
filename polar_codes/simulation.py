...
def run_simulation(
    ...
    if max_frames is None or min_errors is None:
        env_max, env_min = _sim_limits()
        max_frames = max_frames if max_frames is not None else env_max
        min_errors = min_errors if min_errors is not None else env_min
    ...
    if info_indices is None:
        if frozen_bits is not None:
            ...
            raise ValueError("frozen_bits or info_indices required")
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if frame_error:
                num_errors += 1
            num_bit_errors += np.count_nonzero(decoded_info != info_bits)
            num_frames += 1
