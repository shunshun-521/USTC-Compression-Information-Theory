    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if info_indices is None:
                u[:K] = payload
                check_sent = payload if crc_length == 0 else info_bits
                get_recv = lambda uh: uh[:K_info] if crc_length > 0 else uh[:K]
            else:
                u[info_indices] = payload
                check_sent = info_bits if crc_length > 0 else payload
                get_recv = lambda uh: uh[info_indices][:K_info] if crc_length > 0 else uh[info_indices]
            ...
            recv_compare = get_recv(u_hat)
            bit_err = int(np.sum(recv_compare != check_sent))
