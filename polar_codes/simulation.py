...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                frame_err = not np.array_equal(u_hat[info_idx][:K_info], ref_info)
                bit_err = np.sum(u_hat[info_idx][:K_info] != ref_info)
            else:
                frame_err = not np.array_equal(u_hat[info_idx], ref_info)
                bit_err = np.sum(u_hat[info_idx] != ref_info)
