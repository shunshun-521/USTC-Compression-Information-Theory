"""
蒙特卡洛仿真主循环
"""
import time
import numpy as np

from channel import bpsk_modulate, awgn_channel, compute_llr, eb_n0_to_sigma
from encoder import polar_encode
from decoder_scl import crc_encode


def run_simulation(
    N,
    K,
    eb_n0_db_list,
    decoder,
    decoder_type="sc",
    max_frames=100000,
    min_errors=100,
    crc_length=0,
    verbose=True,
    seed=42,
    info_indices=None,
    frozen_bits=None,
):
    """
    蒙特卡洛仿真。

    info_indices: 信息位索引；若提供 frozen_bits 则优先使用 frozen_bits
    frozen_bits: 1=冻结，0=信息
    """
    import os
    fast = os.environ.get("POLAR_FAST_SIM", "0") == "1"
    if fast:
        max_frames = min(max_frames, 5000)
        min_errors = min(min_errors, 20)

    rng = np.random.default_rng(seed)
    rate = K / N
    K_info = K - crc_length

    if frozen_bits is not None:
        frozen_bits = np.asarray(frozen_bits, dtype=np.int8)
        info_indices = np.where(frozen_bits == 0)[0]
    elif info_indices is not None:
        info_indices = np.asarray(info_indices, dtype=np.int64)
        frozen_bits = np.ones(N, dtype=np.int8)
        frozen_bits[info_indices] = 0
    else:
        raise ValueError("Need info_indices or frozen_bits")

    results = []

    for eb_n0_db in eb_n0_db_list:
        sigma = eb_n0_to_sigma(eb_n0_db, rate)
        num_errors = 0
        num_bit_errors = 0
        num_frames = 0
        total_decode_time = 0.0
        total_iters = 0.0

        while num_frames < max_frames and num_errors < min_errors:
            info_raw = rng.integers(0, 2, size=K_info, dtype=np.int8)
            if crc_length > 0:
                payload = crc_encode(info_raw, crc_length)
                u = np.zeros(N, dtype=np.int8)
                u[info_indices] = payload
            else:
                u = np.zeros(N, dtype=np.int8)
                u[info_indices] = info_raw

            x = polar_encode(u)
            s = bpsk_modulate(x)
            y = awgn_channel(s, sigma, rng)
            llr = compute_llr(y, sigma)

            t0 = time.perf_counter()
            u_hat, aux = decoder(llr)
            total_decode_time += time.perf_counter() - t0

            if aux is not None and decoder_type == "bp":
                total_iters += aux

            frame_err = False
            for idx in info_indices[:K_info]:
                if u_hat[idx] != u[idx]:
                    frame_err = True
                    num_bit_errors += int(u_hat[idx] != u[idx])

            if frame_err:
                num_errors += 1
            num_frames += 1

        bler = num_errors / num_frames if num_frames else 0.0
        ber = num_bit_errors / (num_frames * K_info) if num_frames and K_info else 0.0
        avg_time = total_decode_time / num_frames if num_frames else 0.0
        avg_iters = (total_iters / num_frames) if decoder_type == "bp" and num_frames else None

        result = {
            "eb_n0_db": eb_n0_db,
            "bler": bler,
            "ber": ber,
            "num_errors": num_errors,
            "num_frames": num_frames,
            "avg_decode_time": avg_time,
            "avg_iters": avg_iters,
        }
        results.append(result)

        if verbose:
            msg = (
                f"  Eb/N0={eb_n0_db:.2f}dB | BLER={bler:.4e} | BER={ber:.4e} "
                f"| Errors={num_errors} | Frames={num_frames} "
                f"| AvgTime={avg_time * 1000:.2f}ms"
            )
            if avg_iters is not None:
                msg += f" | AvgIter={avg_iters:.1f}"
            print(msg)

    return results
