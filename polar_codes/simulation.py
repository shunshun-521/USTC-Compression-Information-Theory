...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            llr_ch = prepare_decoder_llr(compute_llr(y, sigma), N)
            ...
            if crc_length > 0:
                frame_err = not np.array_equal(decoded_payload[:K_info], info_bits)
                bit_err = int(np.sum(decoded_payload[:K_info] != info_bits))
            else:
                frame_err = not np.array_equal(decoded_payload, info_bits)
                bit_err = int(np.sum(decoded_payload != info_bits))
