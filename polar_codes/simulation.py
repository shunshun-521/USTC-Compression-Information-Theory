    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                frame_err = not crc_check_payload(check_bits, crc_length)
                bit_err_count = np.sum(info_bits != check_bits[:K_info])
            else:
                frame_err = not np.array_equal(u_hat[info_idx], payload)
                bit_err_count = np.sum(u_hat[info_idx] != payload)

            num_frames += 1
            num_bit_errors += bit_err_count
            if frame_err:
                num_errors += 1
...
def crc_check_payload(bits, crc_length):
...
def get_sim_params():
    ...
    max_frames = int(os.environ.get("POLAR_MAX_FRAMES", "100000" if not fast else "300"))
    min_errors = int(os.environ.get("POLAR_MIN_ERRORS", "100" if not fast else "8"))
