...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                frame_error = not crc_check_payload(u_hat, info_indices, crc_length)
                bit_err = np.sum(u_hat[info_indices][:k_info] != info_bits)
            else:
                frame_error = np.any(u_hat[info_indices] != info_bits)
...
def crc_check_payload(u_hat, info_indices, crc_length):
    from decoder_scl import crc_check

    return crc_check(u_hat[info_indices], crc_length)
