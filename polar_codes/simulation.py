    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                bit_error_count = np.sum(info_bits != u_hat[info_indices][:k_info])
            else:
                frame_error = not np.array_equal(payload, u_hat[info_indices])
                bit_error_count = np.sum(payload != u_hat[info_indices])

            num_frames += 1
            num_errors += int(frame_error)
            num_bit_errors += int(bit_error_count)
