...
def run_simulation(
  ...
  for eb_n0_db in eb_n0_db_list:
    ...
    while num_frames < max_frames and num_errors < min_errors:
      payload = rng.integers(0, 2, size=K_info, dtype=np.int8)
      if crc_length > 0:
      ...
      if crc_length > 0:
        frame_error = np.any(u_hat[info_indices][:K_info] != payload)
        bit_errors = int(np.sum(u_hat[info_indices][:K_info] != payload))
      else:
        frame_error = np.any(u_hat[info_indices] != msg)
        bit_errors = int(np.sum(u_hat[info_indices] != msg))
