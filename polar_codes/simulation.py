    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            if crc_length > 0:
                ...
                payload = __import__("decoder_scl", fromlist=["crc_encode"]).crc_encode(
                    info_bits, crc_length
                )
                u = np.zeros(N, dtype=int)
                u[info_indices[:K]] = payload
            else:
                u = np.zeros(N, dtype=int)
                u[info_indices[:K]] = rng.integers(0, 2, K)
            ...
            if crc_length > 0:
                ...
                frame_error = not __import__(
                    "decoder_scl", fromlist=["crc_check"]
                ).crc_check(decoded_info, crc_length)
                bit_err = np.sum(decoded_info[:k_info] != payload[:k_info])
            else:
                frame_error = not np.array_equal(u_hat[info_indices[:K]], u[info_indices[:K]])
                bit_err = np.sum(u_hat[info_indices[:K]] != u[info_indices[:K]])
