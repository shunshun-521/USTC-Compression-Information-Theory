...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            if crc_length > 0:
                info_bits = rng.integers(0, 2, k_info, dtype=int)
                payload = crc_encode(info_bits, crc_length)
            else:
                payload = rng.integers(0, 2, K, dtype=int)
            ...
            sent_info = payload if crc_length == 0 else info_bits
            recv_info = u_hat[info_indices]
            if crc_length > 0:
                recv_info = recv_info[:k_info]

            frame_err = not np.array_equal(sent_info, recv_info)
            if frame_err:
                num_errors += 1
