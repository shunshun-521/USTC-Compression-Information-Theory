    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                frame_error = not _crc_check_bits(decoded_info, crc_length) or np.any(
                    decoded_info[:K_info] != payload[:K_info]
                )
                bit_errors = np.sum(decoded_info[:K_info] != info_bits)
            else:
                decoded_info = u_hat[info_indices]
                frame_error = np.any(decoded_info != info_bits)
...
def _crc_check_bits(bits, crc_length):
    from decoder_scl import crc_check

    return crc_check(bits, crc_length)


def _apply_fast_sim_defaults():
    """通过环境变量 POLAR_FAST_SIM=1 缩短仿真时间。"""
    if os.environ.get("POLAR_FAST_SIM", "0") == "1":
        return {
            "max_frames": 2000,
            "min_errors": 20,
            "eb_n0_range_scale": True,
        }
    return None
