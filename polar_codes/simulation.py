"""
蒙特卡洛仿真主循环
"""
import os
import time

import numpy as np

from channel import awgn_channel, bpsk_modulate, compute_llr, eb_n0_to_sigma
from decoder_scl import crc_encode
from encoder import polar_encode


def _sim_profile():
    """允许通过环境变量缩短仿真时间（自动化/调试）。"""
    if os.environ.get("POLAR_FAST_SIM", "0") == "1":
        return {
            "max_frames": int(os.environ.get("POLAR_MAX_FRAMES", "500")),
            "min_errors": int(os.environ.get("POLAR_MIN_ERRORS", "20")),
            "snr_stride": int(os.environ.get("POLAR_SNR_STRIDE", "2")),
        }
    return None


def run_simulation(
    N,
    K,
    eb_n0_db_list,
    decoder,
    decoder_type="sc",
    max_frames=100000,
    min_errors=100,
    crc_length=0,
    verbose=True,
    seed=42,
    info_indices=None,
):
    """
    蒙特卡洛仿真。
    """
    profile = _sim_profile()
    if profile is not None:
        max_frames = profile["max_frames"]
        min_errors = profile["min_errors"]
        stride = profile.get("snr_stride", 1)
        eb_n0_db_list = eb_n0_db_list[::stride]

    rng = np.random.default_rng(seed)
    rate = K / N
    results = []

    if info_indices is None:
        from construction import ga_construction

        info_indices, _, _ = ga_construction(N, K, 2.5)
    info_indices = np.asarray(info_indices, dtype=int)
    frozen_mask = np.ones(N, dtype=bool)
    frozen_mask[info_indices] = False

    k_info = K - crc_length
    data_info_idx = info_indices[:k_info]
    crc_info_idx = info_indices[k_info:]

    for eb_n0_db in eb_n0_db_list:
        sigma = eb_n0_to_sigma(eb_n0_db, rate)
        num_errors = 0
        num_bit_errors = 0
        num_frames = 0
        total_decode_time = 0.0
        total_iters = 0

        while num_frames < max_frames and num_errors < min_errors:
            u = np.zeros(N, dtype=np.int8)
            info_bits = rng.integers(0, 2, size=k_info, dtype=np.int8)
            u[data_info_idx] = info_bits

            if crc_length > 0:
                payload = crc_encode(info_bits, crc_length)
                u[crc_info_idx] = payload[-crc_length:]

            x = polar_encode(u)
            s = bpsk_modulate(x)
            y = awgn_channel(s, sigma, rng)
            llr = compute_llr(y, sigma)

            t0 = time.perf_counter()
            u_hat, aux = decoder(llr)
            total_decode_time += time.perf_counter() - t0
            if aux is not None and decoder_type == "bp":
                total_iters += aux

            frame_error = not np.array_equal(u_hat[data_info_idx], info_bits)
            if frame_error:
                num_errors += 1
            num_bit_errors += np.count_nonzero(u_hat[data_info_idx] != info_bits)
            num_frames += 1

        bler = num_errors / num_frames
        ber = num_bit_errors / (num_frames * k_info) if k_info > 0 else 0.0
        avg_time = total_decode_time / num_frames
        avg_iters = (total_iters / num_frames) if decoder_type == "bp" else None

        result = {
            "eb_n0_db": eb_n0_db,
            "bler": bler,
            "ber": ber,
            "num_errors": num_errors,
            "num_frames": num_frames,
            "avg_decode_time": avg_time,
            "avg_iters": avg_iters,
        }
        results.append(result)

        if verbose:
            print(
                f"  Eb/N0={eb_n0_db:.2f}dB | BLER={bler:.4e} | BER={ber:.4e} "
                f"| Errors={num_errors} | Frames={num_frames} "
                f"| AvgTime={avg_time * 1000:.2f}ms"
                + (f" | AvgIter={avg_iters:.1f}" if avg_iters is not None else "")
            )

    return results
