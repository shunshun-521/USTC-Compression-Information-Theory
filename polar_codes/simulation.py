    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                sent_info = rng.integers(0, 2, K)
                u[info_idx] = sent_info
            ...
            if crc_length > 0:
                decoded_info = decoded_info[:k_info]

            if not np.array_equal(decoded_info, sent_info):
                num_errors += 1
                num_bit_errors += np.sum(decoded_info != sent_info)
...
def get_sim_params():
    ...
    if fast:
        max_frames = min(max_frames, int(os.environ.get("POLAR_MAX_FRAMES", "800")))
        min_errors = min(min_errors, int(os.environ.get("POLAR_MIN_ERRORS", "15")))
