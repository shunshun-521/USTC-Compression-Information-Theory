...
def run_simulation(
    ...
    参数：
        N: 码长
        K: 信息位数（含 CRC 时 K 为总信息位+CRC 位）
        eb_n0_db_list: Eb/N0 列表（dB）
        decoder: 译码器函数，返回 (u_hat, aux)
        info_indices: 信息位索引
        ...
        verbose: 是否打印进度
        seed: 随机数种子
        design_eb_n0_db: GA 构造设计信噪比（仅用于日志）
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            if crc_length > 0:
                ...
                info_with_crc = rng.integers(0, 2, size=K)
            ...
            if crc_length > 0:
                ...
                bit_errors = np.sum(decoded_info[:K_info] != info_payload)
            else:
                decoded_info = u_hat[info_indices]
