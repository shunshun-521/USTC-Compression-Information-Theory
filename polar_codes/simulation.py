...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if not np.array_equal(u_hat[info_mask], u[info_mask]):
                num_errors += 1
                num_bit_errors += np.count_nonzero(u_hat[info_mask] != u[info_mask])
...
def fast_sim_params():
    """读取 POLAR_FAST_SIM 环境变量以缩短自动化运行时间。"""
    if os.environ.get("POLAR_FAST_SIM", "0") == "1":
        return {
            "max_frames": 500,
            "min_errors": 10,
            "eb_n0_range_sc": np.arange(2.0, 6.5, 0.5),
            "eb_n0_range_other": np.arange(2.0, 6.5, 0.5),
            "n_list_exp1": [256, 512],
            "n_list_exp3": [256, 512],
        }
    return {
        "max_frames": 100000,
        "min_errors": 100,
        "eb_n0_range_sc": np.arange(0.0, 5.5, 0.25),
        "eb_n0_range_other": np.arange(1.0, 5.5, 0.25),
