...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                frame_info = u_hat[info_indices[:K]]
                if not np.array_equal(frame_info, u[info_indices[:K]]):
                    num_errors += 1
                num_bit_errors += np.sum(frame_info != u[info_indices[:K]])
            else:
                sent_info = u[info_indices[:K_info]]
                recv_info = u_hat[info_indices[:K_info]]
                if not np.array_equal(recv_info, sent_info):
                    num_errors += 1
