...
def run_simulation(
    ...
    if info_indices is None:
        ...
        frozen_bits[np.asarray(info_indices)] = 0
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if info_indices is not None:
                ...
                u[~frozen_bits.astype(bool)] = payload
            ...
            if info_indices is not None:
                ...
                sent_info = u[~frozen_bits.astype(bool)]
                recv_info = u_hat[~frozen_bits.astype(bool)]

            if crc_length > 0:
                sent_cmp = sent_info[:K_info]
                recv_cmp = recv_info[:K_info]
            else:
                sent_cmp = sent_info
                recv_cmp = recv_info

            bit_err = np.sum(sent_cmp != recv_cmp)
            if bit_err > 0:
                num_errors += 1
        ...
        bler = num_errors / num_frames if num_frames > 0 else 1.0
        ber = (
            num_bit_errors / (num_frames * K_info)
            if num_frames > 0 and K_info > 0
            else 1.0
        )
