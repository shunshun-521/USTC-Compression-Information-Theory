...
def run_simulation(
    ...
    if frozen_bits is None:
        if info_indices is None:
            raise ValueError("Must provide info_indices or frozen_bits")
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            u_info_hat = u_hat[~frozen_bits][:K_info]

            frame_error = not np.array_equal(u_info_hat, info_bits)
            if frame_error:
                num_errors += 1
            num_bit_errors += np.sum(u_info_hat != info_bits)
            num_frames += 1
