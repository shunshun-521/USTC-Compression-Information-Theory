...
def run_simulation(
    ...
    if info_indices is None or frozen_bits is None:
        info_idx, _, _ = ga_construction(N, K, design_eb_n0_db)
    ...
    sorted_info = np.sort(info_indices)
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                u[sorted_info[:K_info]] = info_payload
                u[sorted_info[K_info : K_info + crc_length]] = crc_bits
            else:
                u[sorted_info] = info_payload
            ...
            if crc_length > 0:
                frame_ok = np.array_equal(
                    u_hat[sorted_info[:K_info]], info_payload
                )
            else:
                frame_ok = np.array_equal(u_hat[sorted_info], info_payload)
        ...
        ber = (
            num_bit_errors / (num_frames * K_info)
            if num_frames and K_info > 0
            else 0.0
        )
