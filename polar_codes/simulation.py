    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            if crc_length > 0:
                ...
                crc_part = crc_encode(info_payload, crc_length)[-crc_length:]
                info_bits = np.concatenate([info_payload, crc_part])
            else:
                info_payload = rng.integers(0, 2, size=k_info)
                info_bits = info_payload
            ...
            if crc_length > 0:
                ...
                bit_errors = np.count_nonzero(decoded_info != info_bits)
            else:
                frame_error = not np.array_equal(u_hat[info_indices], info_payload)
                bit_errors = np.count_nonzero(u_hat[info_indices] != info_payload)
