    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                frame_ok = crc_check_payload(info_bits, decoded_info, crc_length)
            else:
...
def crc_check_payload(info_bits, decoded_payload, crc_length):
    from decoder_scl import crc_check

    if len(decoded_payload) < crc_length:
        return False
    return crc_check(decoded_payload, crc_length) and np.array_equal(
        decoded_payload[: len(info_bits)], info_bits
    )
