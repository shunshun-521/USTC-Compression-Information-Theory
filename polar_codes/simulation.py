...
def run_simulation(
    ...
    """
    蒙特卡洛仿真。
    可通过环境变量 POLAR_MAX_FRAMES / POLAR_MIN_ERRORS 覆盖帧数设置。
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            if crc_length > 0:
                ...
                u_info = crc_encode(info_bits, crc_length)
            else:
                u_info = rng.integers(0, 2, size=K)
            ...
            if crc_length > 0:
                ...
                frame_error = not np.array_equal(u_hat[info_indices], u_info)
                bit_err = np.sum(u_hat[info_indices] != u_info)
