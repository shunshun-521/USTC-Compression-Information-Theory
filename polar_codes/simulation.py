...
def run_simulation(
    ...
    if info_indices is None:
        info_indices = np.where(np.arange(N) >= (N - K))[0]
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                compare_hat = decoded_info
            else:
                compare_sent = sent_info
                compare_hat = decoded_info

            frame_error = not np.array_equal(compare_hat, compare_sent)
            if frame_error:
                num_errors += 1
            num_bit_errors += np.count_nonzero(compare_hat != compare_sent)
            num_frames += 1
