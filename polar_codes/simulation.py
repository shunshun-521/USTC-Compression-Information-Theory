    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                frame_ok = crc_check(u_hat[info_idx[: K]], crc_length) and np.array_equal(
                    u_hat[info_idx[:k_info]], u[info_idx[:k_info]]
                )
            else:
                frame_ok = np.array_equal(u_hat[info_idx[:k_info]], u[info_idx[:k_info]])

            if not frame_ok:
                num_errors += 1
                num_bit_errors += int(
                    np.sum(u_hat[info_idx[:k_info]] != u[info_idx[:k_info]])
                )
            num_frames += 1
