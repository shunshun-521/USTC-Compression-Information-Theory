...
def run_simulation(
    ...
    参数：
        N: 码长
        K: 信息位数（极化码中信息+CRC 所占位置数）
        eb_n0_db_list: Eb/N0 列表（dB）
        decoder: 译码器对象或函数，返回 (u_hat, aux)
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                bit_errors = np.sum(u_hat[info_indices][:k_info] != info_bits)
            else:
                frame_error = not np.array_equal(u_hat[info_indices], payload)
                bit_errors = np.sum(u_hat[info_indices] != payload)
...
def get_sim_params():
    ...
    min_errors = int(os.environ.get("POLAR_MIN_ERRORS", "100" if not fast else "5"))
