...
def run_simulation(
    ...
    if info_indices is None and frozen_bits is not None:
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                check_bits = u_hat_info
                from decoder_scl import crc_check

                frame_error = not crc_check(check_bits, crc_length)
                bit_err = np.sum(info_bits != u_hat_info[:K_info])
            else:
                frame_error = not np.array_equal(info_bits, u_hat_info)
                bit_err = np.sum(info_bits != u_hat_info)
