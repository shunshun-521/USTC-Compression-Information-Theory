...
def run_simulation(
    ...
    参数 info_indices: 长度为 K 的信息位索引（GA 构造结果）
    """
    rng = np.random.default_rng(seed)
    rate = K / N
    info_indices = np.sort(np.asarray(info_indices, dtype=int))
    k_info = K - crc_length
    ber_indices = info_indices[:k_info]
    results = []
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if not np.array_equal(u_hat[info_indices], u[info_indices]):
                num_errors += 1
            num_bit_errors += np.sum(u_hat[ber_indices] != u[ber_indices])
            num_frames += 1
