...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if info_indices is None:
                decoded_info = u_hat[:K]
            else:
                decoded_info = u_hat[info_indices]

            if crc_length > 0:
                compare_bits = info_bits
                decoded_compare = decoded_info[:k_info]
            else:
                compare_bits = info_bits
                decoded_compare = decoded_info

            frame_error = not np.array_equal(decoded_compare, compare_bits)
            bit_errors = np.count_nonzero(decoded_compare != compare_bits)
