...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                err_frame = not np.array_equal(u_hat[info_indices][:k_info], info_raw)
                bit_err = np.sum(u_hat[info_indices][:k_info] != info_raw)
            else:
                err_frame = not np.array_equal(u_hat[info_indices], payload)
