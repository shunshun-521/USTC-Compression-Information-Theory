...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            frame_error = not np.array_equal(u_hat[info_indices][:k_info if crc_length else K], info_bits)
            if frame_error:
                num_errors += 1
            num_bit_errors += np.sum(u_hat[info_indices][:k_info if crc_length else K] != info_bits)
            num_frames += 1
