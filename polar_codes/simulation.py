"""
蒙特卡洛仿真主循环
"""
import os
import time

import numpy as np

from channel import awgn_channel, bpsk_modulate, compute_llr, eb_n0_to_sigma
from decoder_scl import crc_encode
from encoder import polar_encode


def _is_fast_sim():
    return os.environ.get("POLAR_FAST_SIM", "0") == "1"


def _fast_eb_range(eb_n0_db_list):
    if not _is_fast_sim():
        return eb_n0_db_list
    eb = np.asarray(eb_n0_db_list, dtype=float)
    if len(eb) > 8:
        idx = np.linspace(0, len(eb) - 1, 8, dtype=int)
        return eb[idx]
    return eb


def run_simulation(
    N,
    K,
    eb_n0_db_list,
    decoder,
    decoder_type="sc",
    max_frames=100000,
    min_errors=100,
    crc_length=0,
    verbose=True,
    seed=42,
    frozen_bits=None,
    info_indices=None,
):
    """
    蒙特卡洛仿真。
    """
    if _is_fast_sim():
        max_frames = min(max_frames, 200)
        min_errors = min(min_errors, 10)

    eb_n0_db_list = _fast_eb_range(eb_n0_db_list)

    rng = np.random.default_rng(seed)
    rate = K / N
    results = []

    if frozen_bits is None or info_indices is None:
        raise ValueError("frozen_bits and info_indices must be provided")

    frozen_bits = np.asarray(frozen_bits, dtype=int)
    info_indices = np.asarray(info_indices, dtype=int)
    K_info = K - crc_length

    for eb_n0_db in eb_n0_db_list:
        sigma = eb_n0_to_sigma(eb_n0_db, rate)
        num_errors = 0
        num_bit_errors = 0
        num_frames = 0
        total_decode_time = 0.0
        total_iters = 0.0

        while num_frames < max_frames and num_errors < min_errors:
            info_bits = rng.integers(0, 2, size=K_info)
            u = np.zeros(N, dtype=int)
            if crc_length > 0:
                payload = crc_encode(info_bits, crc_length)
                u[info_indices] = payload
            else:
                u[info_indices] = info_bits

            x = polar_encode(u)
            s = bpsk_modulate(x)
            y = awgn_channel(s, sigma, rng)
            llr = compute_llr(y, sigma)

            t0 = time.perf_counter()
            u_hat, aux = decoder(llr)
            total_decode_time += time.perf_counter() - t0

            if decoder_type == "bp" and aux is not None:
                total_iters += aux

            frame_error = False
            if crc_length > 0:
                decoded_info = u_hat[info_indices]
                if not np.array_equal(decoded_info[:K_info], info_bits):
                    frame_error = True
                    num_bit_errors += np.sum(decoded_info[:K_info] != info_bits)
            else:
                decoded_info = u_hat[info_indices]
                if not np.array_equal(decoded_info, info_bits):
                    frame_error = True
                    num_bit_errors += np.sum(decoded_info != info_bits)

            if frame_error:
                num_errors += 1
            num_frames += 1

        bler = num_errors / num_frames
        ber = num_bit_errors / (num_frames * K_info) if K_info > 0 else 0.0
        avg_time = total_decode_time / num_frames
        avg_iters = (total_iters / num_frames) if decoder_type == "bp" else None

        result = {
            "eb_n0_db": eb_n0_db,
            "bler": bler,
            "ber": ber,
            "num_errors": num_errors,
            "num_frames": num_frames,
            "avg_decode_time": avg_time,
            "avg_iters": avg_iters,
        }
        results.append(result)

        if verbose:
            msg = (
                f"  Eb/N0={eb_n0_db:.2f}dB | BLER={bler:.4e} | BER={ber:.4e} "
                f"| Errors={num_errors} | Frames={num_frames} "
                f"| AvgTime={avg_time * 1000:.2f}ms"
            )
            if avg_iters is not None:
                msg += f" | AvgIter={avg_iters:.1f}"
            print(msg)

    return results
