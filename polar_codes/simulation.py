...
def run_simulation(
    ...
    if info_indices is None:
        info_indices = np.where(np.ones(N, dtype=int))[0]
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                sent_payload = u[info_indices[: len(payload)]]
                recv_payload = u_hat[info_indices[: len(payload)]]
                frame_error = not np.array_equal(sent_payload, recv_payload)
                bit_errors = np.count_nonzero(sent_payload != recv_payload)
            else:
