...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            info_data = rng.integers(0, 2, size=K_info)
            u = np.zeros(N, dtype=int)
            info_positions = info_indices

            if crc_length > 0:
                payload = crc_encode(info_data, crc_length)
                u[info_positions] = payload
            else:
                u[info_positions] = info_data
            ...
            info_hat = u_hat[info_positions[:K_info]]
            frame_error = not np.array_equal(info_hat, info_data)
            if frame_error:
                num_errors += 1
            num_bit_errors += np.count_nonzero(info_hat != info_data)
