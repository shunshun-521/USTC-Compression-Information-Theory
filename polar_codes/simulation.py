...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                frame_error = not np.array_equal(info_bits, u_hat[info_indices][:k_info])
            else:
                if info_indices is not None:
                    frame_error = not np.array_equal(info_bits, u_hat[info_indices])
                else:
                    frame_error = not np.array_equal(u[:k_info], u_hat[:k_info])

            if frame_error:
                num_errors += 1
                if info_indices is not None:
                    num_bit_errors += np.sum(info_bits != u_hat[info_indices][:k_info])
                else:
                    num_bit_errors += np.sum(u[:k_info] != u_hat[:k_info])
