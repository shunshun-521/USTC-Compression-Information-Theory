    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                check_bits = np.concatenate(
                    [u_hat[payload_idx], u_hat[info_indices[k_info:]]]
                )
                frame_error = not np.array_equal(u_hat[payload_idx], payload)
            ...
            if frame_error:
                num_errors += 1
            num_bit_errors += np.sum(u_hat[info_indices[:k_info]] != payload)
            num_frames += 1
