...
def run_simulation(
  ...
  参数 info_indices: 信息位索引；若为 None 则假定前 K 个信道（需外部传入 GA 结果）。
  use_br_llr: SC/SCL 译码前是否对比特倒序 LLR 排列。
  """
  ...
  for eb_n0_db in eb_n0_db_list:
    ...
    while num_frames < max_frames and num_errors < min_errors:
      ...
      if use_br_llr and decoder_type in ("sc", "scl"):
        llr_in = channel_llr_to_decoder(llr)
      else:
        llr_in = llr
