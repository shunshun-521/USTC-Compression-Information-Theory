...
def run_simulation(
    ...
    返回每个 Eb/N0 点的 BLER/BER 等统计量。
    """
    if info_indices is None:
        raise ValueError("info_indices must be provided for polar code simulation")
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            if crc_length > 0:
                ...
                info_payload = rng.integers(0, 2, K)
            ...
            if crc_length > 0:
                compare_sent = sent_info[:k_info]
                compare_recv = recv_info[:k_info]
            else:
                compare_sent = sent_info
                compare_recv = recv_info

            frame_error = not np.array_equal(compare_sent, compare_recv)
            if frame_error:
                num_errors += 1
            num_bit_errors += np.count_nonzero(compare_sent != compare_recv)
            num_frames += 1
