...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                frame_error = not np.array_equal(u_hat_info[:k_info], info_bits)
            else:
                frame_error = not np.array_equal(u_hat_info, info_bits)

            if frame_error:
                num_errors += 1
                num_bit_errors += np.sum(u_hat_info[:k_info] != info_bits)
