...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            info_bits = rng.integers(0, 2, k_payload)
            if crc_length > 0:
            ...
            frame_err = not np.array_equal(decoded_payload[:k_payload], info_bits)
            if frame_err:
                num_errors += 1
            num_bit_errors += np.sum(decoded_payload[:k_payload] != info_bits)
            num_frames += 1

        bler = num_errors / num_frames if num_frames else 1.0
        ber = num_bit_errors / (num_frames * k_payload) if num_frames and k_payload else 1.0
