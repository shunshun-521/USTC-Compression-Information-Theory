...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            info_indices_local = np.asarray(info_indices)

            if crc_length > 0:
                payload_idx = info_indices_local[:K_info]
                crc_idx = info_indices_local[K_info:K]
                info_payload = rng.integers(0, 2, K_info)
                coded_info = crc_encode(info_payload, crc_length)
                u[payload_idx] = info_payload
                u[crc_idx] = coded_info[K_info:]
                compare_idx = payload_idx
            else:
                u[info_indices_local] = rng.integers(0, 2, len(info_indices_local))
                compare_idx = info_indices_local
            ...
            if not np.array_equal(u_hat[compare_idx], u[compare_idx]):
                num_errors += 1
        ...
        ber = num_bit_errors / (num_frames * K_info) if num_frames and K_info > 0 else 0.0
