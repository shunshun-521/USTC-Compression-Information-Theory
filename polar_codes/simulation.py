...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            decoded_info = u_hat[info_indices][:K_info if crc_length > 0 else K]
            sent_info = info_bits

            frame_error = not np.array_equal(decoded_info, sent_info)
            bit_errors = int(np.sum(decoded_info != sent_info))
        ...
        denom = num_frames * K_info if crc_length > 0 else num_frames * K
        ber = num_bit_errors / denom if denom > 0 else 0.0
