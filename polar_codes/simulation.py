"""
蒙特卡洛仿真主循环
"""
import os
import time
import numpy as np

from channel import bpsk_modulate, awgn_channel, compute_llr, eb_n0_to_sigma
from encoder import polar_encode


def _env_int(name, default):
    return int(os.environ.get(name, default))


def run_simulation(
    N, K, eb_n0_db_list, decoder,
    decoder_type='sc',
    max_frames=100000,
    min_errors=100,
    crc_length=0,
    verbose=True,
    seed=42,
    info_indices=None,
):
    """蒙特卡洛仿真"""
    if os.environ.get('POLAR_QUICK', '0') == '1':
        max_frames = _env_int('POLAR_MAX_FRAMES', 5000)
        min_errors = _env_int('POLAR_MIN_ERRORS', 20)

    if info_indices is None:
        info_indices = np.arange(K)

    info_indices = np.asarray(info_indices, dtype=int)
    k_info = K - crc_length

    rng = np.random.default_rng(seed)
    rate = K / N
    results = []

    for eb_n0_db in eb_n0_db_list:
        sigma = eb_n0_to_sigma(eb_n0_db, rate)
        num_errors = 0
        num_bit_errors = 0
        num_frames = 0
        total_decode_time = 0.0
        total_iters = 0.0

        while num_frames < max_frames and num_errors < min_errors:
            u_sent = np.zeros(N, dtype=int)
            if crc_length > 0:
                from decoder_scl import crc_encode
                msg = rng.integers(0, 2, k_info)
                coded = crc_encode(msg, crc_length)
                u_sent[info_indices] = coded
                ref_info = msg
            else:
                bits = rng.integers(0, 2, K)
                u_sent[info_indices] = bits
                ref_info = bits

            x = polar_encode(u_sent)
            s = bpsk_modulate(x)
            y = awgn_channel(s, sigma, rng)
            llr = compute_llr(y, sigma)

            t0 = time.perf_counter()
            u_hat, aux = decoder(llr)
            total_decode_time += time.perf_counter() - t0

            if decoder_type == 'bp' and aux is not None:
                total_iters += aux

            hat_info = u_hat[info_indices]
            if crc_length > 0:
                hat_cmp = hat_info[:k_info]
            else:
                hat_cmp = hat_info

            bit_err = int(np.sum(ref_info != hat_cmp))
            num_bit_errors += bit_err
            if bit_err > 0:
                num_errors += 1
            num_frames += 1

        bler = num_errors / num_frames
        ber = num_bit_errors / (num_frames * k_info)
        avg_time = total_decode_time / num_frames
        avg_iters = (total_iters / num_frames) if decoder_type == 'bp' else None

        result = {
            'eb_n0_db': eb_n0_db,
            'bler': bler,
            'ber': ber,
            'num_errors': num_errors,
            'num_frames': num_frames,
            'avg_decode_time': avg_time,
            'avg_iters': avg_iters,
        }
        results.append(result)

        if verbose:
            print(
                f"  Eb/N0={eb_n0_db:.2f}dB | BLER={bler:.4e} | BER={ber:.4e} "
                f"| Errors={num_errors} | Frames={num_frames} "
                f"| AvgTime={avg_time * 1000:.2f}ms"
                + (f" | AvgIter={avg_iters:.1f}" if avg_iters is not None else "")
            )

    return results
