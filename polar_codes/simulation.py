"""
蒙特卡洛仿真主循环
"""
import time
import numpy as np

from encoder import polar_encode
from channel import bpsk_modulate, awgn_channel, compute_llr, eb_n0_to_sigma


def run_simulation(
    N, K, eb_n0_db_list, decoder,
    decoder_type='sc',
    max_frames=100000,
    min_errors=100,
    crc_length=0,
    verbose=True,
    seed=42,
    info_indices=None,
):
    """
    蒙特卡洛仿真。

    返回：dict 列表，每个 Eb/N0 点包含 bler, ber, num_errors, num_frames 等。
    """
    rng = np.random.default_rng(seed)
    rate = K / N
    k_info = K - crc_length
    results = []

    for eb_n0_db in eb_n0_db_list:
        sigma = eb_n0_to_sigma(eb_n0_db, rate)
        num_errors = 0
        num_bit_errors = 0
        num_frames = 0
        total_decode_time = 0.0
        total_iters = 0

        while num_frames < max_frames and num_errors < min_errors:
            # 生成信息比特
            if crc_length > 0:
                info_bits = rng.integers(0, 2, k_info)
                from decoder_scl import crc_encode
                payload = crc_encode(info_bits, crc_length)
            else:
                payload = rng.integers(0, 2, K)

            # 构造 u 向量
            u = np.zeros(N, dtype=int)
            if info_indices is not None:
                u[info_indices] = payload
            else:
                u[:K] = payload

            # 编码、信道
            x = polar_encode(u)
            s = bpsk_modulate(x)
            y = awgn_channel(s, sigma, rng=rng)
            llr = compute_llr(y, sigma)

            # 译码、计时
            t0 = time.perf_counter()
            decode_out = decoder(llr)
            elapsed = time.perf_counter() - t0

            if isinstance(decode_out, tuple):
                u_hat, aux = decode_out
            else:
                u_hat = decode_out
                aux = None

            total_decode_time += elapsed
            if aux is not None and decoder_type == 'bp':
                total_iters += aux

            # 比较错误
            if info_indices is not None:
                sent_info = u[info_indices]
                if crc_length > 0:
                    sent_info = sent_info[:k_info + crc_length]
                    recv_info = u_hat[info_indices][:k_info + crc_length]
                    bit_err = np.sum(sent_info[:k_info] != recv_info[:k_info])
                else:
                    recv_info = u_hat[info_indices]
                    bit_err = np.sum(sent_info != recv_info)
            else:
                sent_info = u[:K]
                recv_info = u_hat[:K]
                bit_err = np.sum(sent_info[:k_info] != recv_info[:k_info])

            frame_err = not np.array_equal(
                sent_info[:k_info if crc_length > 0 else K],
                recv_info[:k_info if crc_length > 0 else K]
            )

            if frame_err:
                num_errors += 1
            num_bit_errors += bit_err
            num_frames += 1

        bler = num_errors / num_frames
        ber = num_bit_errors / (num_frames * k_info)
        avg_time = total_decode_time / num_frames
        avg_iters = (total_iters / num_frames) if decoder_type == 'bp' else None

        result = {
            'eb_n0_db': eb_n0_db,
            'bler': bler,
            'ber': ber,
            'num_errors': num_errors,
            'num_frames': num_frames,
            'avg_decode_time': avg_time,
            'avg_iters': avg_iters,
        }
        results.append(result)

        if verbose:
            msg = (
                f"  Eb/N0={eb_n0_db:.2f}dB | BLER={bler:.4e} | BER={ber:.4e} "
                f"| Errors={num_errors} | Frames={num_frames} "
                f"| AvgTime={avg_time * 1000:.2f}ms"
            )
            if avg_iters is not None:
                msg += f" | AvgIter={avg_iters:.1f}"
            print(msg)

    return results
