"""
蒙特卡洛仿真主循环
"""
import time
import numpy as np

from channel import (
    bpsk_modulate,
    awgn_channel,
    compute_llr,
    eb_n0_to_sigma,
    reverse_llr_channel,
)
from encoder import polar_encode


def run_simulation(
    N,
    K,
    eb_n0_db_list,
    decoder,
    decoder_type="sc",
    max_frames=100000,
    min_errors=100,
    crc_length=0,
    verbose=True,
    seed=42,
    info_indices=None,
    frozen_bits=None,
):
    """
    蒙特卡洛仿真。
    """
    rng = np.random.default_rng(seed)
    rate = K / N
    results = []
    K_info = K - crc_length

    if info_indices is None:
        if frozen_bits is None:
            raise ValueError("info_indices or frozen_bits required")
        info_indices = np.where(~np.asarray(frozen_bits, dtype=bool))[0]

    info_indices = np.asarray(info_indices, dtype=np.int64)

    for eb_n0_db in eb_n0_db_list:
        sigma = eb_n0_to_sigma(eb_n0_db, rate)
        num_errors = 0
        num_bit_errors = 0
        num_frames = 0
        total_decode_time = 0.0
        total_iters = 0

        while num_frames < max_frames and num_errors < min_errors:
            info_payload = rng.integers(0, 2, size=K_info, dtype=np.int32)
            if crc_length > 0:
                from decoder_scl import crc_encode

                info_with_crc = crc_encode(info_payload, crc_length)
            else:
                info_with_crc = info_payload

            u = np.zeros(N, dtype=np.int32)
            u[info_indices] = info_with_crc

            x = polar_encode(u)
            s = bpsk_modulate(x)
            y = awgn_channel(s, sigma, rng)
            llr_raw = compute_llr(y, sigma)
            llr_ch = reverse_llr_channel(llr_raw)

            t0 = time.perf_counter()
            u_hat, aux = decoder(llr_ch)
            total_decode_time += time.perf_counter() - t0

            if aux is not None and decoder_type == "bp":
                total_iters += int(aux)

            decoded_info = u_hat[info_indices]
            if crc_length > 0:
                from decoder_scl import crc_check

                frame_ok = crc_check(decoded_info, crc_length) and np.array_equal(
                    decoded_info[:K_info], info_payload
                )
            else:
                frame_ok = np.array_equal(decoded_info, info_payload)

            if not frame_ok:
                num_errors += 1
                num_bit_errors += np.count_nonzero(decoded_info[:K_info] != info_payload)

            num_frames += 1

        bler = num_errors / num_frames
        ber = num_bit_errors / (num_frames * K_info) if K_info > 0 else 0.0
        avg_time = total_decode_time / num_frames
        avg_iters = (total_iters / num_frames) if decoder_type == "bp" else None

        result = {
            "eb_n0_db": float(eb_n0_db),
            "bler": float(bler),
            "ber": float(ber),
            "num_errors": int(num_errors),
            "num_frames": int(num_frames),
            "avg_decode_time": float(avg_time),
            "avg_iters": float(avg_iters) if avg_iters is not None else None,
        }
        results.append(result)

        if verbose:
            msg = (
                f"  Eb/N0={eb_n0_db:.2f}dB | BLER={bler:.4e} | BER={ber:.4e} "
                f"| Errors={num_errors} | Frames={num_frames} "
                f"| AvgTime={avg_time * 1000:.2f}ms"
            )
            if avg_iters is not None:
                msg += f" | AvgIter={avg_iters:.1f}"
            print(msg)

    return results
