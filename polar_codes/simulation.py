    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                coded = crc_encode(info_bits, crc_length)
            ...
            if crc_length > 0:
                info_pos = info_indices[:K_info]
                frame_error = not np.array_equal(u_hat[info_pos], u[info_pos])
                bit_errors = int(np.sum(u_hat[info_pos] != u[info_pos]))
            else:
                frame_error = not np.array_equal(u_hat[info_indices], u[info_indices])
                bit_errors = int(np.sum(u_hat[info_indices] != u[info_indices]))
