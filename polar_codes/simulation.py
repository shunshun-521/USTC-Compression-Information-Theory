    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                k_payload = k_info
                payload = rng.integers(0, 2, k_payload)
                coded = crc_encode(payload, crc_length)
                u[info_indices] = coded
            else:
                payload = u[info_indices]
            ...
            if crc_length > 0:
                ...
                err_frame = not np.array_equal(payload_hat[:k_info], payload)
                bit_err = np.sum(payload_hat[:k_info] != payload)
            else:
                err_frame = not np.array_equal(u_hat[info_indices], u[info_indices])
