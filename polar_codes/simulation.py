...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                frame_err = not np.array_equal(u_hat[info_indices][:k_info], info_payload)
                bit_err = np.sum(u_hat[info_indices][:k_info] != info_payload)
            else:
                frame_err = not np.array_equal(u_hat[info_indices], info_payload)
