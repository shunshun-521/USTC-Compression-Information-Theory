...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            info_sorted = np.sort(info_idx)
            if crc_length > 0:
                from decoder_scl import crc_encode

                payload = crc_encode(info_bits, crc_length)
                u[info_sorted[: len(payload)]] = payload
            else:
                u[info_sorted[:k_info]] = info_bits
            ...
            frame_error = not np.array_equal(u_hat[info_sorted[:k_info]], info_bits)
            if frame_error:
                num_errors += 1
                num_bit_errors += np.sum(u_hat[info_sorted[:k_info]] != info_bits)
