...
def run_simulation(
    ...
    """
    蒙特卡洛仿真。
    可传入预计算的 info_indices / frozen_bits 以避免重复构造。
    """
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                check_bits = u_hat[info_idx[: K_info + crc_length]]
                frame_ok = crc_check_bits(check_bits, crc_length)
                compare_idx = info_idx[:K_info]
                u_ref = info_raw
            else:
                frame_ok = np.array_equal(u_hat[info_idx], u[info_idx])
                compare_idx = info_idx
                u_ref = u[compare_idx]

            bit_err = np.sum(u_hat[compare_idx] != u_ref)
