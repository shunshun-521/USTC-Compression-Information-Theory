...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                frame_err = not np.array_equal(u[info_payload_idx], u_hat[info_payload_idx])
                bit_err = int(np.sum(u[info_payload_idx] != u_hat[info_payload_idx]))
            else:
                frame_err = not np.array_equal(u[info_indices], u_hat[info_indices])
                bit_err = int(np.sum(u[info_indices] != u_hat[info_indices]))
