...
def run_simulation(
    ...
    """
    蒙特卡洛仿真。
    info_indices: 信息位索引，用于 BER 统计（仅比较这些位置）
    """
    max_frames = _env_int("POLAR_MAX_FRAMES", max_frames)
    min_errors = _env_int("POLAR_MIN_ERRORS", min_errors)
    ...
    if info_indices is None:
        raise ValueError("info_indices required for error counting")
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                ...
                frame_err = not np.array_equal(payload_hat, info_payload)
