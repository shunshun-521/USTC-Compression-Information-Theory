...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                compare_bits = u_hat[info_indices][:K_info]
            else:
                compare_bits = u_hat[info_indices]

            if not np.array_equal(compare_bits, info_bits):
                num_errors += 1
                num_bit_errors += np.sum(compare_bits != info_bits)
