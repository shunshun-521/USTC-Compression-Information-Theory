    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            if crc_length > 0:
                ...
                u_sent_info = rng.integers(0, 2, size=K_info)
                u = np.zeros(N, dtype=int)
                u[info_indices] = u_sent_info
            ...
            if crc_length > 0:
                ...
                frame_error = not np.array_equal(decoded_info, u_sent_info)
                bit_errors = np.sum(decoded_info != u_sent_info)
            else:
        ...
        ber = num_bit_errors / (num_frames * K_info) if num_frames > 0 else 1.0
