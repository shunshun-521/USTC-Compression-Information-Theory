"""
蒙特卡洛仿真主循环
"""
import os
import time

import numpy as np

from channel import awgn_channel, bpsk_modulate, compute_llr, eb_n0_to_sigma, prepare_decoder_llr
from decoder_scl import crc_encode
from encoder import polar_encode


def _fast_sim_defaults(max_frames, min_errors):
    if os.environ.get('POLAR_FAST_SIM', '').strip() in {'1', 'true', 'yes'}:
        return min(max_frames, 8000), min(min_errors, 40)
    return max_frames, min_errors


def run_simulation(
    N, K, eb_n0_db_list, decoder,
    decoder_type='sc',
    max_frames=100000,
    min_errors=100,
    crc_length=0,
    info_indices=None,
    verbose=True,
    seed=42,
):
    """
    蒙特卡洛仿真。
    """
    max_frames, min_errors = _fast_sim_defaults(max_frames, min_errors)
    rng = np.random.default_rng(seed)
    rate = K / N
    results = []
    k_info = K - crc_length

    if info_indices is None:
        raise ValueError('info_indices must be provided')

    for eb_n0_db in eb_n0_db_list:
        sigma = eb_n0_to_sigma(eb_n0_db, rate)
        num_errors = 0
        num_bit_errors = 0
        num_frames = 0
        total_decode_time = 0.0
        total_iters = 0.0

        while num_frames < max_frames and num_errors < min_errors:
            info_bits = rng.integers(0, 2, size=k_info)
            if crc_length > 0:
                payload = crc_encode(info_bits, crc_length)
            else:
                payload = info_bits

            u = np.zeros(N, dtype=int)
            u[info_indices] = payload

            x = polar_encode(u)
            s = bpsk_modulate(x)
            y = awgn_channel(s, sigma, rng)
            llr_raw = compute_llr(y, sigma)

            llr_in = prepare_decoder_llr(llr_raw)

            t0 = time.perf_counter()
            u_hat, aux = decoder(llr_in)
            total_decode_time += time.perf_counter() - t0

            if aux is not None:
                total_iters += aux

            sent_info = u[info_indices[:k_info]]
            recv_info = u_hat[info_indices[:k_info]]
            frame_error = not np.array_equal(sent_info, recv_info)
            num_bit_errors += np.count_nonzero(sent_info != recv_info)
            num_errors += int(frame_error)
            num_frames += 1

        bler = num_errors / num_frames
        ber = num_bit_errors / (num_frames * k_info) if k_info > 0 else 0.0
        avg_time = total_decode_time / num_frames
        avg_iters = (total_iters / num_frames) if decoder_type == 'bp' else None

        result = {
            'eb_n0_db': eb_n0_db,
            'bler': bler,
            'ber': ber,
            'num_errors': num_errors,
            'num_frames': num_frames,
            'avg_decode_time': avg_time,
            'avg_iters': avg_iters,
        }
        results.append(result)

        if verbose:
            print(
                f"  Eb/N0={eb_n0_db:.2f}dB | BLER={bler:.4e} | BER={ber:.4e} "
                f"| Errors={num_errors} | Frames={num_frames} "
                f"| AvgTime={avg_time * 1000:.2f}ms"
                + (f" | AvgIter={avg_iters:.1f}" if avg_iters is not None else '')
            )

    return results
