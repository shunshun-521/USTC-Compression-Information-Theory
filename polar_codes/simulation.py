...
def run_simulation(
    ...
    if info_indices is None:
        raise ValueError("info_indices is required for BER counting")
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                compare_bits = info_payload
                decoded_bits = u_hat[info_indices][:K_info]
            else:
                compare_bits = info_payload
                decoded_bits = u_hat[info_indices]

            bit_err = int(np.sum(decoded_bits != compare_bits))
