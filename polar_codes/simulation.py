...
def run_simulation(
    ...
    返回每个 Eb/N0 点的 BLER、BER、帧数、译码时间等统计量。
    """
    ...
    info_idx = np.where(~frozen_mask)[0]
    K_info = K - crc_length
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            if crc_length > 0:
                if not np.array_equal(u_hat[info_idx][:K_info], u[info_idx][:K_info]):
                    num_errors += 1
                    num_bit_errors += np.sum(
                        u_hat[info_idx][:K_info] != u[info_idx][:K_info]
                    )
            else:
                if not np.array_equal(u_hat[info_idx], u[info_idx]):
                    num_errors += 1
                    num_bit_errors += np.sum(u_hat[info_idx] != u[info_idx])
