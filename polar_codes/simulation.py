...
def run_simulation(
    ...
    for eb_n0_db in eb_n0_db_list:
        ...
        while num_frames < max_frames and num_errors < min_errors:
            ...
            sent_info = payload if crc_length == 0 else payload[:K_info]
            compare_len = K_info if crc_length > 0 else K
            if not np.array_equal(recv_info[:compare_len], sent_info[:compare_len]):
                num_errors += 1
                num_bit_errors += int(np.sum(recv_info[:compare_len] != sent_info[:compare_len]))
