...
if not os.path.exists(f"results/exp2_cascl_L8_N{N}_R0.5.csv"):
    print("CA-SCL 仿真...")
    ...
    all_results[f"CA-SCL (L=8, CRC={CRC_LENGTH})"] = load_results_csv(
        f"results/exp2_cascl_L8_N{N}_R0.5.csv"
    )
...
print("实验二图表已生成。启动实验三...")
import subprocess

subprocess.run([sys.executable, "run_exp3.py"], check=True)
