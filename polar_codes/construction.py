...
def phi(x):
    ...
    result[mask_large] = (
        ...
        * np.exp(-x[mask_large] / 4)
        * (1 - 10.0 / (7.0 * x[mask_large]))
    )
    return result
...
if __name__ == "__main__":
    ...
    print(f"  info_indices: {info_idx}")
    print(f"  frozen_indices: {frozen_idx}")

    info_idx256, _, _ = ga_construction(256, 128, 2.5)
    print(f"N=256, K=128, first 20 info_indices: {info_idx256[:20]}")
