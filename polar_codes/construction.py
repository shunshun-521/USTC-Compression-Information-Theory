"""
极化码构造：高斯近似（GA）方法
适用于 BPSK-AWGN 信道
"""
import numpy as np


def phi(x):
    """
    GA 中的 phi 函数近似（x > 0）
    phi(x) = e^{-0.4527 * x^0.86 + 0.0218},  0 < x < 10
    phi(x) = sqrt(pi/x) * e^{-x/4} * (1 - 10/(7x)), x >= 10
    """
    x = np.asarray(x, dtype=np.float64)
    scalar = x.ndim == 0
    x = np.atleast_1d(x)
    out = np.empty_like(x)
    mask1 = x < 10.0
    mask2 = ~mask1
    out[mask1] = np.exp(-0.4527 * np.power(x[mask1], 0.86) + 0.0218)
    x2 = x[mask2]
    out[mask2] = np.sqrt(np.pi / x2) * np.exp(-x2 / 4.0) * (1.0 - 10.0 / (7.0 * x2))
    if scalar:
        return float(out[0])
    return out


def phi_inv(y):
    """phi 函数的数值逆（二分法，区间 [0, 100]）"""
    y_arr = np.atleast_1d(np.asarray(y, dtype=np.float64))
    lo = np.zeros_like(y_arr)
    hi = np.full_like(y_arr, 100.0)
    for _ in range(60):
        mid = (lo + hi) / 2.0
        pm = phi(mid)
        lo = np.where(pm < y_arr, mid, lo)
        hi = np.where(pm >= y_arr, mid, hi)
    result = (lo + hi) / 2.0
    if result.size == 1:
        return float(result[0])
    return result


def ga_construction(N, K, design_eb_n0_db, rate=None):
    """
    高斯近似构造极化码。

    参数：
        N: 码长（必须是 2 的幂）
        K: 信息位数
        design_eb_n0_db: 设计信噪比 Eb/N0（dB）
        rate: 码率 R=K/N，若为 None 则自动计算

    返回：
        info_indices: 长度为 K 的数组，信息位在 u 向量中的索引（从 0 开始）
        frozen_indices: 长度为 N-K 的数组，冻结位索引
        llr_means: 长度为 N 的数组，每个极化信道的等效 LLR 均值
    """
    if rate is None:
        rate = K / N
    if N & (N - 1):
        raise ValueError("N must be a power of 2")
    if K <= 0 or K > N:
        raise ValueError("K must satisfy 0 < K <= N")

    sigma = 1.0 / np.sqrt(2.0 * rate) * 10.0 ** (-design_eb_n0_db / 20.0)
    m0 = 2.0 / sigma ** 2
    n = int(np.log2(N))

    m = np.array([m0], dtype=np.float64)
    for _ in range(n):
        m_new = np.empty(2 * len(m), dtype=np.float64)
        for i in range(len(m)):
            m_new[2 * i] = phi_inv(1.0 - (1.0 - phi(m[i])) ** 2)
            m_new[2 * i + 1] = 2.0 * m[i]
        m = m_new

    llr_means = m
    info_indices = np.sort(np.argsort(llr_means)[-K:])
    frozen_indices = np.setdiff1d(np.arange(N), info_indices)
    return info_indices, frozen_indices, llr_means


if __name__ == "__main__":
    info, frozen, _ = ga_construction(8, 4, 2.5)
    print("N=8, K=4, Eb/N0=2.5dB")
    print("info_indices:", info)
    print("frozen_indices:", frozen)

    info256, _, _ = ga_construction(256, 128, 2.5)
    print("\nN=256, K=128, first 20 info_indices:", info256[:20])
