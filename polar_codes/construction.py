...
def phi(x):
    ...
    xl = x[~mask_small]
    result[~mask_small] = np.sqrt(np.pi / xl) * np.exp(-xl / 4) * (1 - 10.0 / (7.0 * xl))
    return result
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    eb_n0_linear = 10 ** (design_eb_n0_db / 10.0)
    ...
    for _ in range(n):
        phi_m = phi(means)
        left = np.atleast_1d(phi_inv(1.0 - (1.0 - phi_m) ** 2))
        right = np.atleast_1d(2.0 * means)
        means = np.concatenate([left, right])
