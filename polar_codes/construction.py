...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    info_indices = np.sort(sorted_indices[N - K :])

    # 信道 0 可靠性最低，始终冻结
    if 0 not in frozen_indices:
        if 0 in info_indices:
            replace_idx = info_indices[0]
            info_indices = info_indices[1:]
            info_indices = np.sort(np.append(info_indices, replace_idx))
            frozen_indices = np.sort(np.append(frozen_indices, 0))
            frozen_indices = frozen_indices[frozen_indices != replace_idx]
