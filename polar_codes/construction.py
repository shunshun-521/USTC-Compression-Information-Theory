...
def phi_inv(y):
    ...
    phi_lo = phi(np.full_like(y, 1e-10))
    phi_hi = phi(hi)
    y_clip = np.clip(y, phi_hi + 1e-15, phi_lo - 1e-12)
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    for _ in range(n):
        old_len = len(means)
        new_means = np.empty(2 * old_len, dtype=np.float64)
        phi_m = phi(means)
        new_means[0::2] = phi_inv(1.0 - (1.0 - phi_m) ** 2)
