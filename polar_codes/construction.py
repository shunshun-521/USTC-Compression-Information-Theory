...
def phi(x):
    ...
    small = (x > 0) & (x < 10)
    large = x >= 10
    zero_or_neg = ~(small | large)

    result[small] = np.exp(-0.4527 * np.power(x[small], 0.86) + 0.0218)
    xl = x[large]
    result[large] = np.sqrt(np.pi / xl) * np.exp(-xl / 4.0) * (1.0 - 10.0 / (7.0 * xl))
    result[zero_or_neg] = 1.0
    return result
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    sigma = 1.0 / np.sqrt(2.0 * R) * 10.0 ** (-design_eb_n0_db / 20.0)
    m0 = 2.0 / (sigma ** 2)
    ...
    info_indices = np.argsort(reliabilities)[-K:]
