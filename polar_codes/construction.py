...
def phi_inv(y):
    ...
    if scalar:
        y = np.array([y])
    y = np.clip(y, 1e-300, 1.0 - 1e-15)
    result = np.empty_like(y)
    small = y < 1e-4
    if np.any(small):
        result[small] = (-np.log(y[small]) / 0.4527) ** (1.0 / 0.86)
    large = ~small
    if np.any(large):
        yl = y[large]
        lo = np.zeros_like(yl)
        hi = np.full_like(yl, 500.0)
        for _ in range(64):
            mid = (lo + hi) * 0.5
            pm = phi(mid)
            lo = np.where(pm > yl, lo, mid)
            hi = np.where(pm > yl, mid, hi)
        result[large] = (lo + hi) * 0.5
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    # 在 phi 域递推以避免大均值时的数值饱和
    v = np.array([phi(m0)], dtype=np.float64)
    for _ in range(n):
        v_new = np.empty(2 * len(v), dtype=np.float64)
        v_new[0::2] = 1.0 - (1.0 - v) ** 2
        v_new[1::2] = v ** 2
        v = v_new

    llr_means = phi_inv(v)
    info_indices = np.argsort(v)[:K]
