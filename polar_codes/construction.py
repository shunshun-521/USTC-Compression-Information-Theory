...
def _bhattacharyya_reliability(N, design_eb_n0_db, rate):
    """生成 Bhattacharyya 可靠性排序（用于 GA 平局打破）。"""
    sigma = 1.0 / np.sqrt(2.0 * rate) * 10 ** (-design_eb_n0_db / 20.0)
    m0 = 2.0 / (sigma ** 2)
    z = np.array([np.exp(-0.25 * m0)], dtype=np.float64)
    n = int(np.log2(N))
    for _ in range(n):
        z_new = np.empty(2 * len(z), dtype=np.float64)
        z_new[0::2] = np.clip(2.0 * z - z ** 2, 0.0, 1.0)
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    tiebreak = _bhattacharyya_reliability(N, design_eb_n0_db, rate)
    score = llr_means + tiebreak * 1e-9
    info_indices = np.argsort(score)[-K:]
