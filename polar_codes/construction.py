...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    # 以 Bhattacharyya 参数做 GA 递推，再取 -log(Z) 作为等效 LLR 均值，
    # 避免直接在 LLR 域递推时的浮点下溢与 tie-breaking 误差。
    z = np.array([np.exp(-2.0 / (sigma ** 2))], dtype=np.float64)
    for _ in range(n):
        z_new = np.empty(2 * len(z), dtype=np.float64)
        z_new[0::2] = 1.0 - (1.0 - z) ** 2
        z_new[1::2] = z ** 2
        z = z_new

    llr_means = -np.log(np.clip(z, 1e-300, None))
