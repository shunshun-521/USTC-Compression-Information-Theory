...
def phi_inv(y):
    ...
    for _ in range(60):
        mid = (lo_w + hi_w) / 2.0
        pm = phi(mid)
        # phi decreasing: if phi(mid) > y, x is too large -> hi = mid
        too_large = pm > y_w
        hi_w = np.where(too_large, mid, hi_w)
        lo_w = np.where(too_large, lo_w, mid)
    result[work] = (lo_w + hi_w) / 2.0
...
if __name__ == "__main__":
    ...
    info256, _, llr256 = ga_construction(256, 128, 2.5)
