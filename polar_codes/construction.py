...
def phi_inv(y):
    ...
    if y.ndim == 0:
        return _scalar_phi_inv(float(y))
    return np.array([_scalar_phi_inv(float(v)) for v in y.ravel()]).reshape(y.shape)


def _scalar_phi_inv(y):
    if y <= 0.0:
        return 100.0
    phi_at_100 = _phi_for_inv(100.0)
    if y <= phi_at_100:
        return 100.0
    try:
        return brentq(lambda x: _phi_for_inv(x) - y, 1e-12, 100.0)
    except ValueError:
        return 100.0
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    for _ in range(n):
        ...
        pm = phi(means)
        new_means[0::2] = phi_inv(1.0 - (1.0 - pm) ** 2)
