...
def phi(x):
    ...
    y = np.empty_like(x)
    mask = x < 10.0
    y[mask] = np.exp(-0.4527 * np.power(x[mask], 0.86) + 0.0218)
    xm = x[~mask]
    y[~mask] = np.sqrt(np.pi / xm) * np.exp(-xm / 4.0) * (1.0 - 10.0 / (7.0 * xm))
    return y
    ...
    for idx, val in enumerate(flat):
        ...
        for _ in range(64):
            ...
            if phi(mid) > val:
                ...
                hi = mid
        result[idx] = (lo + hi) * 0.5
    return result.reshape(y.shape)


def ga_construction(N, K, design_eb_n0_db, rate=None):
    """
    高斯近似构造极化码（因子图块结构，与 SC 译码器索引一致）。
    ...
    llr_means = z[:, n]
    order = np.argsort(llr_means, kind="mergesort")
