...
def phi_inv(y):
    """
    phi 函数的数值逆（二分法，在 phi 单调递减区间 [0.01, 100] 上求解）
    """
    ...
    y = np.clip(y, 1e-12, 0.9999)
    lo = np.full_like(y, 0.01)
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    返回：
        ...
        llr_means: 长度为 N 的数组，每个极化信道的等效 LLR 均值（自然顺序）
    """
    if rate is None:
        rate = K / N
...
if __name__ == "__main__":
    ...
    print("\nN=256, K=128, first 20 info_indices:", info256[:20].tolist())
