...
def bhattacharyya_construction(N, K, design_eb_n0_db, rate=None):
    """
    Bhattacharyya 参数递推构造（数值稳定，BPSK-AWGN 设计）。
    """
    if N & (N - 1):
    ...
    z = np.array([np.exp(-snr_linear)], dtype=np.float64)
    for _ in range(n):
        z_new = np.empty(2 * len(z), dtype=np.float64)
        z_new[0::2] = 2.0 * z - z**2
        z_new[1::2] = z**2
        z = np.clip(z_new, 1e-12, 1.0 - 1e-12)
    llr_means = -np.log(z)
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    for _ in range(n):
        ...
        pm = np.clip(phi(m), 1e-12, 1.0 - 1e-12)
        m_new[0::2] = np.maximum(phi_inv(1.0 - (1.0 - pm) ** 2), 1e-12)
        m_new[1::2] = 2.0 * m
        m = m_new
    llr_means = m
    if np.max(m) < 1e-6 or (np.max(m) / (np.min(m[m > 0]) + 1e-12) > 1e8):
        return bhattacharyya_construction(N, K, design_eb_n0_db, rate)
    info_indices = np.argsort(-llr_means)[:K]
    info_indices = np.sort(info_indices)
    frozen_mask = np.ones(N, dtype=bool)
    frozen_mask[info_indices] = False
