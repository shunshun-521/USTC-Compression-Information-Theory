"""
极化码构造：高斯近似（GA）方法
适用于 BPSK-AWGN 信道
"""
import numpy as np


def phi(x):
    """
    GA 中的 phi 函数近似（x > 0）
    phi(x) = e^{-0.4527 * x^0.86 + 0.0218},  0 < x < 10
    phi(x) = sqrt(pi/x) * e^{-x/4} * (1 - 10/(7x)), x >= 10
    """
    x = np.asarray(x, dtype=float)
    out = np.empty_like(x)
    mask = x < 10
    out[mask] = np.exp(-0.4527 * x[mask] ** 0.86 + 0.0218)
    xm = x[~mask]
    out[~mask] = np.sqrt(np.pi / xm) * np.exp(-xm / 4) * (1 - 10 / (7 * xm))
    return out


def phi_inv(y):
    """
    phi 函数的数值逆（二分法，区间 [0, 100]）
    """
    y = np.atleast_1d(y).astype(float)
    lo = np.zeros_like(y)
    hi = np.full_like(y, 100.0)
    for _ in range(60):
        mid = (lo + hi) / 2
        pm = phi(mid)
        lo = np.where(pm < y, mid, lo)
        hi = np.where(pm >= y, mid, hi)
    return (lo + hi) / 2


def ga_construction(N, K, design_eb_n0_db, rate=None):
    """
    高斯近似构造极化码。

    参数：
        N: 码长（必须是 2 的幂）
        K: 信息位数
        design_eb_n0_db: 设计信噪比 Eb/N0（dB）
        rate: 码率 R=K/N，若为 None 则自动计算

    返回：
        info_indices: 长度为 K 的数组，信息位在 u 向量中的索引（从 0 开始）
        frozen_indices: 长度为 N-K 的数组，冻结位索引
        llr_means: 长度为 N 的数组，每个极化信道的等效 LLR 均值
    """
    if N & (N - 1):
        raise ValueError("N must be a power of 2")
    if rate is None:
        rate = K / N

    sigma = 1 / np.sqrt(2 * rate) * 10 ** (-design_eb_n0_db / 20)
    m = np.array([2 / sigma ** 2], dtype=float)
    n = int(np.log2(N))

    for _ in range(n):
        pm = phi(m)
        m_new = np.empty(2 * len(m))
        m_new[0::2] = phi_inv(1 - (1 - pm) ** 2)
        m_new[1::2] = 2 * m
        m = m_new

    info_indices = np.sort(np.argsort(m)[-K:])
    frozen_indices = np.sort(np.setdiff1d(np.arange(N), info_indices))
    return info_indices, frozen_indices, m


if __name__ == "__main__":
    info8, frozen8, _ = ga_construction(8, 4, 2.5)
    print("N=8, K=4, Eb/N0=2.5dB")
    print("info_indices:", info8)
    print("frozen_indices:", frozen8)

    info256, _, _ = ga_construction(256, 128, 2.5)
    print("\nN=256, K=128, info_indices (first 20):", info256[:20])
