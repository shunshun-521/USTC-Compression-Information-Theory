...
def phi_inv(y):
    ...
    for _ in range(60):
        mid = 0.5 * (lo + hi)
        pmid = phi(mid)
        # phi 递减：pmid > y 说明 mid 太小，需要增大下界
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    for _ in range(n):
        ...
        ph = phi(means)
        m_new[0::2] = phi_inv(1.0 - (1.0 - ph) ** 2)  # W^{-} 较差信道
        m_new[1::2] = 2.0 * means  # W^{+} 较好信道
        means = m_new

    llr_means = means
    # 选取可靠性最高的 K 个信道，再映射到编码器/译码器使用的索引顺序
    raw_info = np.argsort(llr_means)[-K:]
    br = bit_reversal_permutation(N)
    info_indices = np.sort(br[raw_info])
