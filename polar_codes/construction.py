...
def phi(x):
    ...
    xl = x[mask_large]
    result[mask_large] = np.sqrt(np.pi / xl) * np.exp(-xl / 4) * (1.0 - 10.0 / (7.0 * xl))
    return result


def _phi_inv_scalar(y, tol=1e-12):
...
def phi_inv(y):
    """phi 函数的数值逆，支持标量或数组"""
    return np.vectorize(_phi_inv_scalar)(np.asarray(y, dtype=np.float64))
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    for _ in range(n):
        m_new = np.zeros(2 * len(means), dtype=np.float64)
        for i in range(len(means)):
            # 规范中 2i-1 / 2i 为 1-based 索引
            m_new[2 * i] = phi_inv(1.0 - (1.0 - phi(means[i])) ** 2)
            m_new[2 * i + 1] = 2.0 * means[i]
        means = m_new

    info_indices = np.sort(np.argsort(means)[-K:])
