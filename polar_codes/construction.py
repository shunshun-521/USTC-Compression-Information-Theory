...
def phi_inv(y, tol=1e-10, max_iter=60):
    ...
    y_clipped = np.clip(y, 1e-15, 1.0 - 1e-15)

    for _ in range(max_iter):
        mid = (lo + hi) / 2.0
        phi_mid = phi(mid)
        lo = np.where(phi_mid > y_clipped, mid, lo)
        hi = np.where(phi_mid > y_clipped, hi, mid)
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    frozen_indices = all_indices[~np.isin(all_indices, info_indices)]
