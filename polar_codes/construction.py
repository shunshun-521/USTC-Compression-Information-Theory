...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    for _ in range(n):
        ...
        for i, m_val in enumerate(llr_means):
    ...
    frozen_indices = sorted_indices[: N - K].astype(int)
    if 0 not in frozen_indices:
        frozen_indices = np.sort(np.concatenate([[0], frozen_indices[:-1]]))

    frozen_mask = np.zeros(N, dtype=bool)
    frozen_mask[frozen_indices] = True
    info_indices = np.sort(np.where(~frozen_mask)[0].astype(int))
