...
def bhattacharyya_construction(N, K, design_eb_n0_db, rate=None):
    ...
    sigma = 1.0 / np.sqrt(2 * rate) * 10 ** (-design_eb_n0_db / 20.0)
    p = np.exp(-1.0 / (2 * sigma ** 2))  # BSC 等效翻转概率
    z = np.array([p])
    ...
    frozen_mask[info_indices] = False
    frozen_indices = all_idx[frozen_mask]
    llr_means = -np.log(z + 1e-300)
    return info_indices, frozen_indices, llr_means


def ga_construction_bh(N, K, design_eb_n0_db, rate=None):
    """默认使用 Bhattacharyya 构造（仿真主用）"""
