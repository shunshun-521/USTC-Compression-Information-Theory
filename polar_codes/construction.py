...
def phi(x):
    ...
    mask_high = x >= 10
  # x <= 0 stays 0
    result[mask_low] = np.exp(-0.4527 * x[mask_low] ** 0.86 + 0.0218)
    xh = x[mask_high]
...
def phi_inv(y):
    ...
    flat_result = np.zeros(y_arr.size, dtype=np.float64)

    for idx, val in enumerate(y_arr):
        if val <= 0:
            flat_result[idx] = 0.0
            continue
        lo, hi = 0.0, 100.0
    ...
    if np.ndim(y) == 0:
        return float(flat_result[0])
    return flat_result.reshape(y_arr.shape)
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    for _ in range(n):
        new_m = np.zeros(2 * len(m), dtype=np.float64)
        for i in range(len(m)):
            new_m[2 * i] = phi_inv(1.0 - (1.0 - phi(m[i])) ** 2)
            new_m[2 * i + 1] = 2.0 * m[i]
        m = new_m
