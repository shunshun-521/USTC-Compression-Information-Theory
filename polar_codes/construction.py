"""
极化码构造：高斯近似（GA）方法
适用于 BPSK-AWGN 信道
"""
import numpy as np

PHI_INV_MAX = 1e4


def phi(x):
    """
    GA 中的 phi 函数近似（x > 0）
    phi(x) = e^{-0.4527 * x^0.86 + 0.0218},  0 < x < 10
    phi(x) = sqrt(pi/x) * e^{-x/4} * (1 - 10/(7x)), x >= 10
    """
    x = np.asarray(x, dtype=np.float64)
    result = np.empty_like(x)
    mask_small = x < 10.0
    mask_large = ~mask_small
    xs = x[mask_small]
    if xs.size > 0:
        result[mask_small] = np.exp(-0.4527 * np.power(xs, 0.86) + 0.0218)
    xl = x[mask_large]
    if xl.size > 0:
        result[mask_large] = (
            np.sqrt(np.pi / xl)
            * np.exp(-xl / 4.0)
            * (1.0 - 10.0 / (7.0 * xl))
        )
    return np.clip(result, 0.0, 1.0 - 1e-12)


def phi_inv(y):
    """phi 函数的数值逆（二分法）"""
    y = np.asarray(y, dtype=np.float64)
    scalar = y.ndim == 0
    if scalar:
        y = y.reshape(1)
    y = np.clip(y, 0.0, 1.0 - 1e-12)
    lo = np.zeros_like(y)
    hi = np.full_like(y, PHI_INV_MAX)
    for _ in range(80):
        mid = (lo + hi) / 2.0
        pm = phi(mid)
        go_hi = pm < y
        lo = np.where(go_hi, mid, lo)
        hi = np.where(go_hi, hi, mid)
    result = (lo + hi) / 2.0
    return result[0] if scalar else result


def ga_construction(N, K, design_eb_n0_db, rate=None):
    """
    高斯近似构造极化码。

    同时执行 GA 均值递推与 Bhattacharyya 参数递推；
    以 -log(Z) 作为等效 LLR 均值进行排序（与 PSC 编码器索引一致），
    选取最大的 K 个信道作为信息位。
    """
    if rate is None:
        rate = K / N
    n = int(np.log2(N))
    if 2 ** n != N:
        raise ValueError("N must be a power of 2")

    sigma = 1.0 / np.sqrt(2.0 * rate * (10.0 ** (design_eb_n0_db / 10.0)))
    m0 = 2.0 / (sigma ** 2)
    z0 = np.exp(-1.0 / (2.0 * sigma ** 2))

    means = np.array([m0], dtype=np.float64)
    z = np.array([z0], dtype=np.float64)

    for _ in range(n):
        phi_m = phi(means)
        bad_mean = phi_inv(1.0 - (1.0 - phi_m) ** 2)
        good_mean = 2.0 * means
        means = np.concatenate([bad_mean, good_mean])

        bad_z = 2.0 * z - z ** 2
        good_z = z ** 2
        z = np.concatenate([bad_z, good_z])

    llr_means = -np.log(np.clip(z, 1e-300, 1.0))

    info_indices = np.argsort(llr_means)[-K:]
    info_indices = np.sort(info_indices)
    all_idx = np.arange(N)
    frozen_mask = np.ones(N, dtype=bool)
    frozen_mask[info_indices] = False
    frozen_indices = all_idx[frozen_mask]

    return info_indices, frozen_indices, llr_means


if __name__ == "__main__":
    info, frozen, _ = ga_construction(8, 4, 2.5)
    print("N=8, K=4, Eb/N0=2.5dB")
    print("info_indices:", info)
    print("frozen_indices:", frozen)

    info256, _, _ = ga_construction(256, 128, 2.5)
    print("\nN=256, K=128, first 20 info_indices:", info256[:20])
