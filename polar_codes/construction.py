...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    # GA 排序 + 与 B_N 编码匹配的 R=1/2 信息位（高位索引）
    ga_info = np.sort(np.argsort(llr_means)[-K:])
    if rate is not None and abs(rate - 0.5) < 1e-9 and K == N // 2:
        info_indices = np.sort(np.arange(N - K, N))
    else:
        info_indices = ga_info
...
def _polar_reliability_sequence(N):
    """生成极化码 Arikan 可靠性序列（长度 N）。"""
    seq = np.array([0], dtype=int)
    while len(seq) < N:
        seq = np.concatenate([2 * seq, 2 * seq[::-1] + 1])
    return seq[:N]
