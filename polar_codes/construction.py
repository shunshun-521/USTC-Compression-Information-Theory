...
def phi(x):
    ...
    xs = x[mask_large]
    result[mask_large] = np.sqrt(np.pi / xs) * np.exp(-xs / 4) * (1 - 10 / (7 * xs))
    return result
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    for _ in range(n):
        ...
        ph = phi(m)
        # f 分支（坏方向）：索引 2i-1 -> 0-based: 2*i+1? 
        # 规范：m_new[2i-1] for i=1..half -> indices 1,3,5,... (0-based)
        # m_new[2i] for i=1..half -> indices 0,2,4,... (0-based)
        for i in range(half):
            m_new[2 * i] = 2 * m[i]  # g 分支（好方向）
            m_new[2 * i + 1] = phi_inv(1 - (1 - ph[i]) ** 2)  # f 分支
        m = m_new
    ...
    frozen_indices = np.setdiff1d(all_indices, info_indices)
