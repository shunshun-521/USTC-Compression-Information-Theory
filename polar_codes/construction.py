"""
极化码构造：高斯近似（GA）方法
适用于 BPSK-AWGN 信道
"""
import numpy as np


def phi(x):
    """GA 中的 phi 函数近似（x > 0）。"""
    x = np.asarray(x, dtype=np.float64)
    scalar = x.ndim == 0
    if scalar:
        x = np.array([x], dtype=np.float64)
    result = np.empty_like(x)
    for i, xi in enumerate(x):
        if xi < 10.0:
            result[i] = np.exp(-0.4527 * (xi ** 0.86) + 0.0218)
        else:
            result[i] = (
                np.sqrt(np.pi / xi) * (1.0 - 10.0 / (7.0 * xi)) * np.exp(-xi / 4.0)
            )
    return float(result[0]) if scalar else result


def _phi_residual(x, val):
    return phi(x) - val


def phi_inv(y):
    """phi 函数的数值逆（二分法，区间 [0, 10000]）。"""
    y = np.asarray(y, dtype=np.float64)
    scalar = y.ndim == 0
    if scalar:
        targets = [float(y)]
    else:
        targets = y.ravel().tolist()

    results = []
    for val in targets:
        lo, hi = 0.0, 10000.0
        while hi - lo >= 0.01:
            mid = 0.5 * (lo + hi)
            if _phi_residual(mid, val) == 0.0:
                lo = hi = mid
                break
            if _phi_residual(mid, val) * _phi_residual(lo, val) < 0:
                hi = mid
            else:
                lo = mid
        results.append(0.5 * (lo + hi))

    if scalar:
        return results[0]
    return np.array(results, dtype=np.float64).reshape(y.shape)


def logQ_Borjesson(x):
    """Borjesson 近似：将对数域可靠性用于 GA 排序。"""
    a = 0.339
    b = 5.510
    half_log2pi = 0.5 * np.log(2.0 * np.pi)
    x = abs(float(x))
    return (
        -np.log((1.0 - a) * x + a * np.sqrt(b + x * x))
        - (x * x / 2.0)
        - half_log2pi
    )


def ga_construction(N, K, design_eb_n0_db, rate=None):
    """
    高斯近似构造极化码（树形密度演化 + Borjesson 排序）。
    """
    if rate is None:
        rate = K / N

    n = int(np.log2(N))
    if 2 ** n != N:
        raise ValueError("N must be a power of 2")

    design_snr = 10.0 ** (design_eb_n0_db / 10.0) * rate
    z0 = 4.0 * design_snr

    z = np.zeros((N, n + 1), dtype=np.float64)
    z[:, 0] = z0

    for j in range(1, n + 1):
        block = 1 << j
        half = block >> 1
        for t in range(0, N, block):
            for s in range(half):
                k = t + s
                z_top = z[k, j - 1]
                z_bottom = z[k + half, j - 1]
                z[k, j] = phi_inv(
                    1.0 - (1.0 - phi(z_top)) * (1.0 - phi(z_bottom))
                )
                z[k + half, j] = z_top + z_bottom

    llr_means = z[:, n]
    m = np.array([logQ_Borjesson(0.707 * np.sqrt(llr_means[i])) for i in range(N)])
    order = np.argsort(m, kind="mergesort")
    # 与 mcba1n 一致：frozen 数组存标准冻结位索引 order[K:]
    frozen_indices = np.sort(order[K:])
    info_indices = np.sort(order[: N - K])

    return info_indices, frozen_indices, llr_means


if __name__ == "__main__":
    info8, frozen8, _ = ga_construction(8, 4, 2.5)
    print("N=8, K=4, Eb/N0=2.5dB")
    print("info_indices:", info8)
    print("frozen_indices:", frozen8)

    info256, _, _ = ga_construction(256, 128, 2.5)
    print("\nN=256, K=128, first 20 info_indices:")
    print(info256[:20])
