...
def phi_inv(y):
    """
    phi 函数的数值逆（二分法 / Brent 法，区间 [0, 100]）
    """
    y = float(np.clip(y, 0.0, 1.0))
    if y <= phi(np.array([100.0]))[0]:
        return 100.0
    if y >= phi(np.array([1e-12]))[0]:
        return 0.0

    lo, hi = 1e-12, 100.0
    f_lo = phi(np.array([lo]))[0] - y
    f_hi = phi(np.array([hi]))[0] - y
    if f_lo * f_hi > 0:
        return hi if abs(f_hi) < abs(f_lo) else lo

    def f(x):
        return float(phi(np.array([x]))[0] - y)
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    for _ in range(n):
        ...
        pm = phi(m)
        m_new[0::2] = np.array([phi_inv(1.0 - (1.0 - p) ** 2) for p in pm])
        m_new[1::2] = 2.0 * m
        m = m_new
