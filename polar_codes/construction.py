...
def phi_inv(y):
    ...
    target = np.clip(y, 0.0, phi(np.array([100.0]))[0])

    for _ in range(60):
        mid = 0.5 * (lo + hi)
        val = phi(mid)
        hi = np.where(val > target, mid, hi)
        lo = np.where(val > target, lo, mid)
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    for _ in range(n):
        m_old = llr_means
        m_new = np.empty(len(m_old) * 2, dtype=np.float64)
        for i in range(len(m_old)):
            m_new[2 * i] = phi_inv(1.0 - (1.0 - phi(m_old[i])) ** 2)
            m_new[2 * i + 1] = 2.0 * m_old[i]
        llr_means = m_new
