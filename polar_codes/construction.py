...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    # Start with single element, expand n layers
    m = np.array([m0], dtype=np.float64)
    for _ in range(n):
        half = len(m)
        m_new = np.empty(2 * half, dtype=np.float64)
        ph = phi(m)
        # f branch (bad): indices 0, 2, 4, ... -> 2i in 0-based: 2*i
        # g branch (good): indices 1, 3, 5, ... -> 2*i+1 in 0-based
        for i in range(half):
            m_new[2 * i] = phi_inv(1.0 - (1.0 - ph[i]) ** 2)
            m_new[2 * i + 1] = 2.0 * m[i]
        m = m_new

    llr_means = m
    # Select K indices with largest LLR means as info bits
