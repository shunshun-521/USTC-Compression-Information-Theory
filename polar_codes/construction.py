...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    # 用 Bhattacharyya 参数 Z = phi(m) 递推，数值更稳定
    z = phi(np.array([m0]))
    z = np.clip(z, 0.0, 1.0 - 1e-12)

    for _ in range(n):
        z_new = np.zeros(2 * len(z), dtype=np.float64)
        z_new[0::2] = np.clip(2.0 * z - z ** 2, 0.0, 1.0 - 1e-12)  # W^- 较差
        z_new[1::2] = np.clip(z ** 2, 0.0, 1.0 - 1e-12)  # W^+ 较好
        z = z_new

    # 转回等效 LLR 均值：m = phi_inv(Z)，Z 越小 m 越大
    llr_means = phi_inv(z)
