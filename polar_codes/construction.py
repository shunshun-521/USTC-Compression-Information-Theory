"""
极化码构造：高斯近似（GA）方法
适用于 BPSK-AWGN 信道
"""
import numpy as np


def phi(x):
    """
    GA 中的 phi 函数近似（x > 0）
    phi(x) = e^{-0.4527 * x^0.86 + 0.0218},  0 < x < 10
    phi(x) = sqrt(pi/x) * e^{-x/4} * (1 - 10/(7x)), x >= 10
    """
    x = np.asarray(x, dtype=np.float64)
    y = np.empty_like(x)
    large = x >= 10.0
    small = ~large
    y[small] = np.exp(-0.4527 * np.power(x[small], 0.86) + 0.0218)
    xs = x[large]
    y[large] = np.sqrt(np.pi / xs) * np.exp(-xs / 4.0) * (1.0 - 10.0 / (7.0 * xs))
    return np.where(x <= 0.0, 1.0, y)


def _phi_inv_scalar(y_val, tol=1e-12):
    """单点 phi 逆（二分法，区间 [0, 100]）"""
    if y_val <= 0.0:
        return 100.0
    if y_val >= 1.0:
        return 0.0
    lo, hi = 0.0, 100.0
    while hi - lo > tol:
        mid = 0.5 * (lo + hi)
        if phi(mid) > y_val:
            lo = mid
        else:
            hi = mid
    return 0.5 * (lo + hi)


def phi_inv(y):
    """phi 函数的数值逆（二分法，区间 [0, 100]）"""
    y_arr = np.asarray(y, dtype=np.float64)
    flat = y_arr.ravel()
    out = np.array([_phi_inv_scalar(v) for v in flat], dtype=np.float64)
    return out.reshape(y_arr.shape)


def ga_construction(N, K, design_eb_n0_db, rate=None):
    """
    高斯近似构造极化码。

    参数：
        N: 码长（必须是 2 的幂）
        K: 信息位数
        design_eb_n0_db: 设计信噪比 Eb/N0（dB）
        rate: 码率 R=K/N，若为 None 则自动计算

    返回：
        info_indices: 长度为 K 的数组，信息位在 u 向量中的索引（从 0 开始）
        frozen_indices: 长度为 N-K 的数组，冻结位索引
        llr_means: 长度为 N 的数组，每个极化信道的等效 LLR 均值
    """
    if rate is None:
        rate = K / N
    if N & (N - 1):
        raise ValueError("N must be a power of 2")

    n = int(np.log2(N))
    sigma = 1.0 / np.sqrt(2.0 * rate) * 10.0 ** (-design_eb_n0_db / 20.0)
    m = np.array([2.0 / (sigma ** 2)], dtype=np.float64)

    for _ in range(n):
        pm = phi(m)
        m_new = np.empty(2 * len(m), dtype=np.float64)
        # 1-indexed m_new[2i-1] -> 0-indexed even: f 分支（可靠性较低）
        m_new[0::2] = phi_inv(1.0 - (1.0 - pm) ** 2)
        # 1-indexed m_new[2i] -> 0-indexed odd: g 分支（可靠性较高）
        m_new[1::2] = 2.0 * m
        m = m_new

    llr_means = m
    info_indices = np.sort(np.argsort(-llr_means)[:K])
    frozen_indices = np.setdiff1d(np.arange(N), info_indices)
    return info_indices, frozen_indices, llr_means


if __name__ == "__main__":
    info8, frozen8, _ = ga_construction(8, 4, 2.5)
    print("N=8, K=4, Eb/N0=2.5dB")
    print("info_indices:", info8)
    print("frozen_indices:", frozen8)

    info256, _, _ = ga_construction(256, 128, 2.5)
    print("\nN=256, K=128, info_indices (first 20):", info256[:20])
