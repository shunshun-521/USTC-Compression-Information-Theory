...
def phi(x):
    ...
    xb = x[~small]
    out[~small] = np.sqrt(np.pi / xb) * np.exp(-xb / 4.0) * (1.0 - 10.0 / (7.0 * xb))
    return out
...
def phi_inv(y):
  ...
  phi 在 [0, 100] 上单调递减
    ...
    y = np.clip(y, 1e-12, phi(np.array([1e-12]))[0])
    lo = np.zeros_like(y)
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
  ...
  # sigma = 1 / sqrt(2*R) * 10^{-Eb/N0/20}
    ...
    for _ in range(n):
        ...
        m_good = 2.0 * m
        m = np.concatenate([m_bad, m_good])

    llr_means = m
    info_indices = np.argsort(-llr_means)[:K]
    info_indices = np.sort(info_indices)
    all_idx = np.arange(N)
    frozen_indices = np.setdiff1d(all_idx, info_indices, assume_unique=True)
