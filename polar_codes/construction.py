...
def phi(x):
    ...
    mask_lo = x < 10.0
    mask_hi = ~mask_lo
    out[mask_lo] = np.exp(-0.4527 * np.power(x[mask_lo], 0.86) + 0.0218)
    xh = x[mask_hi]
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    for _ in range(n):
        half = len(means)
        new = np.empty(2 * half, dtype=np.float64)
        ph = phi(means)
        new[0::2] = phi_inv(1.0 - (1.0 - ph) ** 2)
        new[1::2] = 2.0 * means
        means = new
...
if __name__ == "__main__":
    ...
    print("N=256, K=128, info_indices[:20]:", info256[:20])
