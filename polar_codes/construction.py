...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    for _ in range(n):
        ...
        pm = phi(m)
        m_new[0::2] = 2.0 * m                            # W^+ 好信道
        m_new[1::2] = phi_inv(1.0 - (1.0 - pm) ** 2)    # W^- 坏信道
        m = m_new

    rev = np.array([_bit_reversed_index(i, n) for i in range(N)])
    channel_rel = m[rev]
    llr_means = channel_rel
    info_indices = np.argsort(channel_rel)[:K]
