...
def phi(x):
    ...
    xs = x[mask_large]
    result[mask_large] = np.sqrt(np.pi / xs) * np.exp(-xs / 4) * (1.0 - 10.0 / (7.0 * xs))
    result[mask_nonpos] = 1.0
    return result
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    llr = np.array([m0], dtype=np.float64)
    for _ in range(n):
        m_new = np.empty(2 * len(llr), dtype=np.float64)
        for i in range(len(llr)):
            # f 分支（"坏"方向，0-based: 2i）
            m_new[2 * i] = phi_inv(1.0 - (1.0 - phi(llr[i])) ** 2)
            # g 分支（"好"方向，0-based: 2i+1）
            m_new[2 * i + 1] = 2.0 * llr[i]
        llr = m_new
