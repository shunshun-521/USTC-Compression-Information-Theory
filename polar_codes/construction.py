...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    """
    高斯近似构造极化码（树形密度演化 + Borjesson 可靠性度量）。
    ...
    eb_n0_lin = 10 ** (design_eb_n0_db / 10)
    z0 = 4.0 * rate * eb_n0_lin
    ...
    llr_means = z[:, n]
    reliability = np.array([_log_q_borjesson(0.707 * np.sqrt(z[i, n])) for i in range(N)])
