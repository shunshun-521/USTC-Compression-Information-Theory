...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    for _ in range(n):
        old = means
        new_len = len(old) * 2
        means = np.empty(new_len, dtype=np.float64)
        phi_old = phi(old)
        means[0::2] = phi_inv(1.0 - (1.0 - phi_old) ** 2)
        means[1::2] = 2.0 * old

    llr_means = means
    sorted_indices = np.argsort(llr_means)
    info_indices = np.sort(sorted_indices[-K:])
    frozen_indices = np.sort(sorted_indices[: N - K])

    # 信道 0 可靠性最低，始终冻结
    if 0 in info_indices and K < N:
        worst_frozen = frozen_indices[0]
        info_indices = np.sort(
            np.concatenate([info_indices[info_indices != 0], [worst_frozen]])
        )
        frozen_indices = np.sort(
            np.concatenate([frozen_indices[frozen_indices != worst_frozen], [0]])
        )
