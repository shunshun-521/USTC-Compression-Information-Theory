...
def phi(x):
    ...
    result[mask_large] = (
        np.sqrt(np.pi / x[mask_large])
        * np.exp(-x[mask_large] / 4.0)
        * (1.0 - 10.0 / (7.0 * x[mask_large]))
    )
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    for _ in range(n):
        ...
        phi_old = phi(old)
        # f 分支（坏方向）：索引 2i-1 -> 0-based 偶数位置
        means[0::2] = phi_inv(1.0 - (1.0 - phi_old) ** 2)
        # g 分支（好方向）：索引 2i -> 0-based 奇数位置
        means[1::2] = 2.0 * old
    ...
    frozen_indices = np.setdiff1d(all_indices, info_indices, assume_unique=False)
