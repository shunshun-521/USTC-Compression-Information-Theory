...
def _log_q_borjesson(x):
    ...
    x = np.abs(x)
    y = -np.log((1 - a) * x + a * np.sqrt(b + x * x)) - (x ** 2) / 2 - half_log2pi
    return y
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    llr_means = z[:, n]
    reliability = np.array([_log_q_borjesson(0.707 * np.sqrt(llr_means[i])) for i in range(N)])
    frozen_indices = np.argsort(reliability, kind="mergesort")[K:]
    frozen_indices = np.sort(frozen_indices)
    info_mask = np.ones(N, dtype=bool)
    info_mask[frozen_indices] = False
    info_indices = np.arange(N)[info_mask]
