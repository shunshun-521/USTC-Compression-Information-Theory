...
def phi(x):
    ...
    xl = x[mask_large]
    result[mask_large] = np.sqrt(np.pi / xl) * np.exp(-xl / 4) * (1 - 10 / (7 * xl))
    return result
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    m = np.array([2.0 / sigma ** 2])

    for _ in range(n):
        m_new = np.empty(2 * len(m))
        pm = phi(m)
        m_new[0::2] = phi_inv(1 - (1 - pm) ** 2)
        m_new[1::2] = 2 * m
        m = m_new

    llr_means = m

    # 按比特倒序重排信道可靠性，与 SC 译码器的处理顺序一致
    br = bit_reversal_permutation(N)
    # 按比特倒序重排后的可靠性用于选取信息位，与 SC 译码处理顺序一致
    reliabilities = llr_means[br]
    info_positions = np.argsort(reliabilities)[-K:]
    info_indices = np.sort(br[info_positions])
