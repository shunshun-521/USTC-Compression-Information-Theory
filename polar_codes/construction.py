...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    for _ in range(n):
        ...
        pm = phi(m)
        m_new[0::2] = phi_inv(1.0 - (1.0 - pm) ** 2)  # f 分支（坏）
        m_new[1::2] = 2.0 * m  # g 分支（好）
        m = m_new

    llr_means = m
    sorted_indices = np.argsort(-llr_means)  # 从大到小
...
if __name__ == "__main__":
    ...
    print("N=256, K=128, info_indices (first 20):", info_idx256[:20])
