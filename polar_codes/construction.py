...
def phi_inv(y):
    ...
    y = np.clip(y, 0.0, phi(np.array([100.0]))[0])
    lo = np.zeros_like(y)
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    for _ in range(n):
        ...
        phi_vals = phi(llr_means)
        m_new[0::2] = 2.0 * llr_means
        m_new[1::2] = phi_inv(1.0 - (1.0 - phi_vals) ** 2)
        llr_means = m_new

    indices = np.arange(N)
    order = np.lexsort((-indices, -llr_means))
