...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    llr_means = np.zeros((N, n + 1), dtype=np.float64)
    llr_means[0, 0] = m0
    ...
    for stage in range(1, n + 1):
        ...
        for start in range(0, N, block):
            for s in range(half):
                k = start + s
                top = llr_means[k, stage - 1]
                bottom = llr_means[k + half, stage - 1]
                llr_means[k, stage] = phi_inv(
                    1.0 - (1.0 - phi(top)) * (1.0 - phi(bottom))
                )
                llr_means[k + half, stage] = top + bottom

    llr_means = llr_means[:, n]
