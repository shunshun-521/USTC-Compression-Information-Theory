...
def phi(x):
  ...
  # x=0 时 phi(0)=1
    result = np.where(x <= 0, 1.0, result)
    return result
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    for _ in range(n):
        ...
        # f 分支（坏方向）：索引 2i（0-based: 2i 对应规范中的 2i，2i+1 对应 2i-1）
        # 按规范：m_new[2i-1] = phi_inv(...), m_new[2i] = 2*m[i]
        # 0-based: m_new[2*i] = phi_inv(...), m_new[2*i+1] = 2*m[i]
