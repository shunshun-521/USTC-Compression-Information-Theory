...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    使用 Bhattacharyya 参数 Z 的 GA 递推，选取等效 LLR 最大的 K 个信道为信息位。
    """
    if rate is None:
        rate = K / N
    n = int(np.log2(N))
    ...
    m0 = 2.0 / (sigma ** 2)
    z = np.array([phi(m0)], dtype=np.float64)

    for _ in range(n):
        z_new = np.empty(2 * len(z), dtype=np.float64)
        for i, zi in enumerate(z):
            z_new[2 * i] = phi_inv(1.0 - (1.0 - zi) ** 2)
            z_new[2 * i + 1] = zi ** 2
        z = z_new
    ...
    all_idx = np.arange(N)
    frozen_indices = np.setdiff1d(all_idx, info_indices)
