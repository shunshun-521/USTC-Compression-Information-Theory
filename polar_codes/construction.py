...
def phi(x):
    ...
    out = np.empty_like(x)
    mask = x < 10.0
    out[mask] = np.exp(-0.4527 * np.power(x[mask], 0.86) + 0.0218)
    x_large = x[~mask]
    out[~mask] = (
        np.sqrt(np.pi / x_large)
        * np.exp(-x_large / 4.0)
        * (1.0 - 10.0 / (7.0 * x_large))
    )
    return out
...
def phi_inv(y):
    ...
    for idx, target in enumerate(y_arr.flat):
        lo, hi = 0.0, 100.0
        for _ in range(80):
            mid = 0.5 * (lo + hi)
            if phi(mid) > target:
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    llr_means = np.array([2.0 / (sigma ** 2)], dtype=np.float64)
    n = int(np.log2(N))

    for _ in range(n):
        m_old = llr_means
        m_new = np.empty(2 * len(m_old), dtype=np.float64)
        for i, m_val in enumerate(m_old):
            m_new[2 * i] = 2.0 * m_val
            m_new[2 * i + 1] = phi_inv(1.0 - (1.0 - phi(m_val)) ** 2)
        llr_means = m_new
...
if __name__ == "__main__":
    ...
    print("\nN=256, K=128, first 20 info indices:")
    print(info256[:20])
