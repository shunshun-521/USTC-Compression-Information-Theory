...
def phi(x):
    ...
    mask_small = x < 10
    xs = x[mask_small]
    out[mask_small] = np.exp(-0.4527 * xs ** 0.86 + 0.0218)
    xl = x[~mask_small]
    out[~mask_small] = np.sqrt(np.pi / xl) * np.exp(-xl / 4) * (1 - 10.0 / (7.0 * xl))
    return out
...
def ga_construction(N, K, design_eb_n0_db, rate=None):
    ...
    sigma = 1.0 / np.sqrt(2.0 * rate * 10 ** (design_eb_n0_db / 10.0))
    m0 = 2.0 / sigma ** 2
    ...
    for _ in range(n):
        ...
        m_new[0::2] = phi_inv(1.0 - (1.0 - pm) ** 2)  # f 分支
        m_new[1::2] = 2.0 * m  # g 分支
        m = m_new

    llr_means = m.copy()
    sorted_idx = np.argsort(-llr_means)
    info_indices = np.sort(sorted_idx[:K])
    frozen_indices = np.sort(sorted_idx[K:])
