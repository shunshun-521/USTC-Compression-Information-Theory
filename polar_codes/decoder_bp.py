...
def _f_min_sum(x, y, alpha=0.9375):
    return alpha * np.sign(x) * np.sign(y) * np.minimum(np.abs(x), np.abs(y))


class BPDecoder:
    """BP 译码器（参考 Sionna PolarBPDecoder 消息传递结构）。"""
    ...
    def decode(self, llr_ch):
        ...
        n = self.n
        alpha = self.alpha
        llr_ch = np.clip(llr_ch, -self.LARGE, self.LARGE)
        ...
        u_soft = msg_l[self.max_iter - 1][0]
        u_hat = (u_soft < 0).astype(int)
        u_hat[self.frozen_bits] = 0
        return u_hat, num_iters
