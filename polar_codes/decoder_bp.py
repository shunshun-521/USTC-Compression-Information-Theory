...
class BPDecoder:
    ...
    def _clamp(self, x):
        return np.clip(x, -self.llr_max, self.llr_max)

    def _bp_iteration(self, llr_ch, msg_l_prev, msg_r_in, first_iter):
    ...
    def decode(self, llr_ch):
        llr_raw = llr_ch
        llr_rev = np.zeros(self.N, dtype=np.float64)
        llr_rev[self.rev] = llr_ch
        llr_ch = self._clamp(llr_rev)
        ...
        for it in range(1, self.max_iter + 1):
            num_iters = it
            msg_l_iter, msg_r_iter = self._bp_iteration(
                llr_ch, msg_l_prev, msg_r_in, first_iter=(it == 1)
            )
            msg_l_prev = msg_l_iter

            posterior = msg_l_iter[0] + msg_r_in
        ...
        posterior = msg_l_prev[0] + msg_r_in
        for i in range(self.N):
            if self.frozen_bits[i]:
                u_hat[i] = 0
            else:
