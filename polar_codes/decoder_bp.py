"""
极化码 BP（置信传播）译码器
基于 Arikan 因子图 BP（Sionna 同构实现），min-sum 近似，含早停机制
"""
import numpy as np

from decoder_sc import f_operation, _prepare_llr
from encoder import polar_encode

LLR_MAX = np.float32(19.3)


class BPDecoder:
    """BP 译码器"""

    def __init__(self, N, frozen_bits, max_iter=50, alpha=0.9375):
        self.N = N
        self.n = int(np.log2(N))
        assert 2 ** self.n == N
        self.frozen_bits = np.asarray(frozen_bits, dtype=int)
        self.max_iter = max_iter
        self.alpha = np.float32(alpha)
        self.frozen_idx = np.sort(np.where(self.frozen_bits == 1)[0])
        self.info_idx = np.where(self.frozen_bits == 0)[0]

    def _boxplus(self, x, y):
        """min-sum 近似 boxplus"""
        x = np.clip(x, -LLR_MAX, LLR_MAX)
        y = np.clip(y, -LLR_MAX, LLR_MAX)
        return self.alpha * f_operation(x, y)

    def _run_bp(self, llr_ch, num_iter):
        """完整 BP 迭代"""
        llr_ch = np.asarray(llr_ch, dtype=np.float32)
        n_stages = self.n
        N = self.N
        msg_l = [[None] * (n_stages + 1) for _ in range(num_iter)]
        msg_r = [[None] * (n_stages + 1) for _ in range(num_iter)]
        msg_r_in = np.zeros(N, dtype=np.float32)
        msg_r_in[self.frozen_idx] = LLR_MAX

        for ind_it in range(num_iter):
            for ind_s in range(n_stages):
                ind_range = np.arange(N // 2, dtype=np.int64)
                ind_1 = ind_range * 2 - np.mod(ind_range, 2 ** ind_s)
                ind_2 = ind_1 + 2 ** ind_s

                if ind_s == n_stages - 1:
                    l1_in = llr_ch[ind_1]
                    l2_in = llr_ch[ind_2]
                elif ind_it == 0:
                    l1_in = np.zeros(N // 2, dtype=np.float32)
                    l2_in = np.zeros(N // 2, dtype=np.float32)
                else:
                    l_in = msg_l[ind_it - 1][ind_s + 1]
                    l1_in = l_in[ind_1]
                    l2_in = l_in[ind_2]

                if ind_s == 0:
                    r1_in = msg_r_in[ind_1]
                    r2_in = msg_r_in[ind_2]
                else:
                    r_in = msg_r[ind_it][ind_s]
                    r1_in = r_in[ind_1]
                    r2_in = r_in[ind_2]

                r1_out = self._boxplus(r1_in, l2_in + r2_in)
                r2_out = self._boxplus(r1_in, l1_in) + r2_in
                ind_inv = np.argsort(np.concatenate([ind_1, ind_2]))
                msg_r[ind_it][ind_s + 1] = np.concatenate([r1_out, r2_out])[ind_inv]

            for ind_s in range(n_stages - 1, -1, -1):
                ind_range = np.arange(N // 2, dtype=np.int64)
                ind_1 = ind_range * 2 - np.mod(ind_range, 2 ** ind_s)
                ind_2 = ind_1 + 2 ** ind_s
                ind_inv = np.argsort(np.concatenate([ind_1, ind_2]))

                if ind_s == n_stages - 1:
                    l1_in = llr_ch[ind_1]
                    l2_in = llr_ch[ind_2]
                else:
                    l_in = msg_l[ind_it][ind_s + 1]
                    l1_in = l_in[ind_1]
                    l2_in = l_in[ind_2]

                if ind_s == 0:
                    r1_in = msg_r_in[ind_1]
                    r2_in = msg_r_in[ind_2]
                else:
                    r_in = msg_r[ind_it][ind_s]
                    r1_in = r_in[ind_1]
                    r2_in = r_in[ind_2]

                l1_out = self._boxplus(l1_in, l2_in + r2_in)
                l2_out = self._boxplus(r1_in, l1_in) + l2_in
                msg_l[ind_it][ind_s] = np.concatenate([l1_out, l2_out])[ind_inv]

        return msg_l[num_iter - 1][0]

    def decode(self, llr_ch):
        """主译码函数，返回 (u_hat, num_iters)"""
        llr = _prepare_llr(llr_ch).astype(np.float32)
        u_hat = np.zeros(self.N, dtype=int)
        num_iters = self.max_iter

        for it in range(1, self.max_iter + 1):
            llr_left = self._run_bp(llr, it)
            u_hat[:] = 0
            u_hat[self.info_idx] = (llr_left[self.info_idx] < 0).astype(int)
            x_hat = polar_encode(u_hat)
            if np.array_equal(x_hat, (llr_ch < 0).astype(int)):
                num_iters = it
                break

        return u_hat, num_iters
