...
class BPDecoder:
    """BP 译码器（flooded scheduling，min-sum）"""
            ...
            for j in range(n, 0, -1):
                ...
                for i in range(0, N, 2 * step):
                    for k in range(step):
                        ...
                        L[bot, j - 1] = _ms_f(R[top, j], L[top, j], self.alpha) + L[bot, j]
            ...
            for j in range(1, n + 1):
                ...
                for i in range(0, N, 2 * step):
                    for k in range(step):
                        ...
                        bot = i + k + step
                        R[top, j] = _ms_f(
                            R[bot, j] + L[bot, j], R[top, j - 1], self.alpha
                        )
                        R[bot, j] = _ms_f(R[top, j - 1], L[top, j], self.alpha) + R[bot, j - 1]
