...
from decoder_sc import sc_decode, sc_decode_recursive, path_metric_update
...
def test_path_metric():
    pm = path_metric_update(0.0, 3.5, 0)
    assert pm == 0.0
    pm = path_metric_update(0.0, -2.0, 0)
    assert pm == 2.0
    print("✓ 路径度量校验通过")
...
def test_bp():
    ...
    for _ in range(50):
        ...
        if np.array_equal(u_hat[info_idx], u[info_idx]):
            ok += 1
    assert ok >= 40, f"BP 高信噪比性能不足: {ok}/50"
    print(f"✓ BP 译码校验通过（{ok}/50 帧正确）")
