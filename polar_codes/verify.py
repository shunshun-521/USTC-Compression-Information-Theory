...
def test_ga_construction():
    ...
    assert np.array_equal(info, [0, 3, 5, 6]), f"GA N=8 信息位错误: {info}"
    assert np.array_equal(frozen, [1, 2, 4, 7]), f"GA N=8 冻结位错误: {frozen}"
...
def test_bp_runs():
    ...
    frozen[info] = False
    u = np.zeros(N, dtype=int)
    u[info] = np.ones(K, dtype=int)
    x = polar_encode(u)
    llr = compute_llr(bpsk_modulate(x), eb_n0_to_sigma(6.0, K / N))
    u_hat, iters = BPDecoder(N, frozen, max_iter=10).decode(llr)
    assert len(u_hat) == N
    assert iters >= 1
