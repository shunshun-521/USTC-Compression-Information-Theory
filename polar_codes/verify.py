...
def test_sc_lossless():
    ...
    for _ in range(100):
        ...
        llr = compute_llr(y, sigma)[bit_reversal_permutation(N)]
...
def test_sc_recursive_match():
    ...
    for _ in range(10):
        ...
        u = np.zeros(N, dtype=int)
        u[info_idx] = payload
        llr = compute_llr(bpsk_modulate(polar_encode(u)), 0.001)[bit_reversal_permutation(N)]
...
def test_construction():
    ...
    print(f"  N=256 info_indices[:20]: {info256[:20]}")
    print("✓ GA 构造输出已打印")
