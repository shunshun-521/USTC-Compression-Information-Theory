"""极化码模块数值正确性校验"""
import os
import sys

import numpy as np

sys.path.insert(0, os.path.dirname(__file__))

from channel import awgn_channel, bpsk_modulate, compute_llr, eb_n0_to_sigma
from construction import ga_construction
from decoder_sc import sc_decode, sc_decode_recursive
from decoder_scl import SCLDecoder, crc_encode, crc_check
from encoder import polar_encode, polar_encode_matrix
from simulation import make_frozen_bits, prepare_llr_for_decode


def test_encoder():
    u = np.array([1, 0, 1, 1], dtype=np.int8)
    x = polar_encode(u)
    x_mat = polar_encode_matrix(u)
    assert np.array_equal(x, x_mat), f"蝶形编码与矩阵编码不一致: {x} vs {x_mat}"
    print(f"编码器校验通过: u={u} -> x={x}")


def test_construction():
    info, frozen, _ = ga_construction(8, 4, 2.5)
    assert len(info) == 4 and len(frozen) == 4
    info256, _, _ = ga_construction(256, 128, 2.5)
    print(f"N=8 info={info}, frozen={frozen}")
    print(f"N=256 前20个信息位索引: {info256[:20]}")


def test_sc_lossless():
    N, K = 64, 32
    frozen_bits, info_idx = make_frozen_bits(N, K, 2.5)
    sigma = eb_n0_to_sigma(10.0, K / N)
    rng = np.random.default_rng(0)

    for _ in range(100):
        u = np.zeros(N, dtype=np.int8)
        payload = rng.integers(0, 2, size=K, dtype=np.int8)
        u[info_idx] = payload
        x = polar_encode(u)
        y = awgn_channel(bpsk_modulate(x), sigma, rng=rng)
        llr = prepare_llr_for_decode(compute_llr(y, sigma), N)

        u_hat_rec = sc_decode_recursive(llr, frozen_bits.astype(bool))
        u_hat = sc_decode(llr, frozen_bits.astype(bool))
        assert np.array_equal(u_hat_rec, u_hat), "递归与非递归 SC 不一致"
        assert np.array_equal(u_hat[info_idx], payload), "SC 译码错误"

    print("SC 无损校验通过 (N=64, K=32, 100 帧)")


def test_scl_equals_sc():
    N, K = 64, 32
    frozen_bits, info_idx = make_frozen_bits(N, K, 2.5)
    sigma = eb_n0_to_sigma(4.0, K / N)
    rng = np.random.default_rng(1)

    for _ in range(20):
        u = np.zeros(N, dtype=np.int8)
        payload = rng.integers(0, 2, size=K, dtype=np.int8)
        u[info_idx] = payload
        x = polar_encode(u)
        y = awgn_channel(bpsk_modulate(x), sigma, rng=rng)
        llr = prepare_llr_for_decode(compute_llr(y, sigma), N)

        u_sc = sc_decode(llr, frozen_bits.astype(bool))
        u_scl, _ = SCLDecoder(N, frozen_bits, list_size=1).decode(llr)
        assert np.array_equal(u_sc, u_scl), "L=1 SCL 应与 SC 等价"

    print("SCL(L=1) 与 SC 等价性校验通过")


def test_crc():
    bits = np.array([1, 0, 1, 1, 0, 0, 1, 0], dtype=np.int8)
    encoded = crc_encode(bits, crc_length=8)
    assert crc_check(encoded, crc_length=8)
    encoded[-1] ^= 1
    assert not crc_check(encoded, crc_length=8)
    print("CRC 校验通过")


def main():
    test_encoder()
    test_construction()
    test_crc()
    test_sc_lossless()
    test_scl_equals_sc()
    print("\n全部校验通过。")


if __name__ == "__main__":
    main()
