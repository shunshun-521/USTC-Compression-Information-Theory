...
def test_sc_noiseless():
    ...
    for _ in range(frames):
        ...
        if not np.array_equal(u_hat[info_idx], u[info_idx]):
            errors += 1
    assert errors < frames * 0.6, f"SC 高信噪比错误过多: {errors}/{frames}"
    print(f"sc high-SNR: OK ({frames - errors}/{frames} frames correct)")
...
def test_scl_equals_sc():
    ...
    for _ in range(30):
        ...
        if not np.array_equal(uh_sc, uh_scl):
            mism += 1
    assert mism == 0, f"SCL L=1 与 SC 不一致: {mism}"
    print("scl L=1 == sc: OK")
...
def test_bp():
    ...
    for _ in range(20):
        ...
        if np.array_equal(u_hat[info_idx], u[info_idx]):
            ok += 1
    print(f"bp smoke test: {ok}/20 frames correct")
