...
def test_bp_roundtrip():
  ...
  for _ in range(20):
    ...
    u[info_idx] = payload
    llr = compute_llr(awgn_channel(bpsk_modulate(polar_encode(u)), sigma, rng=rng), sigma)
    u_hat, num_iters = BPDecoder(N, frozen_bits, max_iter=100).decode(llr)
    successes += int(np.array_equal(u_hat[info_idx], payload))
  assert successes >= 5, f"BP 高信噪比测试成功率过低: {successes}/20"
  print(f"[PASS] BP 高信噪比译码 ({successes}/20 帧正确)")
