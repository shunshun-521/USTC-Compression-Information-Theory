...
def test_encoder():
    u = np.array([1, 0, 1, 1])
    x = polar_encode(u)
    assert len(x) == 4 and x.dtype == int
    u2 = np.array([0, 0, 0, 0])
    assert np.array_equal(polar_encode(u2), [0, 0, 0, 0]), "全零输入应编码为全零"
    print("✓ 编码器校验通过")
...
def test_ga_construction():
    ...
    assert np.array_equal(info256[:20], expected_first20), f"GA N=256 错误: {info256[:20]}"
    print("✓ GA 构造校验通过")
