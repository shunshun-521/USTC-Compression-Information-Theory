...
def test_sc_lossless():
    ...
    print("[PASS] SC 译码无损测试 (Eb/N0=12dB, 100帧)")


def test_sc_recursive_match():
    """非递归 SC 与含 BR 编码器的端到端一致性"""
    N = 32
    ...
    for _ in range(50):
        ...
        if not np.array_equal(u_hat[info_idx], u[info_idx]):
            errors += 1
    assert errors == 0, f"非递归 SC E2E 失败: {errors}/50"
    print("[PASS] 非递归 SC 端到端一致性")
...
def test_bp():
    ...
    for _ in range(50):
        ...
        if np.array_equal(u_hat[info_idx], u[info_idx]):
            ok += 1
    print(f"[PASS] BP 高信噪比测试: {ok}/50 帧正确")
