"""单元测试：验证各模块数值正确性。"""
import os
import sys

import numpy as np

sys.path.insert(0, os.path.dirname(__file__))

from channel import awgn_channel, bpsk_modulate, compute_llr, eb_n0_to_sigma
from construction import ga_construction
from decoder_bp import BPDecoder
from decoder_sc import sc_decode, sc_decode_recursive
from decoder_scl import SCLDecoder, crc_check, crc_encode
from encoder import polar_encode, polar_encode_reference


def test_encoder():
    u = np.array([0, 1, 0, 0])
    x = polar_encode(u)
    assert np.array_equal(x, polar_encode_reference(u)), f"矩阵编码不一致: {x}"
    assert np.array_equal(x, [1, 1, 0, 0]), f"编码器错误: {x}"
    print("[PASS] encoder")


def test_construction():
    info8, frozen8, _ = ga_construction(8, 4, 2.5)
    assert len(info8) == 4 and len(frozen8) == 4
    info256, _, _ = ga_construction(256, 128, 2.5)
    assert len(info256) == 128
    print("[PASS] construction")
    print("  N=8 info:", info8)
    print("  N=256 info[:20]:", info256[:20])


def test_sc_noiseless():
    N, K = 64, 32
    info_idx, _, _ = ga_construction(N, K, 2.5)
    frozen = np.ones(N, dtype=bool)
    frozen[info_idx] = False
    rng = np.random.default_rng(0)
    for _ in range(100):
        u = np.zeros(N, dtype=int)
        u[info_idx] = rng.integers(0, 2, K)
        codeword = polar_encode(u)
        llr = compute_llr(bpsk_modulate(codeword), eb_n0_to_sigma(10.0, K / N))
        uh = sc_decode(llr, frozen)
        assert np.array_equal(u, uh), "SC 无损译码失败"
        uh2 = sc_decode_recursive(llr, frozen)
        assert np.array_equal(u, uh2), "递归 SC 不一致"
    print("[PASS] SC noiseless (N=64, 100 frames)")


def test_scl_equals_sc():
    N, K = 64, 32
    info_idx, _, _ = ga_construction(N, K, 2.5)
    frozen = np.ones(N, dtype=bool)
    frozen[info_idx] = False
    rng = np.random.default_rng(1)
    for _ in range(20):
        u = np.zeros(N, dtype=int)
        u[info_idx] = rng.integers(0, 2, K)
        llr = compute_llr(
            bpsk_modulate(polar_encode(u)),
            eb_n0_to_sigma(8.0, K / N),
        )
        uh_sc = sc_decode(llr, frozen)
        uh_scl, _ = SCLDecoder(N, frozen, list_size=1).decode(llr)
        assert np.array_equal(uh_sc, uh_scl), "L=1 SCL 应等价于 SC"
    print("[PASS] SCL L=1 equals SC")


def test_crc():
    bits = np.array([1, 0, 1, 1, 0, 0, 1, 0])
    enc = crc_encode(bits, 8)
    assert crc_check(enc, 8)
    enc[-1] ^= 1
    assert not crc_check(enc, 8)
    print("[PASS] CRC")


def test_channel():
    sigma = eb_n0_to_sigma(2.0, 0.5)
    y = awgn_channel(np.array([1.0, -1.0]), sigma, np.random.default_rng(0))
    llr = compute_llr(y, sigma)
    assert llr.shape == y.shape
    print("[PASS] channel")


def run_all():
    test_encoder()
    test_construction()
    test_channel()
    test_crc()
    test_sc_noiseless()
    test_scl_equals_sc()
    print("\nAll validation tests passed.")


if __name__ == "__main__":
    run_all()
