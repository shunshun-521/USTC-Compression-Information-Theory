...
def test_bp_lossless():
    """BP 译码在极低噪声下应无错误（使用 channel_llr 对齐）。"""
    N, K = 64, 32
    ...
    for _ in range(50):
        ...
        if not np.array_equal(u_bp[info_idx], u[info_idx]):
            errors += 1
    assert errors == 0, f"BP 无损译码失败: {errors}/50 帧错误"
    print("BP 无损译码校验通过 (N=64, 近零噪声, 50帧)")
