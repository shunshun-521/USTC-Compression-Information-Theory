...
def test_sc_lossless():
    ...
    for _ in range(100):
        ...
        u[info_idx] = payload
        x = polar_encode(u)
        y = awgn_channel(bpsk_modulate(x, es=es), es=es, rng=rng)
        llr = prepare_decoder_llr(compute_llr(y, es=es), N)
        u_hat = map_decoder_bits_to_natural(sc_decode(llr, frozen_dec), N)
        u_hat_r = map_decoder_bits_to_natural(sc_decode_recursive(llr, frozen_dec), N)
