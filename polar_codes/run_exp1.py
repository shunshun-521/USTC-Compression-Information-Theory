...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        llr_t = compute_llr(bpsk_modulate(x_t), eb_n0_to_sigma(10.0, K / N))
        u_hat = sc_decode(llr_t, frozen_bits)
        assert np.array_equal(u_hat, u_t)

    # 路径度量校验：L=1 SCL 等价 SC
    from decoder_scl import SCLDecoder
    u_t = np.zeros(N, dtype=int)
    u_t[info_idx] = rng.integers(0, 2, K)
    x_t = polar_encode(u_t)
    llr_t = compute_llr(bpsk_modulate(x_t), eb_n0_to_sigma(2.0, K / N))
    u_sc = sc_decode(llr_t, frozen_bits)
    u_scl, _ = SCLDecoder(N, frozen_bits, list_size=1).decode(llr_t)
