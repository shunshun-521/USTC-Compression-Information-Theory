...
from channel import bpsk_modulate, compute_llr, channel_params, awgn_channel
...
def run_unit_tests():
    ...
    u4 = np.array([1, 0, 1, 1])
    x4 = polar_encode(u4)
    assert np.array_equal(x4, [1, 1, 0, 1]), f"N=4 编码器错误: {x4}"

    u8 = np.array([1, 0, 1, 1, 0, 0, 0, 0])
    x8 = polar_encode(u8)
    assert np.array_equal(x8, [1, 1, 0, 1, 0, 0, 0, 0]), f"N=8 编码器错误: {x8}"
    ...
    es, no, sigma = channel_params(10.0, 0.5)
    errors = 0
    for _ in range(100):
