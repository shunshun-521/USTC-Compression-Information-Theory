...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        if not np.array_equal(u_hat, u):
            errors += 1
    assert errors == 0, f"SC 译码校验失败: {errors}/100 帧有误"
    ...
    for _ in range(100):
        ...
        u_hat = sc_decode(compute_llr(y, sigma), frozen_bits.astype(bool))
        if not np.array_equal(u_hat, u):
            noisy_errors += 1
    assert noisy_errors <= 5, f"高 SNR SC 译码异常: {noisy_errors}/100 帧有误"
