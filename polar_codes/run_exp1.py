...
def run_validations():
    ...
    from decoder_scl import SCLDecoder, scl_decode_equivalent_to_sc

    llr_test = compute_llr(bpsk_modulate(polar_encode(u)), 0.5)
    frozen4 = np.array([False, True, False, False])
    assert scl_decode_equivalent_to_sc(4, frozen4, llr_test), "SCL L=1 与 SC 不等价"
