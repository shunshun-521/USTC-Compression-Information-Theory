...
def validate_modules():
    ...
    for _ in range(100):
        ...
        if not np.array_equal(u_hat[info_idx], u_t[info_idx]):
            errors += 1
    assert errors == 0, f"SC 译码在 Eb/N0=10dB 下 {errors}/100 帧有误"
    ...
    u_t[info_idx] = rng.integers(0, 2, K)
    llr = compute_llr(
        awgn_channel(bpsk_modulate(polar_encode(u_t)), sigma, rng), sigma
    )
    uh_sc = sc_decode(llr, frozen_bits.astype(bool))
    uh_scl, _ = SCLDecoder(N, frozen_bits.astype(bool), list_size=1).decode(llr)
...
MAX_FRAMES = 5000
MIN_ERRORS = 20
EB_N0_RANGE = np.arange(0.0, 5.25, 0.5)
