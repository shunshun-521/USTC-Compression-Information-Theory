...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        assert np.array_equal(u_hat[info_idx], u[info_idx]), "SC 译码在高信噪比下失败"

    info_idx8, _, _ = ga_construction(8, 4, 2.5)
    fb8 = np.ones(8, dtype=bool)
    fb8[info_idx8] = False
    u8 = np.zeros(8, dtype=int)
    u8[info_idx8] = rng.integers(0, 2, 4)
    llr8 = compute_llr(bpsk_modulate(polar_encode(u8)), 0.001)
    assert np.array_equal(sc_decode(llr8, fb8)[info_idx8], u8[info_idx8])
    print("单元测试通过。")
