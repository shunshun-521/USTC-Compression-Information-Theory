...
def run_unit_tests():
    ...
    N, K = 64, 32
    info_idx, _, _ = construct_frozen_ga(N, K, 2.5)
    ...
    u_hat2, _ = scl.decode(llr)
    assert np.array_equal(u_hat2[info_idx], payload)
    print("单元测试通过。")
...
if __name__ == "__main__":
    ...
    N_LIST = [256, 512] if _quick_mode() else [256, 512, 1024]
    RATE = 0.5
    DESIGN_EBN0 = 2.5
    MAX_FRAMES = 5000 if _quick_mode() else 100000
    MIN_ERRORS = 20 if _quick_mode() else 100
    EB_N0_RANGE = (
        np.arange(1.0, 3.5, 0.5) if _quick_mode() else np.arange(0.0, 5.5, 0.25)
    )
