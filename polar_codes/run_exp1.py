"""
实验一：SC 译码基础仿真
"""
import os
import sys

import numpy as np

sys.path.insert(0, os.path.dirname(__file__))

from construction import ga_construction
from encoder import polar_encode
from decoder_sc import sc_decode, sc_decode_recursive, sc_decode_peel
from simulation import run_simulation
from utils import save_results_csv, plot_bler_curves, save_frozen_set_info, find_capacity_limit
from channel import bpsk_modulate, compute_llr, awgn_channel, eb_n0_to_sigma


def _unit_tests():
    u = np.array([1, 0, 1, 1])
    x = polar_encode(u)
    assert np.array_equal(x, [1, 0, 1, 1]), f"编码器错误: {x}"

    N, K = 64, 32
    info, _, _ = ga_construction(N, K, 2.5)
    frozen = np.ones(N, dtype=int)
    frozen[info] = 0
    rng = np.random.default_rng(0)
    sigma = eb_n0_to_sigma(10.0, 0.5)
    err = 0
    for _ in range(100):
        u_full = np.zeros(N, dtype=int)
        u_full[info] = rng.integers(0, 2, K)
        llr = compute_llr(
            awgn_channel(bpsk_modulate(polar_encode(u_full)), sigma, rng), sigma
        )
        u_hat = sc_decode(llr, frozen.astype(bool))
        if not np.array_equal(u_hat[info], u_full[info]):
            err += 1
    assert err < 15, f"SC 高 SNR 错误过多: {err}/100"

    from decoder_scl import SCLDecoder

    scl = SCLDecoder(N, frozen.astype(bool), list_size=1)
    err_scl = 0
    for _ in range(50):
        u_full = np.zeros(N, dtype=int)
        u_full[info] = rng.integers(0, 2, K)
        llr = compute_llr(
            awgn_channel(bpsk_modulate(polar_encode(u_full)), sigma, rng), sigma
        )
        u_scl, _ = scl.decode(llr)
        if not np.array_equal(u_scl[info], u_full[info]):
            err_scl += 1
    assert err_scl < 15, f"SCL L=1 错误过多: {err_scl}/50"
    print("单元测试通过。")


if __name__ == "__main__":
    _unit_tests()

    os.makedirs("results", exist_ok=True)

    quick = os.environ.get("POLAR_QUICK", "0") == "1"
    N_LIST = [256, 512, 1024] if not quick else [256, 512]
    RATE = 0.5
    DESIGN_EBN0 = 2.5
    MAX_FRAMES = int(os.environ.get("POLAR_MAX_FRAMES", "100000" if not quick else "5000"))
    MIN_ERRORS = int(os.environ.get("POLAR_MIN_ERRORS", "100" if not quick else "30"))
    EB_N0_RANGE = np.arange(3.0, 8.5, 0.25) if not quick else np.arange(4.0, 8.0, 0.5)

    save_frozen_set_info(N_LIST, None, DESIGN_EBN0, "results/frozen_sets.txt")

    all_results = {}
    for N in N_LIST:
        K = N // 2
        print(f"\n{'=' * 60}\nSC 仿真: N={N}, K={K}, R={RATE}\n{'=' * 60}")

        info_idx, _, _ = ga_construction(N, K, DESIGN_EBN0)
        frozen_bits = np.ones(N, dtype=int)
        frozen_bits[info_idx] = 0

        def decoder(llr_ch):
            return sc_decode(llr_ch, frozen_bits.astype(bool)), None

        results = run_simulation(
            N=N,
            K=K,
            eb_n0_db_list=EB_N0_RANGE,
            decoder=decoder,
            decoder_type="sc",
            max_frames=MAX_FRAMES,
            min_errors=MIN_ERRORS,
            design_eb_n0_db=DESIGN_EBN0,
        )

        label = f"SC, N={N}, K={K}"
        all_results[label] = results
        save_results_csv(results, f"results/exp1_sc_N{N}_R0.5.csv")

    shannon_db = find_capacity_limit(RATE)
    print(f"\nBPSK 信道容量限（R={RATE}）: Eb/N0 = {shannon_db:.3f} dB")
    plot_bler_curves(
        all_results,
        title=f"SC Decoder BLER vs Eb/N0 (R={RATE})",
        save_path="results/fig1_sc_bler.png",
        shannon_limit_db=shannon_db,
    )
    print("\n实验一完成。结果保存至 results/ 目录。")
