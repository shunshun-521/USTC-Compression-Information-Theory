...
def run_unit_tests():
    u = np.array([1, 0, 1, 1])
    x = polar_encode(u)
    expected = u @ build_generator_matrix(4) % 2
    ...
    for _ in range(100):
        ...
        y_t = bpsk_modulate(x_t) + rng.normal(0, sigma, N)
        llr_t = compute_llr(y_t, sigma)
        assert np.array_equal(sc_decode(llr_t, frozen_bits), u_t)

    from decoder_scl import SCLDecoder
    u_t = np.zeros(N, dtype=int)
    u_t[info_idx] = rng.integers(0, 2, K)
    x_t = polar_encode(u_t)
    llr_t = compute_llr(bpsk_modulate(x_t), sigma)
    uh_sc = sc_decode(llr_t, frozen_bits)
    uh_scl, _ = SCLDecoder(N, frozen_bits, list_size=1).decode(llr_t)
    assert np.array_equal(uh_sc, uh_scl), "L=1 SCL 应等价于 SC"

    info8, frozen8, _ = ga_construction(8, 4, 2.5)
    assert np.array_equal(info8, [3, 5, 6, 7])
    print("单元测试通过。")
...
if __name__ == "__main__":
    ...
    for N in N_LIST:
        ...
        results = run_simulation(
            ...
            info_idx=info_idx, verbose=True,
        )
