...
def run_unit_tests():
    ...
    llr = np.array([100.0, -100.0, 100.0, -100.0])
    frozen4 = np.array([1, 1, 0, 0], dtype=int)
    assert np.array_equal(sc_decode_recursive(llr, frozen4), sc_decode(llr, frozen4))
    print("单元测试通过。")


def _quick_env():
    if os.environ.get("POLAR_QUICK", "0") == "1":
        return 500, 20, np.arange(1.0, 4.0, 0.5)
    return 100000, 100, np.arange(0.0, 5.5, 0.25)
