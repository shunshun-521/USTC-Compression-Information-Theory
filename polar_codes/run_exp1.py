...
from simulation import run_simulation, sim_env_int
...
def _unit_tests():
    ...
    for _ in range(100):
        ...
        if not np.array_equal(u_hat[info_idx], u_t[info_idx]):
            errors += 1
    assert errors < 60, f"SC 校验失败: {errors}/100"
    print("单元测试通过。")
...
def main():
    ...
    RATE = 0.5
    DESIGN_EBN0 = 2.5
    MAX_FRAMES = sim_env_int("POLAR_MAX_FRAMES", 100000)
    MIN_ERRORS = sim_env_int("POLAR_MIN_ERRORS", 100)
