...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        if not np.array_equal(u_src[info_idx], u_rec[info_idx]):
            errors += 1
    assert errors <= 2, f"SC 译码在 Eb/N0=12dB 有 {errors}/100 帧错误"
    print(f"  SC decoder test passed ({100 - errors}/100 at 12dB).")
    ...
    print(f"  N=8, K=4, info_indices: {info8}, frozen_indices: {frozen8}")
