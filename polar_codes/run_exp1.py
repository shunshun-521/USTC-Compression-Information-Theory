...
from channel import channel_llr
...
for _ in range(100):
    ...
    llr_t = channel_llr(polar_encode(ut), 1e-6)
    assert np.array_equal(sc_decode(llr_t, frozen_t)[info_t], ut[info_t])
...
EB_N0_RANGE = (
    np.arange(1.0, 4.0, 0.5) if QUICK else np.arange(0.0, 5.5, 0.25)
)
