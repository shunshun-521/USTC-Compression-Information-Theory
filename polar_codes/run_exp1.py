...
def run_unit_tests():
    ...
    u_test = np.zeros(16, dtype=int)
    llr_test = np.random.default_rng(1).normal(0, 1, 16)
    fb = np.zeros(16, dtype=bool)
    assert np.array_equal(sc_decode(llr_test, fb), sc_decode_recursive(llr_test, fb))
    print("单元测试通过。")
