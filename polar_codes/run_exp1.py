...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        if not np.array_equal(u_hat[info_idx], u_test[info_idx]):
            errors_noisy += 1
    assert errors_noisy == 0, f"SC 高信噪比校验失败: {errors_noisy}/100 帧错误"
...
EB_N0_RANGE = np.arange(0.0, 5.5, 0.5) if os.environ.get("POLAR_QUICK", "0") == "1" else np.arange(0.0, 5.5, 0.25)
