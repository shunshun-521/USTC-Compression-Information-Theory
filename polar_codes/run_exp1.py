...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        if not np.array_equal(u_hat[info_idx], u_test[info_idx]):
            errors += 1
    assert errors == 0, f"SC 无损译码失败 {errors}/100 帧"
    print("单元测试通过。")
...
RATE = 0.5
DESIGN_EBN0 = 2.5
MAX_FRAMES = _env_int("POLAR_MAX_FRAMES", 100000 if not os.environ.get("POLAR_QUICK") else 2000)
MIN_ERRORS = _env_int("POLAR_MIN_ERRORS", 100 if not os.environ.get("POLAR_QUICK") else 20)
EB_N0_RANGE = np.arange(0.0, 5.5, 0.5 if os.environ.get("POLAR_QUICK") else 0.25)
