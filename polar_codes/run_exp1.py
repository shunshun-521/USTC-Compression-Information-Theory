...
assert np.array_equal(x_test, [0, 0, 1, 1]), f"编码器错误: {x_test}"

N_sc, K_sc = 64, 32
info_t, _, _ = ga_construction(N_sc, K_sc, 2.5)
frozen_t = np.ones(N_sc, dtype=int)
frozen_t[info_t] = 0
sigma_t = eb_n0_to_sigma(10.0, K_sc / N_sc)
rng = np.random.default_rng(0)
sc_err = 0
for _ in range(100):
    u = np.zeros(N_sc, dtype=int)
    u[info_t] = rng.integers(0, 2, K_sc)
    llr = compute_llr(bpsk_modulate(polar_encode(u)), sigma_t)
    if not np.array_equal(sc_decode(llr, frozen_t), u):
        sc_err += 1
assert sc_err == 0, f"SC 译码校验失败: {sc_err}/100"
