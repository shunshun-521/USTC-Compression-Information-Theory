...
def run_unit_tests():
    ...
    N64, K = 64, 32
    info_idx, _, _ = ga_construction(N64, K, 2.5)
    frozen_bits = np.ones(N64, dtype=int)
    frozen_bits[info_idx] = 0
    sigma = eb_n0_to_sigma(15.0, K / N64)
    for _ in range(100):
        u = np.zeros(N64, dtype=int)
        u[info_idx] = np.random.randint(0, 2, K)
        x = polar_encode(u)
        y = bpsk_modulate(x) + np.random.normal(0, sigma, N64)
...
MAX_FRAMES = 100000
MIN_ERRORS = 100
EB_N0_RANGE = np.arange(0.0, 8.5, 0.25)
