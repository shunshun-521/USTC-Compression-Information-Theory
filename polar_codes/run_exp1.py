...
for _ in range(100):
    ...
    llr = compute_llr(
        awgn_channel(bpsk_modulate(polar_encode(ut)), eb_n0_to_sigma(10.0, 0.5), rng),
        eb_n0_to_sigma(10.0, 0.5),
    )
    if not np.array_equal(sc_decode(llr, frozen_t)[info_t], ut[info_t]):
        err += 1
assert err == 0, "SC 高信噪比校验失败"
