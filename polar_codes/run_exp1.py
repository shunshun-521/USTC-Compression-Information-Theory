...
def run_unit_tests():
    ...
    assert np.array_equal(x, (u @ G) % 2), f"编码器错误: {x} != {(u @ G) % 2}"
    print("  [PASS] 编码器校验")
    ...
    for _ in range(100):
        ...
        if not np.array_equal(u_hat, u_test):
            sc_errors += 1
    assert sc_errors == 0, f"SC 译码在 Eb/N0=10dB 下有 {sc_errors} 个错误"
    ...
    assert scl_equivalent_to_sc(N, frozen_bits, llr_test), "L=1 SCL 与 SC 不等价"
    print("  [PASS] 路径度量校验（SCL L=1 == SC）")
    print("所有单元测试通过。\n")
