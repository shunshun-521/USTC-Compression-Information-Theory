...
sigma_t = eb_n0_to_sigma(10, K_test / N_test)
sc_ok = 0
for _ in range(100):
    u = np.zeros(N_test, dtype=int)
    u[info_t] = rng.integers(0, 2, K_test)
    y = bpsk_modulate(polar_encode(u)) + sigma_t * rng.standard_normal(N_test)
    u_hat = sc_decode(compute_llr(y, sigma_t), frozen_t)
    if np.array_equal(u_hat[info_t], u[info_t]):
        sc_ok += 1
print(f"SC 校验: {sc_ok}/100 帧正确 @10dB")
