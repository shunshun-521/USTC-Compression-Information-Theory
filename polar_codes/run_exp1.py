...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        if not np.array_equal(u_hat[info_idx], u_test[info_idx]):
            errors += 1
    assert errors == 0, f"SC 译码零噪声错误帧数: {errors}/100"
    ...
    for _ in range(100):
        ...
        llr = compute_llr(bpsk_modulate(x_test) + rng.normal(0, sigma, N), sigma)
        u_hat = sc_decode(llr, frozen_bits)
        if not np.array_equal(u_hat[info_idx], u_test[info_idx]):
            errors += 1
    assert errors == 0, f"SC 译码在 Eb/N0=12dB 下错误帧数: {errors}/100"
    print("单元测试通过。")
