...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        if not np.array_equal(u_hat[info_idx], u_test[info_idx]):
            errors += 1
    assert errors == 0, f"SC 译码校验失败: {errors} 帧错误"

    u_hat_r = sc_decode_recursive(llr_test, frozen_bits)
    u_hat_n = sc_decode(llr_test, frozen_bits)
    assert np.array_equal(u_hat_r, u_hat_n), "递归与非递归 SC 不一致"
