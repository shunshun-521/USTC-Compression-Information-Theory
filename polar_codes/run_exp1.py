...
def run_unit_tests():
    ...
    for _ in range(200):
        ...
        if not np.array_equal(u_hat[info_idx], u[info_idx]):
            errors += 1
    assert errors == 0, f'SC 译码在高 SNR 下失败 {errors}/200 帧'
    ...
    assert np.array_equal(u_sc[info_idx], u_test[info_idx])
...
if __name__ == '__main__':
    ...
    DESIGN_EBN0 = 2.5
    EB_N0_RANGE = (
        np.arange(2.0, 4.5, 0.5) if QUICK else np.arange(0.0, 5.5, 0.25)
    )
    if QUICK:
