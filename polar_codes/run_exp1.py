...
def _run_unit_tests():
    ...
    for _ in range(100):
        ...
        u_t[info_idx] = rng.integers(0, 2, size=K)
        x_t = polar_encode(u_t)
        y_t = awgn_channel(bpsk_modulate(x_t), sigma, rng=rng)
        llr_t = compute_llr(y_t, sigma)
        u_hat = sc_decode(llr_t, frozen_bits)
        assert np.array_equal(u_hat[info_idx], u_t[info_idx])
    print("单元测试通过。")
...
def main():
    ...
    EB_N0_RANGE = np.arange(2.0, 6.5, 0.5) if fast else np.arange(0.0, 5.5, 0.25)
