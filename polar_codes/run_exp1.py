...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        if not np.array_equal(u_hat[info_idx], payload):
            errors += 1
    assert errors == 0, f"SC decode errors at noiseless channel: {errors}"
    print("  [PASS] SC decode test (N=64, 100 frames)")

    llr_test = np.array([1.0, -2.0, 3.0, -1.0])
    u_rec = sc_decode_recursive(llr_test, [True, False, True, False])
    u_nr = sc_decode(llr_test, [True, False, True, False])
    assert np.array_equal(u_rec, u_nr), "Recursive vs non-recursive SC mismatch"
    print("  [PASS] SC recursive consistency")
