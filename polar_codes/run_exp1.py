...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        u_sent[info_idx] = rng.integers(0, 2, size=K, dtype=np.int8)
    ...
    llr_test = np.random.default_rng(2).normal(0, 1, N)
    assert np.array_equal(
        sc_decode(llr_test, frozen_bits),
        SCLDecoder(N, frozen_bits, list_size=1, crc_length=0).decode(llr_test)[0],
    )
