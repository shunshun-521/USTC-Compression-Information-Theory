...
def run_unit_tests():
    ...
    u = np.array([1, 0, 1, 1])
    x = polar_encode(u)
    assert np.array_equal(x, [1, 1, 0, 1]), f"编码器错误: {x}"  # u=[1,0,1,1]
    ...
    for _ in range(100):
        ...
        uh = sc_decode(llr, frozen_b)
        if not np.array_equal(u[info_idx], uh[info_idx]):
            err += 1
    assert err == 0, f"SC 译码在 Eb/N0=10dB 仍有 {err} 帧错误"

    # SCL L=1 应接近 SC
    scl = SCLDecoder(N, frozen_b, list_size=1, crc_length=0)
    err_scl = 0
    for _ in range(20):
        u = np.zeros(N, dtype=int)
        u[info_idx] = np.random.randint(0, 2, K)
        x = polar_encode(u)
        llr = compute_llr(bpsk_modulate(x), sigma)
        uh_sc = sc_decode(llr, frozen_b)
        uh_scl, _ = scl.decode(llr)
        if not np.array_equal(uh_sc, uh_scl):
            err_scl += 1
    assert err_scl == 0, f"SCL(L=1) 与 SC 不一致帧数: {err_scl}"
