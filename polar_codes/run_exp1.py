...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        y = awgn_channel(bpsk_modulate(polar_encode(u_test)), sigma, rng=rng)
        llr = compute_llr(y, sigma)
        if not np.array_equal(sc_decode(llr, fb), u_test):
            sc_errors += 1
    ...
    for _ in range(50):
        ...
        if not np.array_equal(u_scl, u_sc):
            pm_errors += 1
    assert pm_errors == 0, f"SCL(L=1) 与 SC 不一致 {pm_errors}/50 帧"
