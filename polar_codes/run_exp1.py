...
from simulation import quick_env, run_simulation
...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        llr = compute_llr(bpsk_modulate(x_t) + rng.normal(0, sigma, N), sigma)
        u_hat = sc_decode(llr, frozen.astype(bool))
        assert np.array_equal(u_t[info_idx], u_hat[info_idx]), "SC 译码校验失败"

    info8, frozen8, _ = ga_construction(8, 4, 2.5)
    print("GA N=8,K=4 info:", info8, "frozen:", frozen8)
    info256, _, _ = ga_construction(256, 128, 2.5)
    print("GA N=256,K=128 first20 info:", info256[:20])
    print("单元测试通过。")
