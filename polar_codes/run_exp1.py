...
x = polar_encode(u)
assert np.array_equal(x, (u @ build_generator_matrix(4)) % 2), f"编码器错误: {x}"
...
for _ in range(100):
    ...
    llr_t = compute_llr(awgn_channel(bpsk_modulate(polar_encode(u_t)), sigma_t, rng), sigma_t)
    assert np.array_equal(sc_decode(llr_t, frozen_t)[info_t], u_t[info_t])
print("单元测试通过。")
