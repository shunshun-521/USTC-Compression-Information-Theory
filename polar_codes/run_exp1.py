...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        u_test[info] = np.random.randint(0, 2, K)
        x_test = polar_encode(u_test)
        llr = (1 - 2 * x_test) * 100.0
        u_hat = sc_decode(llr, frozen_bits)
        assert np.array_equal(u_hat[info], u_test[info])

    print("单元测试通过：编码器、SC 无损译码")
...
def main():
    ...
    EB_N0_RANGE = np.arange(
        float(os.environ.get("POLAR_EB_START", "6.0")),
        float(os.environ.get("POLAR_EB_STOP", "12.0")),
