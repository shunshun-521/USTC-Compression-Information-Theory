...
sigma_t = eb_n0_to_sigma(12.0, 0.5)
err = 0
for _ in range(100):
    ut = np.zeros(N_test, dtype=int)
    ut[info_t] = rng.integers(0, 2, K_test)
    llr = compute_llr(awgn_channel(bpsk_modulate(polar_encode(ut)), sigma_t, rng), sigma_t)
    if not np.array_equal(sc_decode(llr, frozen_t), ut):
        err += 1
assert err == 0, f"SC 校验失败: {err}/100"
...
EB_N0_RANGE = np.arange(2.0, 4.5, 0.5) if FAST else np.arange(0.0, 5.5, 0.25)
