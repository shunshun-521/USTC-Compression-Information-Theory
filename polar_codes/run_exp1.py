...
assert np.array_equal(x_test, (u_test @ polar_generator_matrix(4)) % 2)
...
for _ in range(100):
    ...
    assert np.array_equal(sc_decode(llr_t, fb_t)[info_t], uu[info_t])
print("实验一：单元测试通过")
...
EB_N0_RANGE = np.arange(1.0, 5.5, 0.5) if QUICK else np.arange(0.0, 5.5, 0.25)
...
for N in N_LIST:
    ...
    def decoder(llr_ch, _fb=frozen_bits):
        return sc_decode(llr_ch, _fb.astype(bool)), None
