...
def run_unit_tests():
    ...
    llr_test = compute_llr(bpsk_modulate(polar_encode(np.zeros(N, dtype=int))), sigma)
    assert np.array_equal(
        sc_decode(llr_test, frozen), sc_decode_recursive(llr_test, frozen)
    ), "递归与非递归 SC 结果不一致"
    print("单元测试通过。")
...
EB_N0_RANGE = np.arange(1.0, 3.0, 0.5) if QUICK else np.arange(0.0, 5.5, 0.25)
