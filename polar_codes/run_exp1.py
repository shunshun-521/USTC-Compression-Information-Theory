...
def run_unit_tests():
    ...
    assert verify_sc_decoders(N=64, K=32, num_frames=50, eb_n0_db=15.0)

    from decoder_scl import SCLDecoder, verify_scl_equals_sc
    assert verify_scl_equals_sc(N=64, K=32, num_frames=20, eb_n0_db=15.0)
    ...
    for _ in range(100):
        ...
        assert np.array_equal(u[info_idx], u_hat[info_idx]), "SCL L=1 != SC path"
    print("所有单元测试通过。")
