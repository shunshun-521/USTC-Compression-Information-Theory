...
def run_unit_tests():
    ...
    assert np.array_equal(x, np.mod(u @ G[:, rev], 2)), f"编码器错误: {x}"

    N, K = 64, 32
    info_idx, _, _ = ga_construction(N, K, 2.5)
    frozen_bits = np.ones(N, dtype=int)
    frozen_bits[info_idx] = 0
    bit_rev = bit_reversal_permutation(N)
    ...
    for _ in range(100):
        ...
        llr = compute_llr(y, sigma)[bit_rev]
    ...
    llr_test = compute_llr(bpsk_modulate(x_test), sigma)[bit_rev]
...
def main():
    ...
    EB_N0_RANGE = np.arange(2.0, 6.5, 0.5) if quick else np.arange(0.0, 5.5, 0.25)
