...
def run_unit_tests():
    ...
    for _ in range(200):
        ...
        u_t[info_idx] = np.random.randint(0, 2, K)
        x_t = polar_encode(u_t)
        y_t = bpsk_modulate(x_t) + np.random.normal(0, sigma, N)
        llr_t = compute_llr(y_t, sigma)
        if not np.array_equal(sc_decode(llr_t, frozen_bits)[info_idx], u_t[info_idx]):
            errors += 1
    assert errors == 0, f"SC 译码校验失败: {errors}/200 帧错误"
    print("单元测试通过。")
...
if __name__ == "__main__":
    ...
    for N in N_LIST:
        ...
        def decoder(llr_ch, fb=frozen_bits, ii=info_idx):
            fb_bool = np.asarray(fb, dtype=bool)
