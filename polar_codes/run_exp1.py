...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        llr = compute_llr(
            bpsk_modulate(polar_encode(u_test)),
            eb_n0_to_sigma(10.0, 0.5),
        )
        assert np.array_equal(u_test, sc_decode(llr, frozen))
        assert np.array_equal(u_test, sc_decode_recursive(llr, frozen))
    ...
    for _ in range(20):
        ...
        uh, _ = SCLDecoder(N, frozen, list_size=1).decode(llr)
        assert np.array_equal(u_test, uh)
