...
if os.environ.get('POLAR_QUICK') == '1':
    N_LIST = [256, 512]
    MAX_FRAMES = int(os.environ.get('POLAR_MAX_FRAMES', 80))
    MIN_ERRORS = int(os.environ.get('POLAR_MIN_ERRORS', 8))
    EB_N0_RANGE = np.array([2.0, 3.0])


def main():
    print('运行内置校验...')
    verify.test_encoder()
    verify.test_ga_construction()
    verify.test_sc_lossless()
    verify.test_scl_equiv_sc()
    verify.test_crc()
    verify.test_bp()
    print()
