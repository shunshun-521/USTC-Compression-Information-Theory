...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        if not np.array_equal(sc_decode(llr, frozen_b), u_t):
            errs += 1
    assert errs == 0, f"SC 译码失败帧数: {errs}"

    llr_test = compute_llr(bpsk_modulate(polar_encode(u_t)), sigma)
    assert np.array_equal(
        sc_decode_recursive(llr_test, frozen_b), sc_decode(llr_test, frozen_b)
    )
    print("单元测试通过。")
...
if __name__ == "__main__":
    ...
    EB_N0_RANGE = np.arange(0.0, 10.5, 0.5)
