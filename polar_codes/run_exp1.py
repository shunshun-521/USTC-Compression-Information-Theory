...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        assert np.array_equal(u_hat, u_sent), "SC 译码在 10dB 下失败"
    ...
    assert np.array_equal(sc_decode(llr, frozen_bits), u_scl), "SCL L=1 应等价于 SC"
...
save_frozen_set_info(N_LIST if not QUICK else [256, 512, 1024], None, DESIGN_EBN0, 'results/frozen_sets.txt')
