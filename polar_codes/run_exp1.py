...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        payload[info_idx] = rng.integers(0, 2, len(info_idx))
        llr = compute_llr(awgn_channel(bpsk_modulate(polar_encode(payload)), sigma, rng), sigma)
        if not np.array_equal(sc_decode(llr, fb), payload):
            errors += 1
