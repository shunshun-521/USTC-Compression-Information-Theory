...
from encoder import polar_encode, polar_encode_matrix, polar_encode_with_reversal
...
def run_unit_tests():
    """模块正确性校验"""
    u = np.array([1, 0, 1, 1])
    x = polar_encode_with_reversal(u)
    ...
    uh_scl, _ = SCLDecoder(N, frozen_bits.astype(bool), 1).decode(llr)
