...
def run_unit_tests():
    ...
    sigma2 = eb_n0_to_sigma(5.0, K / N)
    for _ in range(20):
        info = rng.integers(0, 2, K)
        u_full = np.zeros(N, dtype=int)
        u_full[info_idx] = info
        llr = compute_llr(awgn_channel(bpsk_modulate(polar_encode(u_full)), sigma2, rng), sigma2)
...
for N in N_LIST:
    ...
    def decoder(llr_ch, _frozen=frozen_bits.copy()):
        return sc_decode(llr_ch, _frozen.astype(bool)), None
