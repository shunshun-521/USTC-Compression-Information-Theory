...
def _build_G(N):
    """与 encoder.polar_encode 一致的生成矩阵（蝶形，无额外比特倒序）"""
    F = np.array([[1, 0], [1, 1]])
    G = np.array([[1]])
...
def run_unit_tests():
    ...
    u = np.array([1, 0, 1, 1])
    x = polar_encode(u)
    assert np.array_equal(x, u @ _build_G(4) % 2), f"编码器错误: {x}"
    ...
    for _ in range(100):
        ...
        if not np.array_equal(sc_decode(llr, frozen)[info], u[info]):
            err += 1
    ...
    for _ in range(20):
        ...
        assert np.array_equal(u_scl, sc_decode(llr, frozen))
    print("单元测试通过。")
