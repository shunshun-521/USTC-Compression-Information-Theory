...
def run_unit_tests():
    ...
    rev = [int(format(i, "02b")[::-1], 2) for i in range(4)]
    for i, j in enumerate(rev):
        P[i, j] = 1
    assert np.array_equal(x, (P @ G) @ u % 2), f"编码器错误: {x}"
    ...
    for _ in range(100):
        ...
        if not np.array_equal(sc_decode(llr, frozen)[info_idx], payload):
            errors += 1
    assert errors == 0, f"SC 译码校验失败: {errors} 错误帧"
    print("单元测试通过。")
