...
def run_unit_tests():
    ...
    u = np.array([1, 0, 1, 1])
    x = polar_encode(u)
    assert np.array_equal(x, [1, 1, 0, 1]), f"编码器错误: {x}"  # x = u * F^4
    ...
    for _ in range(100):
        ...
        u_tx[info_idx] = payload
        llr = compute_llr(awgn_channel(bpsk_modulate(polar_encode(u_tx)), sigma, rng), sigma)
        u_hat = sc_decode(llr, frozen_bits)
        assert np.array_equal(u_hat[info_idx], payload)

    llr_test = np.array([1.0, -2.0, 3.0, -1.0])
    assert np.array_equal(
        sc_decode(llr_test, np.array([True, False, True, False])),
        sc_decode_recursive(llr_test, np.array([True, False, True, False])),
    )
    print("单元测试通过。")
...
if os.environ.get("POLAR_QUICK") == "1":
    N_LIST = [256]
    EB_N0_RANGE = np.arange(1.0, 3.0, 0.5)
