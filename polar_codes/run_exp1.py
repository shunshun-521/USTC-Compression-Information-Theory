...
from decoder_sc import sc_decode_with_br
...
def run_unit_tests():
    """单元测试验证各模块正确性。"""
    # 编码器校验（与生成矩阵对照）
    from encoder import polar_encode as pe, bit_reversal_permutation
    N = 4
    ...
    for _ in range(100):
        ...
        u_hat = sc_decode_with_br(llr, frozen_bits)
    ...
    for _ in range(20):
        ...
        u[info_idx] = rng.integers(0, 2, K)
        llr = compute_llr(bpsk_modulate(polar_encode(u)), eb_n0_to_sigma(10, K / N))
        u_sc = sc_decode_with_br(llr, frozen_bits)
...
def main():
    ...
    for N in N_LIST:
        ...
        def decoder(llr_ch):
            return sc_decode_with_br(llr_ch, frozen_bits.astype(bool)), None
