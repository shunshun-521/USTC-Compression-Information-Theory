...
def run_unit_tests():
    ...
    u = np.array([1, 0, 1, 1])
    x = polar_encode(u)
    assert np.array_equal(x, [0, 0, 1, 1]) or np.array_equal(x, [1, 0, 1, 1]), f"编码器错误: {x}"
    ...
    for _ in range(100):
        ...
        llr = compute_llr(bpsk_modulate(polar_encode(u_t)), eb_n0_to_sigma(10.0, K / N))
    ...
    u_t = np.zeros(N, dtype=int)
    u_t[info_idx] = rng.integers(0, 2, K)
    llr = compute_llr(bpsk_modulate(polar_encode(u_t)), 0.01)
