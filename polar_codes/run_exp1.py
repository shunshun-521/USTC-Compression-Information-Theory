...
def run_unit_tests():
    ...
    llr_test = compute_llr(bpsk_modulate(polar_encode(u_sent)), 0.01)
    sc_out = sc_decode(llr_test, frozen_bits)
    scl_out, _ = SCLDecoder(N, frozen_bits, list_size=1).decode(llr_test)
    assert np.array_equal(sc_out, scl_out)
