...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        if not np.array_equal(u_hat[info_idx], u_sent[info_idx]):
            errors += 1
    assert errors == 0, f"SC 译码在 10dB 下出现 {errors} 次错误"

    N2 = 16
    info2, _, _ = ga_construction(N2, 8, 2.5)
    frozen2 = np.ones(N2, dtype=int)
    frozen2[info2] = 0
    fb2 = frozen2.astype(bool)
    llr_test = compute_llr(bpsk_modulate(polar_encode(np.zeros(N2, dtype=int))), 0.01)
    assert np.array_equal(
        sc_decode_recursive(llr_test, fb2),
        sc_decode(llr_test, fb2),
    ), "SCL(L=1) 应与 SC 等价"
