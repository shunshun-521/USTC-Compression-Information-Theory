...
frozen_bits_t = np.ones(N_test, dtype=int)
frozen_bits_t[info_idx_t] = 0
...
for _ in range(100):
    ...
    u_t[info_idx_t] = info
    y_t = awgn_channel(bpsk_modulate(polar_encode(u_t)), sigma_t, rng=rng)
    u_hat = sc_decode(compute_llr(y_t, sigma_t), frozen_bits_t)
    assert np.array_equal(u_hat[info_idx_t], info), "SC 无损验证失败"
print("单元测试通过。")
