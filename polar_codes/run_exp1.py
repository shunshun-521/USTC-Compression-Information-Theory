...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        llr = compute_llr(bpsk_modulate(x_test), sigma=0.01)
        u_hat = sc_decode(llr, frozen_bits)
        assert np.array_equal(u_hat[info_idx], u_test[info_idx])

    u_single = np.zeros(N, dtype=np.int8)
    u_single[info_idx] = rng.integers(0, 2, size=K)
    llr_single = compute_llr(bpsk_modulate(polar_encode(u_single)), 0.01)
    assert np.array_equal(
        sc_decode_recursive(llr_single, frozen_bits),
        sc_decode(llr_single, frozen_bits),
    )
    print("单元测试通过。")
