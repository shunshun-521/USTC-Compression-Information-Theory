...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        u64[info_idx] = info
        codeword = polar_encode(u64)
        llr = compute_llr(awgn_channel(bpsk_modulate(codeword), sigma, rng), sigma)
        u_hat = sc_decode(llr, frozen_bits)
        if not np.array_equal(u_hat[info_idx], info):
            errors += 1
        assert errors <= 5, f"SC 译码错误过多: {errors}/100"
...
MAX_FRAMES = 100000
MIN_ERRORS = 100
EB_N0_RANGE = np.arange(0.0, 9.0, 0.25)
