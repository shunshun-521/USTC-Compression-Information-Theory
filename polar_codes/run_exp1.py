...
def run_unit_tests():
  ...
  for _ in range(100):
    ...
    assert np.array_equal(u[info_idx], u_hat[info_idx]), "SC 无损验证失败"
  ...
  u = np.zeros(N, dtype=int)
  u[info_idx] = rng.integers(0, 2, K)
  llr = compute_llr(awgn_channel(bpsk_modulate(polar_encode(u)), 0.1, rng), 0.1)
  u_scl, _ = scl.decode(llr)
  assert np.array_equal(sc_decode(llr, frozen_bits), u_scl), "SCL L=1 与 SC 不等价"
