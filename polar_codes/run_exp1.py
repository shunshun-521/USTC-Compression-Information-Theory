...
def run_unit_tests():
    ...
    errors = 0
    for _ in range(100):
        u_trial = np.zeros(N, dtype=int)
        u_trial[info_idx] = rng.integers(0, 2, K)
        x_t = polar_encode(u_trial)
        y = awgn_channel(bpsk_modulate(x_t), sigma, rng)
        llr = compute_llr(y, sigma)
        u_hat = sc_decode(llr, frozen_bits)
        if np.any(u_hat[info_idx] != u_trial[info_idx]):
            errors += 1
    ...
    u_trial[info_idx] = rng.integers(0, 2, K)
    x_t = polar_encode(u_trial)
    llr = compute_llr(bpsk_modulate(x_t), 0.01)
