...
def run_unit_tests():
    ...
    for _ in range(100):
        ...
        if np.any(u_hat[info_idx] != u_test[info_idx]):
            errors += 1
    assert errors == 0, f"SC 译码校验失败: {errors}/100 帧错误"

    u_test = np.zeros(N, dtype=int)
    u_test[info_idx] = rng.integers(0, 2, K)
    llr = compute_llr(bpsk_modulate(polar_encode(u_test)), eb_n0_to_sigma(10.0, K / N))
    u_scl, _ = SCLDecoder(N, frozen_bits, list_size=1).decode(llr)
    assert np.array_equal(sc_decode(llr, frozen_bits), u_scl), "L=1 SCL 应等价于 SC"
