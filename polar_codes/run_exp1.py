...
def run_unit_tests():
    ...
    u = np.array([1, 0, 1, 1])
    x = polar_encode(u)
    u_hat = sc_decode(compute_llr(1 - 2 * x, 0.1), np.zeros(4, dtype=bool))
    assert np.array_equal(u, u_hat), f"编码/译码往返失败: u={u}, u_hat={u_hat}"
    ...
    for _ in range(100):
        ...
        llr = compute_llr(1 - 2 * polar_encode(u), sigma)
