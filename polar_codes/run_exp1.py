...
def run_unit_tests():
  ...
  # 标准蝶形+比特倒序编码结果
    ...
    for _ in range(100):
        ...
        assert np.all(u_hat[info_idx] == u_sent[info_idx]), "SC 译码校验失败"

    from decoder_scl import SCLDecoder

    u_sent = np.zeros(N, dtype=int)
    u_sent[info_idx] = rng.integers(0, 2, K)
    llr = compute_llr(bpsk_modulate(polar_encode(u_sent)), 1e-3)
    u_sc = sc_decode(llr, frozen_bits.astype(bool))
    u_scl, _ = SCLDecoder(N, frozen_bits.astype(bool), list_size=1).decode(llr)
    assert np.array_equal(u_sc, u_scl), "路径度量校验失败：L=1 SCL 应等价于 SC"
